...
const runtimePath = path.join(rootDir, '.civweave-server-v129.runtime.mjs');
const VERSION = '1.0.29';
const BUILD = '1.0.29-cabinet-interface-parity';
let source = await fsp.readFile(sourcePath, 'utf8');

function replaceRequired(before, after, label) {
  if (!source.includes(before)) throw new Error(`Civweave v1.0.29 patch could not find ${label}`);
