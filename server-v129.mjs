...
const runtimePath = path.join(rootDir, '.commonweave-server-v129.runtime.mjs');
const VERSION = '1.0.29';
const BUILD = '1.0.29-cabinet-interface-parity';
let source = await fsp.readFile(sourcePath, 'utf8');

function replaceRequired(before, after, label) {
  if (!source.includes(before)) throw new Error(`Commonweave v1.0.29 patch could not find ${label}`);
...
const CW_VERSION = '1.0.29';
const CW_BUILD = '1.0.29-cabinet-interface-parity';
const cwBootLogs = [];
