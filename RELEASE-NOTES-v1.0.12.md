...
Opening the installer registers a service worker even before Commonweave is installed. A worker could become active after its versioned shell cache had been cleared or left empty, then report `0/12` files and turn the primary action into a reset loop.
...
- when shell preparation fails, the primary action becomes **Open Commonweave online** and the update action becomes **Repair shell**
