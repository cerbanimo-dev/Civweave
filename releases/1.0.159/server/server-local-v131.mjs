...
function replaceRequired(before,after,label){if(!source.includes(before))throw new Error(`Civweave local v1.0.159 patch could not find ${label}`);source=source.replace(before,after)}
replaceRequired("const VERSION = '1.0.30';","const VERSION = '1.0.159';",'local version marker');
replaceRequired("const BUILD = '1.0.30-offline-mesh-cabinet-runtime';","const BUILD = '1.0.159-settings-layer-local-runtime';",'local build marker');
replaceRequired("const CW_VERSION = '1.0.30';","const CW_VERSION = '1.0.159';",'generated runtime version marker');
replaceRequired("const CW_BUILD = '1.0.30-offline-mesh-cabinet-runtime';","const CW_BUILD = '1.0.159-settings-layer-local-runtime';",'generated runtime build marker');
...
try{await import(pathToFileURL(runtimePath).href+'?build=1.0.159')}finally{setTimeout(()=>fsp.unlink(runtimePath).catch(()=>{}),1000).unref?.()}
