...
function resolveNodeId(request, env) { const url = new URL(request.url), domain = env.NODE_DOMAIN || 'nodes.commonweave.earth'; return normalizeNodeId(request.headers.get('x-civweave-node-id') || url.searchParams.get('nodeId') || nodeIdFromHostname(url.hostname, domain)); }
function sharedFreeLimitError(error) { return Number(error?.status) === 429 || /3036|daily free allocation|10,000 neurons|account limited/i.test(clean(error?.message || error, 2000)); }
async function reserveSelection(env, nodeId, userId, route, input, model) { return (await capacityPost(env, '/usage/reserve', { nodeId, userId, requestedNeurons: route.quotaNeurons, billingCeilingNeurons: route.providerNeurons, billingRail: route.route, fundingSource: route.pool, billingModel: model, allowLifetimeCredits: input.allowLifetimeCredits === true })).reservation; }
async function settle(env, reservation, quotaNeurons, providerNeurons = quotaNeurons) { return capacityPost(env, '/usage/settle', { reservationId: reservation.reservationId, actualNeurons: quotaNeurons, actualBillingNeurons: providerNeurons }); }
async function runRoute(env, route, gateway, options) { if (route.route === 'ai-gateway-unified-billing') return env.AI.run(gateway.model, options, { gateway: { id: clean(env.CIVWEAVE_AI_GATEWAY_ID || 'default', 120), collectLog: true } }); return env.AI.run(WORKERS_MODEL, options); }
async function handleGenerate(request, env, nodeId) {
  ...
  let session;
  try { session = await verifyCapacitySession(env, bearer(request), nodeId); } catch (error) { return json({ ok: false, error: String(error?.message || error) }, Number.isSafeInteger(error?.status) ? error.status : 401); }
  const input = await request.json().catch(() => ({}));
  const rows = messages(input), maxTokens = Math.max(32, Math.min(MAX_GENERATION_TOKENS, Number(input.maxTokens) || 1024));
  let schema; try { schema = boundedSchema(input.responseSchema); } catch (error) { return json({ ok: false, error: String(error?.message || error) }, 400); }
  let gateway; try { gateway = gatewayConfig(env); } catch (error) { return json({ ok: false, error: String(error?.message || error) }, Number(error?.status) || 503); }
  ...
  let memberStatus; try { memberStatus = await capacityPost(env, '/members/status', { nodeId, userId: session.userId }); } catch (error) { return json({ ok: false, error: String(error?.message || error) }, Number(error?.status) || 503); }
  ...
  if (schema) options.response_format = { type: 'json_schema', json_schema: schema }; else if (input.responseFormat === 'json') options.response_format = { type: 'json_object' };
  let reservation; try { reservation = await reserveSelection(env, nodeId, session.userId, route, input, route.route === 'ai-gateway-unified-billing' ? gateway.model : WORKERS_MODEL); } catch (error) { return json({ ok: false, error: String(error?.message || error), code: Number(error?.status) === 402 ? 'COMPUTE_PAYMENT_REQUIRED' : 'COMPUTE_RESERVATION_FAILED' }, Number.isSafeInteger(error?.status) ? error.status : 503); }
  let result;
...
async function hashCredential(value) { const secret = clean(value, 400); if (!/^[A-Za-z0-9_-]{40,200}$/.test(secret)) throw Object.assign(new TypeError('A valid device login credential is required.'), { status: 400 }); const digest = await crypto.subtle.digest('SHA-256', enc.encode(`civweave.host-login-credential.v1\n${secret}`)); return [...new Uint8Array(digest)].map(byte => byte.toString(16).padStart(2, '0')).join(''); }
...
async function signNode(env, nodeId, rawText) { const stub = env.NODES?.get(env.NODES.idFromName(nodeId)); if (!stub) throw Object.assign(new Error('Node binding is unavailable.'), { status: 503 }); const response = await stub.fetch('https://node.internal/internal/sign-request', { method: 'POST', headers: { 'content-type': 'application/json', 'x-civweave-node-id': nodeId }, body: rawText }); const payload = await response.json().catch(() => ({})); if (!response.ok || !payload.signatureHeader) throw Object.assign(new Error(payload.error || 'Host node could not sign the membership request.'), { status: response.status || 503 }); return payload.signatureHeader; }
async function coreMembership(env, nodeId, body) { const rawText = JSON.stringify(body), signature = await signNode(env, nodeId, rawText); const init = { method: 'POST', headers: { 'content-type': 'application/json', 'x-civweave-node-signature': signature }, body: rawText }; const response = env.CORE?.fetch ? await env.CORE.fetch('https://civweave-core.internal/api/money-edge/memberships', init) : await fetch(new URL('/api/money-edge/memberships', clean(env.CIVWEAVE_CORE_ORIGIN, 2000) || 'https://api.commonweave.earth'), init); const payload = await response.json().catch(() => ({})); if (!response.ok) throw Object.assign(new Error(payload.error || `Core returned HTTP ${response.status}.`), { status: response.status }); return payload.membership; }
async function beginMembership(request, env, nodeId) { try { const input = await request.json().catch(() => ({})), userId = residentId(input.userId), tierId = clean(input.tierId || 'member', 80).toLowerCase(); if (!['member', 'maker', 'builder', 'steward'].includes(tierId)) throw Object.assign(new RangeError('Unknown Civweave membership tier.'), { status: 400 }); const preparation = await capacityPost(env, '/members/prepare-membership', { nodeId, userId, loginCredentialHash: await hashCredential(input.credential) }), origin = nodeOrigin(nodeId, env); const membership = await coreMembership(env, nodeId, { nodeId, userId, tierId, idempotencyKey: `paid-seat:${nodeId}:${userId}:${tierId}`, successUrl: `${origin}/api/commerce/membership/return?result=success&nodeId=${encodeURIComponent(nodeId)}`, cancelUrl: `${origin}/api/commerce/membership/return?result=cancelled&nodeId=${encodeURIComponent(nodeId)}` }); return json({ ok: true, schema: 'civweave.paid-seat-checkout.v1', nodeId, preparation, membership, checkout: membership }, 201); } catch (error) { return json({ ok: false, error: String(error?.message || error) }, Number.isSafeInteger(error?.status) ? error.status : 500); } }
function returnToInstaller(request, env, nodeId) { const source = new URL(request.url), target = new URL('/', clean(env.CIVWEAVE_CANONICAL_INSTALL_ORIGIN, 2000) || 'https://civweave.pages.dev'); target.searchParams.set('host', nodeOrigin(nodeId, env)); target.searchParams.set('node', nodeId); target.searchParams.set('membership', source.searchParams.get('result') === 'success' ? 'success' : 'cancelled'); return Response.redirect(target.href, 303); }

export default { async fetch(request, env, ctx) { const url = new URL(request.url), nodeId = resolveNodeId(request, env); if (nodeId && request.method === 'OPTIONS' && (url.pathname.startsWith('/api/commerce/') || url.pathname === '/api/ai/node/generate')) return new Response(null, { status: 204, headers: AI_CORS }); if (nodeId && request.method === 'POST' && url.pathname === '/api/ai/node/generate') return handleGenerate(request, env, nodeId); if (nodeId && request.method === 'POST' && url.pathname === '/api/commerce/membership/prejoin') return beginMembership(request, env, nodeId); if (nodeId && request.method === 'GET' && url.pathname === '/api/commerce/membership/return') return returnToInstaller(request, env, nodeId); return baseWorker.fetch(request, env, ctx); }, async scheduled(controller, env, ctx) { if (typeof baseWorker.scheduled === 'function') return baseWorker.scheduled(controller, env, ctx); } };