import baseWorker, { CivweaveCloudNode } from './server-ai-entry-v1.mjs';
import { nodeIdFromHostname, normalizeNodeId } from './index.mjs';
import { CivweaveUserPoolCapacityAccount } from './capacity-user-pools-v2.mjs';
import { chooseUserAiPoolRoute } from './user-ai-pool-router-v2.mjs';

export { CivweaveCloudNode };
export { CivweaveUserPoolCapacityAccount as CivweaveCapacityAccount };
...
function usageTokens(result = {}) {
  ...
  const output = explicitOutput != null
...
async function reserveSelection(env, nodeId, userId, route, input, model) {
  return (await capacityPost(env, '/usage/reserve', {
    nodeId,
    userId,
    requestedNeurons: route.quotaNeurons,
    billingCeilingNeurons: route.providerNeurons,
    billingRail: route.route,
    fundingSource: route.pool,
    billingModel: model,
...
async function settle(env, reservation, quotaNeurons, providerNeurons = quotaNeurons) {
  return capacityPost(env, '/usage/settle', {
    ...
    actualNeurons: quotaNeurons,
    actualBillingNeurons: providerNeurons,
  });
}
async function runRoute(env, route, gateway, options) {
  if (route.route === 'ai-gateway-unified-billing') {
    return env.AI.run(gateway.model, options, { gateway: { id: clean(env.CIVWEAVE_AI_GATEWAY_ID || 'default', 120), collectLog: true } });
  }
...
async function handleGenerate(request, env, nodeId) {
  ...
  let route = chooseUserAiPoolRoute({
    workersPlan: memberStatus?.capacity?.workersPlan,
    includedRemainingNeurons: Number(memberStatus?.quota?.includedRemainingNeurons || 0),
    sharedFreeRemainingNeurons: Number(memberStatus?.quota?.workersAiFreeRemainingNeurons ?? memberStatus?.capacity?.workersAiFreeRemainingNeurons ?? 0),
    workersEstimateNeurons: workersEstimate,
    gatewayEstimateNeurons: gatewayEstimate,
  });
  ...
  let reservation;
  try {
    reservation = await reserveSelection(env, nodeId, session.userId, route, input, route.route === 'ai-gateway-unified-billing' ? gateway.model : WORKERS_MODEL);
  } catch (error) {
  ...
  let result;
  try {
    result = await runRoute(env, route, gateway, options);
  } catch (error) {
    ...
    if (route.route === 'workers-ai-free' && String(memberStatus?.capacity?.workersPlan).toLowerCase() === 'free' && sharedFreeLimitError(error)) {
      ...
      try {
        reservation = await reserveSelection(env, nodeId, session.userId, route, input, gateway.model);
        result = await runRoute(env, route, gateway, options);
      } catch (fallbackError) {
        if (reservation) await settle(env, reservation, 0, 0).catch(() => {});
        return json({ ok: false, error: String(fallbackError?.message || fallbackError), code: 'AI_GATEWAY_FALLBACK_FAILED' }, Number.isSafeInteger(fallbackError?.status) ? fallbackError.status : 502);
      }
    } else {
  ...
  try {
    ...
    return json({
      ...
      model: route.route === 'ai-gateway-unified-billing' ? gateway.model : WORKERS_MODEL,
      computeRoute: route.route,
      pool: route.pool,
      text,
      outputJson,
      usage: { ...usage, chargedNeurons: chargedQuota, providerNeurons: chargedProvider },
      settlement,
