...
export function deriveCapacity({
  workersPlan = 'free', memberCount = 0, communityMemberCount = 0, paidExpansionCount = 0,
  operatingReserveMicrocents = 0, communityEndowmentMicrocents = 0, creditReserveMicrocents = 0, dailyUsedNeurons = 0,
  ...
  return Object.freeze({
    schema: HOST_CAPACITY_SCHEMA, workersPlan: paidPlan ? 'paid' : 'free', memberCount, communityMemberCount, paidExpansionCount,
    dailyCeilingNeurons, dailyUsedNeurons, dailyRemainingNeurons: Math.max(0, dailyCeilingNeurons - Number(dailyUsedNeurons || 0)),
    includedDailyNeurons, burstReserveNeurons: Math.max(0, dailyCeilingNeurons - includedDailyNeurons * memberCount), communitySeatLimit,
    paidExpansionSeatLimit, paidExpansionSeatsRemaining: paidExpansionSeatLimit == null ? null : Math.max(0, paidExpansionSeatLimit - paidExpansionCount),
    starterCommunityLimit, fundedOverageNeuronsPerDay: Math.max(0, dailyCeilingNeurons - policy.cloudflareFreeNeuronsPerDay),
...
export function admissionDecision({ seatClass, billingStatus = 'free', capacity, nodeCommunityCount = 0 } = {}, policy = HOST_ECONOMY_POLICY) {
  ...
  if (nextCount > 0 && Math.floor(capacity.dailyCeilingNeurons * (10_000 - policy.burstReserveBps) / 10_000 / nextCount) < policy.survivalFloorNeuronsPerDay) return Object.freeze({ allowed: false, reason: 'funded-daily-floor-would-break' });
  return Object.freeze({ allowed: true, reason: 'funded-paid-expansion-available' });
}

function initialConfig(workersPlan = 'free') {
  return { schema: HOST_CAPACITY_SCHEMA, workersPlan: String(workersPlan).toLowerCase() === 'paid' ? 'paid' : 'free', hostNodeIds: [], operatingReserveMicrocents: 0, communityEndowmentMicrocents: 0, creditReserveMicrocents: 0, updatedAt: new Date().toISOString() };
}

export class CivweaveCapacityAccount {
  constructor(state, env) { this.state = state; this.env = env; }
  async config() { return (await this.state.storage.get('config')) || initialConfig(this.env.CIVWEAVE_WORKERS_PLAN || 'free'); }
  async putConfig(config) { const next = { ...config, updatedAt: new Date().toISOString() }; await this.state.storage.put('config', next); return next; }
  ...
  async snapshot(nodeId = '') {
    ...
    const capacity = deriveCapacity({ workersPlan: config.workersPlan, memberCount: members.length, communityMemberCount: community.length, paidExpansionCount: expansion.length, operatingReserveMicrocents: config.operatingReserveMicrocents, communityEndowmentMicrocents: config.communityEndowmentMicrocents, creditReserveMicrocents: config.creditReserveMicrocents, dailyUsedNeurons });
  ...
  async settleMembership(input) {
    ...
    const residual = systemMicrocents - creditBacking, endowment = Math.floor(residual * HOST_ECONOMY_POLICY.endowmentResidualBps / 10_000), operating = residual - endowment;
    const config = await this.config(); config.creditReserveMicrocents += creditBacking; config.communityEndowmentMicrocents += endowment; config.operatingReserveMicrocents += operating; await this.putConfig(config);
    ...
    await this.state.storage.put(key, nextWallet); await this.setBilling({ nodeId, userId, billingStatus: 'paid', membershipTierId: tierId });
    const result = { schema: HOST_ECONOMY_SCHEMA, kind: 'membership', sourceId: sourceId || null, split, lifetimeCreditsAdded: requestedCredits, reservesAddedMicrocents: { lifetimeCredits: creditBacking, operating, communityEndowment: endowment }, wallet: nextWallet, capacity: await this.snapshot(nodeId), idempotent: false };
  ...
  async settleTopup(input) {
    ...
    const split = splitTopupNetCents(whole(input.netServiceCents, 'netServiceCents', 1)), creditBacking = centsToMicrocents(split.systemCents), credits = microcentsToNeurons(creditBacking);
    const config = await this.config(); config.creditReserveMicrocents += creditBacking; await this.putConfig(config);
    ...
    const result = { schema: HOST_ECONOMY_SCHEMA, kind: 'topup', sourceId: sourceId || null, split, lifetimeCreditsAdded: credits, wallet: nextWallet, capacity: await this.snapshot(nodeId), idempotent: false };
  ...
  async adjustTopup(input) {
    ...
    const amountCents = whole(input.userCreditCents ?? input.amountCents, 'userCreditCents', 1), backingMicrocents = centsToMicrocents(amountCents), debitNeurons = microcentsToNeurons(backingMicrocents);
    ...
    await this.state.storage.put(key, nextWallet);
    const config = await this.config(), cashReserveRemovedMicrocents = Math.min(config.creditReserveMicrocents, backingMicrocents); config.creditReserveMicrocents -= cashReserveRemovedMicrocents; await this.putConfig(config);
    const result = { schema: HOST_ECONOMY_SCHEMA, kind: `topup-${kind}`, sourceId, userCreditCents: amountCents, lifetimeCreditsRemoved: recoveredNeurons, debtAddedNeurons, unbackedAdjustmentMicrocents: Math.max(0, backingMicrocents - cashReserveRemovedMicrocents), wallet: nextWallet, capacity: await this.snapshot(nodeId), idempotent: false };
  ...
  async reserveUsage(input) {
    ...
    const reservationId = `compute:${crypto.randomUUID()}`; await this.state.storage.put(userUsageStorageKey, userUsed + requested); await this.state.storage.put(`daily-total:${day}`, capacity.dailyUsedNeurons + requested);
  ...
  async settleUsage(input) {
    ...
    await this.state.storage.put(userUsageStorageKey, Math.max(0, currentUserUsed - refund)); await this.state.storage.put(`daily-total:${reservation.day}`, Math.max(0, currentTotal - refund));
    if (refundLifetime) { const creditsKey = walletKey(reservation.nodeId, reservation.userId), wallet = await this.wallet(reservation.nodeId, reservation.userId); await this.state.storage.put(creditsKey, { ...wallet, balanceNeurons: wallet.balanceNeurons + refundLifetime, spentNeurons: Math.max(0, wallet.spentNeurons - refundLifetime), updatedAt: new Date().toISOString() }); }
    ...
    if (paidActual > 0) {
      ...
      config.creditReserveMicrocents -= creditCost; let remainingIncludedCost = includedCost;
      ...
      if (remainingIncludedCost > 0) throw Object.assign(new RangeError('Free-service reserve is insufficient for settled overage.'), { status: 503 }); await this.putConfig(config);
    }
    await this.state.storage.delete(key); return { schema: 'civweave.compute-settlement.v1', reservationId: id, requestedNeurons: reservation.requestedNeurons, actualNeurons: actual, refundedNeurons: refund, refundedIncludedNeurons: refundIncluded, refundedLifetimeNeurons: refundLifetime, capacity: await this.snapshot(reservation.nodeId) };
  }
