...
export async function ingestCreationReceipt(storage, nodeId, object, now = Date.now()) {
  ...
  if (!existing) await storage.put(key, Object.freeze({ schema: 'civweave.guild-creator-receipt.v1', nodeId: clean(nodeId, 180), receipt, communityObjectId: clean(object.id, 500), revisionHash: clean(object.revisionHash, 128), originDeviceId: clean(object.origin?.nodeId, 180), receivedAt: new Date(now).toISOString() }));
...
export async function listReceiptsForDay(storage, dayKey) {
  const rows = await storageList(storage, `${RECEIPT_PREFIX}${dayKey}:`);
  return [...rows.values()].map(row => row?.receipt).filter(Boolean);
}
export async function runGuildDailyAudit(storage, nodeId, secretSalt, now = Date.now()) {
  ...
  const record = Object.freeze({ schema: 'civweave.guild-creator-audit-record.v1', nodeId: clean(nodeId, 180), generatedAt: new Date(now).toISOString(), result });
...
export async function pruneGuildAuditStorage(storage, now = Date.now(), { auditDays = 30, receiptDays = 3650 } = {}) {
  const receiptCutoff = now - Math.max(30, receiptDays) * 86400000, auditCutoff = now - Math.max(7, auditDays) * 86400000, deletes = [];
...
export const CREATOR_PROVENANCE_AUDIT_KEYS = Object.freeze({ PREFIX, RECEIPT_PREFIX, AUDIT_PREFIX, POLICY_KEY, LATEST_KEY });
