...
import { PassportAccountService } from '../../account-edge/src/hub-passport-account-v1.mjs';
...
export class CivweaveCloudNode extends BaseCloudNode {
  ...
  async passportAccount(request, nodeId) {
    ...
    try {
      if (url.pathname === '/api/account/passport-account/ensure') { await this.verifyMemberLogin(nodeId, clean(input.userId, 180), clean(input.credential, 400)); return response(await service.ensureAccount(nodeId, input)); }
      if (url.pathname === '/api/account/passkey/register/begin') { await this.verifyMemberLogin(nodeId, clean(input.userId, 180), clean(input.credential, 400)); return response(await service.beginPassportRegistration(input)); }
      ...
      if (url.pathname === '/api/account/recovery-email/begin') { await this.verifyMemberLogin(nodeId, clean(input.userId, 180), clean(input.credential, 400)); return response(await service.beginRecoveryEmail(input)); }
      if (url.pathname === '/api/account/recovery-email/verify') { await this.verifyMemberLogin(nodeId, clean(input.userId, 180), clean(input.credential, 400)); return response(await service.verifyRecoveryEmail(nodeId, input)); }
