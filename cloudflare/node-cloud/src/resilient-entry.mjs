import base, { CivweaveCloudNode, CivweaveCapacityAccount, nodeIdFromHostname } from './entry.mjs';
...
async function secretEqual(left, right) {
  ...
  for (let index = 0; index < Math.min(a.length, b.length); index += 1) diff |= a.charCodeAt(index) ^ b.charCodeAt(index);
  return diff === 0;
}
function bearer(request) {
  const header = clean(request.headers.get('authorization'), 12000);
  return /^Bearer\s+/i.test(header) ? header.replace(/^Bearer\s+/i, '') : '';
}
function anchorStub(env) {
  if (!env.ANCHORS) throw Object.assign(new Error('Anchor registry binding is unavailable.'), { status: 503 });
  return env.ANCHORS.get(env.ANCHORS.idFromName('civweave-anchor-registry'));
}
async function anchorJson(env, pathname, init = {}) {
  const response = await anchorStub(env).fetch(new Request(`https://anchors.internal${pathname}`, init));
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) throw Object.assign(new Error(payload.error || `Anchor registry returned HTTP ${response.status}.`), { status: response.status });
  return payload;
}
async function operatorAuthorized(request, env) {
  return secretEqual(bearer(request), env.NODE_FABRIC_OPERATOR_TOKEN);
}
async function baseJson(request, env, ctx) {
  const response = await base.fetch(request, env, ctx), payload = await response.clone().json().catch(() => ({}));
  if (!response.ok) throw Object.assign(new Error(payload.error || `Base node returned HTTP ${response.status}.`), { status: response.status });
  return payload;
}
async function automaticCheckpoint(env, ctx, nodeId, domain) {
  ...
  return anchorJson(env, '/checkpoint', {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({
      nodeId,
      source: 'cloudflare-node-daily-snapshot',
      nodeManifest: manifest,
      capacitySnapshot: capacityEnvelope,
      ledgerFrontier: {
        mode: 'peer-reconstructable',
        note: 'Shared signed ledger history is recovered from surviving Civweave peers and local devices rather than treated as Cloudflare-owned state.'
      },
      softwareManifest: {
        runtime: manifest.runtime || 'cloudflare-durable-object-v2',
        nodeSchema: manifest.schema || 'civweave.node.v1',
        healthSchema: health.schema || 'civweave.node-health.v1',
        runtimeUpdatedAt: manifest.updatedAt || health.updatedAt || new Date().toISOString()
      },
      recoveryCoverageBps: 10000,
    })
  });
}
async function refreshKnownNodes(env, ctx, domain) {
  const { nodes = [] } = await anchorJson(env, '/nodes');
  const results = [];
  for (const nodeId of nodes.slice(0, 1000)) {
    try { results.push({ nodeId, ok: true, ...(await automaticCheckpoint(env, ctx, nodeId, domain)) }); }
    catch (error) { results.push({ nodeId, ok: false, error: String(error?.message || error) }); }
  }
  return results;
}
function publicCors() {
...
async function publicAnchorRoute(request, env, nodeId) {
  const url = new URL(request.url), headers = publicCors();
  ...
  const mapping = new Map([
    ['/api/node/anchor/pair', '/pair'],
    ['/api/node/anchor/sync', '/sync'],
    ['/api/node/anchor/proof', '/proof'],
  ]);
  const target = mapping.get(url.pathname);
  if (target && request.method === 'POST') {
    let input;
    try { input = JSON.parse(body || '{}'); } catch { return json({ ok: false, error: 'invalid-json' }, 400, headers); }
    input.nodeId = nodeId;
    const payload = await anchorJson(env, target, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(input) });
    return json(payload, target === '/pair' ? 201 : 200, headers);
  }
  return null;
}
async function adminAnchorRoute(request, env, ctx, nodeId, action, domain) {
  ...
  if (action === 'checkpoint' && request.method === 'POST') {
    ...
    const manifest = automatic.checkpoint?.nodeManifest || null, capacitySnapshot = automatic.checkpoint?.capacitySnapshot || null;
    return json(await anchorJson(env, '/checkpoint', {
      method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({
        ...input, nodeId, nodeManifest: input.nodeManifest || manifest, capacitySnapshot: input.capacitySnapshot || capacitySnapshot,
        source: clean(input.source || 'host-published-recovery-state', 120),
      })
    }), 201);
  }
  if (action === 'stipends' && request.method === 'POST') {
    return json(await anchorJson(env, '/stipends/run', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ nodeId }) }));
  }
  return null;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      if (nodeId && url.pathname.startsWith('/api/node/anchor/')) {
        const response = await publicAnchorRoute(request, env, nodeId);
        if (response) return response;
      }
      if (nodeId && request.method === 'GET' && (url.pathname === '/api/node/manifest' || url.pathname === '/api/ai/node/manifest')) {
        const response = await base.fetch(request, env, ctx), payload = await response.json().catch(() => ({}));
        if (!response.ok) return json(payload, response.status);
        const resilience = await anchorJson(env, `/status?nodeId=${encodeURIComponent(nodeId)}`);
        const manifest = payload.manifest || payload;
        const capabilities = [...new Set([...(Array.isArray(manifest.capabilities) ? manifest.capabilities : []), 'local-anchor-recovery'])];
      ...
      if (admin) {
        const response = await adminAnchorRoute(request, env, ctx, admin[1].toLowerCase(), admin[2], domain);
        if (response) return response;
      }
      ...
      return json({ ok: false, error: String(error?.message || error) }, Number.isSafeInteger(error?.status) ? error.status : 500, nodeId ? publicCors() : {});
    }
  },
