...
import {handleBrowserToolRequest}from'./browser-tool-entry-v1.mjs';
...
export default{async fetch(request,env,ctx){const url=new URL(request.url);
  if(url.pathname==='/api/browser/tool'&&(request.method==='POST'||request.method==='OPTIONS'))return handleBrowserToolRequest(request,env);
