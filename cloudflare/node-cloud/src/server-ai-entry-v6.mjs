...
export default{
  async fetch(request,env,ctx){const url=new URL(request.url),match=url.pathname.match(/^\/api\/fabric\/capacity\/guildkeepers\/(appoint|remove|status)$/);if(match){if(request.method!=='POST')return json({ok:false,error:'Method not allowed.'},405);return guildkeeperAdmin(request,env,match[1])}return baseWorker.fetch(request,env,ctx)},
