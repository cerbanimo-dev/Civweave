...
const guildSetupCors = { 'access-control-allow-origin': '*', 'access-control-allow-methods': 'POST, OPTIONS', 'access-control-allow-headers': 'content-type, x-civweave-location-key', 'access-control-max-age': '86400' };
...
export function normalizeGuildRallyPoint(input = {}, now = Date.now()) {
  const name = clean(input.name || input.label, 180), directions = clean(input.directions || input.description, 600);
  ...
  if (!name) throw Object.assign(new RangeError('Guild Rally Point name is required.'), { status: 400 });
  if (input.publicPlaceConfirmed !== true) throw Object.assign(new RangeError('Guild Rally Point must be confirmed as a public or community-accessible place.'), { status: 400 });
  if (!Number.isFinite(latitude) || latitude < -90 || latitude > 90) throw Object.assign(new RangeError('Guild Rally Point latitude is invalid.'), { status: 400 });
  if (!Number.isFinite(longitude) || longitude < -180 || longitude > 180) throw Object.assign(new RangeError('Guild Rally Point longitude is invalid.'), { status: 400 });
  if (!Number.isFinite(accuracyMeters) || accuracyMeters <= 0 || accuracyMeters > 250) throw Object.assign(new RangeError('Guild Rally Point requires a location reading within 250 meters.'), { status: 400 });
  if (!Number.isFinite(capturedMs) || Math.abs(now - capturedMs) > 10 * 60 * 1000) throw Object.assign(new RangeError('Guild Rally Point location reading is stale.'), { status: 400 });
  return Object.freeze({
    ...
    precisionMeters: Math.max(1, Math.ceil(accuracyMeters)),
...
export function normalizeHubLocation(input = {}, now = Date.now()) {
  ...
  if (!['rounded', 'precise'].includes(precisionRequest)) throw Object.assign(new RangeError('Guild public precision must be rounded or precise.'), { status: 400 });
  const precise = precisionRequest === 'precise', coordinateDecimals = precise ? 6 : 3;
  if (!Number.isFinite(latitude) || latitude < -90 || latitude > 90) throw Object.assign(new RangeError('Guild latitude is invalid.'), { status: 400 });
  if (!Number.isFinite(longitude) || longitude < -180 || longitude > 180) throw Object.assign(new RangeError('Guild longitude is invalid.'), { status: 400 });
  if (!Number.isFinite(accuracyMeters) || accuracyMeters <= 0 || accuracyMeters > 5_000) throw Object.assign(new RangeError('Guild location accuracy must be between 1 and 5,000 meters.'), { status: 400 });
  if (precise && accuracyMeters > 250) throw Object.assign(new RangeError('A precise public Guild pin requires a location reading within 250 meters.'), { status: 400 });
  if (!Number.isFinite(capturedMs) || Math.abs(now - capturedMs) > 10 * 60 * 1000) throw Object.assign(new RangeError('Guild location reading is stale.'), { status: 400 });
  return Object.freeze({
...
function nodePage(manifest) { const esc = value => String(value).replace(/[&<>"']/g, ch => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[ch])); const caps = manifest.capabilities.map(cap => `<li>${esc(cap)}</li>`).join(''); return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(manifest.displayName)} · Civweave Guild</title><style>body{font:16px system-ui;background:#11152b;color:#f5f2ea;max-width:760px;margin:auto;padding:48px 22px}main{border:1px solid #6f77a8;border-radius:24px;padding:28px;background:#171d3a}code{color:#9de3cf}a{color:#f5ca77}</style></head><body><main><p>Civweave public Guild</p><h1>${esc(manifest.displayName)}</h1><p><code>${esc(manifest.nodeId)}</code></p><p>Runtime: ${esc(manifest.runtime)}</p><h2>Capabilities</h2><ul>${caps}</ul><p><a href="/api/ai/node/manifest">Guild manifest</a> · <a href="/api/ai/node/capacity">Capacity</a> · <a href="/api/node/health">Health</a></p></main></body></html>`; }
...
export class CivweaveCloudNode {
  ...
  async updateHubLocation(nodeId, input, claimKey) {
    if (!/^[A-Za-z0-9_-]{40,200}$/.test(clean(claimKey, 220))) throw Object.assign(new Error('A valid Guild location claim key is required.'), { status: 401 });
    ...
    if (stored?.ownerKeyHash && stored.ownerKeyHash !== ownerKeyHash) throw Object.assign(new Error('This Guild location is already paired to another Guildkeeper browser.'), { status: 409 });
    const normalized = normalizeHubLocation(input), rallyPoint = stored?.rallyPoint?.schema === 'civweave.guild-rally-point.v1' ? stored.rallyPoint : null;
    const location = Object.freeze({ ...normalized, ...(rallyPoint ? { rallyPoint } : {}) }), manifest = Object.freeze({ ...(await this.manifest(nodeId)), location, updatedAt: location.syncedAt });
  ...
  async updateGuildRallyPoint(nodeId, input, claimKey) {
    if (!/^[A-Za-z0-9_-]{40,200}$/.test(clean(claimKey, 220))) throw Object.assign(new Error('A valid Guild location claim key is required.'), { status: 401 });
    const ownerKeyHash = await sha256Hex(`civweave.hub-location-owner.v1\n${claimKey}`), stored = await this.state.storage.get('hub-location');
    if (!stored?.ownerKeyHash) throw Object.assign(new Error('Place this Guild on the Guild Map before setting its Rally Point.'), { status: 409 });
    if (stored.ownerKeyHash !== ownerKeyHash) throw Object.assign(new Error('This Guild is paired to another Guildkeeper browser.'), { status: 409 });
    const rallyPoint = normalizeGuildRallyPoint(input), { ownerKeyHash: _ownerKeyHash, ...storedLocation } = stored;
    const location = Object.freeze({ ...storedLocation, rallyPoint }), manifest = Object.freeze({ ...(await this.manifest(nodeId)), location, updatedAt: rallyPoint.updatedAt });
    await this.state.storage.put({ 'hub-location': { ...location, ownerKeyHash }, manifest });
    return { rallyPoint, location, manifest };
  }
  ...
  async fetch(request) {
    ...
    if (request.method === 'POST' && url.pathname === '/internal/rally-point') {
      try { return json({ ok: true, ...(await this.updateGuildRallyPoint(nodeId, await request.json().catch(() => ({})), request.headers.get('x-civweave-location-key'))) }); }
      catch (error) { return json({ ok: false, error: String(error?.message || error) }, Number.isSafeInteger(error?.status) ? error.status : 400); }
    }
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'OPTIONS' && ['/api/fabric/location', '/api/fabric/rally-point'].includes(url.pathname) && !nodeFromHost) return new Response(null, { status: 204, headers: guildSetupCors });
    ...
    if (request.method === 'POST' && url.pathname === '/api/fabric/location' && !nodeFromHost) {
      ...
      if (!/^[A-Za-z0-9_-]{40,200}$/.test(claimKey)) return json({ ok: false, error: 'location-claim-key-required' }, 401, guildSetupCors);
      ...
      if (!capacityResponse.ok) return json({ ok: false, error: capacity.error || 'capacity-unavailable' }, capacityResponse.status, guildSetupCors);
      if (requested.length < 1 || requested.length > 3 || requested.some(nodeId => !registered.has(nodeId))) return json({ ok: false, error: 'registered-node-selection-required' }, 400, guildSetupCors);
      const nodes = [];
      for (const nodeId of requested) {
        ...
        if (!nodeResponse.ok) return json({ ok: false, error: result.error || 'node-location-sync-failed', nodeId, syncedNodeIds: nodes.map(item => item.nodeId) }, nodeResponse.status, guildSetupCors);
        nodes.push({ nodeId, location: result.location, claimed: result.claimed, core: await syncCore(env, result.manifest) });
      }
      return json({ schema: 'civweave.hub-location-sync.v1', ok: true, nodeIds: requested, location: nodes[0].location, nodes }, 200, guildSetupCors);
    }
    if (request.method === 'POST' && url.pathname === '/api/fabric/rally-point' && !nodeFromHost) {
      const claimKey = clean(request.headers.get('x-civweave-location-key'), 220), input = await request.json().catch(() => ({}));
      if (!/^[A-Za-z0-9_-]{40,200}$/.test(claimKey)) return json({ ok: false, error: 'location-claim-key-required' }, 401, guildSetupCors);
      const requested = [...new Set((Array.isArray(input.nodeIds) ? input.nodeIds : []).map(normalizeNodeId).filter(Boolean))];
      const capacityResponse = await callCapacity(env, '/snapshot'), capacity = await capacityResponse.json().catch(() => ({})), registered = new Set(Array.isArray(capacity.hostNodeIds) ? capacity.hostNodeIds : []);
      ...
      const nodes = [];
      for (const nodeId of requested) {
        const nodeResponse = await callNode(env, nodeId, new Request(`https://${nodeId}.${domain}/internal/rally-point`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-civweave-location-key': claimKey }, body: JSON.stringify(input) }), '/internal/rally-point'), result = await nodeResponse.json().catch(() => ({}));
        if (!nodeResponse.ok) return json({ ok: false, error: result.error || 'guild-rally-point-sync-failed', nodeId, syncedNodeIds: nodes.map(item => item.nodeId) }, nodeResponse.status, guildSetupCors);
        nodes.push({ nodeId, rallyPoint: result.rallyPoint, core: await syncCore(env, result.manifest) });
      }
      return json({ schema: 'civweave.guild-rally-point-sync.v1', ok: true, nodeIds: requested, rallyPoint: nodes[0].rallyPoint, nodes }, 200, guildSetupCors);
    }
    ...
    return json({ schema: 'civweave.node-cloud-fabric.v1', ok: true, message: 'Civweave Cloudflare Guild fabric', nodeDomain: domain, nodeRoute: `https://<node-id>.${domain}`, capacityRoute: '/api/fabric/capacity', locationRoute: '/api/fabric/location', rallyPointRoute: '/api/fabric/rally-point', moneyAuthority: 'cloudflare-core' });
  }
};