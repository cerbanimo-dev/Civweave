...
export function buildCloudNodeManifest(nodeId, input = {}, domain = 'nodes.commonweave.earth') {
  ...
  return Object.freeze({
    ...
    publicKey: clean(input.publicKey, 20000) || null,
    keyId: clean(input.keyId, 120) || null,
    capabilities,
    services: Array.isArray(input.services) ? input.services : [],
    transport: { publicHttps: true, webSocket: true, tunnelRequired: false, relayRequired: false },
    security: {
...
export class CivweaveCloudNode {
  ...
  async identity() {
    let identity = await this.state.storage.get('identity');
    if (identity?.privateJwk?.d && identity?.publicKey) return identity;
    const pair = await crypto.subtle.generateKey({ name: 'Ed25519' }, true, ['sign', 'verify']);
    const privateJwk = await crypto.subtle.exportKey('jwk', pair.privateKey);
    const spki = await crypto.subtle.exportKey('spki', pair.publicKey);
    const publicKey = derToPem(spki);
    const fingerprint = await sha256Hex(publicKey);
    identity = {
      schema: 'civweave.node-cloud-identity.v1',
      privateJwk,
      publicKey,
      keyId: `node-${fingerprint.slice(0, 12)}`,
      fingerprint,
  ...
  async manifest(nodeId) {
    ...
    if (stored) {
      if (stored.publicKey === identity.publicKey && stored.keyId === identity.keyId) return stored;
      const repaired = buildCloudNodeManifest(nodeId, { ...stored, publicKey: identity.publicKey, keyId: identity.keyId }, this.env.NODE_DOMAIN || 'nodes.commonweave.earth');
      await this.state.storage.put('manifest', repaired);
      return repaired;
    }
    const created = buildCloudNodeManifest(nodeId, { publicKey: identity.publicKey, keyId: identity.keyId }, this.env.NODE_DOMAIN || 'nodes.commonweave.earth');
    await this.state.storage.put('manifest', created);
    return created;
  }

  async sign(domain, timestamp, raw) {
    const identity = await this.identity();
    const privateKey = await crypto.subtle.importKey('jwk', identity.privateJwk, { name: 'Ed25519' }, false, ['sign']);
    const message = concatBytes(enc.encode(`${domain}\n${timestamp}\n`), raw instanceof Uint8Array ? raw : enc.encode(String(raw)));
    const signature = await crypto.subtle.sign({ name: 'Ed25519' }, privateKey, message);
    return { keyId: identity.keyId, signature: b64url(signature) };
  }
  ...
  async fetch(request) {
    ...
    if (request.method === 'GET' && (url.pathname === '/api/node/manifest' || url.pathname === '/api/ai/node/manifest')) {
      return json({ manifest, marketplace: { schema: 'civweave.node-ai-http.v1', enabled: true, nodeId, runtime: CLOUD_NODE_RUNTIME } });
    }
    if (request.method === 'GET' && url.pathname === '/api/node/health') {
      return json({ schema: 'civweave.node-health.v1', ok: true, nodeId, runtime: CLOUD_NODE_RUNTIME, connections: this.state.getWebSockets?.().length || 0, updatedAt: manifest.updatedAt });
    }
    if (request.method === 'POST' && url.pathname === '/api/ai/node/live/challenge') {
      const input = await request.json().catch(() => ({}));
      if (clean(input.nodeId, 180) !== nodeId || !/^[A-Za-z0-9_-]{32,200}$/.test(clean(input.challenge, 300))) return json({ error: 'invalid-challenge' }, 400);
      const signed = await this.sign('civweave.node-live-challenge.v1', 0, enc.encode(`${nodeId}\n${input.challenge}`));
      return json({ schema: 'civweave.node-ai-live-commerce.v1', nodeId, keyId: signed.keyId, signature: signed.signature });
    }
    if (request.method === 'POST' && url.pathname === '/api/ai/node/live/payments/webhook') {
      const body = await request.text();
      const events = await this.state.storage.get('payment-events') || [];
      let event;
      ...
      if (!events.some(item => item.id === event.id)) {
        events.push({ id: event.id, type: event.type, receivedAt: new Date().toISOString(), payload: event });
        while (events.length > 200) events.shift();
        await this.state.storage.put('payment-events', events);
      }
      return json({ ok: true, stored: true, nodeId, eventId: event.id });
    }
    ...
    if (request.method === 'POST' && url.pathname === '/internal/configure') {
      const identity = await this.identity();
      const next = buildCloudNodeManifest(nodeId, { ...(await request.json()), publicKey: identity.publicKey, keyId: identity.keyId }, this.env.NODE_DOMAIN || 'nodes.commonweave.earth');
      await this.state.storage.put('manifest', next);
      return json({ ok: true, manifest: next });
    }
    if (request.method === 'POST' && url.pathname === '/internal/sign-request') {
      const raw = await request.text();
      const timestamp = Math.floor(Date.now() / 1000);
      const signed = await this.sign('civweave.node-money-edge-request.v1', timestamp, enc.encode(raw));
      return json({ keyId: signed.keyId, signatureHeader: `t=${timestamp},kid=${signed.keyId},sig=${signed.signature}` });
    }
...
async function coreJson(env, pathname, init = {}) {
  const response = await env.CORE.fetch(`https://civweave-core.internal${pathname}`, init);
...
export default {
  async fetch(request, env) {
    ...
    if (payoutMatch && request.method === 'POST' && !nodeFromHost) {
      if (!await secretEqual(request.headers.get('authorization')?.replace(/^Bearer\s+/i, ''), env.NODE_FABRIC_OPERATOR_TOKEN)) return json({ ok: false, error: 'forbidden' }, 403);
      const nodeId = normalizeNodeId(payoutMatch[1]);
      const manifestResponse = await callNode(env, nodeId, new Request(`https://${nodeId}.${domain}/api/ai/node/manifest`, { method: 'GET' }), '/api/ai/node/manifest');
      const envelope = await manifestResponse.json();
      const manifest = envelope.manifest;
      const input = await request.json().catch(() => ({}));
      const identity = { nodeId: manifest.nodeId, operatorId: manifest.operatorId, callbackUrl: manifest.publicOrigin };
      ...
      const registration = (await coreJson(env, '/api/money-edge/nodes/register', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ ...identity, enrollmentGrant: enrollment.token, email: input.email || undefined, country: input.country || undefined })
      })).registration;
