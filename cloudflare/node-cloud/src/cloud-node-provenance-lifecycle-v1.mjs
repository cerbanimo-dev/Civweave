...
export class CivweaveCloudNode extends BaseCloudNode {
  ...
  async signedGuildReviewReceipt(nodeId,auditRequest,review={},reviewedAt){
    const identity=await this.identity(),method=clean(review.reviewMethod,40),human=method==='human-tribunal',commitments=human?(auditRequest.tribunalVotes||[]).map(vote=>clean(vote?.endorsementCommitment,240)).filter(Boolean):[],commitmentSetHash=await endorsementCommitmentSetHash(commitments),base=reviewReceiptSignable({guildId:nodeId,sampleId:auditRequest.sampleId,creationReceipt:{sessionId:auditRequest.receipt?.sessionId,headHash:auditRequest.receipt?.headHash,receiptHash:auditRequest.receipt?.receiptHash,origin:auditRequest.receipt?.origin},reviewOutcome:review.outcome,reviewMethod:method,voteCount:human?Number(review.voteCount)||0:0,endorsementCount:human?Number(review.endorsementCount)||0:0,endorsementCommitmentSetHash:commitmentSetHash,reviewedAt,guildIdentity:{keyId:identity.keyId,publicKey:identity.publicKey}}),timestamp=Math.floor(Date.parse(reviewedAt)/1000),signed=await this.sign(REVIEW_RECEIPT_DOMAIN,timestamp,reviewReceiptCanonical(base));
  ...
  signedReceiptReference(receipt){return receipt?Object.freeze({schema:receipt.schema,guildId:receipt.guildId,sampleId:receipt.sampleId,reviewMethod:receipt.reviewMethod,keyId:receipt.signature?.keyId}):null}
  async creatorProvenanceInternal(request,nodeId){
    const url=new URL(request.url),storage=this.state.storage;
    const lifecycleRoute=['/internal/creator-provenance/audit/requests','/internal/creator-provenance/audit/evidence','/internal/creator-provenance/audit/appeal','/internal/creator-provenance/audit/human/pending','/internal/creator-provenance/audit/human/finding'].includes(url.pathname);
    ...
    try{
      ...
      if(url.pathname==='/internal/creator-provenance/audit/evidence'&&request.method==='POST'){
        const copy=request.clone(),response=await super.creatorProvenanceInternal(request,nodeId),payload=await response.json().catch(()=>({}));if(!response.ok||payload.reviewStatus!=='reviewed')return json(payload,response.status);
        const input=await copy.json().catch(()=>({})),auditRequest=await this.requestBySample(storage,input.sampleId);if(!auditRequest||auditRequest.status!=='reviewed')return json(payload,response.status);if(auditRequest.signedReviewReceipt)return json({...payload,signedReviewReceipt:auditRequest.signedReviewReceipt,request:this.publicReviewRequest(auditRequest)},response.status);
        const finding=auditRequest.finding||payload.finding,reviewMethod=finding?.reviewerKind==='model'?'model':'deterministic',reviewedAt=auditRequest.reviewedAt||new Date().toISOString(),signedReviewReceipt=await this.signedGuildReviewReceipt(nodeId,auditRequest,{outcome:finding?.outcome,reviewMethod},reviewedAt),reviewRecord={...(auditRequest.reviewRecord||{}),signedReceipt:this.signedReceiptReference(signedReviewReceipt)},updated=await updateAuditRequest(storage,auditRequest.deviceId,auditRequest.sampleId,{signedReviewReceipt,reviewRecord,originImmutable:true});
        return json({...payload,signedReviewReceipt,request:this.publicReviewRequest(updated)},response.status);
      }
      ...
      if(url.pathname==='/internal/creator-provenance/audit/human/finding'&&request.method==='POST'){
        ...
        let tribunal;try{tribunal=addTribunalVote({votes:auditRequest.tribunalVotes||[]},endorsement.finding,{request:auditRequest,reviewerId:memberUserId,endorsement:protectedEndorsement,endorsementCommitment})}catch(error){throw Object.assign(error,{status:/already voted/i.test(String(error.message))?409:400})}const decision=tribunalDecision(tribunal.votes),patch={tribunalVotes:tribunal.votes,tribunalDecision:decision,status:decision.status,originImmutable:true};let signedReviewReceipt=null;if(decision.status==='reviewed'){const reviewedAt=new Date().toISOString(),receiptContext={...auditRequest,tribunalVotes:tribunal.votes};signedReviewReceipt=await this.signedGuildReviewReceipt(nodeId,receiptContext,{outcome:decision.outcome,reviewMethod:'human-tribunal',voteCount:decision.voteCount,endorsementCount:decision.endorsementCount},reviewedAt);patch.finding=decision.finding;patch.reviewRecord={...additiveReviewRecord(auditRequest.receipt,decision.finding),tribunal:{voteCount:decision.voteCount,endorsementCount:decision.endorsementCount,individualReviewerIdentitiesPublic:false},signedReceipt:this.signedReceiptReference(signedReviewReceipt)};patch.signedReviewReceipt=signedReviewReceipt;patch.reviewedAt=reviewedAt;patch.reviewReason='Independent Guild human tribunal reached a device-endorsed, Guild-signed decision.'}const updated=await updateAuditRequest(storage,auditRequest.deviceId,auditRequest.sampleId,patch);return json({ok:true,nodeId,status:decision.status,decision,finding:decision.finding,endorsementAccepted:true,endorsementCommitment,signedReviewReceipt,request:this.publicReviewRequest(updated)});
      }
