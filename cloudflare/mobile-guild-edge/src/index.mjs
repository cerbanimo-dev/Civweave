...
const WORKER_VERSION='1.1.0-full-guild-cloud';
...
export class CivweaveGuildEdgeState extends DurableObject{
  ...
  claim({guildId,displayName,foundingDeviceId,membershipKeyHash}={}){const existingGuild=this.meta('guildId'),existingDevice=this.meta('foundingDeviceId'),existingHash=this.meta('membershipKeyHash');if(existingGuild){if(existingGuild!==guildId||existingDevice!==foundingDeviceId||existingHash!==membershipKeyHash)return{ok:false,status:409,error:'This Cloudflare Guild is already claimed by a different Guild or founding device.'};return{...this.status(),idempotent:true}}const claimedAt=now();this.putMeta('guildId',guildId);this.putMeta('displayName',displayName);this.putMeta('foundingDeviceId',foundingDeviceId);this.putMeta('membershipKeyHash',membershipKeyHash);this.putMeta('claimedAt',claimedAt);return{...this.status(),idempotent:false}}
...
export class CivweaveGuildNodeState extends DurableObject{
  ...
  meta(key){const row=[...this.sql.exec('SELECT value FROM meta WHERE key = ?',key)][0];return row?JSON.parse(row.value):null}
  putMeta(key,value){this.sql.exec('INSERT OR REPLACE INTO meta (key,value) VALUES (?,?)',key,JSON.stringify(value))}
  configure({guildId,nodeId,displayName,publicOrigin}={}){const existingNode=this.meta('nodeId'),existingGuild=this.meta('guildId');if(existingNode&&(existingNode!==nodeId||existingGuild!==guildId))throw Object.assign(new Error('This Cloudflare node is already bound to another Guild node.'),{status:409});const createdAt=this.meta('createdAt')||now();this.putMeta('guildId',guildId);this.putMeta('nodeId',nodeId);this.putMeta('displayName',displayName);this.putMeta('publicOrigin',publicOrigin);this.putMeta('createdAt',createdAt);this.putMeta('updatedAt',now());return this.manifest()}
  ...
  manifest(){return{ok:Boolean(this.meta('nodeId')),manifest:{schema:NODE_MANIFEST_SCHEMA,guildId:this.meta('guildId'),nodeId:this.meta('nodeId'),displayName:this.meta('displayName'),publicOrigin:this.meta('publicOrigin'),runtime:'cloudflare-workers-ai',capabilities:['guild-relay','workers-ai','signed-object-mesh','creator-provenance'],createdAt:this.meta('createdAt'),updatedAt:this.meta('updatedAt')}}}
}
...
async function buildCloudFabric(env,guildStatus,origin){const capacity=await capacityStub(env).bootstrap({guildId:guildStatus.guildId,displayName:guildStatus.displayName,origin});for(const node of capacity.starterNodes){await nodeStub(env,node.nodeId).configure({guildId:guildStatus.guildId,nodeId:node.nodeId,displayName:`${guildStatus.displayName} · ${node.nodeId.slice(-1).toUpperCase()}`,publicOrigin:node.publicOrigin})}return{schema:FABRIC_SCHEMA,status:'ready',capacityOrigin:`${origin}/api/fabric/capacity`,manifestOrigin:`${origin}/api/fabric/manifest`,aiEnabled:Boolean(env.AI),starterNodes:capacity.starterNodes}}
...
export default{
  async fetch(request,env,ctx){
    ...
    try{
      ...
      if(request.method==='GET'&&url.pathname==='/api/health'){const status=await guildState.status(),infrastructure=await fabricStatus(env,url.origin);return json({ok:true,service:'civweave-guild-cloud',version:WORKER_VERSION,claimed:status.claimed,guildId:status.guildId,infrastructure})}
      ...
      if(request.method==='POST'&&url.pathname==='/api/guild/claim'){
        ...
        const claimed=await guildState.claim({guildId,displayName,foundingDeviceId,membershipKeyHash:await membershipHash(membershipKey)});if(claimed?.ok===false)return json({ok:false,error:claimed.error},claimed.status||409);const infrastructure=await buildCloudFabric(env,claimed,url.origin);return json({...claimed,primaryOrigin:url.origin,primaryGateway:url.origin,infrastructure});
      }
      if(routed){
