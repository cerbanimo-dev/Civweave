...
const WORKER_VERSION='1.0.1-mobile-guild-edge';
...
export class CivweaveGuildEdgeState extends DurableObject{
  ...
  claim({guildId,displayName,foundingDeviceId,membershipKeyHash}={}){const existingGuild=this.meta('guildId'),existingDevice=this.meta('foundingDeviceId'),existingHash=this.meta('membershipKeyHash');if(existingGuild){if(existingGuild!==guildId||existingDevice!==foundingDeviceId||existingHash!==membershipKeyHash)return{ok:false,status:409,error:'This Cloudflare edge is already claimed by a different Guild or founding device.'};return{...this.status(),idempotent:true}}const claimedAt=now();this.putMeta('guildId',guildId);this.putMeta('displayName',displayName);this.putMeta('foundingDeviceId',foundingDeviceId);this.putMeta('membershipKeyHash',membershipKeyHash);this.putMeta('claimedAt',claimedAt);return{...this.status(),idempotent:false}}
  ...
  async fetch(request,env){
    ...
    try{
      ...
      if(request.method==='POST'&&url.pathname==='/api/guild/claim'){
        ...
        const claimed=await stub.claim({guildId,displayName,foundingDeviceId,membershipKeyHash:await membershipHash(membershipKey)});if(claimed?.ok===false)return json({ok:false,error:claimed.error},claimed.status||409);return json({...claimed,primaryOrigin:url.origin,primaryGateway:url.origin});
      }
