import accountEdge, { CivweaveAccountNode, accountNodePath } from './index.mjs';
...
export { CivweaveAccountNode, CivweaveCapacityAccount };
...
function isGenerate(pathname) {
  return pathname === '/api/ai/node/generate';
}

export default {
  async fetch(request, env, ctx) {
    if (request.method !== 'POST') return accountEdge.fetch(request, env, ctx);
    ...
    return accountEdge.fetch(request, env, ctx);
  },
};
