...
export class CivweaveAccountNode extends CivweaveCloudNode {
  async accountOrigin() { return trimOrigin(await this.state.storage.get(ACCOUNT_ORIGIN_KEY)); }
  async manifest(nodeId) {
    ...
    const repaired = Object.freeze({ ...manifest, publicOrigin, transport: Object.freeze({ ...manifest.transport, accountEdgePath: true }), updatedAt: new Date().toISOString() });
  ...
  async fetch(request) {
    ...
    if (response.ok && nodeId && request.method === 'GET' && url.pathname === '/' && response.headers.get('content-type')?.includes('text/html')) {
      const manifest = await this.manifest(nodeId), headers = new Headers(response.headers);
      headers.delete('content-length');
      return new Response(scopeAccountNodeHtml(await response.text(), manifest.publicOrigin), { status: response.status, statusText: response.statusText, headers });
    }
    if (response.ok && nodeId && request.method === 'POST' && url.pathname === '/internal/configure') return json({ ok: true, manifest: await this.manifest(nodeId) }, response.status);
    return response;
  }
}
...
export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url), accountOrigin = url.origin, routed = accountNodePath(url.pathname);
    if (routed) {
    ...
    if (/^\/api\/fabric\/nodes\/[a-zA-Z0-9-]+\/payouts$/.test(url.pathname)) return json({ ok: false, error: 'central-money-edge-required', message: 'Account-local starter nodes never receive the central payment authority secret.' }, 409);
    const headers = new Headers(request.headers);
    headers.set('x-civweave-account-edge-origin', accountOrigin);
