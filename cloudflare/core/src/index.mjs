...
export class CivweaveCoreIdentity {
  ...
  async identity() {
    ...
    if (stored?.privateJwk?.d && stored?.publicKeyPem) return stored;
    ...
    stored = {
      ...
      keyId: `cerbanimo-cloudflare-${fingerprint.slice(0, 12)}`,
      algorithm: 'Ed25519',
      publicKeyPem,
      privateJwk,
      fingerprint,
  ...
  async fetch(request) {
    ...
    if (request.method === 'POST' && url.pathname === '/sign') {
      ...
      if (input.domain !== 'civweave.money-edge-event.v1') return json({ error: 'signing-domain-not-allowed' }, 403);
      const timestamp = Number(input.timestamp);
      if (!Number.isSafeInteger(timestamp)) return json({ error: 'invalid-signing-timestamp' }, 400);
      const raw = fromB64url(clean(input.payload, 1000000));
      const message = concatBytes(encoder.encode(`${input.domain}\n${timestamp}\n`), raw);
...
export async function verifyStripeWebhook({ rawBody, signatureHeader, secret, now = Date.now(), toleranceSeconds = 300 }) {
  ...
  const expectedBytes = await crypto.subtle.sign('HMAC', key, encoder.encode(`${timestamp}.${rawBody}`));
...
async function stripeWebhook(request, env, edge) {
  ...
  const inserted = await env.DB.prepare(`INSERT OR IGNORE INTO stripe_events(event_id,event_type,livemode,payload_json,received_at)
    VALUES(?1,?2,?3,?4,?5)`)
    .bind(clean(event.id, 180), clean(event.type, 180), event.livemode ? 1 : 0, rawBody, nowIso()).run();
  if (Number(inserted?.meta?.changes ?? inserted?.changes ?? 0) === 0) return json({ ok: true, duplicate: true, received: event.id });
  try {
    const result = await edge.handleProviderEvent(event);
...
export async function handleMoneyEdgeRequest(request, env, options = {}) {
  ...
  if (pathname === '/api/money-edge/status' && request.method === 'GET') {
    return json({ moneyEdge: edge.readiness(), authority: 'cloudflare-core', canonical: true });
  }
  if (pathname === '/api/money-edge/trust' && request.method === 'GET') {
    try { return json({ trust: await edge.trustDocument(url.origin) }); }
    catch (error) { const safe = moneyEdgeError(error); return json(safe.body, safe.status); }
  }
  try {
    if (pathname === '/api/money-edge/enrollment/start' && request.method === 'POST') {
      return json({ enrollment: await edge.createEnrollmentGrant(await request.json()) }, 201);
    }
    if (pathname === '/api/money-edge/nodes/register' && request.method === 'POST') {
    ...
    if (statusMatch && request.method === 'GET') {
      ...
      return json({ operator: await edge.operatorStatus(decodeURIComponent(statusMatch[1]), raw, request.headers.get('x-civweave-node-signature')) });
    }
    if (pathname === '/api/money-edge/topups' && request.method === 'POST') {
      const rawText = await request.text();
      const input = JSON.parse(rawText || '{}');
      return json({ topup: await edge.createTopUp(input, encoder.encode(rawText), request.headers.get('x-civweave-node-signature')) }, 201);
    }
    const refundMatch = pathname.match(/^\/api\/money-edge\/topups\/([^/]+)\/refund$/);
    if (refundMatch && request.method === 'POST') {
      const rawText = await request.text();
      const input = JSON.parse(rawText || '{}');
      return json({ refund: await edge.refundTopUp({
        nodeId: input.nodeId,
        topupId: decodeURIComponent(refundMatch[1]),
        amountCents: input.amountCents
      }, encoder.encode(rawText), request.headers.get('x-civweave-node-signature')) }, 202);
    }
    if ((pathname === '/api/money-edge/webhooks/stripe' || pathname === '/api/stripe/webhook') && request.method === 'POST') {
      return stripeWebhook(request, env, edge);
    }
...
async function routeApi(request, env) {
  const url = new URL(request.url);
  const money = await handleMoneyEdgeRequest(request, env);
  ...
  if (request.method === 'GET' && url.pathname === '/api/trust') {
    const edge = new CloudflareMoneyEdge(env);
    return json({ trust: await edge.trustDocument(url.origin) });
  }
  if (request.method === 'GET' && url.pathname === '/api/launch-topology') {
    return json({ ...launchTopology, platformFeeBps: Number(env.CIVWEAVE_PLATFORM_FEE_BPS || 1500) });
  }
  if (request.method === 'GET' && url.pathname === '/api/nodes') {
    return json({ schema: 'civweave.node-directory.v1', nodes: await listNodes(env, url.searchParams.get('limit')) });
  }
...
export default {
  ...
  async scheduled(_controller, env) {
    const edge = new CloudflareMoneyEdge(env);
    if (!edge.readiness().integrationDoorReady) return;
    await edge.deliverPending({ limit: 100 });
  }
};
