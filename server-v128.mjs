...
const runtimePath = path.join(rootDir, '.civweave-server-v128.runtime.mjs');
const VERSION = '1.0.28';
const BUILD = '1.0.28-parity-ledger-foundation';
let source = await fsp.readFile(sourcePath, 'utf8');

function replaceRequired(before, after, label) {
  if (!source.includes(before)) throw new Error(`Civweave v1.0.28 patch could not find ${label}`);
...
const CW_VERSION = '1.0.28';
const CW_BUILD = '1.0.28-parity-ledger-foundation';
const cwBootLogs = [];
...
async function cwServeLoom(req, res, originalPathname, requestId) {
  ...
  if (originalPathname === '/lite' || originalPathname === '/lite/' || originalPathname === '/lite/index.html') {
    const text = await fsp.readFile(path.join(PUBLIC_DIR, 'app', 'lite-v128.html'), 'utf8');
  ...
  if (liteSource) {
    ...
    const injection = '<base href="/app/services/' + service + '/"><script>window.__CIVWEAVE_LITE_SOURCE__=true;try{if(navigator.serviceWorker)navigator.serviceWorker.register=async()=>({scope:location.origin+"/lite/source-disabled/"})}catch{}</script>';
  ...
  if (originalPathname === '/loom/version.json' || originalPathname === '/lite/version.json') {
    cwSend(res, 200, JSON.stringify({schema:'civweave.version.v1',version:CW_VERSION,build:CW_BUILD,channel:'main',parityLedger:'/app/shared/civweave-parity-ledger.json',updatedAt:new Date().toISOString()},null,2), 'application/json; charset=utf-8', requestId); return true;
  }
  return false;
}
`;
