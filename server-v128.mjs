...
const runtimePath = path.join(rootDir, '.commonweave-server-v128.runtime.mjs');
const VERSION = '1.0.28';
const BUILD = '1.0.28-parity-ledger-foundation';
let source = await fsp.readFile(sourcePath, 'utf8');

function replaceRequired(before, after, label) {
  if (!source.includes(before)) throw new Error(`Commonweave v1.0.28 patch could not find ${label}`);
...
async function cwServeLoom(req, res, originalPathname, requestId) {
  ...
  if (liteSource) {
    ...
    const injection = '<base href="/app/services/' + service + '/"><script>window.__COMMONWEAVE_LITE_SOURCE__=true;try{if(navigator.serviceWorker)navigator.serviceWorker.register=async()=>({scope:location.origin+"/lite/source-disabled/"})}catch{}</script>';
