#!/usr/bin/env python3
"""Loopback-only Commonweave static server with narrow update/model proxying."""
...
class CommonweaveHandler(http.server.SimpleHTTPRequestHandler):
    server_version = "CommonweaveLoopback/1.0"
    ...
    def fetch_deferred_asset_if_needed(self) -> None:
        ...
        request = urllib.request.Request(self.source + pathname, headers={"X-Commonweave-Package": "mobile-model-on-demand"})
        try:
    ...
    def proxy_api(self) -> None:
        ...
        try:
            ...
            payload = (f'{{"error":"Local Commonweave proxy failed: {str(error)}"}}').encode()
...
def main() -> None:
    ...
    os.chdir(root)
    handler = lambda *a, **k: CommonweaveHandler(*a, directory=str(root), source=args.source, **k)
    server = http.server.ThreadingHTTPServer((args.host, args.port), handler)
    print(f"Commonweave loopback server: http://{args.host}:{args.port}/", flush=True)
