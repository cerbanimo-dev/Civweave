...
check('downloads route carries explicit host and finder context',topbar.includes("current.get('host')||current.get('hostNode')")&&topbar.includes("['node','nodeId','node_id','finder','federationFinder']")&&topbar.includes("Manage Civweave installs, downloads, and host node connection"));
