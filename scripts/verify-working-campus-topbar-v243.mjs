...
check('downloads route carries explicit host and finder context',topbar.includes("['host','hostNode','node','nodeId','node_id','finder','federationFinder']")&&topbar.includes("Manage Civweave installs, downloads, and host node connection"));
...
console.log(JSON.stringify({ok:true,version,revision:'working-campus-topbar-v243-downloads-entry-v1',checks:checks.length,stickyTop:true,chatSafe:true,downloadsManager:'/app/index.html?manage=downloads',mapHandshake:['federation-finder-v1.7.2','same-origin-pwa-navigation','explicit-remote-node-fallback','direct-api-fallback','register-fallback','event-fallback','same-origin-route-fallback']},null,2));
