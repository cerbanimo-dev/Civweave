...
assert.ok(runtimeSource.includes("VERSION='1.0.111-local-ai-runtime-v300-windows-memory'")&&runtimeSource.includes("LEGACY_VERSION='1.0.87-local-ai-runtime-v287-gemma4-mobile'")&&runtimeSource.includes("REVISION='1.0.88-local-ai-runtime-v283-small-model-fast-path'")&&runtimeSource.includes('promptTokenBudget:requestOptions.promptTokenBudget')&&runtimeSource.includes('smallModelFastPath:true')&&runtimeSource.includes('wasmPerformanceDiagnostics:true')&&runtimeSource.includes('stalledWebGPUFallback:true')&&runtimeSource.includes('webgpuSessionQuarantine:true')&&runtimeSource.includes('serializedInference:true')&&runtimeSource.includes('coldStartBenchmarkOptIn:true'));
...
assert.ok(bootstrapSource.includes("REVISION='1.0.114-local-ai-bootstrap-v301-bounded-startup'")&&bootstrapSource.includes("RUNTIME_REVISION='1.0.88-local-ai-runtime-v283-small-model-fast-path'")&&bootstrapSource.includes('/app/local-ai/small-model-policy-v283.js')&&bootstrapSource.includes('/app/local-ai/primary-route-v283.js')&&bootstrapSource.includes('/app/local-ai/runtime-v266.js?v=1.0.111-v300-windows-memory')&&bootstrapSource.includes('componentCompatibility')&&bootstrapSource.includes('capability-contract-v301')&&bootstrapSource.includes('continuationValidation:true')&&bootstrapSource.includes('wasmPerformanceDiagnostics:true')&&bootstrapSource.includes('stalledWebGPUFallback:true')&&bootstrapSource.includes('boundedStartup:true'));
...
console.log(JSON.stringify({ok:true,revision:'small-model-fast-path-v301-bounded-startup',features:{embeddedLocalPrimary:true,adaptiveOutputSlices:true,clippingValidation:true,boundedContinuation:true,streamedContinuation:true,thinkingProfilesPreserved:true,wasmPerformancePreserved:true,stalledWebGPUFallback:true,webgpuSessionQuarantine:true,serializedInference:true,coldStartBenchmarkOptIn:true,boundedStartup:true,componentCompatibility:'capability-contract-v301'}},null,2));
