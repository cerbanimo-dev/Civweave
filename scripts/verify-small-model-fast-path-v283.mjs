...
const completedGuide=JSON.stringify({answer:"Then we'll plan different areas for growing, storing, and composting.",choice:{mode:'Plan',system:'civweave',room:'',nextAction:'Choose the site.'},assumptions:[],requiresConsent:false,confidence:.8,questDraft:null});
assert.equal(policy.validateCompletion({},clippedGuide,{structured:true,jsonValid:true}).reason,'structured-answer-abrupt','valid JSON with a clipped answer must continue');
...
const compatibleBootstrapRevision=bootstrapSource.includes("REVISION='1.0.115-local-ai-bootstrap-v302-session-handoff'")||bootstrapSource.includes("REVISION='1.0.117-local-ai-bootstrap-v306-browser-tools'");
assert.ok(compatibleBootstrapRevision&&bootstrapSource.includes("RUNTIME_REVISION='1.0.88-local-ai-runtime-v283-small-model-fast-path'")&&bootstrapSource.includes('/app/local-ai/small-model-policy-v283.js')&&bootstrapSource.includes('/app/local-ai/primary-route-v283.js')&&bootstrapSource.includes('/app/local-ai/runtime-v266.js?v=1.0.121-v307-coherence-reload')&&bootstrapSource.includes('componentCompatibility')&&bootstrapSource.includes('capability-contract-v307')&&bootstrapSource.includes('continuationValidation:true')&&bootstrapSource.includes('freshWorkerFallback:true')&&bootstrapSource.includes('boundedStartup:true')&&bootstrapSource.includes('coherenceReload:true')&&bootstrapSource.includes('packageRevisionGuard:true')&&bootstrapSource.includes('smoothFitRuntime:true'));
if(bootstrapSource.includes("REVISION='1.0.117-local-ai-bootstrap-v306-browser-tools'"))assert.ok(bootstrapSource.includes('/app/browser-tool-v1.js')&&bootstrapSource.includes('/app/local-ai/browser-agent-v1.js')&&bootstrapSource.includes('delegatedBrowserTools:true')&&bootstrapSource.includes('offlineArchiveSearch:true'),'Browser-enabled bootstrap must preserve the small-model runtime while adding delegated browser tools.');
...
console.log(JSON.stringify({ok:true,revision:'small-model-fast-path-v314-smooth-fit-browser-compatible',features:{embeddedLocalPrimary:true,adaptiveOutputSlices:true,clippingValidation:true,structuredAnswerCompletionValidation:true,boundedContinuation:true,streamedContinuation:true,thinkingProfilesPreserved:true,wasmPerformancePreserved:true,adaptiveWasmThreads:true,compatibilityPromptCap:true,intentPrewarm:true,stalledWebGPUFallback:true,webgpuSessionQuarantine:true,freshWorkerFallback:true,promptBudgetEnforced:true,boundedStartup:true,coherenceReload:true,packageRevisionGuard:true,delegatedBrowserTools:bootstrapSource.includes('delegatedBrowserTools:true'),componentCompatibility:'capability-contract-v307'}},null,2));