...
// Retired Q4 presentation is now removed at source, not merely hidden by the service worker.
...
assert.match(controller,/1\.0\.25-model-settings-controller-v173-passive-gemma-support-downloads/);
...
assert.match(controller,/gemma4-dual-actions-v2\.js\?v=1\.3\.0-support-downloads/);
assert.match(controller,/await actions\?\.synchronizeImportedModels\?\.\(\)/,'Imported OPFS receipt must reconcile before current phone decoration.');
assert.match(controller,/actions\?\.refreshSupportStatus\?\.\(\{autoReconcile:true\}\)/,'Current phone chain must resolve missing support state after the two Gemma imports.');
...
const takeoverImport=worker203.indexOf("importScripts('/service-worker-gemma4-current-phone-v1.js?v=gemma4-current-phone-worker-v3-support-downloads')");
...
assert.match(takeover,/gemma4-current-phone-worker-v3-support-downloads/,'Takeover generation must change so installed PWAs receive current support-download code.');
assert.match(takeover,/staging-gemma4-current-phone-owner-v3-support-downloads/,'Takeover must use a fresh one-shot recovery marker.');
...
assert.match(currentActions,/Download missing support files/);
...
assert.match(currentActions,/downloadSupportFiles/,'Missing support components must have an explicit action path.');
...
console.log(JSON.stringify({ok:true,contract:'gemma4-phone-single-owner-v7',retiredQ4SourcesPhysicallyStubbed:true,currentController:'litert-support-single-owner',opfsReceiptReconciledBeforeDecorate:true,currentActionsOwnPresentation:true,phoneCoreRuntimeOnly:true,workerTakeoverBeforeLegacyCoherence:true,registeredWorkerGeneration:'v3-support-downloads',reloadsOldPageRealm:true,settingsPassive:true,downloadedModelsPreserved:true},null,2));