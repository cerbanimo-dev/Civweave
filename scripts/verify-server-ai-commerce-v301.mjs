...
const [version, router, settings, mesh, spine, cloudEntryV1, cloudEntryV2, cloudEntryV3, cloudEntryV4, cloudEntryV5, cloudEntryV6, cloudEntryV7, cloudEntryV8, capacityExtension, hostingCapacity, hostingNode, recoveryNode, accountEdge, legacyAccountEdge, wrangler, offlineText] = await Promise.all([
  'VERSION',
...
assert.match(cloudEntryV6, /capacity-membership-admin-v1\.mjs/);
assert.match(cloudEntryV6, /bindSession/);
assert.match(cloudEntryV7, /server-ai-entry-v6\.mjs/);
assert.match(cloudEntryV7, /accountRoute/);
assert.match(cloudEntryV8, /server-ai-entry-v7\.mjs/);
assert.match(cloudEntryV8, /cloud-node-recovery-v3\.mjs/);
...
console.log(JSON.stringify({
  ...
  revision: 'server-ai-commerce-v301-composed-user-pools-community-dividend-hosting-recovery-directory-account-lifecycle-v3',
  ...
  accountLifecycleComposition: true,
  deviceBoundSessions: true,
  memberStripeSurface: true,
  stewardMemberManagement: true,
