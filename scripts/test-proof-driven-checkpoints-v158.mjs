...
const [source,boundary,legacyWorker,wrapper,core,offline,workingCampus,manifestText,css]=await Promise.all([
  read('public/extensions/commonweave-proof-progress-v158.js'),
...
for(const token of ['input.disabled=true','stopImmediatePropagation','MutationObserver',"dataset.proofDriven='true'",'commonweave.working-campus.v1','commonweave.living-school.cabinet.v151','cerbanimo.quest-engine.v144','fellowfare.mvp.state.v3','assessmentPassed','proofItems(task)','fulfilled','syncProgress']){
...
if(lightweightMode){
  ...
  assert(legacyWorker.includes('/extensions/commonweave-proof-progress-v158.js'),'Offline extension package does not contain proof progress.');
}
...
assert(JSON.parse(localStorage.getItem('commonweave.intentions.v127'))[0].done===true,'Canonical intention was not completed.');
assert(JSON.parse(localStorage.getItem('commonweave.realm-inbox.v1')).every(row=>row.status==='completed'),'Realm handoffs were not completed.');
console.log(JSON.stringify({
  ...
  offlinePackageMode:lightweightMode?'v218-wrapper-resumable-campus':'layered-extension-package-v158'
},null,2));
