...
assert(runtime.includes('fallbackGenerate=spine.generate.bind(spine);'),'terminal fallback must pin the pre-wrapper runtime spine generator');
assert(runtime.includes("if(typeof fallbackGenerate!=='function')return primary;"),'terminal fallback must not recurse through a later wrapper');
assert(runtime.includes('const fallback=await fallbackGenerate(fallbackRequest(request));'),'terminal fallback must make exactly one prepared 3.5 retry');
...
console.log(JSON.stringify({ok:true,contract:'living-school-terminal-fallback-v1',primary:'gemini-3.7-flash',fallback:'gemini-3.5-flash',fallbackOn:'any-3.7-error',maxFallbackCalls:1,flashLiteFallback:false,pinnedSpineGenerator:true},null,2));
