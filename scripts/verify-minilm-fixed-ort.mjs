...
const [pkgText,adapter,worker,manifestText,stage,materializer]=await Promise.all([
  read('package.json'),
...
console.log(JSON.stringify({ok:true,runtime:'onnxruntime-web/wasm',version:pkg.dependencies['onnxruntime-web'],graph:'model_quantized.onnx',threads:1,loaderRemoved:true,backendSelectionRemoved:true,cleanBuildMaterializesModel:true},null,2));
