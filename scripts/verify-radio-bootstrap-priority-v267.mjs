...
const safeIndex=entries.indexOf('RADIO_SAFE_STATIONS');
...
assert.equal(safeIndex,radioIndex+1,'S.A.F.E. station policy must bootstrap immediately after the base radio owner');
assert.equal(trackIndex,safeIndex+1,'tier-aware track suggestions must bootstrap immediately after the S.A.F.E. station policy');
...
assert.equal(entries.filter(entry=>entry==='RADIO_SAFE_STATIONS').length,1,'S.A.F.E. station owner must be injected exactly once');
assert.equal(entries.filter(entry=>entry==='RADIO_TRACK_SUGGESTIONS').length,1,'track suggestion helper must be injected exactly once');
assert.ok(
...
assert.ok(
  boundary.indexOf("const RADIO_SAFE_STATIONS='/app/radio-safe-stations-v1.js'")<boundary.indexOf('const SYSTEM_EXPERIENCE_SCRIPTS=['),
  'S.A.F.E. station path must be declared before the experience registry'
);
assert.ok(
...
console.log('Civweave radio bootstrap priority contract verified for base radio -> S.A.F.E. policy -> tier-aware suggestions.');
