...
const files={
  controller:'public/app/model-settings-controller-v173.js',settings:'public/app/unified-ai-settings-v175.js',deterministic:'public/app/deterministic-mode-v175.js',delegation:'public/app/settings-delegation-v175.js',cleanup:'public/app/shared-tools-cleanup-v175.js',loader:'public/app/family-ai-loader-v105.js',additions:'public/extensions/commonweave-additions-v156.js',mobile:'public/app/mobile-regression-v170.js',rails:'public/app/local-rails-validator-v170.js',merlinites:'public/app/merlinites-semantic-planner-v164.js',boundary:'public/app/install-boundary-v146.js',worker:'public/service-worker.js',additiveWorker:'public/service-worker-v156.js',installer:'public/install-v130.js',installerHtml:'public/index.html',pwa:'public/app/pwa-v130.js',prepare:'scripts/prepare-start-v131.mjs',pkg:'package.json',directWorkflow:'.github/workflows/verify-v126.yml',additionsWorkflow:'.github/workflows/verify-working-campus-additions-v156.yml',campusTail:'public/app/working-campus-v156.part5.txt'
};
...
for(const key of ['controller','settings','deterministic','delegation','cleanup','loader','additions','mobile','rails','merlinites','boundary','worker','additiveWorker','installer','pwa'])new Function(source[key]);
...
for(const active of ['loader','mobile','rails','merlinites'])for(const forbidden of ['/app/minilm-reflex-runtime-v138.js','/app/minilm-model-settings-v138.js','/app/models/all-minilm-l6-v2','Xenova/all-MiniLM-L6-v2','ADAPTER_URL'])assert(!source[active].includes(forbidden),`${active} still activates or advertises ${forbidden}`);
...
for(const token of ["EXTENSION_VERSION='working-campus-additions-v175-deterministic-single-ai-settings'","defaultProvider:'deterministic'",'transformerActive:false','APP_FILES'])assert(source.additiveWorker.includes(token),`additive worker missing ${token}`);
