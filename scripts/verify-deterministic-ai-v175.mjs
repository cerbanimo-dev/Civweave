...
const files={
  ...
  additions:'public/extensions/civweave-additions-v156.js',
...
for(const token of ["VERSION='175.0-deterministic-single-authority'",'<option value="deterministic">Deterministic local mode</option>','Gemini API key','data-paste-key','data-import-key','data-forget-key','Save Civweave AI settings','migrateDeterministicDefault'])assert(source.settings.includes(token),`unified settings missing ${token}`);
...
for(const token of ["VERSION='175.2-deterministic-default'",'civweave-deterministic-v175','transformerActive:false','currentProvider','installAssistantPatch','stabilize'])assert(source.deterministic.includes(token),`deterministic runtime missing ${token}`);
...
for(const token of ["VERSION='177.0-final-legacy-ai-retirement'",'form[data-cw143-settings]','civweave.model-setup','data-open-unified-ai-settings','migrateLegacyAI','savePlatform'])assert(source.delegation.includes(token),`legacy settings retirement missing ${token}`);
for(const token of ['PLATFORM CONFIGURATION','Save platform settings','data-open-ai-settings','Open Civweave AI settings'])assert(source.surface.includes(token),`platform surface missing ${token}`);
...
for(const forbidden of ['MODEL_DOWNLOAD_SCRIPT','civweave-model-download-v157','addScript(MODEL_DOWNLOAD_SCRIPT)'])assert(!source.boundary.includes(forbidden),`install boundary still activates ${forbidden}`);
...
for(const forbidden of ['civweave-model-download-v157','minilm-model-settings','all-minilm-l6-v2','MODEL_PACKAGE'])assert(!source.additiveWorker.includes(forbidden),`additive worker still activates ${forbidden}`);
...
assert(sandbox.CivweaveDeterministicModeV175.transformerActive===false,'deterministic runtime reports an active Transformer');
assert(sandbox.CivweaveDeterministicModeV175.route('I need to borrow a trailer').system==='fellowfare','deterministic router lost resource routing');
console.log(JSON.stringify({ok:true,defaultProvider:'deterministic',settingsAuthority:'CivweaveUnifiedAISettingsV175',visiblePlatformSurface:'platform-only',apiKeyField:'Gemini API key',sharedToolsTabs:['Node & friends','Rewards','Active thread'],activeTransformer:false,activeModelChecks:false,activeModelDownload:false,activeSemanticAdapter:false,archivedTransformerLab:true,deviceRevision:'device-package-r39-no-legacy-ai'},null,2));
