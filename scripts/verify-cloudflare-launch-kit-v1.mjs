...
const [topologyText, transportsText, coreWranglerText, nodeWranglerText, coreSource, liveCoreSource, moneySource, stripeSource, nodeSource, capacitySource, migration1, migration2, migration3, bootstrapSource, envExample, guide, validationOptIn] = await Promise.all([
...
assert.ok(capacitySource.includes('Paid expansion residency requires an active membership'));
...
const fakeFetch = async (url, init = {}) => {
  ...
  if (String(url).endsWith('/v1/transfers')) return new Response(JSON.stringify({ id: 'tr_test', amount: 242 }), { status: 200, headers: {'content-type':'application/json'} });
  if (String(url).includes('/v1/transfers/tr_test/reversals')) return new Response(JSON.stringify({ id: 'trr_test', amount: 121 }), { status: 200, headers: {'content-type':'application/json'} });
