...
localStorage.setItem('civweave.universal-ai.v127',JSON.stringify({provider:'gemini',route:'gemini',model:'gemini-test',externalConsent:true}));
...
const context={
  ...
  CivweaveParity:{load:async()=>({systems:[
    {id:'civweave',roomIds:['civweave.quad']},
  ...
  CivweaveReflexRuntime:{route:(text,current)=>({system:current,mode:current==='civweave'?'Reflect':current==='living-school'?'Learn':current==='cerbanimo'?'Build':current==='fellowfare'?'Acquire':'Govern',confidence:.8,evidence:[]})},
...
const a=context.CivweaveAssistantV141,core=context.CivweaveCoreLoopV152,chat=context.CivweaveGuideChatV153,readiness=context.CivweaveCapabilityReadinessV154;
const assert=(x,m)=>{if(!x)throw new Error(m)};

let result=await a.respond({text:'I want to make a video game with my friends about a time traveler facing off with a time looper',systemId:'civweave',history:[]});
...
assert(JSON.parse(localStorage.getItem('civweave.intentions.v127')).some(i=>i.id===result.planItemId),'Plan must be persisted.');
...
const assessedPlan=JSON.parse(localStorage.getItem('civweave.intentions.v127')).find(item=>item.id===result.plan.id).plan;
...
assert((await chat.ask('civweave','I want to make a tiny game plan',[])).approvalGate?.kind==='intention-activation','Weaveling chat must return a reviewable plan gate.');
...
result=await a.respond({text:'retry one more time',systemId:'civweave',history:[{role:'user',text:'I want to make a video game with my friends about a time traveler facing off with a time looper'}]});
...
let approval=context.CivweaveActionUI.approve(result.action.id);
...
approval=context.CivweaveActionUI.approve(result.action.id);
assert(approval.ok&&approval.action.state==='published','Completed food request should publish after approval.');

const before=JSON.parse(localStorage.getItem('civweave.intentions.v127')).length;
...
const after=JSON.parse(localStorage.getItem('civweave.intentions.v127')).length;
assert(before===after,'Moss must not create a Civweave intention automatically.');
