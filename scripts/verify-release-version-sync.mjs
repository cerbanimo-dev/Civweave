...
const [versionText,packageText,indexHtml,manifestText,installRuntime,appIndex,installedEntry,installBoundary,releaseRuntime,workerCore,workerWrapper,legacyWorker,gateway,localServer]=await Promise.all([
  read('VERSION'),
  read('package.json'),
...
assert(gateway.includes("readFile(path.join(rootDir,'VERSION'),'utf8')")||gateway.includes("readFile(path.join(rootDir, 'VERSION'), 'utf8')"),'Gateway wrapper does not read the canonical VERSION file.');
assert(localServer.includes("readFile(path.join(rootDir,'VERSION'),'utf8')")||localServer.includes("readFile(path.join(rootDir, 'VERSION'), 'utf8')"),'Local server wrapper does not read the canonical VERSION file.');

for(const [label,source] of [['installer page',indexHtml],['manifest',manifestText],['installer runtime',installRuntime],['stable app entry',appIndex],['installed entry',installedEntry],['worker core',workerCore],['worker wrapper',workerWrapper],['legacy worker',legacyWorker]]){
...
console.log(JSON.stringify({ok:true,version,packageVersion:pkg.version,installerVersion:true,manifestVersion:true,workerVersion:true,visibleVersionRuntime:true,gatewayCanonicalVersion:true,localCanonicalVersion:true},null,2));
