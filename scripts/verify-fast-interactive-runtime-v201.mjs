...
assert.match(critical,/critical-package-completion-v201-fast-runtime-proxy/,'critical boot revision must rotate');
assert.ok(critical.includes("'/app/fast-interactive-runtime-v192.js'"),'critical boot must refresh the corrected runtime');
assert.match(worker,/service-worker-critical-v199\.js\?v=fast-runtime-proxy-v201/,'the registered worker must force an imported-script refresh');
...
console.log(JSON.stringify({
  ok:true,
  revision:'fast-interactive-runtime-v201',
