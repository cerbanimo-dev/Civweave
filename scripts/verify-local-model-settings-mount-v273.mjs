...
assert.doesNotMatch(lifecycle,/globalThis\.MutationObserver\s*=/,'management lifecycle must not patch MutationObserver');
...
const settingsOpenBody=lifecycle.match(/function ensureLocalAISettingsManagement\(layer\)\{([\s\S]*?)\}\nfunction scheduleSettingsManagement/)?.[1]||'';
assert.ok(settingsOpenBody,'Downloaded-model Settings management function must remain inspectable.');
...
assert.match(controller,/compatibilityFacade:true/);assert.match(controller,/canonical:'CivweaveSettingsV320'/);assert.match(controller,/inputOwnership:false/);assert.match(controller,/presentationOwnership:false/);assert.match(controller,/credentialOwnership:false/);assert.match(controller,/domCreation:false/);
...
console.log(JSON.stringify({ok:true,revision:'local-model-settings-mount-v320-single-owner',canonicalSettingsMount:true,settingsOwner:'settings-v320',gatewayOwner:'settings-gateway-v317',managementService:true,managementAfterPaint:true,noSettingsCaptureInLifecycle:true,inferenceDormantUntilNeeded:true,legacyControllerFacadeOnly:true},null,2));
