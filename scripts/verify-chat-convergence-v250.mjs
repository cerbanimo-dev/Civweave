...
check('worker runs the current avatar-visible cache repair and retains the v343 migration marker',workerEntry.includes("importScripts('/service-worker-chat-repair-v245.js?v=chat-avatar-visible-v346&purge=chat-avatar-visible-v346')")&&workerRepair.includes("const REVISION='chat-avatar-visible-v346'")&&workerEntry.includes('chat-css-contract-v343')&&workerRepair.includes('chat-css-contract-v343'));
...
console.log(JSON.stringify({ok:true,version,revision:'chat-convergence-v250-with-chat-avatar-visible-v346-navigation-lifecycle-v424',checks:checks.length,installedLaunch:'updater-first-clean-url',canonicalRuntime:'guide-workspace-v242',canonicalDuplicateChatOwners:0,retiredChatRuntimeCount:retiredPaths.length,embeddedSurfaceDelegates:true,cacheMigration:true,checkedInReleaseIdentitiesCurrent:true,releaseGeneratorsPreserveRetirement:true,navigationLifecycle:'v424'},null,2));
