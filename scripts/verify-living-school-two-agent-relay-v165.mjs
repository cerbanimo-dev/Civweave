...
assert(worker.includes("TWO_AGENT_RELAY_REVISION='living-school-two-agent-youtube-v166'"),'Service worker is missing the relay revision.');
assert(boundary.includes("additionsVersion:'v166-two-agent-youtube-reviewed-handoffs'"),'Install boundary did not rotate for the relay.');
assert(worker.includes('reviewed-merlin-rook-proof-attachments-v165'),'The combined package lost the workflow-handoff compatibility layer.');
...
console.log(JSON.stringify({ok:true,relay:'antigravity-research-to-moss-teaching',agenticVideoRule:'seek-and-open-when-available',offlineFallback:'complete-module-plus-search-phrase',moduleContract:{multipleChoice:3,shortAnswer:1},preserved:['merlin-review','rook-followthrough','cerbanimo-proof-attachments']},null,2));
