...
new Function(shell);
assert.ok(shell.includes("const VERSION='1.1.2-canonical-local-settings-refresh';"),'Persistent shell chrome owner revision is missing.');
...
assert.ok(shell.includes("/app/settings-local-route-v325.js"),'Persistent Settings must load the canonical local-model view.');
assert.ok(shell.includes("SETTINGS_LOCAL_ROUTE_VERSION='1.1.3-settings-local-route-v326-canonical-inert-hard-local'"),'Persistent Settings must require the exact inert local-model view revision.');
...
assert.ok(page.includes('/app/persistent-system-shell-v1.js?v=1.1.2-canonical-local-settings-refresh'),'Persistent shell HTML must cache-bust the current chrome/settings owner revision.');
...
console.log(JSON.stringify({ok:true,revision:'persistent-shell-chrome-owner-v1',updateSignal:'platform-backlight-v2',sharedQuickFeatures:['settings','chat'],childChatLauncherPolicy:'parent-only',localSettings:'canonical-v325-exact-version'},null,2));