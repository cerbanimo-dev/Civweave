...
assert.equal(enhanced.semanticPlan.schema,'commonweave.sol-semantic-plan.v1');
...
assert.equal(created.response.approvalGate.required,true,'Sol must preserve the approval gate');
assert.equal(created.plan.semanticPlan.schema,'commonweave.sol-semantic-plan.v1');
const persisted=JSON.parse(context.localStorage.getItem('commonweave.intentions.v127'));
assert.equal(persisted[0].plan.semanticPlan.schema,'commonweave.sol-semantic-plan.v1','the saved weave must receive the additive plan');

const learning=await sol.evaluateLearning({
...
const task=await sol.evaluateTask({
...
assert.match(pwa,/working-campus-additions-v167-two-agent-workflow-sol-semantic-planning/,'installed updater must advance to the combined two-agent, workflow-handoff, and Sol package');
