...
for(const token of["const installedEntryRevision='boot-recovery-v427-installed-capability-v1'","const boundaryRevision='browser-install-boundary-v228-chat-escape-install-only-pwa-v1'","const boundaryRuntimeRevision='browser-install-boundary-v228-chat-escape-install-only-pwa-v1'",'guideChatCanonicalPolicy','guideChatWorkspaceRevision','guideChatOwnershipPolicy',"ownership?.systems?.['guide-chat']?.owner","browserRuntimePolicy:'installed-display-or-verified-installed-capability'",'installedQueryIsAuthorization:false',"installedCapability:'v1'",'installOnlyPwa'])assert(coherenceSync.includes(token),`Release coherence synchronizer is missing ${token}.`);
...
assert(installedEntry.includes('async function installedLaunchAuthorized()'),'Installed entry must own one capability-aware launch authorization boundary.');
assert(installedEntry.includes('await installedLaunchAuthorized()'),'Installed entry must await verified installed authorization before routing.');
assert(installedEntry.includes("browserRuntimePolicy:'installed-display-or-verified-installed-capability'"),'Installed entry must require installed display or verified capability.');
...
assert(boundary.includes("const INSTALL_CAPABILITY_KEY='civweave.pwa.installed-capability.v1'"),'Shared boundary must use the durable same-origin install capability.');
assert(boundary.includes('function allowed(){return installedDisplay()||installedCapability()||developer()}'),'Shared boundary must accept installed display or verified capability, never embedded browser state.');
assert(boundary.includes("browserRuntimePolicy:'installed-display-or-verified-installed-capability'"),'Shared boundary must declare capability-aware installed policy.');
assert(boundary.includes('installedQueryIsAuthorization:false'),'Query parameters alone must never authorize the campus.');
assert(!boundary.includes('function allowed(){return installedDisplay()||developer()||embedded()}'),'Embedded browser documents must not authorize Civweave runtime.');
...
console.log(JSON.stringify({ok:true,version,revision:'navigation-generator-installed-capability-v1-v350-chat-owner',canonicalChatOwner:chatOwner,singleRouteAuthority:true,builderRouteFirst:true,builderIntegrityOrder:true,builderCanonicalAfterRepair:true,coherenceCannotRestoreAnonymousBrowserRuntime:true,updaterFirstEntry:true,installedLaunchNeverInstaller:true,installedCapability:true,queryAuthorization:false,repairOnly:true,cacheDistinctRepair:true,stableV225WorkerEpoch:true,workingCampusReturn:'v425',moduleSyntaxCheckedByWorkflow:true},null,2));