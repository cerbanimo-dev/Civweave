...
assert(/if\s*\(\s*installedDisplay\(\)\s*\)/.test(installerFallback)&&installerFallback.includes('event.stopImmediatePropagation()')&&installerFallback.includes("},true);"),'Installed Open button no longer preempts the legacy /app/ click handler in capture phase.');
...
assert(wrapper.includes(`/service-worker-core-v208.js?v=${version}-chat-convergence-v250-installer-brand-v1-working-campus-return-v425`),'Checked-in worker wrapper lost the v425 cache epoch.');
...
console.log(JSON.stringify({ok:true,version,revision:'navigation-generator-invariants-v426-avatar-v346',singleRouteAuthority:true,builderRouteFirst:true,builderIntegrityOrder:true,builderCanonicalAfterRepair:true,coherenceCannotDowngrade:true,coherenceCannotRestoreRetiredChatOwners:true,compatibilityLauncherUsesContract:true,updaterFirstEntry:true,installedLaunchCampusRecovery:true,installedLaunchNeverInstaller:true,installerOpenPreempted:true,installerGuardOfflineCached:true,navigationLifecycle:'v424',workingCampusReturn:'v425',avatarCacheRepair:'v346',moduleSyntaxCheckedByWorkflow:true},null,2));
