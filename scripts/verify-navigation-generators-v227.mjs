...
for(const token of ["const boundaryRevision='chat-convergence-v250'","const routeRevision='five-system-route-contract-v227'","canonicalPolicy:'five-system-first-class-routes-v242-canonical-chat-owner'",'canonicalSystemCount:5','Canonical navigation must remain the final worker policy'])assert(coherenceSync.includes(token),`Release coherence synchronizer is missing ${token}.`);
...
console.log(JSON.stringify({ok:true,version,revision:'navigation-generator-invariants-v227',singleRouteAuthority:true,builderRouteFirst:true,builderCanonicalLast:true,coherenceCannotDowngrade:true,compatibilityLauncherUsesContract:true,moduleSyntaxCheckedByWorkflow:true,boundaryRevision:'chat-convergence-v250'},null,2));
