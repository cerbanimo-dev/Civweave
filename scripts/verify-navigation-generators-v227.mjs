...
const [versionText,builder,versionSync,coherenceSync,installedEntry,installedLaunch,repairOnly,manifestText,shellAssets,shellRepair,wrapper,boundary,ownershipText]=await Promise.all([
...
for(const token of['public/app/themed-system-nav-v178.js','five-system-navigation-v227','browser-install-boundary-v228-chat-escape-install-only-pwa-v1','working-campus-return-v425-install-only-pwa-v1','installOnlyPwa'])assert(versionSync.includes(token),`Release version synchronizer is missing ${token}.`);
...
assert(shellAssets.includes("policy:'declarative-shell-assets-only-no-repair-or-message-ownership'"),'Shell asset helper must remain non-owning.');
...
console.log(JSON.stringify({ok:true,version,revision:'navigation-generator-five-guide-rail-v228-pwa-launch-session-v1-v350-chat-owner-single-repair',canonicalChatOwner:chatOwner,singleRouteAuthority:true,fiveGuideRailReleaseSync:true,builderRouteFirst:true,builderIntegrityOrder:true,shellRepairOwner:'v293',retiredV225Absent:true,updaterFirstEntry:true,installedLaunchNeverInstaller:true,pwaLaunchSession:true,queryAuthorization:false,repairOnly:true,cacheDistinctRepair:true,workingCampusReturn:'v425'},null,2));
