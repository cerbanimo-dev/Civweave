...
const expectedSeeds=[
  ...
  '/app/federation-finder-local-v269.html',
  '/app/federation-finder-local-v269.js',
...
assert(JSON.stringify(manifest.seeds)===JSON.stringify(expectedSeeds),`Offline manifest must seed exactly installed entry + reward runtimes + Systems Mesh + local Federation Finder + five canonical systems; got ${manifest.seeds.length} seeds.`);
assert(manifest.revision==='canonical-background-campus-v241-systems-mesh-v251-federation-finder-v269','Offline manifest does not identify the approved Systems Mesh + local Federation Finder seed revision.');
