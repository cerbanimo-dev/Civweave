...
assert(coreSource.startsWith('// GENERATED: lightweight-shell-v208 core + offline-campus-seed-provenance-v211'),'Retained core banner is missing.');
...
assert(workerPackage.includes("const V211_REVISION = 'offline-campus-seed-provenance-v211'"),'v211 retry provenance override is missing from the worker package.');
...
const context=vm.createContext({
  console,Date,Map,Set,Promise,Number,String,Array,Math,URL,self,
  VERSION:'1.0.6',OFFLINE_CACHE:'commonweave-offline-test',offlinePacket:()=>({}),offlineStatus:async()=>({}),
  downloadOfflinePackage:async()=>({}),writeOfflineMeta:async packet=>{persisted=packet;return packet},
  loadOfflineManifest:async()=>({seeds:[]}),readOfflineMeta:async()=>null,
  cacheOfflineAsset:async()=>{throw new Error('not used by metadata migration test')},discoverReferences:()=>[],TEXT_CONTENT:/text/,post:()=>{}
});
...
console.log(JSON.stringify({ok:true,revision:'offline-campus-seed-provenance-v211',workerComposition:'v218-cleanroom-plus-retained-core',legacyAttempted:205,acceptedDownloaded:migrated.downloaded,staleReferencesSkipped:migrated.skippedCount,requiredFailuresPreserved:requiredFailure.failedCount},null,2));
