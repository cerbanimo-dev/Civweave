...
assert(overrideSource.includes('const V211_BATCH_SIZE = 12'),'Offline current-graph download concurrency is not the expected bounded value.');
...
const context=vm.createContext({console,Date,Map,Set,Promise,Number,String,Array,Math,URL,RegExp,self,VERSION:'1.0.32',OFFLINE_CACHE:'civweave-offline-test',offlinePacket:()=>({}),offlineStatus:async()=>({}),downloadOfflinePackage:async()=>({}),writeOfflineMeta:async packet=>{persisted=packet;return packet},loadOfflineManifest:async()=>({seeds:[]}),readOfflineMeta:async()=>null,cacheOfflineAsset:async()=>{throw new Error('not used by metadata migration test')},discoverReferences:()=>[],TEXT_CONTENT:/text/,post:()=>{}});
...
assert(api?.batchSize===12,'Current-graph API does not publish bounded concurrency.');
...
console.log(JSON.stringify({ok:true,revision:'offline-campus-current-graph-v238',workerComposition:'v218-cleanroom-plus-retained-core-plus-current-graph-v238',legacyCacheCandidates:migrated.total,staleReferencesSkipped:migrated.skippedCount,requiredFailuresPreserved:requiredFailure.failedCount,batchSize:api.batchSize,currentGraphOnly:api.currentGraphOnly},null,2));
