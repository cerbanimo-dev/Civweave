...
assert.equal(guide.guide.learning.moduleCompletionAcorns,10);
assert.equal(guide.guide.learning.externalValidationBonusAcorns,20);
assert.equal(guide.guide.learning.validatorContributionAcorns,5);
...
const chart=guide.chartRows().map(row=>`${row.label} ${row.value} ${row.note}`).join('\n');assert.match(chart,/Starting wage/);assert.match(chart,/5 🔘 Buttons \/ hour/);assert.match(chart,/Learning module completed 10 🌰 Acorns/);assert.match(chart,/External successful validation \+20 🌰 Acorns/);assert.match(chart,/Validate someone’s learning 5 🌰 Acorns/);assert.match(chart,/100 🌰 Acorns \/ hour/);assert.match(chart,/5 🔘 Buttons \+ 50 🌰 Acorns/);assert.match(chart,/more granular numeric scale/i);
...
assert.deepEqual(entries.map(row=>[row.assetType,row.amount,row.sourceKey]),[
 ['acorn',10,'basic-value:module-a:module-complete'],
 ['acorn',20,'basic-value:module-a:external-validation'],
 ['acorn',5,'basic-value:receipt-b:validator:model-b']
]);
...
assert.match(valueModel.promptContract,/Learning module completion grant: 10 🌰 Acorns/);
assert.match(valueModel.promptContract,/validation bonus: \+20 🌰 Acorns/);
assert.match(valueModel.promptContract,/validating somebody else's learning: 5 🌰 Acorns/);
...
for(const token of ['marketplace-v2-value-guide.css?v=basic-value-v1','/app/civweave-basic-value-v1.js?v=acorn-scale-v2','marketplace-v2-value-guide.js?v=acorn-scale-v2'])assert.ok(fellowfare.includes(token),`FellowFare does not load ${token}`);
...
for(const shell of [living,cerbanimo])for(const token of ['/app/cw-reward-ledger-v2.js?v=basic-value-v1','/app/civweave-basic-value-v1.js?v=acorn-scale-v2','/app/cw-reward-receivers-v2.js?v=basic-value-v1','/app/civweave-basic-value-model-v1.js?v=acorn-scale-v2'])assert.ok(shell.includes(token),`Cross-system shell is missing ${token}`);
...
console.log(JSON.stringify({ok:true,schema:guide.schema,labor:'uniform starting wage: 5 🔘 / human-equivalent hour for every worker',learning:'10 🌰 completion + 20 🌰 external validation + 5 🌰 validator contribution',education:'100 🌰/hour; curriculum 50–500 🌰',mentorship:['5 🔘 + 50 🌰','15 🔘','200 🌰'],crossSystem:true,valuationPersists:true},null,2));
