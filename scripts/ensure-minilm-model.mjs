...
async function valid(spec){try{const file=path.join(modelRoot,spec.name),info=await stat(file);if(spec.size&&info.size!==spec.size)return false;if(spec.min&&info.size<spec.min)return false;if(spec.sha&&(await hash(file))!==spec.sha)return false;return true}catch{return false}}
async function download(spec){
  ...
  const response=await fetch(`${base}/${spec.name}?download=true`,{redirect:'follow',headers:{'user-agent':'Commonweave/1.0.30'}});
  ...
  const info=await stat(temp);if(spec.size&&info.size!==spec.size)throw new Error(`${spec.name} downloaded ${info.size} bytes, expected ${spec.size}`);if(spec.min&&info.size<spec.min)throw new Error(`${spec.name} downloaded only ${info.size} bytes`);if(spec.sha&&(await hash(temp))!==spec.sha)throw new Error(`${spec.name} hash mismatch`);
...
async function main(){
  ...
  if(!missing.length){console.log('[Commonweave] MiniLM semantic package is ready.');return}
