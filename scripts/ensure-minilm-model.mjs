...
async function main(){
  if(skip){
    console.log('[Commonweave] MiniLM download skipped by COMMONWEAVE_SKIP_MODEL_PULL.');
    return;
  }
  const missing=[];
  ...
  for(const spec of missing){
    ...
    console.log(`[Commonweave] Downloaded ${spec.name} (${bytes} bytes).`);
  }
  console.log('[Commonweave] MiniLM semantic package is ready.');
}
main().catch(error=>{
  if(soft){
    console.warn(`[Commonweave] MiniLM package remains optional: ${error.message}`);
    return;
  }
  console.error(error);
  process.exitCode=1;
