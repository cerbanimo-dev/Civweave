...
for(const token of ['data-download-local-model','adapter.install','commonweave:model-package-ready'])assert(modelDownload.includes(token),`Model download control missing ${token}`);
...
for(const token of ["const VERSION='1.0.4-direct-settings-v173'","const SYSTEM_ORDER=['commonweave','living-school','cerbanimo','fellowfare','anarchadia']",'tray.innerHTML=SYSTEM_ORDER.map',"document.documentElement.dataset.familyShell='direct'",'commonweave.family-status.v105','timestamp(item)>last','badge.hidden=value.count<1',"settingsOwner:'CommonweaveModelSettingsControllerV173'"])assert(familyRuntime.includes(token),`Direct family runtime missing ${token}`);
...
assert(realmHtml.includes('cabinet-surfaces-v143.js'),'Commonweave/Cerbanimo shared work surfaces are not mounted.');
assert(fellowfareHtml.includes('/app/services/fellowfare/cabinet.html')&&!fellowfareHtml.includes('cabinet-home-v142.js'),'FellowFare must mount its native Rook market without the Commonweave opening overlay.');
