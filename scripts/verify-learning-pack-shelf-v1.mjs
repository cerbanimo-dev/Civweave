...
assert(shelf.includes("adapter.createQuest(row.id")&&shelf.includes("adapter.generateCurriculum(row.id"),'Shelf must hand selected content to existing realm engines.');
assert(shelf.includes("if(row.kind==='labor-reference')")&&shelf.includes('Reference only. Adapt this occupational description'),'Labor references must be visibly non-executable.');
...
assert(shelf.includes("record.packType==='reference'")&&shelf.includes('never become executable Cerbanimo work'),'Reference packs need an explicit adaptation boundary in the UI.');
...
console.log('Learning Pack Shelf v1 contract passed.',{sharedMount:true,offlineControls:true,referenceGuard:true,livingSchool:true,cerbanimo:true,mobile:true});
