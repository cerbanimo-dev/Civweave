...
const [pkg,gateway,localHost,prepareStart,startRuntime,stageRuntime,policy,parity,compatHost,familyShell,aiLoader,marketingCabinet,legacyVisual,launcher,sw,installerHtml,installerJs,boundary,entryHtml,entryJs,mesh,realmHtml,livingHtml,fellowfareHtml,anarchadiaHtml]=await Promise.all([
...
for(const token of ["const VERSION='1.0.4'","WORKER_REVISION='device-package-r35-direct'",'GET_DEVICE_PACKAGE_STATUS','packageReady','beforeinstallprompt'])assert(installerJs.includes(token),`installer runtime missing ${token}`);
...
for(const token of ['/app/installed-entry-v146.html','/app/family-ai-loader-v105.js',"INSTALL_REVISION='direct-entry-r35'","DEVICE_REVISION='device-package-r35-direct'",'GET_DEVICE_PACKAGE_STATUS',"'x-commonweave-package':'install'"])assert(sw.includes(token),`service worker missing ${token}`);
...
new Function(familyShell);new Function(aiLoader);new Function(entryJs);
console.log(JSON.stringify({ok:true,version:packageJson.version,publicOrigin:'installer and marked package distribution only',defaultStart:'direct Commonweave software',compatibilityHost:'redirect only',aiRuntime:'lazy on interaction',notifications:'unseen actionable records',workerRevision:'device-package-r35-direct',ordinaryApplicationTraffic:'cache-only',communityObjects:'local canonical outbox'},null,2));
