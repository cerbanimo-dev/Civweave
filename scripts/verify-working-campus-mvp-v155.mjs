...
])assert(settings.includes(token),`Canonical Settings V320 are missing ${token}`);
...
assert(settings.includes('globalThis.CivweaveSettingsV320=api'),'Settings gateway must remain the canonical V320 owner.');
...
console.log(JSON.stringify({ok:true,surface:'working-campus-v156',sourceFiles:parts.length,coreLoop:'wish -> aptitude -> review -> activation -> realm handoffs',aiSettings:'Settings V320 single-owner deterministic-default gateway with explicit provider options',providerTests:'retired from Working Campus',navigation:'canonical five-system route owner with direct Working Campus fallback',installedBoot:'bounded service-worker update before routing',offlineState:'local canonical'},null,2));
