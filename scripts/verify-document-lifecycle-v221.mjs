...
assert.doesNotMatch(lifecycle,/globalThis\.MutationObserver\s*=|document\.addEventListener\('click'/);
assert(additions.includes('civweaveAdditionsNavigating'));
assert(workerCore.includes("'/app/document-lifecycle-v221.js'"),'Dormant lifecycle service must remain cacheable for offline first-click management activation.');
...
for(const [name,source] of [['installed entry',installedEntry],['campus loader',campusLoader],['lifecycle service',lifecycle],['install boundary',boundary],['route contract',routes],['shared additions',additions],['release coherence',releaseCoherence],['canonical navigation',canonicalNavigation]])assert.doesNotThrow(()=>new vm.Script(source,{filename:name}),`${name} does not compile.`);
console.log(JSON.stringify({ok:true,revision:'canonical-campus-startup-v320',updaterFirstStart:true,canonicalSystems:5,navigationOwner:'system-routes-v227',settingsOwner:'settings-gateway-v317',settingsApi:'CivweaveSettingsV320',settingsManagementOwner:'settings-v320',settingsManagementActivation:'explicit-after-open',managementAfterPaint:true,globalObserverPatch:false},null,2));
