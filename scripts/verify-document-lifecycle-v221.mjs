...
const [manifestText,installedEntry,campusHtml,campusLoader,campusPart4,lifecycle,orchestrator,installBoundary,routes,additions,workerCore,releaseCoherence,wrapper,canonicalNavigation]=await Promise.all([
...
assert(installedEntry.includes('/app/manifest.webmanifest?boot=${Date.now()}')&&installedEntry.includes("cache:'no-store'")&&installedEntry.includes('bounded(fetch('),'Installed entry no longer resolves the current release through the bounded no-store manifest path.');
assert(installedEntry.includes("updateViaCache:'none'")&&installedEntry.includes('registration.update()')&&installedEntry.includes('bounded(registration.update()'),'Installed entry no longer performs a bounded worker refresh before routing.');
...
for(const token of ['document-lifecycle-v316-nonblocking-no-global-observer-patch','ensureLocalAISettingsManagement','function scheduleSettingsManagement(',"addEventListener('pagehide',stopOnPageHide)","addEventListener('pageshow',reviveFromPageShow)","settingsEntryOwner:'experience-orchestrator'",'managementAfterPaint:true','globalObserverPatch:false'])assert(lifecycle.includes(token),`Lifecycle guard is missing ${token}.`);
assert(!/globalThis\.MutationObserver\s*=/.test(lifecycle),'Lifecycle guard replaced the browser MutationObserver primitive.');
assert(!lifecycle.includes('captureSettingsOpen'),'Lifecycle guard regained Settings input ownership.');
assert(!lifecycle.includes('warmVisibleManagement()'),'Lifecycle guard prewarms local-AI management before Settings is visible.');
assert(orchestrator.includes("globalThis.addEventListener('click',earlySettings,true)")&&orchestrator.includes("settingsCaptureOwner:'window-only-v316'"),'Experience orchestrator lost canonical Settings ownership.');
assert(orchestrator.includes('afterSettingsPaint'),'Settings management no longer yields a paint before mounting.');
...
for(const [name,source] of [['installed entry',installedEntry],['campus loader',campusLoader],['lifecycle guard',lifecycle],['experience orchestrator',orchestrator],['install boundary',installBoundary],['route contract',routes],['shared additions',additions],['release coherence',releaseCoherence],['canonical navigation',canonicalNavigation]])assert.doesNotThrow(()=>new vm.Script(source,{filename:name}),`${name} does not compile.`);
console.log(JSON.stringify({ok:true,revision:'canonical-campus-startup-v316-boot-recovery-v426',updaterFirstStart:true,boundedUpdater:true,pageshowResume:true,directCampusStart:false,canonicalSystems:5,canonicalChatOwner:'guide-workspace-v242',civweaveCoreOnly:true,settingsOwner:'experience-orchestrator',managementAfterPaint:true,globalObserverPatch:false,navigationErrorsSilent:true,campusFragmentsNetworkFirst:true,packageAuthenticatedNavigation:true,exactRouteFallback:true,nonInterruptingCoreWorker:true,v250WrapperActivation:true},null,2));
