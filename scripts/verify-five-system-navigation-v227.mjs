...
const optionalApprovedExperienceScripts=[
...
const experienceScripts=[
  ...coreExperienceScripts.slice(0,5),
  ...optionalApprovedExperienceScripts.filter(script=>boundarySource.includes(script)),
  ...coreExperienceScripts.slice(5)
];
...
const radioIndex=experienceScripts.indexOf('/app/system-radio-agent-v233.js'),trackIndex=experienceScripts.indexOf('/app/radio-track-suggestions-v240.js'),playlistIndex=experienceScripts.indexOf('/app/canonical-playlists-v1.js'),playlistGovernanceIndex=experienceScripts.indexOf('/app/radio-playlist-governance-v1.js'),meshIndex=experienceScripts.indexOf('/app/civweave-systems-mesh-v251.js'),hostSessionIndex=experienceScripts.indexOf('/app/host-node-session-v1.js'),nodeMeshIndex=experienceScripts.indexOf('/app/node-ai-mesh-v1.js'),veilMeshIndex=experienceScripts.indexOf('/app/quest-veil-mesh-v1.js'),veilGateIndex=experienceScripts.indexOf('/app/quest-veil-ledger-gate-v1.js'),veilIndex=experienceScripts.indexOf('/app/quest-veil-v1.js');assert.equal(radioIndex,3,'Radio must remain immediately after the Settings gateway, mobile AI guard, and experience orchestrator.');assert.equal(trackIndex,radioIndex+1,'Exact-track suggestions must remain immediately after radio.');if(playlistIndex>=0){assert.equal(playlistIndex,trackIndex+1,'Canonical playlists must load immediately after exact-track suggestions.');assert.equal(playlistGovernanceIndex,playlistIndex+1,'Playlist governance must load immediately after canonical playlists.');assert.equal(meshIndex,playlistGovernanceIndex+1,'Systems mesh must remain immediately after playlist governance.')}else{assert.equal(playlistGovernanceIndex,-1,'Playlist governance cannot load without canonical playlists.');assert.equal(meshIndex,trackIndex+1,'Systems mesh must remain immediately after radio suggestions.')}assert.equal(hostSessionIndex,meshIndex+1,'Hub Node session ownership must load immediately after systems mesh.');assert.equal(nodeMeshIndex,hostSessionIndex+1,'Node AI mesh must follow Hub Node session ownership.');assert.equal(veilMeshIndex,nodeMeshIndex+1,'Quest Veil mesh must follow the node AI mesh.');assert.equal(veilGateIndex,veilMeshIndex+1,'Mandatory Quest Veil ledger gate must follow its mesh runtime.');assert.equal(veilIndex,veilGateIndex+1,'Quest Veil finale renderer must remain downstream of the mandatory gate.');
