...
for(const marker of ['exact-route-current-package-first-no-live-network-runtime-fallback','runtimeNetworkFallback:false','precacheCanonicalRoutes','currentPackage(pathname)',"canonicalHandler:'packageOnlyCanonical'"])assert.ok(workerNavigation.includes(marker));
const handlerStart=workerNavigation.indexOf('async function packageOnlyCanonical(request)'),handlerEnd=workerNavigation.indexOf("self.addEventListener('install'",handlerStart),canonicalHandler=workerNavigation.slice(handlerStart,handlerEnd);
assert.ok(canonicalHandler.length>0&&!canonicalHandler.includes('fetch(')&&!canonicalHandler.includes('originalNetworkFirst'),'Canonical package-only handler can still escape to the hosted network.');
const wrapperStart=workerNavigation.indexOf('networkFirst=async function canonicalFiveSystemPackageFirst'),wrapperEnd=workerNavigation.indexOf('self.CivweaveCanonicalNavigationV227',wrapperStart),wrapperBranch=workerNavigation.slice(wrapperStart,wrapperEnd);
assert.ok(wrapperBranch.includes('if(canonical(request))return packageOnlyCanonical(request);'),'Canonical routes are not delegated to the isolated package-only handler before generic network fallback.');
...
assert.match(gatewayBase,/x-civweave-package/,'Gateway base no longer requires an explicit package marker for runtime assets.');
assert.match(gatewayBase,/const installerSurface = installerAssets\.has\(pathname\);/,'Gateway base lost the explicit installer dependency graph.');
assert.match(gatewayBase,/if \(gatewayRequest && applicationSurface && !installerSurface && !packageInstall\)/,'Gateway base no longer blocks non-installer application surfaces without a package request.');
assert.ok(gatewayBase.includes("'/app/index.html'"),'Gateway base does not explicitly expose the installer page.');
assert.ok(!gatewayBase.includes("pathname !== '/app' && !pathname.startsWith('/app/')"),'Gateway base contains the retired live-/app relaxation.');
assert.ok(gatewayWrapper.includes('forbiddenLiveAppRelaxation'),'Render gateway wrapper no longer guards against reopening /app as a live runtime.');
assert.ok(gatewayWrapper.includes('refused to start because /app runtime was relaxed into a live hosted surface'),'Render gateway wrapper does not fail closed if the live-/app relaxation returns.');
assert.ok(!gatewayWrapper.includes('source = source.replace(before, after)'),'Render gateway wrapper still rewrites the package boundary open.');
...
console.log(JSON.stringify({ok:true,version,revision:'five-system-navigation-v266-downloaded-runtime',systems:Object.keys(paths),routeMatrix:25,routeIdentityAuthorization:false,boundaryIntrinsicAuthorization:false,queryInstalledIsAuthorization:false,standaloneAuthorization:true,canonicalRuntimeSource:'downloaded-package',canonicalNetworkFallback:false,canonicalHandler:'packageOnlyCanonical',offlineRuntimeBoundary:'v266',gatewayInstallerGraph:true,gatewayLiveAppRelaxation:false,gatewayWrapperFailClosed:true,canonicalChatOwner:'guide-workspace-v242',workerFallback:'current-package-or-visible-repair',hostRole:'installer-update-recovery-only'},null,2));
