...
assert.match(boundarySource,/canonicalSystemCount:5/);assert.match(boundarySource,/canonicalExperienceScripts:SYSTEM_EXPERIENCE_SCRIPTS\.length/);assert.match(boundarySource,/five-system-first-class-routes-civweave-core-only/);assert.match(boundarySource,/radioTrackSuggestionRevision:'v241-playlist-context-track-links'/);assert.match(boundarySource,/campusBackgroundDownloadRevision:'v241-worker-owned-download-bottom-progress-rail'/);assert.match(boundarySource,/guideWorkspaceRevision:'v242-five-window-local-ledgers-no-scroll-trap'/);assert.match(boundarySource,/workingCampusTopbarRevision:'v243-sticky-top-map-launch-contract'/);assert.match(boundarySource,/mapLaunchRevision:'v243-register-route-handler-or-open-event'/);
...
console.log(JSON.stringify({ok:true,version,revision:'five-system-navigation-v227',systems:Object.keys(paths),routeMatrix:25,boundaryIntrinsicAuthorization:true,experienceScripts,canonicalExperienceScriptCount:experienceScripts.length,workerPackageHeader:true,workerFallback:'exact-route-or-visible-recovery',backgroundCampus:true,guideWorkspace:'v242',workingCampusTopbar:'v243',launcherSubstitution:false,installerSubstitution:false},null,2));
