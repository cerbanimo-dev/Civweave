...
assert.match(boundarySource,/canonicalSystemCount:5/);assert.match(boundarySource,/canonicalExperienceScripts:SYSTEM_EXPERIENCE_SCRIPTS\.length/);assert.match(boundarySource,/systemsMeshRevision:'v251-five-system-non-privileged-event-contract'/);assert.match(boundarySource,/five-system-first-class-routes-v242-canonical-chat-owner/);assert.match(boundarySource,/radioTrackSuggestionRevision:'v241-playlist-context-track-links'/);assert.match(boundarySource,/campusBackgroundDownloadRevision:'v241-worker-owned-download-bottom-progress-rail'/);assert.match(boundarySource,/guideWorkspaceRevision:'v250-v242-canonical-owner'/);assert.match(boundarySource,/guideSurfaceOwnershipPolicy:'v250-single-v242-runtime-five-local-window-ledgers-handover-only-cross-realm'/);assert.match(boundarySource,/workingCampusTopbarRevision:'v243-sticky-top-map-launch-contract'/);assert.match(boundarySource,/mapLaunchRevision:'v243-register-route-handler-or-open-event'/);
...
console.log(JSON.stringify({ok:true,version,revision:'five-system-navigation-v227',systems:Object.keys(paths),routeMatrix:25,boundaryIntrinsicAuthorization:true,experienceScripts,canonicalExperienceScriptCount:experienceScripts.length,canonicalChatOwner:'guide-workspace-v242',retiredCanonicalChatScripts,workerPackageHeader:true,workerFallback:'exact-route-or-visible-recovery',backgroundCampus:true,guideWorkspace:'v250-v242-canonical-owner',workingCampusTopbar:'v243',systemsMesh:'v251-five-system-non-privileged-event-contract',launcherSubstitution:false,installerSubstitution:false},null,2));
