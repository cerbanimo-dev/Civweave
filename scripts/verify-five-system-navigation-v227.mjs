...
for(const [system,pathname] of Object.entries(paths)){
  ...
  const scriptPaths=result.appended.map(node=>{try{return new URL(String(node.src||''),'https://civweave.test').pathname}catch{return''}}).filter(Boolean);
  assert.deepEqual(scriptPaths,[coreRuntimePath],`${system} must receive exactly one canonical interface runtime from the install boundary.`);
  for(const retired of retiredCanonicalChatScripts)assert.ok(!scriptPaths.includes(retired),`${system} resurrected retired canonical chat runtime ${retired}.`);
  if(system==='civweave'){assert.equal(result.root.dataset.installBoundary,'canonical');assert.equal(result.root.dataset.civweaveCanonicalCore,'only');}
  else{assert.equal(result.root.dataset.installBoundary,'canonical-system');assert.equal(result.root.dataset.civweaveCanonicalRealm,'self-contained');}
}
...
console.log(JSON.stringify({ok:true,version,revision:'five-system-navigation-v227-core-interface-runtime-v1-browser-boundary-v228-install-only-pwa-v1',systems:Object.keys(paths),routeMatrix:25,browserRequiresInstalledDisplay:true,boundaryInstalledAuthorization:true,boundaryRuntimeScripts:1,sharedBootScripts,sharedBootScriptCount:sharedBootScripts.length,settingsOwner:'settings-gateway-v317',canonicalChatOwner:'guide-workspace-v242',interfaceRuntime:'core-interface-runtime-v1',retiredCanonicalChatScripts,workerPackageHeader:true,workerFallback:'exact-route-or-visible-recovery',backgroundCampus:true,guideWorkspace:'v250-v242-canonical-owner',workingCampusTopbar:'v243',systemsMesh:'v251-five-system-non-privileged-event-contract',nodeAiMesh:'v1-node-owned-service-discovery-routing',questVeil:'v1-mandatory-human-ledger-gate-plus-mesh-batches',launcherSubstitution:false,installerSubstitution:false},null,2));
