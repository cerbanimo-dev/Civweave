...
const prettyPaths={
  civweave:'/app/working-campus-v156',
  'living-school':'/app/cabinets/living-school',
  cerbanimo:'/app/realm-console-v140',
  fellowfare:'/app/fellowfare-cabinet-v144',
  anarchadia:'/app/anarchadia-console-v139'
};
...
for(const [system,pathname] of Object.entries(prettyPaths))assert.equal(routeApi.identify(pathname),system,`${system} does not recognize its Cloudflare Pages pretty URL.`);
...
for(const [system,pathname] of Object.entries(prettyPaths)){
  const result=boundaryRuntime(pathname);assert.equal(result.replaced.length,0,`${system} pretty URL redirects to the installer.`);assert.equal(result.context.CivweaveInstallBoundaryV146.systemSurface(),system,`${system} pretty URL loses first-class route identity.`);for(const script of experienceScripts)assert.ok(result.appended.some(node=>String(node.src||'').includes(script)),`${system} pretty URL does not load ${script}.`);
}
...
console.log(JSON.stringify({ok:true,version,revision:'five-system-navigation-v227',systems:Object.keys(paths),routeMatrix:25,boundaryIntrinsicAuthorization:true,experienceScripts,canonicalExperienceScriptCount:experienceScripts.length,canonicalChatOwner:'guide-workspace-v242',retiredCanonicalChatScripts,workerPackageHeader:true,workerFallback:'exact-route-or-visible-recovery',backgroundCampus:true,guideWorkspace:'v250-v242-canonical-owner',workingCampusTopbar:'v243',systemsMesh:'v251-five-system-non-privileged-event-contract',nodeAiMesh:'v1-node-owned-service-discovery-routing',questVeil:'v1-mandatory-human-ledger-gate-plus-mesh-batches',launcherSubstitution:false,installerSubstitution:false},null,2));
