...
const [routesSource,boundarySource,navSource,campusSource,campusPart4,workerWrapper,workerNavigation,gatewayBase,gatewayWrapper,...pages]=await Promise.all([
  ...
  read(`releases/${version}/server/server-gateway-v131-base.mjs`),
...
const routeImport=workerWrapper.indexOf("importScripts('/app/system-routes-v227.js");const coreImport=workerWrapper.indexOf("importScripts('/service-worker-core-v208.js");const shellRepairImport=workerWrapper.indexOf("importScripts('/service-worker-shell-repair-v225.js");const canonicalImport=workerWrapper.indexOf("importScripts('/service-worker-canonical-navigation-v227.js");assert.ok(routeImport>=0&&routeImport<coreImport,'Worker does not load the route contract before the core.');assert.ok(canonicalImport>shellRepairImport,'Canonical navigation is not the final worker policy.');assert.match(workerNavigation,/headers\.set\('x-civweave-package',REVISION\)/);assert.match(workerNavigation,/exact-route-cache-only-never-runtime-network-fallback/);assert.match(workerNavigation,/runtimeNetworkFallback:false/);assert.match(workerNavigation,/precacheCanonicalRoutes/);assert.doesNotMatch(workerNavigation,/fetch\(packageRequest\(request\)\)/,'Canonical runtime navigation must never fetch a missing route from the network.');assert.doesNotMatch(workerNavigation,/stableAppEntry\(/);assert.doesNotMatch(workerNavigation,/findCached\('\/app\/index\.html'\)/);assert.doesNotMatch(workerNavigation,/findCached\('\/offline\.html'\)/);for(const pathname of Object.values(paths))assert.ok(routesSource.includes(`pathname:'${pathname}'`),`Route contract is missing ${pathname}.`);
...
console.log(JSON.stringify({ok:true,version,revision:'five-system-navigation-v227-local-first-settings-gateway-v317-browser-boundary-v228-install-only-pwa-v1',systems:Object.keys(paths),routeMatrix:25,browserRequiresInstalledDisplay:true,boundaryInstalledAuthorization:true,experienceScripts,canonicalExperienceScriptCount:experienceScripts.length,settingsOwner:'settings-gateway-v317',canonicalChatOwner:'guide-workspace-v242',retiredCanonicalChatScripts,workerPackageHeader:true,workerRuntimeNetworkFallback:false,workerFallback:'exact-local-route-or-visible-package-recovery',backgroundCampus:true,guideWorkspace:'v250-v242-canonical-owner',workingCampusTopbar:'v243',systemsMesh:'v251-five-system-non-privileged-event-contract',nodeAiMesh:'v1-node-owned-service-discovery-routing',questVeil:'v1-mandatory-human-ledger-gate-plus-mesh-batches',launcherSubstitution:false,installerSubstitution:false},null,2));
