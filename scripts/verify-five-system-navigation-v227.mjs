...
assert.ok(chatRevision,'Canonical guide-chat owner must expose a revisioned filename.');
const paths={
...
assert.match(boundarySource,/canonicalSystemCount:5/);assert.match(boundarySource,/canonicalExperienceScripts:SYSTEM_EXPERIENCE_SCRIPTS\.length/);assert.match(boundarySource,/settingsGatewayRevision:'v317-single-owner-first-click-only'/);assert.match(boundarySource,/systemsMeshRevision:'v251-five-system-non-privileged-event-contract'/);assert.match(boundarySource,/nodeAiMeshRevision:'v1-node-owned-service-discovery-routing'/);assert.match(boundarySource,/questVeilRevision:'v1-mandatory-human-ledger-gate-plus-mesh-batches'/);assert.match(boundarySource,/questVeilHumanLedgerPolicy:'raw-task-chronicle-never-human-visible'/);assert.match(boundarySource,/questVeilMeshBountyPolicy:'per-item:learning=1-acorn;labor-material-exchange=1-button'/);assert.ok(boundarySource.includes(`canonicalPolicy:'five-system-first-class-routes-${chatRevision}-canonical-chat-owner'`));assert.match(boundarySource,/radioTrackSuggestionRevision:'v241-playlist-context-track-links'/);assert.match(boundarySource,/campusBackgroundDownloadRevision:'v241-worker-owned-download-bottom-progress-rail'/);assert.ok(boundarySource.includes(`guideWorkspaceRevision:'${chatRevision}-single-current-chat-surface'`));assert.ok(boundarySource.includes(`guideSurfaceOwnershipPolicy:'${chatRevision}-single-current-surface-five-private-ledgers-handover-only-cross-realm'`));assert.match(boundarySource,/workingCampusTopbarRevision:'v243-sticky-top-map-launch-contract'/);assert.match(boundarySource,/mapLaunchRevision:'v243-register-route-handler-or-open-event'/);assert.match(boundarySource,/browserBoundaryRevision:'v228-installed-only-stale-session-chat-escape-install-only-pwa-v1'/);assert.match(boundarySource,/browserRuntimePolicy:'installed-display-only'/);assert.match(boundarySource,/installedQueryIsAuthorization:false/);
const realmIndex=experienceScripts.indexOf('/app/realm-session-integrity-v237.js'),chatIndex=experienceScripts.indexOf(chatOwnerPath),topbarIndex=experienceScripts.indexOf('/app/working-campus-topbar-v243.js');assert.equal(chatIndex,realmIndex+1,'Canonical guide chat must load immediately after realm-local thread ownership.');assert.equal(topbarIndex,chatIndex+1,'Working Campus topbar must load immediately after canonical guide-chat ownership.');
...
console.log(JSON.stringify({ok:true,version,revision:'five-system-navigation-v227-settings-gateway-v317-browser-boundary-v228-v350-chat-install-only-pwa-v1',systems:Object.keys(paths),routeMatrix:25,browserRequiresInstalledDisplay:true,boundaryInstalledAuthorization:true,experienceScripts,canonicalExperienceScriptCount:experienceScripts.length,settingsOwner:'settings-gateway-v317',canonicalChatOwner:chatOwner,retiredCanonicalChatScripts,workerPackageHeader:true,workerFallback:'exact-route-or-visible-recovery',backgroundCampus:true,guideWorkspace:`${chatRevision}-single-current-chat-surface`,workingCampusTopbar:'v243',systemsMesh:'v251-five-system-non-privileged-event-contract',nodeAiMesh:'v1-node-owned-service-discovery-routing',questVeil:'v1-mandatory-human-ledger-gate-plus-mesh-batches',launcherSubstitution:false,installerSubstitution:false},null,2));