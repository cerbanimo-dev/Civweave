...
const checks = [
  ...
  [installerBridge.includes("new URLSearchParams(location.search).get('host')"), 'Host Node lobby is host-query gated'],
  ...
  [lobby.includes("/api/host-node-status"), 'lobby reads same-origin Host Node status proxy'],
  [lobby.includes('Free slots') && lobby.includes('Paid slots'), 'lobby exposes free and paid slot counters'],
  [lobby.includes('Join this Host'), 'lobby exposes Join this Host action'],
  ...
  [status.includes('host-node-not-allowed'), 'status proxy rejects arbitrary host targets'],
];
