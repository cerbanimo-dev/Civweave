...
assert.ok(bridge.includes("const INSTALL_CAPABILITY_KEY='civweave.pwa.installed-capability.v1'"),'native install bridge must persist a same-origin installed capability');
assert.ok(bridge.includes('navigator.getInstalledRelatedApps()'),'installer must discover related installs when supported');
assert.ok(bridge.includes("rememberInstalled('appinstalled')"),'successful app installation must persist the installed capability');
assert.ok(bridge.includes("rememberInstalled('getInstalledRelatedApps')"),'existing related installs must recover the installed capability');
assert.ok(bridge.includes("text:'Open Civweave'"),'installed state must expose an Open Civweave action');
assert.ok(bridge.includes("browserRuntimePolicy:'installed-capability-or-installed-display'"),'installer bridge must authorize installed capability when display-mode reporting is unreliable');
...
assert.ok(bridge.includes('eagerRelatedAppDiscovery:true'),'installer must recover already-installed apps without forcing reinstall');
...
assert.ok(bridge.includes("if(standalone()||installed||capability)"),'installer must open the campus for either installed display or verified installed capability');
...
assert.ok(installedEntryHtml.includes("const INSTALL_CAPABILITY_KEY='civweave.pwa.installed-capability.v1'"),'installed-entry gate must share the persisted capability key');
assert.ok(installedEntryHtml.includes('navigator.getInstalledRelatedApps()'),'installed-entry gate must recover existing installs when display mode is misreported');
assert.ok(installedEntryHtml.includes("policy:'installed-display-or-verified-installed-capability'"),'installed-entry gate must declare its authorization policy');
...
assert.ok(installedEntry.includes('await installedLaunchAuthorized()'),'installed bootstrap must await installed capability recovery before redirecting');
assert.ok(installedEntry.includes("browserRuntimePolicy:'installed-display-or-verified-installed-capability'"),'installed bootstrap must declare capability-aware runtime policy');
...
console.log(JSON.stringify({
  ok:true,
  revision:'pwa-install-campus-v250-installed-capability-v1',
  canonicalOrigin,
  stagingOrigin,
  previousCanonicalOrigin,
  browserRuntime:'installed-display-or-verified-installed-capability',
