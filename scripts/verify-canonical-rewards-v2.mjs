...
const sources=['public/app/cw-reward-ledger-v2.js','public/app/cw-reward-receivers-v2.js','public/app/cw-reward-surfaces-v2.js'];
...
const noop=()=>{},document={readyState:'complete',documentElement:{},body:{},getElementById:()=>null,querySelector:()=>null,querySelectorAll:()=>[]};
...
assert.equal(exact.exactSkillAmounts,true);assert.match(receivers.promptContract,/Never emit a generic rewardXp pool/);assert.match(receivers.promptContract,/Each skill amount is absolute/);
