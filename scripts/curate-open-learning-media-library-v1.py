...
EDUCATIONAL=re.compile(r"\b(?:teach|teaching|learn|learning|lesson|lecture|course|classroom|student|tutorial|guide|workshop|training|education|curriculum|explain|introduction|overview|demonstration|demo|example|walkthrough|how[- ]to|presentation|seminar|method|framework|technique|practice|basics|beginner|fundamentals)\b",re.I)
TITLE_INSTRUCTIONAL=re.compile(r"\b(?:learn|lesson|lecture|course|tutorial|guide|workshop|training|introduction|intro|overview|demo|walkthrough|how[- ]to|basics|beginner|fundamentals|explained|understanding|history|geography|civics|biology|physics|chemistry|astronomy|math|computing|art|health|nutrition|philosophy|ethics|coding|programming|prompt|algorithm|logic|reasoning|finance|cooking|first aid|maintenance|career|electronics|woodworking|sewing|drawing|design|photography|statistics|climate|rights|law)\b",re.I)
...
def main():
    ...
    for r in before:
        ...
        for slug in sorted(x for x in candidates if x in topics):
            t=topics[slug]; alias_hits=hits(t.get("aliases"),f"{r.get('title','')} {r.get('description','')}")
            concept_hits=hits(t.get("concepts"),f"{r.get('title','')} {r.get('description','')}")
        ...
        elif len(rel_rejected)<120: rel_rejected.append({"provider":r.get("provider"),"title":r.get("title"),"topic_slug":r.get("topic_slug")})

    pedagogical=[]; ped_rejected=[]
    for r in relevant:
        title=str(r.get("title") or ""); desc=str(r.get("description") or ""); educational=bool(EDUCATIONAL.search(title+" "+desc)); clickbait=bool(CLICKBAIT.search(title)); decisions={}
        for slug in sorted((r.get("relevance") or {}).keys()):
            aliases=topics[slug].get("aliases") or []; title_hits=hits(aliases,title); desc_hits=hits(aliases,desc)
            ok=bool(title_hits) or (bool(desc_hits) and educational and not clickbait)
            ...
            if not ok: continue
            score=20*len(title_hits)+8*len(desc_hits)+(5 if educational else 0)+(6 if r.get("mesh_redistributable") else 0)+min(6,int(r.get("quality_score") or 0)//15)-(30 if clickbait else 0)
            decisions[slug]={"strong_title_matches":title_hits,"strong_description_matches":desc_hits,"educational_intent":educational,"clickbait_title":clickbait,"pedagogy_score":score}
        if decisions: r["pedagogy"]=decisions; pedagogical.append(r)
        elif len(ped_rejected)<120: ped_rejected.append({"provider":r.get("provider"),"title":r.get("title"),"relevance_topics":sorted((r.get("relevance") or {}).keys())})

    kept=[]; sel_rejected=[]
    for r in pedagogical:
        accepted={}; reasons={}
        for slug,e in sorted((r.get("pedagogy") or {}).items()):
            if e.get("strong_title_matches"): ok,reason=True,"strong-topic-anchor-in-title"
            elif e.get("strong_description_matches") and TITLE_INSTRUCTIONAL.search(str(r.get("title") or "")): ok,reason=True,"strong-description-anchor-plus-instructional-title"
            else: ok,reason=False,"description-only-without-instructional-title"
            reasons[slug]=reason
            if ok: accepted[slug]={"reason":reason,"pedagogy_score":int(e.get("pedagogy_score") or 0)}
        if not accepted:
            if len(sel_rejected)<120: sel_rejected.append({"provider":r.get("provider"),"title":r.get("title"),"reasons":reasons})
            continue
    ...
    outputs={
      ...
      "relevance-audit.json":{"schema":"civweave.open-learning-media-relevance-audit.v2","built_at":catalog.get("built_at"),"before":len(before),"after":len(relevant),"rejected":len(before)-len(relevant),"topics":topic_stats,"rejected_samples":rel_rejected},
      "pedagogy-audit.json":{"schema":"civweave.open-learning-media-pedagogy-audit.v2","built_at":catalog.get("built_at"),"before":len(relevant),"after":len(pedagogical),"rejected":len(relevant)-len(pedagogical),"topics":topic_stats,"rejected_samples":ped_rejected},
      "selection-audit.json":{"schema":"civweave.open-learning-media-selection-audit.v2","built_at":catalog.get("built_at"),"before":len(pedagogical),"after":len(kept),"rejected":len(pedagogical)-len(kept),"topics":topic_stats,"packs":pack_stats,"rejected_samples":sel_rejected},
