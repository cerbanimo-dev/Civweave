...
const [identity,boundary,workspace,assistant,platform]=await Promise.all([
  ...
  read('public/app/platform-experience-v160.js')
]);

new Function(identity);new Function(boundary);new Function(workspace);new Function(platform);
for(const token of [
  ...
  "reason:explicit?'explicit-handoff':routed?'explicit-action-route'",
...
assert(!platform.includes('patchAssistant')&&!platform.includes("Object.defineProperty(globalThis,'CivweaveAssistantV141'"),'Platform HUD regained assistant interception.');
...
sandbox.CivweaveAssistantV141={respond:async options=>({response:{answer:'Identity held.',choice:{system:options.systemId,nextAction:'Continue.'}},action:{id:'a1'},context:{guide:{system:options.systemId}},provider:'test-provider',model:'test-model'})};
...
sandbox.location.pathname='/app/working-campus-v156.html';sandbox.location.search='';sandbox.document.documentElement.dataset.civweaveSystemRoute='civweave';
const routed=await sandbox.CivweaveAssistantV141.respond({systemId:'civweave',text:'Please add dark mode',history:[]});
assert.equal(routed.respondingSystem,'anarchadia');assert.equal(routed.handoffReason,'explicit-action-route');assert.match(routed.response.answer,/Weaveling routed this to Anarchadia/);
console.log(JSON.stringify({ok:true,revision:'v216-identity-integrity-v242-routing-owner',canonicalChatOwner:'guide-workspace-v242',actionRoutingOwner:'guide-identity-integrity-v216',legacyChatMigration:false,documentObservers:0,parallelAssistantPatchers:0},null,2));