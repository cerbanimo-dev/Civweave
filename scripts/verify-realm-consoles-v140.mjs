...
const [html,css,js,launcher,launcherCss,shellsText,sw,server]=await Promise.all([
  read('public/app/realm-console-v140.html'),read('public/app/realm-console-v140.css'),read('public/app/realm-console-v140.js'),read('public/app/v130-cabinet-launcher.js'),read('public/app/v130-cabinet-launcher.css'),read('public/app/shared/cabinet-shells-v129.json'),read('public/service-worker.js'),read('server-v130.mjs')
]);
...
assert(launcher.includes('/app/realm-console-v140.html'),'non-Anarchadia realms do not use the new realm console');
...
assert(sw.includes("CACHE_REVISION='realm-cabinets-r18'"),'service worker cache was not rotated');
...
assert(server.includes("'/app/realm-console-v140.html'"),'host does not serve realm console HTML before legacy redirects');
console.log(JSON.stringify({ok:true,systems:ledger.systems.map(s=>({id:s.id,rooms:s.rooms.length,capabilities:ledger.capabilities.filter(c=>c.system===s.id).length})),physicalCabinets:Object.keys(shells),cacheRevision:'realm-cabinets-r18'},null,2));
