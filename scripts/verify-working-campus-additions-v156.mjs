...
for(const token of ['createOffer','acceptOffer','acceptAnswer','friend-offer-v156','friend-answer-v156','/api/envelopes','publishTrade','requestValidation','publishValidation','scrub','syncNode'])assert(mesh.includes(token),`Mesh tools are missing ${token}.`);
assert(mesh.includes('BarcodeDetector')&&mesh.includes('CommonweaveQRV156.canvas'),'QR friend pairing is incomplete.');
for(const token of ["const PENDING_KEY='commonweave.friend-pairings.pending.v156'",'sessionStorage.getItem(PENDING_KEY)','sessionStorage.setItem(PENDING_KEY','crypto.subtle.digest(\'SHA-256\'','name:\'HMAC\'','answerProof','verifyAnswerProof','/#cwfriend='])assert(mesh.includes(token),`Private QR rendezvous is missing ${token}.`);
assert(!mesh.includes('localStorage.setItem(PENDING_KEY'),'Pending QR secrets are stored persistently.');
assert(!mesh.includes("payload:{schema:'commonweave.friend-offer.v156',token"),'Friend offer exposes the QR invitation secret to the node.');
assert(!mesh.includes("payload:{schema:'commonweave.friend-answer.v156',token"),'Friend answer echoes the QR invitation secret to the node.');
assert(mesh.includes("to:id,kind:'friend-offer-v156'")&&mesh.includes('correlationId:id'),'Friend offers are not addressed by their non-secret rendezvous hash.');
...
console.log(JSON.stringify({ok:true,foundation:'working-campus-v155-retained',additions:['encrypted-shared-ai-vault','typed-cross-system-requests','append-only-rewards','private-qr-friend-rendezvous','signed-mesh-trade','peer-ai-validation'],offlineWorker:'service-worker-v156',qrVersion:matrix.version},null,2));
