...
const [campus,campusPart4,boundary,worker,installer,pwa,vault,domain,qr,mesh,additions,css,validator,modelDownload,baseWorker]=await Promise.all([
  ...
  read('public/extensions/civweave-secure-vault-v156.js'),
  read('public/extensions/civweave-domain-bridge-v156.js'),
  read('public/extensions/civweave-qr-v156.js'),
  read('public/extensions/civweave-mesh-tools-v156.js'),
...
assert(campus.includes('Civweave Working Campus')&&campus.includes('/app/working-campus-v156.js'),'PR #56 source-based Working Campus is not the retained foundation.');
assert(!campus.includes("const payload='H4sI"),'The obsolete packed Working Campus payload was reintroduced.');
for(const token of ['test-gemini','test-antigravity','civweave-model-profiles-v1','agenticEnabled'])assert(campus+campusPart4.includes(token),`PR #56 native AI setup is missing ${token}.`);
for(const token of ['/extensions/civweave-additions-v156.js','/extensions/civweave-additions-v156.css','/extensions/civweave-model-download-v157.js','installAdditions','v157-fast-core'])assert(boundary.includes(token),`Install boundary is missing direct source-page additive hook ${token}.`);
...
for(const token of ["importScripts('/service-worker.js?v=1.0.4-base-r37-core')",'cwext-working-campus-additions-v157-fast-core','civweave-secure-vault-v156.js','civweave-domain-bridge-v156.js','civweave-qr-v156.js','civweave-mesh-tools-v156.js','civweave-model-download-v157.js','patchInstalledBoundary','GET_ADDITIONS_STATUS'])assert(worker.includes(token),`Additive device worker is missing ${token}.`);
...
for(const token of ['data-download-local-model','adapter.install','civweave:model-package-ready','Download local model'])assert(modelDownload.includes(token),`Explicit model download runtime is missing ${token}.`);
for(const token of ['AES-GCM','PBKDF2','250000','civweave-model-profiles-v1','civweave.model-secret.v1','sessionStorage','remember','unlock','scrubPlaintext'])assert(vault.includes(token),`Secure vault is missing ${token}.`);
...
for(const token of ['civweave.learning-request.v156','civweave.task-request.v156','civweave.materials-request.v156','civweave.reward-ledger.v156','cerbanimo.ai-reviews.v156','civweave.cerbanimo.peer-reviews.v156','qualifiedProvider','recordPeerReview','peerReviewStatus','patchCerbanimoEngine'])assert(domain.includes(token),`Domain and reward bridge is missing ${token}.`);
...
assert(mesh.includes('BarcodeDetector')&&mesh.includes('CivweaveQRV156.canvas'),'QR friend pairing is incomplete.');
for(const token of ["const PENDING_KEY='civweave.friend-pairings.pending.v156'",'sessionStorage.getItem(PENDING_KEY)','crypto.subtle.digest(\'SHA-256\'','name:\'HMAC\'','answerProof','verifyAnswerProof','/#cwfriend='])assert(mesh.includes(token),`Private QR rendezvous is missing ${token}.`);
...
const sandbox={console,TextEncoder,Uint8Array,Array,Math,Error,Object,String,Number,Boolean,JSON,globalThis:null};sandbox.globalThis=sandbox;vm.createContext(sandbox);vm.runInContext(qr,sandbox,{filename:'civweave-qr-v156.js'});
const code='https://civweave-host-node.onrender.com/#cwfriend=123e4567-e89b-12d3-a456-426614174000';
const matrix=sandbox.CivweaveQRV156.matrix(code);
