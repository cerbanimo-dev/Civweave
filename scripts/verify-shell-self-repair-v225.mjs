...
for(const token of ['REPAIR_DEVICE_PACKAGE','repairableCacheShell','selfRepairingShellStatus','reject-incomplete-install-repair-active-shell-no-browser-runtime'])assert(source.includes(token),`Shell repair is missing ${token}.`);
assert(!source.includes('cacheShell = async function'),'Shell repair must not replace the strict install-time cacheShell binding.');
assert(source.includes('if (!self.CivweaveInstalledShellRepairV293)'),'Legacy repair responder must stand down when v293 is present.');
...
await assert.rejects(context.cacheShell(),/App shell incomplete/);assert.equal(attempts,1);assert.equal(cached,false);
...
const handler=listeners.find(row=>row.type==='message')?.handler;assert(handler);let promise=null;handler({data:{type:'REPAIR_DEVICE_PACKAGE'},waitUntil:value=>{promise=value},ports:[],source:null});await promise;assert.equal(replies.at(-1)?.type,'CIVWEAVE_DEVICE_PACKAGE_REPAIR');assert.equal(replies.at(-1)?.ready,true);
console.log(JSON.stringify({ok:true,version,revision,activatedIncomplete:false,strictInstall:true,retryAttempts:attempts,repaired:true,onlineFallback:false,repairOnly:true,cacheDistinctRepair:true,stableWorkerEpoch:true,canonicalNavigationFinal:true,manualFirst:true},null,2));