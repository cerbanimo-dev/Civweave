...
for(const token of ['Open online campus','Open Civweave online','installed-entry-v146','legalGate','launch','online'])assert(installerFallback.includes(token),`Installer fallback is missing ${token}.`);
assert.ok(!installerFallback.includes('working-campus-v156.html'),'Installer fallback must not bypass the Civweave Terms gate by routing directly into campus.');
...
console.log(JSON.stringify({ok:true,version,revision,activatedIncomplete:true,retryAttempts:attempts,repaired:true,onlineFallback:true,canonicalNavigationFinal:true,manualFirst:true,termsGatePreserved:true},null,2));