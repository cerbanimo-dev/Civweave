...
const [source,wrapper,indexHtml,installerRepair,canonicalNavigation,versionText,packageText]=await Promise.all([
  readFile(path.join(root,'public/service-worker-shell-repair-v225.js'),'utf8'),readFile(path.join(root,'public/service-worker-v203.js'),'utf8'),readFile(path.join(root,'public/app/index.html'),'utf8'),readFile(path.join(root,'public/app/installer-repair-only-v1.js'),'utf8'),readFile(path.join(root,'public/service-worker-canonical-navigation-v227.js'),'utf8'),readFile(path.join(root,'VERSION'),'utf8'),readFile(path.join(root,'package.json'),'utf8')
]);
...
assert(indexHtml.includes('offline campus remains a separate resumable code download'),'Installer copy no longer preserves the manual-first campus boundary.');
assert(!indexHtml.includes('id="open-online-campus-v225"'),'Installer must not expose an online-campus fallback.');
assert(!indexHtml.includes('Browser fallback'),'Installer must not advertise browser runtime.');
for(const token of ['REPAIR_DEVICE_PACKAGE','repairableCacheShell','selfRepairingShellStatus','activate-incomplete-retry-required-shell-and-report-paths-install-only'])assert(source.includes(token),`Shell repair is missing ${token}.`);
for(const token of ['repairInstalledShell','REPAIR_DEVICE_PACKAGE','browserRuntimePolicy','installer-only-until-installed-display'])assert(installerRepair.includes(token),`Installer repair bridge is missing ${token}.`);
assert(!installerRepair.includes('function openCampus'),'Installer repair bridge must not contain an online-campus launcher.');
...
assert.doesNotThrow(()=>new vm.Script(source));assert.doesNotThrow(()=>new vm.Script(installerRepair));
...
console.log(JSON.stringify({ok:true,version,revision,activatedIncomplete:true,retryAttempts:attempts,repaired:true,onlineFallback:false,browserRuntime:false,canonicalNavigationFinal:true,manualFirst:true},null,2));