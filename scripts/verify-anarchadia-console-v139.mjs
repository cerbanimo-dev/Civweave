...
const [html,css,js,launcher,adapter,worker,serviceWorker]=await Promise.all([
  ...
  read('public/app/anarchadia-console-v139.css'),
  read('public/app/anarchadia-console-v139.js'),
...
for(const required of [
  ...
  'CommonweaveReflexRuntime',
  'CommonweaveModelRuntime',
  'commonweave-safe-scaffolder',
  'validatePatch',
  'sandbox-install',
  'preview-ready',
  'vote-signal',
...
assert(serviceWorker.includes("CACHE_REVISION='minilm-runtime-r18'"),'service worker revision was not rotated');
...
console.log(JSON.stringify({
  ...
  minilmStatusProbe:'small cached files measured; ONNX binaries stream without blocking on Cache Storage',
