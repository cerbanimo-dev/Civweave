...
assert.ok(bridge.includes('globalThis.CivweavePWAInstallV249=api'),'v249 must remain the canonical install API');
assert.ok(bridge.includes('globalThis.CivweavePWAInstallV248=api')&&bridge.includes('globalThis.CivweavePWAInstallV247=api')&&bridge.includes('globalThis.CivweavePWAInstallV246=api'),'v249 must retain compatibility aliases without shipping parallel old runtimes');
...
console.log(JSON.stringify({
  ok:true,
  revision:'fresh-install-paths-v3-canonical-v249',
  exactEntry,
  canonicalBridge:'/app/pwa-install-prompt-v249.js',
  parallelPromptRuntimes:false,
