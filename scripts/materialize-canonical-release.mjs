...
const versions=[];for(const entry of await fsp.readdir(releasesRoot,{withFileTypes:true})){if(entry.isDirectory()&&entry.name!==version&&/^\d+\.\d+\.\d+$/.test(entry.name)&&await exists(path.join(releasesRoot,entry.name,'release.json')))versions.push(entry.name)}versions.sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));const previous=versions.at(-1);if(!previous)throw new Error('No prior canonical release exists to materialize from.');
