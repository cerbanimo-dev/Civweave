...
assert.equal(vars.find(row=>row.id==='education.acornsPerHour').currentValue,100);
assert.equal(vars.find(row=>row.id==='education.curriculumMinAcorns').currentValue,50);
assert.equal(vars.find(row=>row.id==='education.curriculumMaxAcorns').currentValue,500);
...
assert.match(review.rubric.find(row=>row.id==='education-baseline').text,/100 Acorns per educational hour/);
assert.match(review.rubric.find(row=>row.id==='education-baseline').text,/50–500 Acorns/);
...
const curriculum=review.valuationFrom({subject:{id:'module-a',kind:'curriculum',existing:{}},estimate:{educationalHours:2,curriculumAcorns:300,rationale:'two-hour module'},review:{decision:'fair',confidence:.9,criteria:passes('curriculum'),rationale:'within rubric',stabilityImpact:'neutral'}});
assert.equal(curriculum.baseline.acorns,300);assert.equal(curriculum.pricingReady,true);
...
assert.equal(schema.properties.modules.items.properties.curriculumAcorns.minimum,50);
assert.equal(schema.properties.modules.items.properties.curriculumAcorns.maximum,500);
...
assert.match(model.promptContract,/100 🌰 Acorns per hour/);
assert.match(model.promptContract,/50–500 🌰 Acorns/);

for(const token of ['civweave.working-campus.v1','civweave.living-school.cabinet.v151','cerbanimo.quest-engine.v144','review.reviewSubjects','model-reviewed-fair','valuationRationale','Economic value review','uniform rate','eventDriven:true','polling:false','initialRun:false','domObserver:false'])assert.ok(systemsSource.includes(token),`systems propagation omits ${token}`);
assert.doesNotMatch(systemsSource,/setInterval\s*\(/,'economic valuation must not poll while Civweave is idle');
assert.doesNotMatch(systemsSource,/MutationObserver/,'economic valuation must not use DOM churn as an inference trigger');
assert.doesNotMatch(systemsSource,/civweave:assistant-runtime-ready[^\n]*schedule/,'assistant readiness must not trigger a background valuation run');
for(const token of ['/app/civweave-basic-value-v1.js?v=economic-review-v1','/app/civweave-economic-policy-v1.js?v=economic-review-v1','/app/civweave-basic-value-model-v1.js?v=economic-review-v1','/app/civweave-basic-value-review-v1.js?v=economic-review-v1','/app/civweave-basic-value-systems-v1.js?v=economic-review-v1-idle-safe-v1','await loadValueCore()','await loadValueModel()','requestEconomicReview','civweave:working-campus-plan-built','cerbanimo:quest-engine-changed','localAIOptionalSideEffects:false'])assert.ok(loaderSource.includes(token),`family AI loader omits ${token}`);
assert.doesNotMatch(loaderSource,/const OPTIONAL=[\s\S]*?local-ai\/bootstrap-v266\.js[\s\S]*?\];/,'generic assistant optional warmup must not bootstrap local AI');
...
console.log(JSON.stringify({ok:true,authority:'model-interim',futureAuthority:'anarchadia-democratic',laborWage:'5 Buttons/hour uniform starting wage',acornPriceScale:'100 Acorns/hour; curriculum 50–500 Acorns',hierarchy:'models estimate hours only; wage rate is identical for every worker',review:'upstream model proposal -> independent rubric review -> fair/adjust/reject',rubricGate:'all required fairness criteria must pass before price readiness',backgroundReview:'event-driven-only-no-idle-polling',stability:'candidate sinks/demurrage/issuance ceilings are explicit but disabled and unset'},null,2));
