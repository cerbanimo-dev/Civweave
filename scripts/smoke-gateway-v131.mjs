...
const PORT=18792,origin=`http://127.0.0.1:${PORT}`,VERSION='1.0.7',BUILD='1.0.7-install-only-fullscreen-family-gateway';
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..'),dataDir=await mkdtemp(path.join(os.tmpdir(),'commonweave-gateway-v107-')),output=[];
...
try{
  ...
  const rootResponse=await fetch(`${origin}/`,{cache:'no-store'}),rootHtml=await rootResponse.text();assert(rootResponse.ok,'gateway installer root failed');assert(rootHtml.includes('Install Commonweave'),'gateway root is not the Commonweave installer');assert(rootHtml.includes('No native modal'),'gateway root does not describe the fixed settings layer');assert(rootHtml.includes('/app/logos/commonweave-app-icon.png'),'gateway root does not use the app icon');assert(rootHtml.includes('/app/offline-campus-status-v210.js'),'gateway root is missing the release-version synchronizer');
  ...
  for(const forbidden of ['all-minilm-l6-v2','transformers.min.js','ort-wasm','minilm-reflex','minilm-model-settings','commonweave-settings-safe-open'])assert(!requiredAssets.some(route=>route.includes(forbidden)),`v1.0.7 package still includes ${forbidden}`);
  ...
  const manifest=await fetch(`${origin}/app/manifest.webmanifest`,{headers:packageHeaders}).then(response=>response.json());
  assert(manifest.name==='Commonweave v1.0.7','PWA manifest name is not v1.0.7');
  assert(String(manifest.start_url||'').includes('version=1.0.7'),'PWA manifest start URL is not v1.0.7');
  const releaseSync=await fetch(`${origin}/app/offline-campus-status-v210.js`,{headers:packageHeaders}).then(response=>response.text());
  assert(releaseSync.includes("const RELEASE_VERSION='1.0.7'")&&releaseSync.includes('syncReleaseVersion()'),'installer release synchronizer is not v1.0.7');
  ...
  const campus=await fetch(`${origin}/app/working-campus-v156.html`,{headers:packageHeaders}).then(response=>response.text());assert(campus.includes('/app/logos/commonweave-app-icon.png')&&campus.includes('version-chip')&&!campus.includes('/app/logos/commonweave.webp'),'Working Campus header is stale');
  const themedNav=await fetch(`${origin}/app/themed-system-nav-v178.js`,{headers:packageHeaders}).then(response=>response.text());
  assert(themedNav.includes("const RELEASE_VERSION='1.0.7'")&&themedNav.includes('--cw-themed-nav-button-width:200px')&&themedNav.includes('max-width:var(--cw-themed-nav-button-width)'),'family navigation release marker or size cap is stale');
  ...
  console.log(JSON.stringify({ok:true,version:VERSION,build:BUILD,requiredCoreAssetCount:requiredAssets.length,defaultProvider:'deterministic',settingsPresentation:'cleanroom-v188',nativeDialog:false,outsideTap:'safe-close',settingsTransformerWork:false,providerRuntimeOnOpen:false,providerTestsAvailable:false,modelDiscoveryAvailable:false,captureListener:false,mutationObserver:false,polling:false,timers:false,campusIconPixels:176,navigationButtonCap:'200x100'},null,2));
