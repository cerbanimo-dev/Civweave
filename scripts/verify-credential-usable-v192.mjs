...
const [settingsSource,deviceSource,runtimeSource,legacyWorker,workerWrapper,workerCore,offlineManifestText,installBoundary]=await Promise.all([
  readFile('public/app/settings-gateway-v317.js','utf8'),readFile('public/extensions/civweave-device-credentials-v160.js','utf8'),readFile('public/app/shared/civweave-model-runtime.js','utf8'),readFile('public/service-worker-v156.js','utf8'),readFile('public/service-worker-v203.js','utf8'),readFile('public/service-worker-core-v208.js','utf8'),readFile('public/app/offline-package-v208.json','utf8'),readFile('public/app/install-boundary-v146.js','utf8')]);
const offlineManifest=JSON.parse(offlineManifestText);const assert=(condition,message)=>{if(!condition)throw new Error(message)};
...
const localStorage=new MemoryStorage({'civweave-model-persistent-secrets-v191':JSON.stringify(persistent),'civweave-model-credential-policy-v191':'device','civweave-model-profiles-v1':JSON.stringify({interactive:config,agentic:null,agenticEnabled:false}),'civweave.universal-ai.v127':JSON.stringify({...config,consent:true})});
const sessionStorage=new MemoryStorage();const listeners=new Map();
const settingsUrl='https://civweave.test/app/settings-gateway-v317.js';
...
const sandbox={console,Date,Math,JSON,URL,TextEncoder,TextDecoder,AbortController,Headers,Request,Response,Promise,Set,Map,crypto:webcrypto,performance,localStorage,sessionStorage,CustomEvent,Element,HTMLElement,document,location:{href:settingsUrl,origin:'https://civweave.test'},navigator:{userAgent:'Civweave v192 verifier'},fetch:async()=>new Response('{}',{status:200,headers:{'content-type':'application/json'}}),setTimeout,clearTimeout,setInterval,clearInterval,requestAnimationFrame:null,addEventListener(type,handler){const rows=listeners.get(type)||[];rows.push(handler);listeners.set(type,rows)},removeEventListener(){},dispatchEvent(event){for(const handler of listeners.get(event.type)||[])handler(event);return true},globalThis:null,window:null,self:null};
sandbox.globalThis=sandbox;sandbox.window=sandbox;sandbox.self=sandbox;vm.createContext(sandbox);
vm.runInContext(settingsSource,sandbox,{filename:'settings-gateway-v317.js'});
const settings=sandbox.CivweaveSettingsV320;
assert(settings?.credentialOwner===true,'Canonical Settings does not own credential state.');
assert(settings.singleMenu===true,'Credential verifier is not exercising the single Settings menu.');
assert(!sessionStorage.getItem('civweave-model-session'),'Settings restored credentials merely because its module was parsed.');
assert(settings.restoreRememberedCredential()===true,'Explicit canonical Settings credential restore failed.');
const brokenSession=JSON.parse(sessionStorage.getItem('civweave-model-session'));
assert(brokenSession.apiKey===apiKey,'Canonical Settings did not restore the remembered key fixture.');
...
const secrets=JSON.parse(sessionStorage.getItem('civweave-model-secrets-v1'));const secretRows=Object.values(secrets);
...
console.log(JSON.stringify({ok:true,revision:'v192-credential-usable-v320',canonicalSettingsOwner:true,moduleLoadMutation:false,reproduced:{keyPresent:true,consentLost:true},repaired:{keyPresent:true,consentRestored:true,fingerprintedSecret:true,runtimeUsable:true},offlinePackaged:'discovered-through-working-campus-install-boundary',freezeBoundary:{observer:false,polling:false}},null,2));
