...
const [pkgText,adapter,worker,serviceWorker,materializer,workflow]=await Promise.all([
  read('package.json'),
...
for(const required of [
  "WORKER_URL = `${MODEL_ROOT}/worker.js?v=onnx-runtime-r9`",
...
assert(serviceWorker.includes("CACHE_REVISION='smollm2-backend-r9'"),'service worker cache revision is stale');
...
assert(workflow.includes("CACHE_REVISION='smollm2-backend-r9'"),'workflow does not verify r9 cache');
assert(workflow.includes('onnx-r9'),'workflow does not probe versioned backend URLs');
...
console.log(JSON.stringify({
  ...
  serviceWorkerRevision:'smollm2-backend-r9',
