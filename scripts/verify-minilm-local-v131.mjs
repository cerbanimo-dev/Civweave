...
assert(!pkg.scripts?.prestart?.includes('ensure-minilm-model.mjs'),'public gateway startup still materializes MiniLM through npm prestart');
...
assert(/^(?:cabinet-mode|fullscreen-family|direct-family)-r\d+(?:-[a-z0-9-]+)?$/i.test(cacheRevision),'service worker is not on a versioned direct Cabinet Mode cache policy');
...
console.log(JSON.stringify({ok:true,model:manifest.id,renderMaterialization:false,localMaterialization:true,graphs:'installed during device-package update',pageStartup:'model dormant until chat or settings',ordinaryRuntimeTraffic:'cache-storage only',lexicalFallback:true,cacheRevision,deviceRevision},null,2));
