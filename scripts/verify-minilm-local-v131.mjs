...
for(const token of ["pipeline('feature-extraction'",'validateProfile','env.useWasmCache=true','EMBED_BATCH_SIZE=4','output.dispose?.()','stateProfileKey','MINILM_PROFILE_CONFLICT','verifyWasm','fromDevicePackage','caches.match'])assert(worker.includes(token),`MiniLM worker missing ${token}`);
for(const retired of ['const attempts=','failures=[]','for(const [device,dtype] of attempts'])assert(!worker.includes(retired),`MiniLM worker can still retry multiple backends in one activation: ${retired}`);
assert(worker.includes("env.backends.onnx.wasm.numThreads=1"),'MiniLM WASM backend is not capped to one thread');
assert(worker.includes("env.backends.onnx.wasm.initTimeout=30000"),'MiniLM backend initialization is not bounded');
assert(!worker.includes("env.useWasmCache=false"),'MiniLM disables the ONNX WASM cache');
...
for(const token of ['BODY_PROBE_LIMIT=2_000_000','MODEL_CACHE','CIRCUIT_KEY','FAILURE_COOLDOWN_MS=10*60_000','explicit-activation-required','MINILM_CIRCUIT_OPEN','MINILM_NOT_READY','selectProfile','resetCircuit','export function shutdown','worker.js?v=device-package-r39-minilm-safe-session'])assert(adapter.includes(token),`MiniLM adapter missing ${token}`);
assert(adapter.includes("await prewarm({explicit:true"),'MiniLM benchmark does not explicitly establish a safe session');
assert(adapter.includes("if(!explicit)return{ready:false,dormant:true"),'Incidental prewarm calls can still start MiniLM');
assert(adapter.includes("if(readyState)return")&&adapter.includes("if(initPromise)return initPromise"),'MiniLM session startup is not deduplicated');
...
console.log(JSON.stringify({ok:true,model:manifest.id,serverMaterialization:true,coreInstallBlocksOnModel:false,modelDelivery:'explicit same-origin download into separate cache',pageStartup:'lexical-only until explicit safe semantic session',ordinaryRuntimeTraffic:'cache-only after explicit download',lexicalFallback:true,sessionPolicy:'one worker, one backend, chunked embeddings, failure circuit breaker',wasmCache:true,cacheRevision,deviceRevision},null,2));
