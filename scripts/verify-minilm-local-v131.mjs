...
assert(sw.includes("CACHE_REVISION='local-first-r20'"),'service worker is not on the local-first cache policy');
...
console.log(JSON.stringify({ok:true,model:manifest.id,renderMaterialization:false,localMaterialization:true,graphs:'cache on first local use',lexicalFallback:true},null,2));
