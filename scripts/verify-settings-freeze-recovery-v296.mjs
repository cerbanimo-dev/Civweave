...
assert.match(gateway,/afterPaint\(requestInferenceQuiescence\)/);
assert.match(gateway,/afterPaint\(\(\)=>void ensureManagement\(layer\)\)/);
assert.match(gateway,/removeEventListener\('click',previous,true\)/);
assert.doesNotMatch(gateway,/bootstrap-v266|local-chat-runtime-v295|CivweaveLocalModelRuntimeV266|new Worker\(/,'Settings gateway reintroduced generative startup work.');
...
const managementIndex=openBlock.indexOf('afterPaint(()=>void ensureManagement(layer))');
...
assert.ok(managementIndex>visibleIndex,'Downloaded-model management must be scheduled only after Settings is visible.');
...
assert.doesNotMatch(controller,/requestInferenceQuiescence|document\.addEventListener\('click'/,'Legacy controller regained active Settings behavior.');
