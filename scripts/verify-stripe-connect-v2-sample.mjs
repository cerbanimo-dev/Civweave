...
const [pkgText, sampleSource, entrySource, migrationSource, prepareSource, deployWorkflow] = await Promise.all([
...
// Accounts V2 creation contract. The source must use the user-requested V2
// shape and must never fall back to legacy top-level account types.
for (const needle of [
...
// 22.4.0 exposes parseEventNotificationAsync rather than the old sample name.
