...
const [platform,platformCss,realm,validator,living,livingRuntime,livingCss,anarchadia,anarchadiaRuntime,worker,transport]=await Promise.all([
...
for(const retired of ['anarchadia-governance-bridge-v145.js','anarchadia-cabinet-workbench-v144.js'])assert(!anarchadia.includes(retired),`Anarchadia still boots delayed runtime ${retired}`);
for(const token of ['data-ag145-open','data-anarchadia-workbench="workbench"','anarchadia-runtime-stability-v159.js','platform-stability-v159.js','/app/family-ai-loader-v105.js'])assert(anarchadia.includes(token),`Anarchadia stable entry missing ${token}`);
for(const eager of ['/app/assistant-runtime-v141.js','/app/shared/commonweave-model-runtime.js','/app/minilm-model-settings-v138.js'])assert(!anarchadia.includes(eager),`Anarchadia eagerly embeds ${eager} instead of using the bounded settings loader.`);
...
for(const token of ['working-campus-additions-v159-gemini-interactions-chat-tray-review-stability-researched-learning','gemini-interactions-v159','/extensions/commonweave-gemini-interactions-v159.js','/app/platform-stability-v159.js','/app/cerbanimo-ai-validator-v159.js','/app/cabinets/living-school/living-school-runtime-stability-v159.js','/app/anarchadia-runtime-stability-v159.js'])assert(worker.includes(token),`Installed-device patch missing ${token}`);
for(const token of ['CommonweaveGeminiInteractionsV159','generateWithInteractions','executionProfile','profile!==\'agentic\''])assert(transport.includes(token),`Gemini Interactions transport missing ${token}`);
new Function(platform);new Function(validator);new Function(livingRuntime);new Function(anarchadiaRuntime);new Function(transport);
console.log(JSON.stringify({ok:true,chat:'minimizable-to-tray',dialogs:'sticky-and-bottom-close',livingSchool:'direct-lessons-and-opt-in-public-research',cerbanimo:'idempotent-ai-review',anarchadia:'compacted-state-native-runtime-bounded-shared-settings',gemini:'interactions-transport-preserved'},null,2));
