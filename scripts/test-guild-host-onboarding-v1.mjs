...
assert.match(onboardingSource,/membershipKey=null/);
...
assert.match(pocketSource,/membershipKey=null/);
assert.match(pocketSource,/cloudAttached:Boolean\(origin\)/);
assert.match(pocketSource,/authorization':`Bearer \$\{value\.membershipKey\}`/);
assert.match(pocketSource,/object\.consent==='group'/);
...
assert.match(mobileCreateSource,/cloudPairingCode/);
assert.match(mobileCreateSource,/attachCloudflareEdge/);
assert.match(mobileCreateSource,/deploy\.workers\.cloudflare\.com/);
assert.match(downloadsHtml,/1\. Create local Guild/);
assert.match(downloadsHtml,/Step 2 of 2/);
assert.match(downloadsHtml,/Connect Cloudflare &amp; create public edge/);
assert.match(downloadsHtml,/CIVWEAVE_GUILD_CLAIM_TOKEN/);
assert.match(downloadsHtml,/local Guild remains valid if Cloudflare setup is interrupted/i);
...
console.log(JSON.stringify({ok:true,schema:GUILD_HOST_ONBOARDING_SCHEMA,mobilePremierRoute:'pocket-node',mobileFallback:'pocket-node-pending',mobileCloudFlow:'local-first-then-guildkeeper-edge',downloadOriginInherited:false,desktopRoute:'persistent-local-node',stewardSessionHook:true,downloadsCreationSurface:true,emergencyAiMesh:true,emergencyAiOperatorControl:true,offlineCore:true}));
