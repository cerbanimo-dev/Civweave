...
for(const token of ['CommonweaveGuideChatV153','CommonweaveModelSettingsV133','async function ensure()','openChat','openSettings','promise=null'])assert(loader.includes(token),`Lazy AI loader is missing ${token}`);
...
for(const [name,page] of [['realm',realm],['living-school',living],['fellowfare',fellowfare],['anarchadia',anarchadia]]){assert(page.includes('/app/family-shell-v104.js')&&page.includes('/app/family-ai-loader-v105.js'),`${name} does not mount the direct family shell and lazy AI loader.`);for(const eager of ['/app/shared/commonweave-model-runtime.js','/app/minilm-reflex-runtime-v138.js','/app/assistant-runtime-v141.js','/app/guide-chat-v153.js'])assert(!page.includes(eager),`${name} still eagerly loads ${eager}`)}
