...
for(const token of ['/app/realm-console-v140.html','/app/anarchadia-console-v139.html','cabinet-shells-v129.json','renderControls','preloadAll','swapArt','viewBox','Cabinet mode could not open'])assert(runtime.includes(token),`Cabinet Mode runtime missing ${token}`);
...
for(const token of ["CALIBRATION_REVISION='source-svg-r24'",'/app/cabinet-only-v144.html','/app/cabinet-calibrator-v144.html','/app/cabinet-calibration-v144.js','/app/shared/cabinet-calibration-v144.json'])assert(worker.includes(token),`Service worker does not install ${token}`);
...
console.log(JSON.stringify({ok:true,mode:'Cabinet Mode + Cabinet Only',coordinatePlane:'941x1672 source pixels',calibratedPanels:Object.keys(calibration.systems),controls:25,swapMs:120,editor:'/app/cabinet-calibrator-v144.html',closeTarget:'/loom/',hubImage:'retained',locationScenes:'excluded from release payload'},null,2));
