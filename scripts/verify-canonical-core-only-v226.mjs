...
const systems={commonweave:'/app/working-campus-v156.html','living-school':'/app/cabinets/living-school/index.html',cerbanimo:'/app/realm-console-v140.html',fellowfare:'/app/fellowfare-cabinet-v144.html',anarchadia:'/app/anarchadia-console-v139.html'};
function runBoundary(pathname){
  ...
  const location={pathname,search:'',hash:'',hostname:'commonweave.invalid',origin:'https://commonweave.invalid',href:`https://commonweave.invalid${pathname}`,replace:url=>replaced.push(String(url))};
...
for(const [system,pathname] of Object.entries(systems)){
  ...
  assert.equal(result.documentElement.dataset.commonweaveSystemRoute,system);
  if(system==='commonweave'){
    assert.equal(result.appended.length,0,'Commonweave auto-loaded global additions.');
    assert.equal(result.documentElement.dataset.installBoundary,'canonical');
    assert.equal(result.documentElement.dataset.commonweaveCanonicalCore,'only');
  }else{
    ...
    assert(result.appended.some(node=>String(node.src||'').includes('/extensions/commonweave-additions-v156.js')),`${system} lost shared additions compatibility.`);
  }
}
const api=runBoundary(systems.commonweave).api;
assert.equal(api.canonicalSystemCount,5);
assert.equal(api.canonicalAutoScripts,0);
assert.equal(api.canonicalPolicy,'five-system-first-class-routes-commonweave-core-only');
console.log(JSON.stringify({ok:true,version,revision:api.revision,canonicalSystems:Object.keys(systems),emptySessionAuthorized:true,commonweaveAutoScripts:0,legacyCompatibility:true},null,2));
