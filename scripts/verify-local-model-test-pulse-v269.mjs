...
check('runtime selects tuned v302 worker and preserves compatibility plus tier fallback',runtime.includes("VERSION='1.0.115-local-ai-runtime-v302-session-handoff'")&&runtime.includes("worker-v266.js?v=1.0.125-v314-smooth-fit")&&runtime.includes('compatibilitySpec')&&runtime.includes('freshFallbackWorker')&&runtime.includes('installedTierFallbacks'));
check('runtime uses adaptive threaded compatibility on eligible desktops',!runtime.includes("forceSingleThread:spec.device==='wasm'")&&runtime.includes("spec.device==='wasm'&&(!globalThis.crossOriginIsolated||mobileLike()||hardwareConcurrency()<4)")&&runtime.includes('adaptiveWasmThreads:true'));
check('runtime caps compatibility prompt and output work',runtime.includes('compatibility?Math.min(512')&&runtime.includes('compatibility?Math.min(48')&&runtime.includes('compatibilityPromptCap:true'));
...
check('worker supports prewarm and monotonic model progress',worker.includes("message.type==='prewarm'")&&worker.includes('progress_total??p.progress')&&worker.includes('high=Math.max(high,pct)'));
...
console.log(JSON.stringify({ok:true,revision:'local-model-test-pulse-v314-smooth-fit',checks:checks.length,directRuntime:'CivweaveLocalModelRuntimeV266.generate',thinkingDisabled:true,stagedHealth:true,wasmPerformanceDiagnostics:true,mobileSafeHealth:true,interruptedTestRecovery:true,benchmarkDisabledOnMobile:true,tierFallback:true,adaptiveWasmThreads:true,compatibilityPromptCap:true,intentPrewarm:true,gemma3OptimizedQ4:true},null,2));