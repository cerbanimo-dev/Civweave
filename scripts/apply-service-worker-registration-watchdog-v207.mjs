...
await patch('public/install-v130.js',source=>{
  ...
  source=replaceRequired(
    source,
    "const PREPARE_TIMEOUT_MS=180000;\nconst AUTO_RESET_KEY='civweave.device-package.auto-reset.v106-r51';\nconst PREVIOUS_AUTO_RESET_KEY='civweave.device-package.auto-reset.v106-r48';\nconst EARLIER_AUTO_RESET_KEY='civweave.device-package.auto-reset.v106-r47';",
    "const PREPARE_TIMEOUT_MS=180000;\nconst REGISTRATION_TIMEOUT_MS=15000;\nconst UPDATE_TIMEOUT_MS=15000;\nconst REGISTRATION_QUERY_TIMEOUT_MS=6000;\nconst AUTO_RESET_KEY='civweave.device-package.auto-reset.v106-r52';\nconst PREVIOUS_AUTO_RESET_KEY='civweave.device-package.auto-reset.v106-r51';\nconst EARLIER_AUTO_RESET_KEY='civweave.device-package.auto-reset.v106-r48';\nconst WATCHDOG_RECOVERY_KEY='civweave.device-package.registration-watchdog.v107-r1';",
  ...
  source=replacePattern(
    ...
    block(
      ...
      "      worker=activeWorker;",
      "      help('Using the current Civweave package registration…');",
      "    }else{",
      "      if(exactCandidate){",
      "        registration=existing;",
      ...
      "          await pause(250);",
      "        }",
      "        help('Registering the Civweave app package…');",
      "        registration=await withTimeout(",
      "          navigator.serviceWorker.register(WORKER_URL,{scope:'/',updateViaCache:'none'}),",
      "          REGISTRATION_TIMEOUT_MS,",
      "          'Chrome did not finish registering the Civweave app package.',",
      ...
      "          help('Refreshing the Civweave worker package…');",
      "          await withTimeout(",
      "            registration.update(),",
      "            UPDATE_TIMEOUT_MS,",
      "            'Chrome did not finish checking the Civweave worker package.',",
      ...
      "      REGISTRATION_TIMEOUT_MS,",
      "      'Chrome did not finish activating the Civweave app package.',",
      "      'service-worker readiness'",
      "    );",
      "    await confirmReady(worker);",
      "    sessionStorage.removeItem(WATCHDOG_RECOVERY_KEY);",
      "    if(options.manual)help('Civweave is updated and ready. Your saved knowledge library was preserved.');",
      "  }catch(error){",
      "    if(error?.code==='CIVWEAVE_PACKAGE_TIMEOUT'){",
      "      try{await recoverStalledRegistration(error)}catch(recoveryError){",
      "        if(recoveryError?.code!=='CIVWEAVE_RECOVERY_RELOAD')failPackage(recoveryError);",
      "      }",
      "    }else if(error?.code!=='CIVWEAVE_RECOVERY_RELOAD')failPackage(error);",
      "  }finally{",
      "    preparing=false;",
...
await patch('public/app/pwa-update-controller-v204.js',source=>{
  ...
  source=replaceRequired(
    ...
    "registration=registration||await withTimeout(navigator.serviceWorker.getRegistration('/'),REGISTRATION_QUERY_TIMEOUT_MS,'Chrome did not return the Civweave update registration.');",
