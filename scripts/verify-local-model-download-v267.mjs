...
check('background service worker handles success',backgroundWorker.includes('backgroundfetchsuccess')&&backgroundWorker.includes('copyRecords'));
...
check('background service worker handles failure and abort',backgroundWorker.includes('backgroundfetchfailure')&&backgroundWorker.includes('backgroundfetchabort'));
...
check('local runtime carries token callbacks from worker',runtime.includes("message.type==='token'")&&runtime.includes('task.onToken?.(message.token)')&&runtime.includes('stream:Boolean(stream)'));
...
check('raw pulse streams visible direct model output',pulse.includes('onToken:token=>')&&pulse.includes('stream:true')&&pulse.includes('streaming from WebGPU worker'));
...
check('runtime spine applies capability normalization before middleware',spine.includes("id:'capability-normalizer'")&&spine.includes('normalizeRequest'));
check('runtime spine exposes middleware registry',spine.includes('middleware=new Map()')&&spine.includes('function register(')&&spine.includes('__civweaveRuntimeSpineV270:true'));
...
check('local bridge marks downloaded model as actual provider',bridge.includes("provider:'downloaded-local'")&&bridge.includes('local-ai-bridge-v270-streaming'));
check('local bridge uses extended first-load timeout',bridge.includes('900000')&&bridge.includes('600000')&&bridge.includes('360000'));
check('local bridge registers a runtime spine handler',bridge.includes("MIDDLEWARE_ID='downloaded-local-v270'")&&bridge.includes('runtimeSpine.register(MIDDLEWARE_ID,middleware(),100)'));
...
check('bootstrap advertises local streaming and integrity repair',bootstrap.includes('localStreaming:true')&&bootstrap.includes('integrityRepair:true')&&bootstrap.includes('agenticToolSemantics:true'));
...
const legacyLocal=capability.normalizeRequest({purpose:'threat-model',executionProfile:'agentic',background:true,requiresTools:true,config:{service:'anarchadia'},messages:[{role:'user',content:'Review this local charter diff for abuse paths.'}]});
assert.equal(legacyLocal.requiresTools,false,'Anarchadia local threat reasoning must not be treated as tool use merely because it is agentic.');
assert.equal(capability.requirements(legacyLocal).profile,'agentic');
assert.equal(capability.requirements(legacyLocal).requiresTools,false);
...
assert.equal(live.requiresTools,true,'Live web research must keep its tool requirement.');
...
assert.equal(explicit.requiresTools,true,'Explicit modern capability declarations must override legacy normalization.');
checks.push('agentic/tool normalization executable cases');

console.log(JSON.stringify({ok:true,revision:'local-model-streaming-v270',checks:checks.length,features:{byteProgress:true,backgroundFetch:true,foregroundFallback:true,integrityValidation:true,targetedRepair:true,localStreaming:true,capabilityRouting:true,runtimeSpine:true,localAgenticReasoning:true,agenticToolSeparation:true,toolEscalation:true,cloudflareRuntimeStaged:true}},null,2));
