...
assert(hologram.includes('left:calc(50% + 5px)'),'Weaveling was not moved 5px right');
assert(hologram.includes('top:calc(72% + 50px)'),'Weaveling was not moved 50px higher from the prior placement');
assert(hologram.includes('width:min(16vw,136px)'),'Weaveling scale changed unexpectedly');
assert(hologram.includes('height:min(18vh,164px)'),'Weaveling height changed unexpectedly');
...
assert(worker.includes("CACHE_REVISION='smollm2-route-lock-r12'"),'service worker was not rotated for the route-lock update');
for(const required of ['assistant-runtime-v133.js','smollm2-fallback-runtime-v134.js','smollm2-small-model-v137.js','model-settings-v133.js','weaveling-hologram-v133.css','smollm2-360m-instruct/model-manifest.json','smollm2-360m-instruct/adapter.js','smollm2-360m-instruct/worker.js','smollm2-360m-instruct/tokenizer.json','wasm/ort-wasm-simd-threaded.jsep.mjs','wasm/ort-wasm-simd-threaded.jsep.wasm','ONNX_BACKEND_PREFIX'])assert(worker.includes(required),`service worker does not include ${required}`);
