...
assert(entry.includes('/app/manifest.webmanifest?boot=${Date.now()}')&&entry.includes("cache:'no-store'")&&entry.includes('bounded(fetch('),'Installed entry does not resolve the current release through the bounded no-store manifest path.');
assert(entry.includes("updateViaCache:'none'"),'Installed entry can still reuse HTTP-cached worker script bytes.');
assert(entry.includes('registration.update()')&&entry.includes('bounded(registration.update()'),'Installed entry does not explicitly perform a bounded worker refresh before routing.');
assert(entry.includes('revision=boot-recovery-v426'),'Installed entry does not register the current boot-recovery worker before routing.');
...
assert(entry.indexOf('await refreshWorker(releaseVersion)')<entry.indexOf('const requested='),'Installed entry routes before the worker refresh completes.');
...
console.log(JSON.stringify({
  ok:true,
  revision:'release-coherence-v226-boot-recovery-v426-entry',
  installedEntryRefresh:'boot-recovery-v426-bounded-before-routing',
