...
let approvalCalls=0;
const rook=context();
rook.localStorage.setItem('commonweave.realm-actions.v141',JSON.stringify([{
...
rook.CommonweaveGuideContractsV141={approve(id){approvalCalls++;const rows=JSON.parse(rook.localStorage.getItem('commonweave.realm-actions.v141'));const item=rows.find(row=>row.id===id);item.state='published';item.updatedAt=new Date().toISOString();rook.localStorage.setItem('commonweave.realm-actions.v141',JSON.stringify(rows));return{ok:true,action:item}}};
rook.CommonweaveAssistantV141={async respond(){throw new Error('The model must not be called for explicit approval.')}};
rook.CommonweaveFamilyAILoaderV105={async ensure(){return true}};
vm.runInContext(followthrough,rook,{filename:'action-followthrough-v165.js'});
const approved=await rook.CommonweaveAssistantV141.respond({text:'Go ahead and submit this draft',systemId:'fellowfare',history:[]});
...
rookDraft.CommonweaveGuideContractsV141={approve(){return{ok:false}}};
rookDraft.CommonweaveAssistantV141={async respond(){const action={id:'materials-2',system:'fellowfare',kind:'trade-request',title:'Resource request',state:'review',sourceText:'Need soil, seedlings, lumber, and hand tools for a community garden.',fields:{category:'Other',need:'Need soil, seedlings, lumber, and hand tools for a community garden.'},missingRequired:[],approval:{required:true,label:'Approve & publish request'}};rookDraft.localStorage.setItem('commonweave.realm-actions.v141',JSON.stringify([action]));return{action,response:{answer:"I don't publish for you."},provider:'gemini',model:'gemini-test'}}};
rookDraft.CommonweaveFamilyAILoaderV105={async ensure(){return true}};
vm.runInContext(followthrough,rookDraft,{filename:'action-followthrough-v165.js'});
const drafted=await rookDraft.CommonweaveAssistantV141.respond({text:'I need soil, seedlings, lumber, and hand tools for a community garden.',systemId:'fellowfare'});
...
merlin.localStorage.setItem('commonweave.realm-actions.v141',JSON.stringify([{
...
merlin.AnarchadiaCitizenConsoleV158={async runPipeline(id){pipelineRuns++;const state=JSON.parse(merlin.localStorage.getItem('commonweave.anarchadia.citizen-console.v139'));const item=state.proposals.find(row=>row.id===id);item.status='preview-ready';item.stage='preview-ready';item.preview={srcdoc:'<!doctype html><p>preview</p>',generator:'test'};merlin.localStorage.setItem('commonweave.anarchadia.citizen-console.v139',JSON.stringify(state))}};
...
const reviewedState=JSON.parse(merlin.localStorage.getItem('commonweave.anarchadia.citizen-console.v139'));
...
const proofApi=proof.CommonweaveCerbanimoProofAttachmentsV165;
