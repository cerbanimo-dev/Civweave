...
assertIncludes(boundary,["const PWA_UPDATE_SCRIPT='/app/pwa-update-controller-v204.js'",'PWA_UPDATE_SCRIPT',"guideWorkspaceRevision:'v350-single-current-chat-surface'", "campusBackgroundDownloadRevision:'v241-worker-owned-download-bottom-progress-rail'"],'install boundary');
assertIncludes(installedEntry,["updateViaCache:'none'",'registration.update()',"bounded(registration.update(),WORKER_STEP_TIMEOUT_MS,'service worker update')","candidate.postMessage({type:'SKIP_WAITING'})",'bounded(fetch(',"/app/manifest.webmanifest?boot=${Date.now()}","cache:'no-store'",'release manifest'],'updater-first installed entry');
...
console.log(JSON.stringify({schools:catalog.schools.length,articles:articleCount,compressedBytes,compressedMiB:Number((compressedBytes/1024/1024).toFixed(2)),largestSchoolMiB:Number((Math.max(...catalog.schools.map(school=>school.zip_bytes))/1024/1024).toFixed(2)),crossroads:catalog.reconciliation.crossroads_articles,knowledgeCache:'cwknowledge-school-seeds-v2',externalOriginReady:true,legacyOriginCacheCompatible:true,installedUpdateOwner:'installed-entry-v146',installedUpdateBounded:true,manualUpdateControl:'visible-v207-registration-watchdog',workerComposition:'v281-integrity-v280-resumable-campus-v343-chat-css-repair',currentFilesSkipRedownload:true,backgroundCampus:true,storagePreflight:true,shellIntegrity:true},null,2));
