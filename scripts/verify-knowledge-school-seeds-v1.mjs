...
assertIncludes(boundary,["const PWA_UPDATE_SCRIPT='/app/pwa-update-controller-v204.js'",'PWA_UPDATE_SCRIPT',"pwaUpdateRevision:'v250-installed-entry-every-launch'", "campusBackgroundDownloadRevision:'v241-worker-owned-download-bottom-progress-rail'"],'install boundary');
...
console.log(JSON.stringify({schools:catalog.schools.length,articles:articleCount,compressedBytes,compressedMiB:Number((compressedBytes/1024/1024).toFixed(2)),largestSchoolMiB:Number((Math.max(...catalog.schools.map(school=>school.zip_bytes))/1024/1024).toFixed(2)),crossroads:catalog.reconciliation.crossroads_articles,knowledgeCache:'cwknowledge-school-seeds-v2',appUpdateControl:'v250-installed-entry-every-launch',workerComposition:'v218-cleanroom-plus-retained-core-plus-current-graph-v238-fast-background-v241',currentFilesSkipRedownload:true,backgroundCampus:true},null,2));
