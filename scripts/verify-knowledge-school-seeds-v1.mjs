...
assertIncludes(installedEntry,["updateViaCache:'none'",'registration.update()',"bounded(registration.update(),WORKER_STEP_TIMEOUT_MS,'service worker update')","candidate.postMessage({type:'SKIP_WAITING'})","fetch(`/app/manifest.webmanifest?boot=${Date.now()}`,{cache:'no-store'})"],'updater-first installed entry');
