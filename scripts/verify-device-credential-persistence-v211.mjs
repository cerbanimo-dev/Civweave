...
for (const token of ['Remember on this device', 'civweave-model-persistent-secrets-v191', 'civweave:model-settings-saved', "document.addEventListener('click',openSettings,true)"]) {
...
const first = boot({
  'civweave.universal-ai.v127': JSON.stringify({ provider: 'gemini', route: 'gemini', model: 'gemini-test', endpoint: 'https://generativelanguage.googleapis.com/v1beta', consent: true }),
}, { 'civweave-model-session': JSON.stringify(sessionPacket) });
assert(first.sandbox.CivweaveDeviceCredentialPersistenceV211.persistFromSession('device'), 'Device persistence did not save the active session key.');
...
assert(second.sandbox.CivweaveDeviceCredentialPersistenceV211.status().mode === 'device', 'Restored credential did not remain in device mode.');
