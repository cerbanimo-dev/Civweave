...
for(const html of [loom,lite])assert(!html.includes('/app/device-credential-persistence-v211.js'),'Legacy entry auto-loads the credential compatibility helper instead of leaving it explicit-only.');
...
const fixtureValue=['fixture','value','v211'].join('-');
const sessionPacket={[secretField]:fixtureValue,provider:'gemini',remoteConsent:true,savedAt:'2026-08-05T00:00:00.000Z'};
...
assert(first.sandbox.CivweaveDeviceCredentialPersistenceV211.persistFromSession('device'),'Explicit device persistence did not save the active session fixture.');
const durable=JSON.parse(first.localStorage.getItem('civweave-model-persistent-secrets-v191')||'{}');
assert(durable[secretField]===fixtureValue,'Durable credential fixture was not written.');
...
assert(restored[secretField]===fixtureValue,'Explicit restore did not recover the remembered fixture.');
...
assert(!second.localStorage.getItem('civweave-model-persistent-secrets-v191'),'Forget did not clear the durable fixture.');
assert(!second.sessionStorage.getItem('civweave-model-session'),'Forget did not clear the session fixture.');
console.log(JSON.stringify({ok:true,revision:'v211-explicit-credential-compatibility',save:true,automaticEntryLoad:false,automaticRestore:false,explicitRestore:true,forget:true,settingsInputOwnership:false,settingsApiPatching:false,legacyEntries:['loom','lite']},null,2));
