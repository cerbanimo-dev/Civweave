...
check('installed cache repair purges stale face icon launcher',repair.includes("const REVISION='chat-bubble-anchor-v342'")&&repair.includes("'/app/shared-chat-face-icons-v255.js'"));
check('active worker imports matching chat bubble repair revision',worker.includes("/service-worker-chat-repair-v245.js?v=chat-bubble-anchor-v342&purge=chat-bubble-anchor-v342"));
...
console.log(JSON.stringify({ok:true,revision:'mobile-chat-bubble-anchor-v342',checks:checks.length,launcher:{position:'fixed',shape:'circle',desktopPx:52,mobilePx:48,narrowPx:46},installedPwa:{staleLauncherPurge:true,workerRotation:true}},null,2));
