...
const tracks=sandbox.CivweaveRadioTrackSuggestionsV245;
assert.ok(radio,'v233 radio runtime must initialize');
assert.ok(tracks,'v245 track suggestion behavior must initialize from stable v240 file');
assert.equal(sandbox.CivweaveRadioRecommendationAgentV232,radio,'v232 compatibility alias must point to v233 runtime');
assert.equal(sandbox.CivweaveRadioTrackSuggestionsV244,tracks,'v244 compatibility alias must point to v245 behavior');
assert.equal(sandbox.CivweaveRadioTrackSuggestionsV243,tracks,'v243 compatibility alias must point to v245 behavior');
assert.equal(sandbox.CivweaveRadioTrackSuggestionsV242,tracks,'v242 compatibility alias must point to v245 behavior');
assert.equal(sandbox.CivweaveRadioTrackSuggestionsV241,tracks,'v241 compatibility alias must point to v245 behavior');
assert.equal(sandbox.CivweaveRadioTrackSuggestionsV240,tracks,'stable v240 compatibility alias must point to v245 behavior');
assert.equal(radio.revision,'system-radio-agent-v233');
assert.equal(tracks.revision,'radio-track-suggestions-v245');
...
assert.equal(contextUrl.pathname,`/track/${testTrackId}`,'suggested-track context must target the exact suggested Spotify track');
assert.equal(contextUrl.searchParams.get('context'),`spotify:playlist:${expectedIds.cerbanimo}`,'suggested-track context must retain its approved station playlist');
const smartLink=new URL(tracks.spotifyContentLink(enriched,'cerbanimo'));
assert.equal(smartLink.hostname,'spotify.link','suggested station tracks must use Spotify content linking for native-app handoff');
assert.equal(smartLink.pathname,'/content_linking','suggested station tracks must use Spotify content-linking route');
assert.equal(smartLink.searchParams.get('~campaign'),'civweave-pwa','Spotify handoff must identify Civweave traffic');
assert.equal(smartLink.searchParams.get('$deeplink_path'),contextUrl.href,'native deeplink must preserve exact track + playlist context');
assert.equal(smartLink.searchParams.get('$fallback_url'),contextUrl.href,'web fallback must preserve exact track + playlist context');
assert.equal(smartLink.searchParams.get('$canonical_url'),contextUrl.href,'iOS canonical URL must preserve exact track + playlist context');
assert.equal(tracks.spotifyHighlightedPlaylistUrl(enriched,'cerbanimo'),tracks.spotifyContentLink(enriched,'cerbanimo'),'v243 compatibility helper must resolve to the current Spotify content-linking handoff');
...
assert.match(trackSource,/Play suggested station track ↗/,'exact track metadata must expose a station-aware CTA');
assert.match(trackSource,/Open station ↗/,'playlist fallback must remain available');
assert.match(trackSource,/new URL\(`https:\/\/open\.spotify\.com\/track\/\$\{id\}`\)/,'suggested-track context must target the exact Spotify track URL');
assert.match(trackSource,/searchParams\.set\('context',meta\.playlistUri\)/,'exact-track context must preserve the approved playlist');
assert.match(trackSource,/new URL\('https:\/\/spotify\.link\/content_linking'\)/,'PWA handoff must use Spotify content linking');
assert.match(trackSource,/searchParams\.set\('\$deeplink_path',contentUrl\)/,'native handoff must carry the exact contextual track URL');
assert.match(trackSource,/searchParams\.set\('\$fallback_url',contentUrl\)/,'browser fallback must carry the exact contextual track URL');
assert.match(trackSource,/searchParams\.set\('\$canonical_url',contentUrl\)/,'iOS canonical handoff must carry the exact contextual track URL');
...
assert.doesNotMatch(trackSource,/open\.spotify\.com\/search\//,'v245 behavior must never strand users in Spotify search');
...
console.log('Civweave system radio v233 + stable-v240/v245 exact-station-track content-linking contract verified.');
