...
console.log(JSON.stringify({ok:true,version,revision:'mobile-v302-fullscreen-ai-hardening',checks:checks.length,mobile:{topbarRows:'brand / modes / map+downloads / settings+review / theme',realmCards:'2-column then 1-column',dynamicViewport:true,chat:'full-visual-viewport'},chat:{canonicalOwner:'v242',deletedLegacyOwners:3,embeddedComposerDelegates:true,modelFallback:true},localAI:{interruptedTestRecovery:true,downloadsPreserved:true,mobileSafeHealth:true},installedBoot:{updaterFirst:true,boundedWorkerUpdate:true,workerActivation:true,legacyCachePurge:true}},null,2));
