...
for(const path of ['/app/manifest.webmanifest','/app/installed-entry-v146.js','/app/install-boundary-v146.js','/app/guide-identity-integrity-v216.js','/app/realm-session-integrity-v237.js','/app/guide-workspace-v242.js','/app/chat-runtime-failsafe-v266.js','/app/shared-guide-surface-v236.js','/app/shared-guide-surface-v236-core-v244.js','/app/family-ai-loader-v105.js','/app/assistant-runtime-v141.js','/app/shared/civweave-model-runtime.js','/app/persistent-guide-chat-v215.js','/app/persistent-guide-viewport-v216.js','/app/chat-single-owner-v245.js','/app/working-campus-v156.part5.txt'])check(`phone cache repair evicts ${path}`,workerRepair.includes(`'${path}'`));
check('service worker retains v250 convergence import and activates v266 chat runtime recovery',workerEntry.includes('chat-convergence-v250')&&workerRepair.includes("const REVISION='chat-runtime-recovery-v266'"));
...
console.log(JSON.stringify({ok:true,version,revision:'chat-runtime-recovery-v266',checks:checks.length,mobile:{topbarRows:'brand / modes / map+settings / review+theme',realmCards:'2-column then 1-column'},chat:{canonicalOwner:'v242',duplicateOwners:0,embeddedComposerDelegates:true,modelFallback:true,failOpenRecovery:true},installedBoot:{updaterFirst:true,workerActivation:true}},null,2));
