...
for(const key of facadeKeys){
  assert.match(src[key],/(compatibilityFacade:true|retired:true)/,`${paths[key]} is not explicitly a compatibility facade.`);
  assert.doesNotMatch(src[key],/document\.addEventListener\('click'/,`${paths[key]} regained document Settings input ownership.`);
  assert.doesNotMatch(src[key],/document\.createElement\(['"]dialog['"]\)|showModal\(/,`${paths[key]} regained dialog presentation ownership.`);
}
...
for(const key of ['delegation','bindGuard','deviceRepair'])assert.match(src[key],/canonical:'CivweaveSettingsV320'/,`${paths[key]} does not identify V320 as canonical.`);
