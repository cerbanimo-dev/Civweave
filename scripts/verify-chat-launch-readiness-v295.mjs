...
const [orchestrator,fullscreen,store,ui,localRuntime,localOwner,settings,workspace,runtime266,bootstrap,gateway,repair,minilm]=await Promise.all([
...
for(const source of [orchestrator,fullscreen,store,ui,localRuntime,localOwner,settings,workspace,runtime266,bootstrap,gateway,minilm])new Function(source);
...
assert.doesNotMatch(orchestrator,/chatOpenPrewarm:true/,'chat readiness must never require chat-open model prewarm');
assert.doesNotMatch(orchestrator,/intentPrewarm===true/,'chat readiness must never require focus/intent model prewarm');
...
// MiniLM is separate semantic infrastructure and is deliberately exempt from the generative-model UI ban.
