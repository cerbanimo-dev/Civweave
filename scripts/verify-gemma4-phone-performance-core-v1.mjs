...
includes(phone,"VERSION='1.1.0-gemma4-phone-performance-core-v1-registry-authority'",'phone authority');
...
before(phone,'const registryReady=watchRegistry();','const packsReady=watchPacks();','phone authority');

const repair=read(repairPath);
includes(repair,"VERSION='1.0.7-gemma4-inference-repair-v1-post-core-authority'",'Gemma repair');
includes(repair,"const DEEP_VERSION='1.0.0-gemma4-e4b-q4-extension-v1'",'Gemma repair');
includes(repair,"const PHONE_AUTH_VERSION='1.1.0-gemma4-phone-performance-core-v1-registry-authority'",'Gemma repair');
includes(repair,"const PHONE_AUTH_SRC='/app/local-ai/gemma4-phone-performance-core-v1.js?v=1.1.0-registry-authority'",'Gemma repair');
...
console.log('PASS Gemma 4 phone path keeps E2B/E4B and both LiteRT models in one registry authority, installs that authority after the inference core bootstrap, and makes the LiteRT pair the 12 GB phone performance core without silent ONNX fallback.');
