...
async function staged(){if(!await exists(manifestPath))return false;try{const manifest=JSON.parse(await fsp.readFile(manifestPath,'utf8'));if(manifest.schema!==SCHEMA||manifest.corePackage!==CORE_PACKAGE||manifest.coreVersion!==CORE_VERSION||manifest.utilsPackage!==UTILS_PACKAGE||manifest.utilsVersion!==UTILS_VERSION)return false;const required=['dist/index.js','wasm','wasm-utils/dist/index.js'];return (await Promise.all(required.map(name=>exists(path.join(destination,name))))).every(Boolean)}catch{return false}}
...
async function main(){
  ...
  try{
    ...
    const wasmFiles=(await walk(path.join(destination,'wasm'))).map(file=>path.relative(destination,file).split(path.sep).join('/'));
    ...
    const files=(await walk(destination)).filter(file=>file!==manifestPath).map(file=>path.relative(destination,file).split(path.sep).join('/'));
    await fsp.writeFile(manifestPath,`${JSON.stringify({schema:SCHEMA,corePackage:CORE_PACKAGE,coreVersion:CORE_VERSION,utilsPackage:UTILS_PACKAGE,utilsVersion:UTILS_VERSION,entry:'/app/vendor/litert-lm/dist/index.js',wasmRoot:'/app/vendor/litert-lm/wasm/',backend:'webgpu-gpu-artisan',selfHosted:true,rewrittenBareImports,wasmFiles,files,stagedAt:new Date().toISOString()},null,2)}\n`,'utf8');
    console.log(`[Civweave] Staged self-hosted LiteRT-LM Web ${CORE_VERSION}: ${files.length} files, ${wasmFiles.length} WASM runtime files, ${rewrittenBareImports} bare import rewrite(s).`);
