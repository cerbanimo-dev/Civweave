...
for(const token of [
  '1.0.4-platform-experience-v160.1-hud-stable',
  'function writeText',
...
class Element{
  ...
  matches(selector){
    return selector.split(',').some(raw=>{
      ...
      if(part==='.top')return this.className.split(/\s+/).includes('top')||this.classList.contains('top');
      if(part==='#cwf104-head')return this.id==='cwf104-head';
      if(part==='meta[name="theme-color"]')return this.tagName==='META'&&this.getAttribute('name')==='theme-color';
      if(part==='link[data-cw160-frame-theme]')return this.tagName==='LINK'&&Object.hasOwn(this.dataset,'cw160FrameTheme');
      const match=part.match(/^\[data-([a-z0-9-]+)\]$/i);
      if(match)return Object.hasOwn(this.dataset,camel(match[1]));
      return false;
    });
  }
...
meta.setAttribute('name','theme-color');host.className='top';host.classList.add('top');
documentElement.append(head,body);head.append(meta);body.append(host);
const document={
  readyState:'complete',documentElement,head,body,
  createElement:tag=>new Element(tag),
  getElementById:id=>documentElement.querySelector(`#${id}`),
  querySelector:selector=>documentElement.matches(selector)?documentElement:documentElement.querySelector(selector),
...
for(let index=0;index<250;index++){
  const node=new Element('div');node.className='subsystem-update';body.append(node);
  api.refreshControls();
}
