...
async function sameSize(sourceFile,targetFile){
  try{const [a,b]=await Promise.all([fsp.stat(sourceFile),fsp.stat(targetFile)]);return a.size===b.size&&a.size>0}catch{return false}
}
async function main(){
  ...
  for(const name of required){
    ...
    const info=await fsp.stat(targetFile);
    staged.push({name,bytes:info.size});
  }
  await fsp.writeFile(path.join(destination,'stage-manifest.json'),JSON.stringify({
    schema:'commonweave.onnxruntime-stage.v1',
    package:'onnxruntime-web',
    version:packageJson.version,
    entry:'/app/vendor/onnxruntime/ort.wasm.min.mjs',
