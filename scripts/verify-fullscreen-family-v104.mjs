...
assert(entry.includes("destination.searchParams.set('system'"),'Installed entry must preserve the requested system.');
...
for(const token of ["const VERSION='1.0.4'","const SYSTEM_ORDER=['commonweave','living-school','cerbanimo','fellowfare','anarchadia']",'tray.innerHTML=SYSTEM_ORDER.map','data-cwf-badge','data-cwf-state','data-cwf-chat',"document.documentElement.dataset.familyShell='direct'","document.documentElement.dataset.visualShell='sol-r1'",'Talk to Commonweave with Weaveling'])assert(shell.includes(token),`Family shell contract missing ${token}`);
...
for(const token of ['CommonweaveModelSettingsV133','CommonweaveGuideChatV153','[data-ai-settings]','[data-capability="commonweave.model-setup"]'])assert(loader.includes(token),`Lazy family AI contract missing ${token}`);
...
console.log('Fixed five-realm Commonweave family v1.0.4 verification passed.');
