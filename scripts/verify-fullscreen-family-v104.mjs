...
const expectedSystems={
  civweave:{guide:'Weaveling',artifact:'/app/assets/ai/weaveling-compass.png',avatar:'/app/assets/ai/weaveling.png'},
...
assert(entry.includes("const requested=params.get('system')||params.get('target')||'civweave'")&&entry.includes('(sites[system]||sites.civweave)')&&entry.includes("destination.searchParams.set('installed','1')"),'Installed entry must resolve the requested system through direct installed routes.');
const manifest=JSON.parse(manifestRaw);assert(manifest.start_url.includes('system=civweave'),'Civweave must remain the first installed screen.');assert(manifest.shortcuts.length===5,'Every system needs a direct shortcut.');
for(const token of ["const VERSION='1.0.4-direct-settings-v173'","const SYSTEM_ORDER=['civweave','living-school','cerbanimo','fellowfare','anarchadia']",'tray.innerHTML=SYSTEM_ORDER.map','data-cwf-badge','data-cwf-state','data-cwf-chat','[data-ai-settings]','[data-capability="civweave.model-setup"]',"document.documentElement.dataset.familyShell='direct'","document.documentElement.dataset.visualShell='merlinites-r1'",'Talk to Civweave with Weaveling','/app/merlinites-shell-fix-v166.css?v=merlinites-r2',"settingsOwner:'CivweaveModelSettingsControllerV173'"])assert(shell.includes(token),`Family shell contract missing ${token}`);
...
for(const token of ['async function ensure()',"settingsOwner:'CivweaveModelSettingsControllerV173'",'/app/minilm-reflex-runtime-v138.js'])assert(loader.includes(token),`Lazy family chat contract missing ${token}`);
...
const sandbox={console,URLSearchParams,location:{search:'?system=civweave',pathname:'/app/fullscreen-family-v104.html',assign(){},replace(){}},localStorage:{getItem(){return null},setItem(){}},document:{readyState:'loading'},addEventListener(){},setInterval(){},globalThis:null};sandbox.globalThis=sandbox;vm.createContext(sandbox);vm.runInContext(shell,sandbox);
const runtime=sandbox.CivweaveFamilyShellV104;assert(runtime,'Family shell did not expose its runtime contract.');assert(JSON.stringify(Array.from(runtime.systemOrder))===JSON.stringify(expectedOrder),'Runtime realm order does not match the fixed five-realm order.');assert(JSON.stringify(Object.keys(runtime.systems))===JSON.stringify(expectedOrder),'Runtime systems are missing or reordered.');
...
console.log('Fixed five-realm Civweave family v1.0.4 verification passed with direct settings ownership and merlinites only.');
