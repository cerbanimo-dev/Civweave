...
check('canonical owner supplies the complete chat presentation contract',workspace.includes('position:fixed!important')&&workspace.includes('display:grid!important')&&workspace.includes('#${ROOT_ID}>header')&&workspace.includes('#${ROOT_ID} [data-log] article')&&workspace.includes('#${ROOT_ID} [data-persistent-form]'));
check('canonical presentation covers every themed guide',workspace.includes('[data-guide="living-school"]')&&workspace.includes('[data-guide="cerbanimo"]')&&workspace.includes('[data-guide="fellowfare"]')&&workspace.includes('[data-guide="anarchadia"]'));
check('saved chat row receives an explicit optional grid track',workspace.includes(':has(>.cw295-saved-chats){grid-template-rows:auto auto auto minmax(96px,1fr) auto!important'));
