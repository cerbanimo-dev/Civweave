...
for(const token of ['/app/cabinets/living-school/living-school-research-v162.js','agentic-research-v162','platform-experience-v160','living-school-paths-v160'])assert(indexHtml.includes(token),`Living School index is missing ${token}.`);
for(const token of ['/app/intention-planner-v141.js','/app/cabinets/living-school/living-school-research-v162.js','/app/platform-experience-v160.js','/app/cabinets/living-school/living-school-paths-v160.js',"INTENTION_RESEARCH_REVISION='latest-intention-agentic-research-v163'"])assert(worker.includes(token),`Installed package is missing ${token}.`);
assert(worker.includes("PLATFORM_EXPERIENCE_REVISION='dark-review-rook-learning-v160'"),'PR #79 platform experience revision was lost.');
assert(installBoundary.includes("additionsVersion:'v163-latest-intention-agentic-research'"),'Install boundary did not rotate to v163.');
console.log(JSON.stringify({ok:true,planner:'latest-intention-only',corrections:['not-a-game','fresh-wish'],researchPrimary:'antigravity-agentic',researchFallback:'gemini-training-data-flagged',liveSourcesRequired:true,preserved:['dark-mode','rook-flow','active-learning-paths']},null,2));
