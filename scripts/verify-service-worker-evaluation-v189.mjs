...
const orderedImports=['/app/system-routes-v227.js','/service-worker-living-school-cleanroom-v218.js','/service-worker-code-coherence-v288.js','/service-worker-core-v208.js','/service-worker-installed-launch-v282.js','/service-worker-installer-state-v280.js','/service-worker-shell-integrity-v281.js','/service-worker-shell-repair-v293.js','/service-worker-offline-v211-override.js','/service-worker-campus-completion-v246.js','/service-worker-release-coherence-v220.js','/service-worker-navigation-safety-v224.js','/service-worker-canonical-navigation-v227.js','/service-worker-chat-repair-v245.js','/service-worker-local-model-download-v267.js'];
...
assert(!wrapper.includes('service-worker-shell-repair-v225.js'),'Worker wrapper resurrected retired shell repair v225.');
assert(wrapper.indexOf('/service-worker-shell-repair-v293.js')<wrapper.indexOf('/service-worker-canonical-navigation-v227.js'),'Canonical navigation must remain after the sole installed repair responder.');
...
const combinedSource=[routes,cleanup,coherence,core,installedLaunch,installerState,integrity,installedRepair,offline,campusCompletion,release,navigation,canonical,chatRepair,localModel].join('\n');
...
assert(combined.filter(row=>row.type==='message').length>=2,'Combined worker lost package/repair messaging.');
...
assert(installedRepair.includes("event.data?.type !== 'REPAIR_DEVICE_PACKAGE'"),'Installed shell repair v293 lost explicit repair message ownership.');
assert(installedRepair.includes("verified-shell-only-preserve-campus-model-school-storage"),'Installed shell repair v293 lost protected-storage policy.');
assert(pwa.includes("requiredNextOwner:'pwa-install-prompt-v249'"),'PWA controller lost required-next ownership.');
assert(!pwa.includes('REPAIR_DEVICE_PACKAGE'),'Ordinary installer controller must not invoke shell repair.');
...
console.log(JSON.stringify({ok:true,revision:'v189-install-only-pwa-v1-worker-stack-source-truth',legacyBridge:true,codeCoherence:'network-first',retainedOfflineCore:true,installedLaunch:true,installedShellRepair:'v293-sole-owner',browserRuntime:false,shellIntegrity:true,canonicalSystems:5,importLayers:orderedImports.length},null,2));