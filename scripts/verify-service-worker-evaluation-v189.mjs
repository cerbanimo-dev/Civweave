...
assert(shellRepair.includes("const V225_OPTIONAL_ASSETS = ['/app/installer-repair-only-v2.js']"),'Shell repair must retain the current cache-distinct repair-only installer bridge v2.');
assert(!shellRepair.includes("const V225_OPTIONAL_ASSETS = ['/app/installer-repair-only-v1.js']"),'Shell repair must not seed the stale shell-cached repair bridge v1.');
...
console.log(JSON.stringify({ok:true,revision:'v189-install-only-pwa-v2-cache-distinct-repair',legacyBridge:true,codeCoherence:'network-first',retainedOfflineCore:true,installedLaunch:true,repairOnly:true,cacheDistinctRepair:true,browserRuntime:false,shellIntegrity:true,canonicalSystems:5,importLayers:orderedImports.length},null,2));
