...
const [legacy,wrapper,routes,cleanup,coherence,core,installedLaunch,installerState,integrity,repair293,offline,campusCompletion,release,navigation,canonical,chatRepair,localModel,repairOnly]=await Promise.all([
...
assert.ok(!wrapper.includes('/service-worker-shell-repair-v225.js'),'Worker wrapper resurrected the parallel v225 repair owner.');
assert.equal(await exists('public/service-worker-shell-repair-v225.js'),false,'Parallel v225 repair worker must remain deleted.');
...
assert(wrapper.includes('installed-shell-repair-v293'),'Worker wrapper does not force the canonical repair refresh.');
...
const combinedSource=[routes,cleanup,coherence,core,installedLaunch,installerState,integrity,repair293,offline,campusCompletion,release,navigation,canonical,chatRepair,localModel].join('\n');
...
const repairMessages=(repair293.match(/REPAIR_DEVICE_PACKAGE/g)||[]).length;assert(repairMessages>=2,'Canonical repair message contract missing.');
assert.equal((combinedSource.match(/event\.data\?\.type!=='REPAIR_DEVICE_PACKAGE'/g)||[]).length,1,'Worker stack must have exactly one explicit REPAIR_DEVICE_PACKAGE repair owner.');
...
assert(core.includes("'/app/installer-repair-only-v1.js'"),'Core must directly cache the repair-only installer bridge.');
assert(repair293.includes("policy:'verified-shell-only-preserve-campus-model-school-storage'"),'Canonical shell repair policy drifted.');
...
console.log(JSON.stringify({ok:true,revision:'v189-single-shell-repair-owner',legacyBridge:true,codeCoherence:'network-first',retainedOfflineCore:true,installedLaunch:true,repairOwner:'v293',parallelRepairOwners:0,repairOnly:true,browserRuntime:false,shellIntegrity:true,canonicalSystems:5,importLayers:orderedImports.length},null,2));