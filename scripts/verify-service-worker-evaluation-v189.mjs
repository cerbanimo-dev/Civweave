...
const [legacy,wrapper,routes,cleanup,coherence,core,installerState,integrity,offline,campusCompletion,release,navigation,shellRepair,canonical,chatRepair,localModel]=await Promise.all([
...
assert(combined.filter(row=>row.type==='fetch').length>=3,'Combined worker lost clean-room, code-coherence, or core fetch listeners.');
