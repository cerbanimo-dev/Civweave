...
const [loom,realm,lite,contract,worker,manifestText,generationText,generator]=await Promise.all([
  ...
  read('public/app/logos/civweave-icon-generation.json'),
...
for(const [name,html] of [['loom',loom],['realm',realm],['lite',lite]]){
  ...
  assert(html.includes('civweave-icon-192.png?v=tight-r2'),`${name} does not use the tightly cropped Civweave icon`);
  assert(html.includes('manifest.webmanifest?v=1.0.30-icon-r2'),`${name} manifest cache key is stale`);
}
for(const html of [loom,realm])assert(!html.includes('<img src="/app/logos/civweave.webp"'),'active top bar still uses the old wordmark image');

for(const required of [
  'CIVWEAVE_SMALL_MODEL_CONTRACT_V2',
  'const ROUTE_CODES=',
  ...
  'maxNewTokens:4',
  'maxNewTokens:36',
  'Production guide responses use the canonical Civweave route lock',
...
for(const required of [
  ...
  '/app/logos/civweave-icon-generation.json'
])assert(worker.includes(required),`service worker does not cache ${required}`);

assert(generation.schema==='civweave.icon-generation.v1','unexpected icon generation schema');
...
for(const required of [
  'civweave-icon-192.png',
  'civweave-icon-512.png',
...
for(const [relative,width,height,minBytes] of [
  ['public/app/logos/civweave-icon-192.png',192,192,20000],
  ['public/app/logos/civweave-icon-512.png',512,512,100000],
  ['public/app/logos/civweave-icon-maskable-192.png',192,192,18000],
  ['public/app/logos/civweave-icon-maskable-512.png',512,512,90000]
]){
