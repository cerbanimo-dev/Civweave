...
const now = Date.parse('2026-08-11T23:00:00Z');
const checkpoint = { checkpointId: 'cp-1', createdAt: new Date(now - 60_000).toISOString(), recoveryCoverageBps: 10000 };
const healthy = (id, recipient, pairedOffset = 0, key = id) => ({
  anchorId: id, recipientId: recipient, keyFingerprint: `sha256:${key}`, state: 'active',
  pairedAt: new Date(now - 7 * 86400000 + pairedOffset).toISOString(), lastProofAt: new Date(now - 60_000).toISOString(),
  lastCheckpointId: 'cp-1', recoveryCoverageBps: 10000, proofCount: 3
});
...
test('duplicate device keys cannot farm redundant stipends', () => {
...
test('stale storage proof blocks stipend eligibility', () => {
...
test('incomplete recovery records work but freezes irreversible external settlement', () => {
  ...
  const normal = externalSettlementPolicy({ cloudReachable:true, reconciled:true, recoveryCoverageBps:10000 });
  assert.equal(normal.externalSettlementAllowed, true);
});
