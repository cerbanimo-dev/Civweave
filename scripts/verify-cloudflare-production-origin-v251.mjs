...
const PUBLIC_CANONICAL='https://civweave.cc';
const PAGES_UNDERLAY='https://civweave.pages.dev';
assert.equal(wrangler.name,'civweave','Wrangler must reserve the civweave Pages project for the canonical root underlay.');
assert.equal(wrangler.pages_build_output_dir,'./.cloudflare-pages','Wrangler Pages output directory drifted.');
assert.equal(launch.canonicalInstallOrigin,PUBLIC_CANONICAL,'Launch topology must use civweave.cc as the public canonical root.');
assert.equal(launch.backbone.staticProject,'civweave','Launch topology must reserve the civweave Pages project.');
assert.equal(launch.transition.previousCanonicalInstallOrigin,PAGES_UNDERLAY,'Launch topology must preserve civweave.pages.dev as the previous canonical/Pages underlay.');
assert.match(setup,/canonicalProjectName = "civweave"/,'Cloudflare setup helper does not reserve the civweave project.');
assert.match(setup,/canonicalOrigin = "https:\/\/civweave\.cc"/,'Cloudflare setup helper does not name the civweave.cc public canonical root.');
assert.match(setup,/canonicalPagesOrigin = `https:\/\/\$\{canonicalProjectName\}\.pages\.dev`/,'Cloudflare setup helper does not preserve the Pages underlay.');
...
assert.match(setup,/Visible status: \$\{pagesOrigin\}\/host-setup\.html/,'Incomplete account-edge setup is not routed to the durable Pages steward status.');
...
assert.match(docs,/Public canonical URL: https:\/\/civweave\.cc/,'Cloudflare guide does not name civweave.cc as the public canonical origin.');
assert.match(docs,/Canonical Pages underlay: https:\/\/civweave\.pages\.dev/,'Cloudflare guide does not name the canonical Pages underlay.');
...
assert.match(canonicalWorkflow,/Verify stable production origin/,'Canonical workflow does not verify the stable Pages underlay after deployment.');
assert.match(canonicalWorkflow,/https:\/\/civweave\.pages\.dev/,'Canonical workflow does not poll the stable Pages underlay.');
...
assert.match(canonicalWorkflow,/ensure-cloudflare-pages-domain-v1\.mjs[\s\S]*civweave\.cc/,'Canonical workflow does not attach/verify the civweave.cc custom domain.');
assert.match(docs,/green workflow therefore proves the Pages underlay updated before civweave\.cc is treated as canonical/,'Cloudflare guide does not explain the guarded public-domain cutover.');
...
assert.match(docs,/specifically for the OG `civweave\.pages\.dev` Pages underlay/,'Cloudflare guide does not distinguish OG credentials from community host credentials.');
...
assert.equal(webmanifest.related_applications[0]?.url,'https://civweave.cc/app/manifest.webmanifest','Manifest must prefer civweave.cc as the public canonical root.');
assert.equal(webmanifest.related_applications[1]?.url,'https://civweave.pages.dev/app/manifest.webmanifest','Manifest must preserve the Pages underlay for installed-app migration.');
console.log(JSON.stringify({ok:true,revision:'cloudflare-production-origin-v259-civweave-cc',project:'civweave',publicCanonical:PUBLIC_CANONICAL,pagesUnderlay:PAGES_UNDERLAY,communityPattern:'https://<project>.pages.dev',paidCommunityPattern:'https://<host>.civweave.cc',previewInstallSupported:false,localAnchorReminder:true,idempotentProjectCreate:true,automaticCanonicalUpdates:true,guardedCustomDomainCutover:true,nonBlockingCommunityAutomation:true},null,2));