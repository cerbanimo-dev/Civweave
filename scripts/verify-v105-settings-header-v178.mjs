...
assert(await missing('public/extensions/commonweave-settings-safe-open-v171.js'),'v171 MiniLM settings capture extension still exists');
assert(await missing('public/extensions/commonweave-settings-safe-open-v172.js'),'v172 window-capture MiniLM settings extension still exists');
for(const key of ['settings','controller','delegation','campus','worker','additive','installer','pwa','boundary']){
  assert(!source[key].includes('commonweave-settings-safe-open-v171'),`${key} still references safe-open v171`);
  assert(!source[key].includes('commonweave-settings-safe-open-v172'),`${key} still references safe-open v172`);
}
...
assert(source.settings.includes("provider:'deterministic'")&&source.settings.includes('commonweave-deterministic-v178'),'settings migration does not force deterministic state');
for(const token of ['/app/logos/commonweave-app-icon.png','LOCAL WORKING CAMPUS','v1.0.5','mode-switch','settings-pill','working-campus-v178-v105'])assert(source.campus.includes(token),`Working Campus header missing ${token}`);
assert(!source.campus.includes('/app/logos/commonweave.webp'),'Working Campus still uses the old Commonweave logo');
...
console.log(JSON.stringify({ok:true,version:'1.0.5',settingsOwner:'controller-only',dismissal:'explicit-only',legacyCaptureExtensions:false,defaultRoute:'deterministic',campusIcon:'/app/logos/commonweave-app-icon.png',campusIconPixels:176,devicePackage:'r40-settings-stable'},null,2));
