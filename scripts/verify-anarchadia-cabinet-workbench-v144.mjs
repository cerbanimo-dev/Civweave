...
assert(worker.includes("DEVICE_REVISION='device-package-r25-anarchadia-fellowfare'"),'Shared restored-workbench device package revision was not activated.');
...
console.log(JSON.stringify({ok:true,cabinet:'anarchadia',surfaces:['citizen-console','governance-workbench'],classicRoutes:8,devicePackage:'r25-anarchadia-fellowfare',serviceWorkerAuthority:'root-only',sourceBoundary:'cabinet-only compatibility loader',archivedSceneRequests:false},null,2));
