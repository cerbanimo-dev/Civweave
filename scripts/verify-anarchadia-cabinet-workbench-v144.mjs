...
assert(/const DEVICE_REVISION='device-package-r\d+-[a-z0-9-]+';/.test(worker),'Device package does not expose a versioned revision.');
assert(worker.includes("DEVICE_REVISION='device-package-r27-local-sovereignty'")&&worker.includes("INSTALL_REVISION='install-only-r26'"),'Local sovereignty and install-only package revisions are not active together.');
...
console.log(JSON.stringify({ok:true,cabinet:'anarchadia',surfaces:['citizen-console','governance-workbench','local-sovereignty'],classicRoutes:8,devicePackage:'install-only-local-sovereignty-r27',serviceWorkerAuthority:'root-only',sourceBoundary:'cabinet-only compatibility loader',archivedSceneRequests:false},null,2));
