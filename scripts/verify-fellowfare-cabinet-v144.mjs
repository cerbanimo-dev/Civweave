...
])assert(worker.includes(`'${asset}'`),`Installed device package omits ${asset}.`);
assert(worker.includes("DEVICE_REVISION='device-package-r25-anarchadia-fellowfare'"),'Service worker revision was not rotated for both restored realm workbenches.');
...
console.log(JSON.stringify({
  ok:true,
  renderer:'Cabinet Mode',
  fellowfare:{
    guide:'Rook',
    surface:'pre-visual exchange application',
    restored:['threads','matching','proposals','messages','assemblies','agreements','milestones','evidence','settlement','repair','trust','recurrence','portable bundles'],
    archivedMallScenesReintroduced:false
  }
},null,2));
