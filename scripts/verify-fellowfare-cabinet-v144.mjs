...
assert(cabinetMode.includes("system?.id==='fellowfare'")&&cabinetMode.includes('/app/services/fellowfare/cabinet.html')&&cabinetMode.includes("query.set('cabinet','1')"),'Cabinet Mode does not route FellowFare directly to its exchange subsystem.');
for(const token of ['cabinet-home-v142.js','fellowfare-cabinet-v144.js','install-boundary-v146.js','local-object-mesh-v146.js'])assert(outerHtml.includes(token),`Dedicated FellowFare compatibility cabinet is missing ${token}.`);
assert(outerHtml.includes('/app/services/fellowfare/cabinet.html?commonweave=1&cabinet=1#market'),'Dedicated compatibility cabinet does not load the pre-visual market engine.');
...
assert(worker.includes("INSTALL_REVISION='install-only-r26'")&&worker.includes('/app/services/fellowfare/cabinet.html'),'FellowFare is not installed inside the local-sovereignty install-only package.');
