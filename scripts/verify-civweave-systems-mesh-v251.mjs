...
assert.ok(routes.includes("const SYSTEMS_MESH_RUNTIME='/app/civweave-systems-mesh-v251.js'"));
assert.ok(routes.includes('installSystemsMeshRuntime();'));
...
assert.equal(api.receiveProjection(projection).added,true);
assert.equal(api.receiveProjection(projection).added,false);
