...
const BUILD_VERSION = '1.0.83-install-only-fullscreen-family-gateway';
...
function releasePacket(baseUrl = DEFAULT_PUBLIC_HOST) {
  ...
  return {
    ...
    releasedAt: STARTED_AT, appUrl: `${root}/`, installUrl: `${root}/`, sourceUrl: CIVWEAVE_SOURCE_URL,
    downloadUrl: CIVWEAVE_RELEASE_URL, sha256: '', bytes: 0, mandatory: false, localInstallRequired: true,
    notes: 'The public origin distributes the Civweave v1.0.83 fixed-settings-layer device package.'
  };
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (gatewayRequest && applicationSurface && !installerSurface && !packageInstall && pathname !== '/app' && !pathname.startsWith('/app/')) { return json(res,410,{error:'Installed runtime required',localInstallRequired:true,installUrl:requestOrigin(req,url)+'/',sourceUrl:CIVWEAVE_SOURCE_URL,releaseUrl:CIVWEAVE_RELEASE_URL,message:'This public origin distributes the complete device package but does not run the Civweave software family in browser mode.'}); }
