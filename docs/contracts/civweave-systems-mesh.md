...
Every canonical system loads it through the existing shared experience boundary in `public/app/install-boundary-v146.js`. Routing remains responsible only for canonical system identity/navigation. The runtime can:
...
The verifier asserts the canonical five-system registry, source ownership, target restrictions, shared-boundary loading, bounded local outbox/inbox behavior, portable draft/projection bundles, and the absence of Node Host credentials/network primitives or canonical ledger writes from the browser runtime.
