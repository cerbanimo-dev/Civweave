...
export function createNodeTerritoryHostAuthorityHandler({
  ...
  const coreOrigin = (() => { try { const parsed = new URL(coreUrl); return parsed.protocol === 'https:' ? parsed.origin : ''; } catch { return ''; } })();
  ...
  async function handle(req, res, url) {
    ...
    try {
      ...
      sendJson(res, safeStatus(error), { error: safeStatus(error) === 500 ? 'Territory host authority request failed.' : clean(error?.message || error, 1200) });
      return true;
    }
  }
