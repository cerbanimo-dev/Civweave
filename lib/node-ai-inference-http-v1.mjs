...
function statusFor(error) {
  ...
  if (/capability|signature|bound to|expired|revoked|wallet update/i.test(message)) return 401;
  if (/no implementation handler|does not advertise|No node AI wallet/i.test(message)) return 404;
  if (error instanceof TypeError || error instanceof RangeError || /exceeds|insufficient|invalid|unsupported/i.test(message)) return 400;
  return 500;
}
