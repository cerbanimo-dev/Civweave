...
function statusFor(error) {
  ...
  if (error?.code === 'NODE_AI_REQUEST_IN_PROGRESS' || /already in progress/i.test(message)) return 409;
...
export function createNodeAiInferenceHttpHandler({ ledger, manifest, inferenceGate = null, capabilitySecret = '', now = () => Date.now() } = {}) {
  ...
  async function handle(req, res, url) {
    ...
    if (pathname === '/api/ai/node/inference/status') {
      if (req.method !== 'GET') { sendJson(res, 405, { error: 'Method not allowed.' }); return true; }
      sendJson(res, 200, { inference: status() });
      return true;
    }
    ...
    try {
      ...
      const requestId = clean(input.requestId, 180) || undefined;
      ...
      const settledReplay = requestId && typeof ledger.getInferenceSettlement === 'function'
        ? ledger.getInferenceSettlement({ userId: capability.sub, requestId, serviceId })
        : null;
      if (!settledReplay) {
      ...
      sendJson(res, 200, {
        ...
        replayOutputAvailable: execution.replayOutputAvailable !== false
      });
    } catch (error) {
