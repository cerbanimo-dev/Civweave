...
function errorStatus(error) { if (Number.isSafeInteger(error?.status)) return error.status; const message = String(error?.message || ''); if (/signature|credential|authorization|registration invitation/i.test(message)) return 401; if (/not registered|not owned|not found|unknown/i.test(message)) return 404; if (error instanceof TypeError || error instanceof RangeError || /invalid|blocked|must|cannot|exceed|incomplete|disabled|mismatch|public internet/i.test(message)) return 400; return 500; }
...
export function createNodeMoneyEdgeHttpHandler({
  ...
  registrationSecret = process.env.CIVWEAVE_MONEY_EDGE_REGISTRATION_SECRET || '',
  ...
  function status() {
    ...
    return Object.freeze({ schema: NODE_MONEY_EDGE_SCHEMA, requested: Boolean(requested), enabled: Boolean(service), startupError: startupError ? clean(startupError.message, 500) : null, provider: stripe?.id || null, providerMode: stripe?.mode || null, operatorPayouts: stripe?.operatorPayouts || null, registrationInvitesReady: secretReady(registrationSecret), readiness });
  }
  ...
  function requireRegistrationInvitation(req) {
    if (!secretReady(registrationSecret)) throw new Error('Money-edge registration invitation is not configured.');
    const supplied = clean(req.headers['x-civweave-money-edge-registration'], 10000);
    if (!constantTime(supplied, registrationSecret)) throw new Error('Invalid money-edge registration invitation.');
  }
  async function handle(req, res, url) {
    ...
    try {
      if (pathname === '/api/money-edge/nodes/register') {
        ...
        requireRegistrationInvitation(req);
