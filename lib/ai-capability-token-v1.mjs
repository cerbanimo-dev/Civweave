...
export const AI_CAPABILITY_SCHEMA = 'civweave.ai-capability.v1';
...
export function verifyAiCapability(token, {
  ...
  if (parts.length !== 3) throw new Error('Malformed Civweave AI capability.');
  const [encodedHeader, encodedPayload, signature] = parts;
  const unsigned = `${encodedHeader}.${encodedPayload}`;
  if (!constantTimeEqual(signature, sign(unsigned, secret))) throw new Error('Invalid Civweave AI capability signature.');
  ...
  try {
    ...
    throw new Error('Civweave AI capability is not valid JSON.');
  }
  if (header.alg !== 'HS256' || header.typ !== 'CWAI1') throw new Error('Unsupported Civweave AI capability header.');
  if (payload.schema !== AI_CAPABILITY_SCHEMA) throw new Error('Unsupported Civweave AI capability schema.');

  const now = Math.floor(nowMs / 1000);
  if (!Number.isSafeInteger(payload.iat) || !Number.isSafeInteger(payload.exp) || payload.exp <= payload.iat) {
    throw new Error('Civweave AI capability has invalid timestamps.');
  }
  if (payload.iat > now + 30) throw new Error('Civweave AI capability was issued in the future.');
  if (payload.exp <= now) throw new Error('Civweave AI capability has expired.');
  if (deviceId && payload.device !== deviceId) throw new Error('Civweave AI capability is bound to a different device.');
  ...
  if (expectedWalletVersion && payload.walletVersion !== expectedWalletVersion) {
    throw new Error('Civweave AI capability was revoked by a wallet update.');
  }
