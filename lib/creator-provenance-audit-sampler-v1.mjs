...
export async function sampleReceipts(receipts = [], options = {}) {
  const policy = normalizeAuditPolicy(options.policy), dayKey = clean(options.dayKey, 40), secretSalt = clean(options.secretSalt, 4000);
  ...
  for (const receipt of unique.values()) {
    ...
    candidates.push(Object.freeze({
      ...
      sampleId: `audit:${dayKey}:${receipt.sessionId}:${receipt.headHash.slice(0, 16)}`,
      dayKey,
      receipt,
      score,
      threshold,
  ...
  return Object.freeze({
    ...
    selectedCount: Math.min(policy.maxDailySamples, candidates.length),
    samples: candidates.slice(0, policy.maxDailySamples),
  });
}
