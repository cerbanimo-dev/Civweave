...
export class NodeAiLedger {
  ...
  #initialize() {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS node_ai_inference_settlements (
      ...
      CREATE INDEX IF NOT EXISTS node_ai_inference_settlement_user_idx ON node_ai_inference_settlements(node_id, user_id, created_at DESC);
    `);
  }
  ...
  #publicInferenceSettlement(row) {
    ...
    return Object.freeze({
      ...
      retailCostCents: safeInt(row.retail_cost_cents),
  ...
  getInferenceSettlement({ userId, requestId, serviceId }) {
    const owner = text(userId, 'userId', 180);
    const request = text(requestId, 'requestId', 180);
    const service = text(serviceId, 'serviceId', 120);
    const row = this.db.prepare('SELECT * FROM node_ai_inference_settlements WHERE request_id = ?').get(request);
    if (!row) return null;
    if (row.node_id !== this.nodeId || row.user_id !== owner || row.service_id !== service) throw new Error('Inference request ID is already bound to a different node user or service.');
    return this.#publicInferenceSettlement(row);
  }

  settleInference({ userId, reservationId, requestId, serviceId, actualRetailCostCents, receipt, metadata = {}, at = new Date().toISOString() }) {
    const owner = text(userId, 'userId', 180);
    const id = text(reservationId, 'reservationId', 180);
    const request = text(requestId, 'requestId', 180);
    const service = text(serviceId, 'serviceId', 120);
    ...
    const timestamp = iso(at);
    return this.#tx(() => {
      const prior = this.db.prepare('SELECT * FROM node_ai_inference_settlements WHERE request_id = ?').get(request);
      if (prior) {
        if (prior.node_id !== this.nodeId || prior.user_id !== owner || prior.service_id !== service) throw new Error('Inference request ID is already bound to a different node user or service.');
        return { wallet: this.#publicWallet(this.#walletRow(owner)), settlement: this.#publicInferenceSettlement(prior), serviceId: prior.service_id, retailCostCents: safeInt(prior.retail_cost_cents), receipt: parseJson(prior.receipt_json), idempotent: true };
      }
      ...
      if (reservation.service_id !== service) throw new Error('Inference reservation is bound to a different service.');
      ...
      const metadataJson = json({ reservationId: id, ...metadata });
      ...
      this.db.prepare('INSERT INTO node_ai_ledger(node_id,user_id,kind,amount_cents,related_id,metadata_json,created_at) VALUES(?,?,?,?,?,?,?)')
        .run(this.nodeId, owner, 'inference-retail-charge', -actual, request, json({ reservationId: id, serviceId: service, ...metadata }), timestamp);
      this.db.prepare('INSERT INTO node_ai_inference_settlements(request_id,node_id,user_id,service_id,retail_cost_cents,receipt_json,metadata_json,created_at) VALUES(?,?,?,?,?,?,?,?)')
        .run(request, this.nodeId, owner, service, actual, receiptJson, metadataJson, timestamp);
      const row = this.db.prepare('SELECT * FROM node_ai_inference_settlements WHERE request_id = ?').get(request);
      return { wallet: this.#publicWallet(this.#walletRow(owner)), settlement: this.#publicInferenceSettlement(row), serviceId: service, retailCostCents: actual, receipt: parseJson(row.receipt_json), idempotent: false };
    });
  }
