...
export const AI_PLAN_CATALOG_VERSION = 'commonweave.ai-plans.v1';
...
export function getAiPlan(planId) {
  ...
  if (!plan) throw new RangeError(`Unknown Commonweave AI plan: ${id}`);
  return plan;
}
...
export function allocateSubscriptionCharge({
  ...
  return deepFreeze({
    schema: 'commonweave.subscription-allocation.v1',
...
export function quoteTopUp({ grossCents, netDistributableCents }) {
  ...
  return deepFreeze({
    schema: 'commonweave.ai-top-up-quote.v1',
    grossCents: gross,
    netDistributableCents: net,
...
export function createWallet({
  ...
  const plan = getAiPlan(planId);
  return {
    schema: 'commonweave.ai-wallet.v1',
