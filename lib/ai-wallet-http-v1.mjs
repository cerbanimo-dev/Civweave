...
const API_SCHEMA = 'civweave.ai-wallet-http.v1';
const PAYMENT_SCHEMA = 'civweave.payment-event.v1';
...
export function createAiWalletHttpHandler({ walletService, requested = false, authSecret = '', paymentSecret = '', internalSecret = '', capabilitySecret = '', paymentToleranceSeconds = 300, now = () => Date.now() } = {}) {
  ...
  function status() { return Object.freeze({ schema: API_SCHEMA, requested: Boolean(requested), enabled, storage: walletService?.storage || 'unavailable', registeredDevicesRequired: Boolean(walletService?.requireRegisteredDevices), missing: [...missing] }); }
  ...
  function requireInternal(req) { const supplied = clean(req.headers['x-civweave-internal-secret'], 10000); if (!constantTimeEqual(supplied, internalSecret)) throw new Error('Invalid Civweave internal wallet credential.'); }
  ...
  async function handle(req, res, url) {
    ...
    if (pathname === '/api/ai/plans') { if (req.method !== 'GET') sendJson(res, 405, { error: 'Method not allowed.' }); else sendJson(res, 200, { schema: AI_PLAN_CATALOG_VERSION, hostedWalletEnabled: enabled, plans: Object.values(AI_PLAN_CATALOG).map(publicPlan) }); return true; }
    ...
    try {
      if (pathname === '/api/ai/wallet/payments/webhook') {
        ...
        const raw = await readRaw(req, 256 * 1024); verifyPaymentSignature(raw, req.headers['x-civweave-payment-signature'], paymentSecret, now(), paymentToleranceSeconds);
      ...
      if (pathname === '/api/ai/wallet/internal/sessions') {
        ...
        sendJson(res, 201, { schema: 'civweave.ai-wallet-session-envelope.v1', session: token, expiresInSeconds: ttlSeconds }); return true;
      }
      ...
      if (pathname === '/api/ai/wallet/internal/ledger') { if (req.method !== 'GET') { sendJson(res, 405, { error: 'Method not allowed.' }); return true; } requireInternal(req); const userId = clean(url.searchParams.get('userId'), 180); if (!userId) throw new TypeError('userId is required.'); const limit = Math.min(500, Math.max(1, Number(url.searchParams.get('limit') || 100))); sendJson(res, 200, { schema: 'civweave.ai-wallet-ledger-page.v1', entries: await walletService.listLedgerEntries({ userId, limit, before: url.searchParams.get('before') || null }) }); return true; }
      if (pathname === '/api/ai/wallet/internal/reconcile') { if (req.method !== 'POST') { sendJson(res, 405, { error: 'Method not allowed.' }); return true; } requireInternal(req); const input = await readJson(req, 64 * 1024); sendJson(res, 200, { schema: 'civweave.ai-wallet-reconciliation.v1', reconciled: await walletService.reconcileExpiredReservations({ at: input.at || new Date(now()).toISOString(), limit: input.limit || 100 }) }); return true; }
      ...
      if (pathname === '/api/ai/wallet/capability' && req.method === 'POST') { const session = await requireSession(req); const input = await readJson(req, 64 * 1024); await walletService.expireReservations({ userId: session.sub }); const deviceId = clean(input.deviceId || session.device, 180); if (deviceId !== session.device) throw new Error('Wallet session cannot issue a capability for a different device.'); const token = await walletService.issueCapability({ userId: session.sub, deviceId, models: input.models, maxRequestCents: input.maxRequestCents, ttlSeconds: input.ttlSeconds || 900 }); sendJson(res, 201, { schema: 'civweave.ai-capability-envelope.v1', capability: token, expiresInSeconds: input.ttlSeconds || 900 }); return true; }
      if (pathname === '/api/ai/wallet/reservations' && req.method === 'POST') { const session = await requireSession(req); const input = await readJson(req, 64 * 1024); await walletService.expireReservations({ userId: session.sub }); const wallet = await walletService.getWallet(session.sub); if (!wallet) throw new RangeError(`No hosted-AI wallet exists for ${session.sub}.`); const reservationId = clean(input.reservationId, 180), model = clean(input.model, 120); if (!reservationId || !model) throw new TypeError('reservationId and model are required.'); const capability = verifyAiCapability(req.headers['x-civweave-ai-capability'], { secret: capabilitySecret, nowMs: now(), deviceId: session.device, model, estimatedCostCents: input.maxCostCents, expectedWalletVersion: wallet.walletVersion }); if (capability.sub !== session.sub) throw new Error('AI capability belongs to a different user.'); const next = await walletService.reserve({ userId: session.sub, reservationId, maxCostCents: input.maxCostCents, model, metadata: { purpose: clean(input.purpose, 120), capabilityId: capability.jti }, ttlSeconds: 900 }); sendJson(res, 201, { wallet: publicWallet(next), reservation: next.reservations[reservationId] }); return true; }
