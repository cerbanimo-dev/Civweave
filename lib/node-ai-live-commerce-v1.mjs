...
export function createNodeAiLiveCommerceHandler({
  ...
  async function handle(req, res, url) {
    ...
    try {
      ...
      if (pathname === '/api/ai/node/live/operator/charters' && req.method === 'GET') {
        requireInternal(req);
        const result = await edgeRequest(`/api/money-edge/charters?nodeId=${encodeURIComponent(manifest.nodeId)}`, { method: 'GET', body: null });
        sendJson(res, 200, { schema: NODE_AI_LIVE_COMMERCE_SCHEMA, charterkeeping: result }); return true;
      }
      if (pathname === '/api/ai/node/live/operator/charters' && req.method === 'POST') {
        requireInternal(req);
        const input = parseJson(await readRaw(req, 128 * 1024));
        const body = { ...input, charterkeeperNodeId: manifest.nodeId, idempotencyKey: clean(input.idempotencyKey, 180) || `charter:${manifest.nodeId}:${crypto.randomUUID()}` };
        const result = await edgeRequest('/api/money-edge/charters', { body });
        sendJson(res, 201, { schema: NODE_AI_LIVE_COMMERCE_SCHEMA, charterkeeping: result }); return true;
      }
      const charterAction = pathname.match(/^\/api\/ai\/node\/live\/operator\/charters\/([^/]+)\/(training|prepare|accept|agreement|end)$/);
      if (charterAction && req.method === 'POST') {
        requireInternal(req);
        const input = parseJson(await readRaw(req, 128 * 1024));
        const action = charterAction[2];
        const body = action === 'accept' ? { ...input, childNodeId: manifest.nodeId }
          : action === 'end' ? { ...input, nodeId: manifest.nodeId }
            : input;
        const result = await edgeRequest(`/api/money-edge/charters/${encodeURIComponent(charterAction[1])}/${action}`, { body });
        sendJson(res, 200, { schema: NODE_AI_LIVE_COMMERCE_SCHEMA, charterkeeping: result }); return true;
      }
