...
function safeStatus(error) { if (Number.isSafeInteger(error?.status)) return error.status; const message = String(error?.message || ''); if (/session|signature|credential|authorization|registered|revoked|invitation/i.test(message)) return 401; if (/not found|not registered/i.test(message)) return 404; if (error instanceof TypeError || error instanceof RangeError || /invalid|blocked|must|cannot|disabled|unavailable|insufficient/i.test(message)) return 400; return 500; }
...
export function createNodeAiLiveCommerceHandler({
  ...
  function status() {
    return Object.freeze({ schema: NODE_AI_LIVE_COMMERCE_SCHEMA, requested: Boolean(requested), enabled, nodeId: manifest?.nodeId || null, edgeOrigin, callbackOrigin: origin, operatorPayouts: 'stripe-connected-account-native', livePayments: enabled, registrationInvitationStored: false, missing: [...missing] });
  }
  ...
  async function handle(req, res, url) {
    ...
    try {
      ...
      if (pathname === '/api/ai/node/live/operator/connect') {
        ...
        const registrationSecret = clean(input.registrationSecret, 10000);
        if (!secretReady(registrationSecret)) throw new Error('A valid money-edge registration invitation is required.');
        const result = await edgeRequest('/api/money-edge/nodes/register', {
          signed: false,
          headers: { 'x-civweave-money-edge-registration': registrationSecret },
          body: { nodeId: manifest.nodeId, operatorId: manifest.operatorId, callbackUrl: origin, email: clean(input.email, 320) || undefined, country: clean(input.country, 4) || undefined }
        });
