...
const RECEIPT_SCHEMAS=new Set(['civweave.creation-receipt.v1','civweave.creation-receipt-summary.v1']);
...
export function buildC2paManifestIntent({receipt={},packet={},software={}}={}){
  if(!RECEIPT_SCHEMAS.has(receipt?.schema))throw new TypeError('A finalized or compact Civweave creation receipt is required.');
  ...
  for(const event of substantive){const seq=Number(event?.seq)||0;if(seq===firstSeq&&!isExternal(event))continue;const action=editAction(event,agent,ingredientBySeq.get(seq)||'');if(action)actions.push(action);if(actions.length>=256)break}
