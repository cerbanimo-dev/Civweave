...
export class StripeConnectDirectProvider {
  constructor({
    ...
    this.connectedAccountDashboard = 'full';
    this.connectedAccountFeePayer = 'account';
    this.connectedAccountLosses = 'stripe';
    this.connectedAccountRequirements = 'stripe';
  }
  ...
  async createConnectedAccount({ nodeId, operatorId, email = null, country = null, metadata = {} } = {}) {
    const form = {
    ...
    const meta = cleanMetadata({
      ...
      civweave_operator_id: text(operatorId, 'operatorId', 180),
      civweave_connect_model: STRIPE_CONNECT_ACCOUNT_MODEL,
      ...metadata
    });
  ...
  // Compatibility alias for the existing money-edge service. New accounts no longer use Stripe's legacy type=standard parameter.
  async createStandardAccount(input = {}) {
    return this.createConnectedAccount(input);
  }
