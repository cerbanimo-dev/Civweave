...
export function aggregateValidations(validations, policyInput = {}) {
  ...
  // Accumulate absolute evidence strength rather than normalizing weak evidence to 100%.
  // Support asymptotically approaches 1 as independent weighted evidence accumulates;
  // contradictory evidence applies a nonlinear drag without erasing the underlying receipts.
  const supportScore = 1 - Math.exp(-(1.15 * positive));
  const oppositionScore = 1 - Math.exp(-negative);
  const confidence = Math.max(0, Math.min(1, supportScore * (1 - (0.85 * oppositionScore))));
