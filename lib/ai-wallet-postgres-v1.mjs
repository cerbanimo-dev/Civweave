...
export class PostgresAiWalletService {
  ...
  async load() {
    ...
    const rows = await this.#query("SELECT to_regnamespace('commonweave_ai_wallet') IS NOT NULL AS ready");
    if (!rows?.[0]?.ready) throw new Error('The Commonweave AI wallet schema is not installed. Run db/migrations/001-ai-wallet-ledger.sql first.');
    return this;
  }
  async flush() {}
  async getWallet(userId) {
    const rows = await this.#query('SELECT commonweave_ai_wallet.wallet_snapshot($1) AS wallet', [clean(userId, 'userId')]);
    return normalizeWallet(firstValue(rows, 'wallet'));
  }
  async getSourceEvent(sourceId) {
    const rows = await this.#query(`SELECT source_id AS "sourceId", user_id AS "userId", event_type AS "type", amount_cents::int AS "amountCents", payload_hash AS "payloadHash", metadata, processed_at AS "processedAt" FROM commonweave_ai_wallet.payment_events WHERE source_id = $1`, [clean(sourceId, 'sourceId')]);
    return rows?.[0] || null;
  }
  async ensureWallet({ userId, planId = 'local' }) {
    getAiPlan(planId);
    const rows = await this.#query('SELECT commonweave_ai_wallet.ensure_wallet($1,$2) AS wallet', [clean(userId, 'userId'), planId]);
    return normalizeWallet(firstValue(rows, 'wallet'));
  }
  async registerDevice({ userId, deviceId, publicKey = null, label = null, metadata = {} }) {
    const rows = await this.#query('SELECT commonweave_ai_wallet.register_device($1,$2,$3,$4,$5::jsonb) AS registered', [clean(userId, 'userId'), clean(deviceId, 'deviceId'), publicKey || null, label || null, metadataJson(metadata)]);
    return Boolean(firstValue(rows, 'registered'));
  }
  async revokeDevice({ userId, deviceId }) {
    const rows = await this.#query('SELECT commonweave_ai_wallet.revoke_device($1,$2) AS revoked', [clean(userId, 'userId'), clean(deviceId, 'deviceId')]);
    return Boolean(firstValue(rows, 'revoked'));
  }
  async isDeviceActive({ userId, deviceId }) {
    const rows = await this.#query('SELECT commonweave_ai_wallet.device_is_active($1,$2) AS active', [clean(userId, 'userId'), clean(deviceId, 'deviceId')]);
  ...
  async credit({ userId, amountCents, sourceId, planId, eventType = 'credit', payloadHash = null, metadata = {} }) {
    ...
    const rows = await this.#query('SELECT commonweave_ai_wallet.credit_wallet($1,$2,$3,$4,$5,$6,$7::jsonb) AS wallet', [clean(userId, 'userId'), amountCents, clean(sourceId, 'sourceId'), planId || null, clean(eventType, 'eventType'), payloadHash || null, metadataJson(metadata)]);
    return normalizeWallet(firstValue(rows, 'wallet'));
  }
  async debit({ userId, amountCents, sourceId, planId, eventType = 'debit', payloadHash = null, metadata = {} }) {
    positiveInteger(amountCents, 'amountCents');
    if (planId) getAiPlan(planId);
    const rows = await this.#query('SELECT commonweave_ai_wallet.debit_wallet($1,$2,$3,$4,$5,$6,$7::jsonb) AS wallet', [clean(userId, 'userId'), amountCents, clean(sourceId, 'sourceId'), planId || null, clean(eventType, 'eventType'), payloadHash || null, metadataJson(metadata)]);
    return normalizeWallet(firstValue(rows, 'wallet'));
  }
  async reserve({ userId, reservationId, maxCostCents, model, metadata = {}, ttlSeconds = 900 }) {
    ...
    const rows = await this.#query('SELECT commonweave_ai_wallet.reserve_wallet($1,$2,$3,$4,$5::jsonb,$6,$7,$8) AS wallet', [clean(userId, 'userId'), clean(reservationId, 'reservationId'), positiveInteger(maxCostCents, 'maxCostCents'), clean(model, 'model'), metadataJson(metadata), ttlSeconds, plan.maxRequestCents, plan.dailyHostedLimitCents]);
    return normalizeWallet(firstValue(rows, 'wallet'));
  }
  async expireReservations({ userId, at = new Date().toISOString() }) {
    ...
    const rows = await this.#query('SELECT commonweave_ai_wallet.expire_reservations($1,$2::timestamptz) AS wallet', [clean(userId, 'userId'), timestamp.toISOString()]);
    return normalizeWallet(firstValue(rows, 'wallet'));
  }
  async reconcileExpiredReservations({ at = new Date().toISOString(), limit = 100 } = {}) {
    ...
    const rows = await this.#query(`WITH targets AS (SELECT DISTINCT user_id FROM commonweave_ai_wallet.reservations WHERE status='active' AND expires_at <= $1::timestamptz ORDER BY user_id LIMIT $2) SELECT user_id AS "userId", commonweave_ai_wallet.expire_reservations(user_id,$1::timestamptz) AS wallet FROM targets`, [timestamp.toISOString(), limit]);
  ...
  async settle({ userId, reservationId, actualCostCents, requestId = null, metadata = {} }) {
    ...
    const rows = await this.#query('SELECT commonweave_ai_wallet.settle_wallet($1,$2,$3,$4,$5::jsonb) AS wallet', [clean(userId, 'userId'), clean(reservationId, 'reservationId'), actualCostCents, requestId || null, metadataJson(metadata)]);
    return normalizeWallet(firstValue(rows, 'wallet'));
  }
  async cancel({ userId, reservationId, metadata = {} }) {
    const rows = await this.#query('SELECT commonweave_ai_wallet.cancel_wallet($1,$2,$3::jsonb) AS wallet', [clean(userId, 'userId'), clean(reservationId, 'reservationId'), metadataJson(metadata)]);
    return normalizeWallet(firstValue(rows, 'wallet'));
  }
  async listLedgerEntries({ userId, limit = 100, before = null }) {
    ...
    return this.#query(`SELECT entry_id::text AS "entryId", user_id AS "userId", entry_type AS "entryType", delta_balance_cents::int AS "deltaBalanceCents", delta_reserved_cents::int AS "deltaReservedCents", delta_debt_cents::int AS "deltaDebtCents", source_id AS "sourceId", reservation_id AS "reservationId", request_id AS "requestId", wallet_version::text AS "walletVersion", metadata, created_at AS "createdAt" FROM commonweave_ai_wallet.ledger_entries WHERE user_id=$1 AND ($2::timestamptz IS NULL OR created_at < $2::timestamptz) ORDER BY created_at DESC, entry_id DESC LIMIT $3`, [clean(userId, 'userId'), beforeDate?.toISOString() || null, limit]);
  }
