...
function optionalBps(value) {
  if (value == null || String(value).trim() === '') return null;
  return bps(Number(value), 'CIVWEAVE_MONEY_PLATFORM_FEE_BPS');
}
...
export class NodeMoneyEdgeService {
  constructor({
    ...
    enrollmentTtlSeconds = Number(process.env.CIVWEAVE_MONEY_ENROLLMENT_TTL_SECONDS || 600),
    ...
    this.publicKey = this.privateKey ? crypto.createPublicKey(this.privateKey).export({ type: 'spki', format: 'pem' }).toString().trim() : '';
    this.publicKeyFingerprint = this.publicKey ? sha256(this.publicKey) : '';
    this.platformFeeBps = optionalBps(platformFeeBps);
    this.config = { ...config };
    this.fetch = fetchImpl;
    this.now = now;
    this.signatureToleranceSeconds = signatureToleranceSeconds;
    this.enrollmentTtlSeconds = Math.max(60, Math.min(3600, Number(enrollmentTtlSeconds) || 600));
  ...
  #init() {
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS money_edge_enrollment_node_idx ON money_edge_enrollment_grants(node_id,created_at DESC);
  ...
  trustDocument() {
    if (!this.publicKey) throw new Error('Money-edge signing key is unavailable.');
    return Object.freeze({
      schema: 'civweave.money-edge-trust.v1',
      keyId: this.keyId,
      algorithm: 'Ed25519',
  ...
  readiness() {
    ...
    if (this.platformFeeBps == null) structuralBlockers.push('platform-fee-policy-missing');
  ...
  async #probeNode({ nodeId, operatorId, callbackOrigin }) {
    ...
    if (manifest.nodeId !== nodeId || manifest.operatorId !== operatorId) throw new Error('Node manifest identity does not match money-edge enrollment.');
    ...
    return { manifest, publicKey: manifest.publicKey };
  }
  ...
  async createEnrollmentGrant({ nodeId, operatorId, callbackUrl } = {}) {
    ...
    if (this.platformFeeBps == null) throw new Error('Cerbanimo platform fee policy is not configured.');
    const id = text(nodeId, 'nodeId', 180), operator = text(operatorId, 'operatorId', 180);
    const callbackOrigin = cleanOrigin(callbackUrl, { live: this.provider?.mode === 'live' });
    const proof = await this.#probeNode({ nodeId: id, operatorId: operator, callbackOrigin });
    const token = crypto.randomBytes(32).toString('base64url');
    const grantHash = sha256(token), createdAt = isoNow(this.now), expiresAt = new Date(this.now() + this.enrollmentTtlSeconds * 1000).toISOString();
    this.db.prepare(`INSERT INTO money_edge_enrollment_grants(grant_hash,node_id,operator_id,callback_origin,receipt_public_key,platform_fee_bps,created_at,expires_at,consumed_at)
      VALUES(?,?,?,?,?,?,?,?,NULL)`).run(grantHash, id, operator, callbackOrigin, proof.publicKey, this.platformFeeBps, createdAt, expiresAt);
    return Object.freeze({
  ...
  #consumeEnrollmentGrant({ token, nodeId, operatorId, callbackOrigin, publicKey }) {
    const grantHash = sha256(text(token, 'enrollmentGrant', 1000));
    const grant = this.db.prepare('SELECT * FROM money_edge_enrollment_grants WHERE grant_hash = ?').get(grantHash);
    ...
    if (grant.node_id !== nodeId || grant.operator_id !== operatorId || grant.callback_origin !== callbackOrigin || grant.receipt_public_key !== publicKey) throw Object.assign(new Error('Enrollment grant is bound to a different node identity.'), { status: 401 });
    const at = isoNow(this.now);
    const changed = this.db.prepare('UPDATE money_edge_enrollment_grants SET consumed_at=? WHERE grant_hash=? AND consumed_at IS NULL').run(at, grantHash);
    if (Number(changed.changes || 0) !== 1) throw Object.assign(new Error('Enrollment grant could not be consumed exactly once.'), { status: 401 });
    return grant;
  }

  async registerNode({ nodeId, operatorId, callbackUrl, enrollmentGrant, email = null, country = null } = {}) {
    if (this.config.emergencyStop) throw new Error('Money edge emergency stop is active.');
    const id = text(nodeId, 'nodeId', 180), operator = text(operatorId, 'operatorId', 180);
    const callbackOrigin = cleanOrigin(callbackUrl, { live: this.provider?.mode === 'live' });
    const proof = await this.#probeNode({ nodeId: id, operatorId: operator, callbackOrigin });
    const grant = this.#consumeEnrollmentGrant({ token: enrollmentGrant, nodeId: id, operatorId: operator, callbackOrigin, publicKey: proof.publicKey });
    ...
    this.db.prepare(`INSERT INTO money_edge_nodes(node_id,operator_id,connected_account_id,callback_origin,receipt_public_key,platform_fee_bps,created_at,updated_at)
      ...
      .run(id, operator, accountId, callbackOrigin, proof.publicKey, Number(grant.platform_fee_bps), existing?.created_at || at, at);
    ...
    return Object.freeze({ schema: NODE_MONEY_EDGE_SCHEMA, nodeId: id, operatorId: operator, connectedAccountId: accountId, onboardingUrl: link.url, onboardingExpiresAt: link.expires_at ? new Date(Number(link.expires_at) * 1000).toISOString() : null, platformFeeBps: Number(grant.platform_fee_bps), operatorPayouts: this.provider.operatorPayouts, enrollment: 'proof-of-key-short-lived-grant' });
  }
  ...
  async settleTopUpBySession(sessionId, sourceEventId) {
    ...
    const payload = Object.freeze({
      ...
      metadata: { topupId: current.topup_id, stripeSessionId: current.stripe_session_id, stripeChargeId: current.stripe_charge_id, feeAuthority: 'cerbanimo-money-edge' },
      mintEffect: 0,
      supplyEffect: 0
    });
