'use strict';
(()=>{
const CACHE='cwupdate-visible-v204';
const FILES=[
...
async function populate(){
  ...
  for(const url of FILES){
    const response=await fetch(`${url}${url.includes('?')?'&':'?'}v=visible-update-library-v204`,{cache:'no-store',headers:{'x-commonweave-package':'update-controls'}});
...
self.CommonweaveUpdatePackageV204=Object.freeze({version:'v204-visible-update-library-preservation',cache:CACHE,files:[...FILES],knowledgeCache:'cwknowledge-school-seeds-v2'});
})();
