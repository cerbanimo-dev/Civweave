...
async function populate(){
  ...
  for(const url of FILES){
    const response=await fetch(`${url}${url.includes('?')?'&':'?'}v=visible-update-library-v207-registration-watchdog`,{cache:'no-store',headers:{'x-civweave-package':'update-controls'}});
...
self.CivweaveUpdatePackageV204=Object.freeze({version:'v207-registration-watchdog',cache:CACHE,files:[...FILES],knowledgeCache:'cwknowledge-school-seeds-v2'});
})();
