'use strict';
(()=>{
const REVISION='decentralized-guild-release-v1-20260917';
...
async function verify(record,jwk){
  if(!record?.signature)return false;
  const body={...record};delete body.signature;
  try{
    const key=await crypto.subtle.importKey('jwk',jwk,{name:'ECDSA',namedCurve:'P-256'},false,['verify']);
    return crypto.subtle.verify({name:'ECDSA',hash:'SHA-256'},key,unb64(record.signature),enc.encode(canonical(body)));
  }catch{return false}
}
async function anchors(){const rows=await get('anchors');return Array.isArray(rows)?rows:[]}
async function addAnchor(input){
  const jwk=input?.publicKey;
  if(!jwk||jwk.kty!=='EC'||jwk.crv!=='P-256')throw new Error('Release trust anchor must be a P-256 public JWK.');
  await crypto.subtle.importKey('jwk',jwk,{name:'ECDSA',namedCurve:'P-256'},false,['verify']);
  const id=await keyId(jwk),permissions=caps(input.capabilities);
  if(input.keyId&&input.keyId!==id)throw new Error('Release trust anchor key id mismatch.');
  if(!permissions.length)throw new Error('Release trust anchor has no capabilities.');
  const row={schema:'civweave.local-release-trust-anchor.v1',keyId:id,publicKey:jwk,capabilities:permissions,label:clean(input.label||id,160),trustedAt:new Date().toISOString(),source:'explicit-local-import'};
  const rows=(await anchors()).filter(x=>x.keyId!==id);rows.push(row);await put('anchors',rows);return row;
}
...
async function trust(bundle={}){
  ...
  async function expand(blocked=new Set()){
    ...
    for(let pass=0;pass<8;pass++){
      ...
      for(const d of Array.isArray(bundle.delegations)?bundle.delegations:[]){
        ...
        map.set(d.subjectKeyId,{keyId:d.subjectKeyId,publicKey:d.subjectPublicKey,capabilities:permissions,issuerKeyId:d.issuerKeyId});
        changed=true;
      }
      if(!changed)break;
    }
  ...
  for(const r of Array.isArray(bundle.revocations)?bundle.revocations:[]){
    const issuer=first.get(r?.issuerKeyId);
    if(r?.schema===REVOCATION_SCHEMA&&active(r)&&issuer?.capabilities.includes('revoke')&&await verify(r,issuer.publicKey))revoked.add(r.revokedKeyId);
  }
  return{map:await expand(revoked),revoked};
}
function owned(path){
  if(path==='/'||path==='/index.html'||path==='/offline.html'||PREFIXES.includes(path))return true;
  return PREFIXES.some(p=>path.startsWith(p))&&TEXT.test(path);
}
...
function valid(response,path){
  ...
  if(/\.html?$/i.test(path)||PREFIXES.includes(path))return type.includes('text/html')||!type;
  if(/\.(?:m?js|css)$/i.test(path))return !type.includes('text/html');
  return true;
}
function head(r){return new Response(null,{status:r.status,statusText:r.statusText,headers:r.headers})}
async function bounded(input,ms=TIMEOUT){
  const c=new AbortController(),timer=setTimeout(()=>c.abort(),ms);
  try{return await fetch(new Request(input,{cache:'no-store',signal:c.signal}))}finally{clearTimeout(timer)}
}
...
async function bundle(guild){
  try{
    const r=await bounded(`${guild.origin}/api/civweave-trust/bundle`);
    if(!r.ok)throw new Error(`Guild trust relay returned ${r.status}.`);
    const value=await r.json();
    if(value?.schema!==TRUST_SCHEMA)throw new Error('Unsupported Guild trust bundle.');
    const safe={schema:TRUST_SCHEMA,delegations:Array.isArray(value.delegations)?value.delegations:[],revocations:Array.isArray(value.revocations)?value.revocations:[]};
    await put('relayedTrustBundle',safe);return safe;
  }catch(error){const saved=await get('relayedTrustBundle');if(saved)return saved;throw error}
}
async function verifyManifest(manifest,remote){
  ...
  const seen=new Set();
  for(const a of manifest.assets){const path=assetPath(a.path);if(seen.has(path)||!/^[A-Za-z0-9_-]{40,100}$/.test(clean(a.sha256,120)))throw new Error(`Guild release asset entry is invalid: ${path}`);seen.add(path)}
...
async function install(guild,manifest){
  const name=`${GUILD_CACHE_PREFIX}${(await digest(manifest.releaseId)).slice(0,24)}`,stage=`${name}-stage`;
  await caches.delete(stage);const staging=await caches.open(stage);
  try{
    for(const item of manifest.assets){
      const path=assetPath(item.path),u=new URL('/api/civweave-release/asset',guild.origin);
      u.searchParams.set('releaseId',manifest.releaseId);u.searchParams.set('path',path);
      const r=await bounded(u.href,12000);if(!r.ok)throw new Error(`Guild release asset ${path} returned ${r.status}.`);
      const bytes=new Uint8Array(await r.arrayBuffer());if(await digest(bytes)!==item.sha256)throw new Error(`Guild release asset hash mismatch for ${path}.`);
      const h=new Headers(r.headers);h.set('x-civweave-guild-release',manifest.releaseId);
      await staging.put(requestFor(path),new Response(bytes,{status:200,headers:h}));
    }
    await caches.delete(name);const final=await caches.open(name);
    for(const item of manifest.assets){const path=assetPath(item.path),r=await staging.match(requestFor(path),{ignoreSearch:true});if(!r)throw new Error(`Guild release staging cache lost ${path}.`);await final.put(requestFor(path),r)}
    await put('activeRelease',{releaseId:manifest.releaseId,cacheName:name,guildOrigin:guild.origin,signerKeyId:manifest.signerKeyId,activatedAt:new Date().toISOString()});
    for(const n of await caches.keys())if(n.startsWith(GUILD_CACHE_PREFIX)&&n!==name&&n!==stage)await caches.delete(n);
    return{ok:true,releaseId:manifest.releaseId,cacheName:name};
  }finally{await caches.delete(stage)}
}
async function check(force=false){
  const guild=await get('guild');if(!guild)return{ok:false,reason:'no-guild-selected'};
  const last=Number(await get('lastCheck')||0);if(!force&&Date.now()-last<CHECK_MS)return{ok:true,reason:'check-throttled'};
  await put('lastCheck',Date.now());
  const [remote,r]=await Promise.all([bundle(guild),bounded(`${guild.origin}/api/civweave-release/current`)]);
  if(r.status===404)return{ok:false,reason:'guild-has-no-release'};
  if(!r.ok)throw new Error(`Guild release relay returned ${r.status}.`);
  const manifest=await r.json();await verifyManifest(manifest,remote);
  const current=await get('activeRelease');if(current?.releaseId===manifest.releaseId&&current?.guildOrigin===guild.origin)return{ok:true,reason:'already-current',releaseId:manifest.releaseId};
  const result=await install(guild,manifest);await notify({type:'CIVWEAVE_GUILD_RELEASE_READY',...result,guildOrigin:guild.origin});return result;
}
...
async function localFallback(path,method){
  const r=await caches.match(requestFor(path),{ignoreSearch:true});if(!valid(r,path))return null;return method==='HEAD'?head(r):r;
}
async function responseFor(request,path){
  const guild=await get('guild');
  if(guild){
    const current=await get('activeRelease');
    if(current?.cacheName){const r=await(await caches.open(current.cacheName)).match(requestFor(path),{ignoreSearch:true});if(valid(r,path))return request.method==='HEAD'?head(r):r}
    const local=await localFallback(path,request.method);if(local)return local;
    return new Response('Verified Guild release asset unavailable on this device.',{status:503,headers:{'content-type':'text/plain; charset=utf-8','cache-control':'no-store','x-civweave-release-policy':REVISION}});
  }
  try{
    const r=await bounded(request);if(valid(r,path)){if(request.method==='GET')await(await caches.open(LEGACY_CACHE)).put(requestFor(path),r.clone());return request.method==='HEAD'?head(r):r}
  }catch{}
  return await localFallback(path,request.method)||new Response('Civweave runtime asset unavailable offline.',{status:503});
}
async function purge(){
  const activeRelease=await get('activeRelease'),names=await caches.keys();
  for(const name of names){
    if(name===LEGACY_CACHE||name===activeRelease?.cacheName)continue;
    if(name.startsWith(GUILD_CACHE_PREFIX)){await caches.delete(name);continue}
    const cache=await caches.open(name);
    for(const req of await cache.keys()){let u;try{u=new URL(req.url)}catch{continue}if(u.origin===self.location.origin&&owned(u.pathname))await cache.delete(req,{ignoreSearch:true})}
  }
}
...
self.addEventListener('fetch',e=>{
  const request=e.request;if(!['GET','HEAD'].includes(request.method))return;
  let u;try{u=new URL(request.url)}catch{return}if(u.origin!==self.location.origin)return;
  const path=u.pathname;if(request.mode!=='navigate'&&!owned(path))return;
  if(request.mode==='navigate'&&!(path==='/'||path==='/index.html'||PREFIXES.some(p=>path.startsWith(p))))return;
  e.stopImmediatePropagation();if(request.mode==='navigate')e.waitUntil(check(false).catch(()=>null));e.respondWith(responseFor(request,path));
});
