// GENERATED: five-system route contract v227 + living-school clean-room cache boundary + local-ai-code-coherence-v309-lifecycle-deferred + code-coherence-v289-lifecycle-deferred + retained lightweight shell core + shell-assets-v1-repair-v12-persistent-shell-route-safe + installed-pwa-launch-v295-entry-integrity + installer-state-machines-v280 + shell-integrity-v281-required-only-install-v2 + radio-core-shell-v305 + installed-shell-repair-v293 + offline-campus-current-graph-v280 current-manifest-only-v282 + campus-current-completion-v250 + release-coherence-v226 + navigation-redirect-safety-v224 + canonical-navigation-v228-lifecycle-deferred + chat-avatar-visible-v346-lifecycle-deferred + mobile-chat-main-thread-quiescence-v349 + mobile-chat-long-thread-fit-v362 + selected-local-minilm-v357 + server-auto-local-failover-v358 + guide-capability-passover-v361 + local-ai-bootstrap-capability-v359 + local-model-background-v267 + open-learning-media-v1 + working-campus-return-v425 + guild-quest-browser-v430 + boot-recovery-v432-lifecycle-deferred + atomic-update-handoff-v427 + install-only-pwa-v1 + family-nav-single-owner-r1 + family-navigation-live-r2 + persistent-family-shell-v1 + persistent-shell-route-safe-v1 + required-nav-media-v1 + standard-ai-isolation-v1 + cerbanimo-boot-network-first-v1 + desktop-civweave-boot-recovery-v363 + staging-installed-entry-takeover-v1
...
importScripts('/service-worker-shell-integrity-v281.js?v=shell-integrity-v281-required-only-install-v2');
...
importScripts('/service-worker-canonical-navigation-v227.js?v=canonical-five-system-navigation-v228-lifecycle-deferred');
importScripts('/service-worker-chat-repair-v245.js?v=chat-avatar-visible-v346-lifecycle-deferred&purge=chat-avatar-visible-v346&freeze=mobile-chat-main-thread-quiescence-v349&layout=mobile-chat-long-thread-fit-v362&threads=saved-tabs-contained-v354&party=lazy-v353&model=selected-local-minilm-v357&failover=server-auto-local-failover-v358&passover=guide-capability-passover-v361');
...
// staging-installed-entry-takeover-v1: one emergency activation is allowed on the isolated staging origin so a poisoned start document cannot deadlock its own repair.
const V203_STAGING_RECOVERY_HOST='civweave-staging.pages.dev';
const V203_STAGING_RECOVERY_CACHE='cwrecovery-v426';
const V203_STAGING_RECOVERY_MARKER='/__civweave/staging-installed-entry-takeover-v1';
...
if(self.location.hostname===V203_STAGING_RECOVERY_HOST){
  ...
  self.addEventListener('activate',event=>{event.waitUntil((async()=>{const cache=await caches.open(V203_STAGING_RECOVERY_CACHE);await cache.put(v203StagingRecoveryRequest(),new Response('installed-entry-takeover-complete',{headers:{'content-type':'text/plain','cache-control':'no-store'}}));await self.clients.claim()})())});
}
