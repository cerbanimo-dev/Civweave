...
// guild-discovery-v312: rotate the top-level worker so already-installed PWAs adopt network-first Guild search/location code instead of retaining v1.4 from an older cache.
'use strict';
...
importScripts('/service-worker-local-ai-coherence-v307.js?v=1.0.163-local-ai-code-coherence-v312-guild-discovery');
...
// Keep compatibility import identities expected by release-coherence audits; imported files declare their newer internal lifecycle revisions.
...
// staging-installed-entry-takeover-v6-guild-discovery: rotate the one-shot staging marker so every staging install immediately adopts current Guild discovery code.
const V203_STAGING_RECOVERY_HOST='civweave-staging.pages.dev';
const V203_STAGING_RECOVERY_CACHE='cwrecovery-v438-guild-discovery';
const V203_STAGING_RECOVERY_MARKER='/__civweave/staging-installed-entry-takeover-v6-guild-discovery';
...
if(self.location.hostname===V203_STAGING_RECOVERY_HOST){
  ...
  self.addEventListener('activate',event=>{event.waitUntil((async()=>{const cache=await caches.open(V203_STAGING_RECOVERY_CACHE);await cache.put(v203StagingRecoveryRequest(),new Response('guild-discovery-v312-activated',{headers:{'content-type':'text/plain','cache-control':'no-store'}}));await self.clients.claim()})())});
}
