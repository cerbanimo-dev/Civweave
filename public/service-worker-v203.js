...
// direct-first-class-routes-v4: Civweave v440 and each realm own their canonical page; themed-system-nav-v178 is the sole five-system navigation owner.
...
// five-system-pages-v1: exact realm navigation is bounded, validated, and uses a cache-distinct current realm page instead of falling back to stale shell HTML.
...
// family-nav-single-owner-r2: themed-system-nav-v178 remains the sole visual and interaction owner; legacy capture/context observers are not auto-installed.
...
importScripts('/service-worker-five-system-pages-v1.js?v=five-system-pages-v1-bounded-realm-navigation-r4-cache-distinct-shared-nav');
...
// staging-installed-entry-takeover-v17-fresh-realm-cache: one-shot staging activation for the shared-navbar owner repair plus stale realm-page cache purge without interrupting a visible installer.
const V203_STAGING_RECOVERY_HOST='civweave-staging.pages.dev';
const V203_STAGING_RECOVERY_CACHE='cwrecovery-v449-fresh-realm-cache';
const V203_STAGING_RECOVERY_MARKER='/__civweave/staging-installed-entry-takeover-v17-fresh-realm-cache';
...
if(self.location.hostname===V203_STAGING_RECOVERY_HOST){
  ...
  self.addEventListener('activate',event=>{event.waitUntil((async()=>{const cache=await caches.open(V203_STAGING_RECOVERY_CACHE);await cache.put(v203StagingRecoveryRequest(),new Response('fresh-realm-cache-r1-activated',{headers:{'content-type':'text/plain','cache-control':'no-store'}}));await self.clients.claim()})())});
}
