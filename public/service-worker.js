...
const effectiveRevision=`${CACHE_REVISION}-${CABINET_REVISION}`;
const report=async(kind,detail={})=>{
  try{await fetch('/api/boot-log',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({schema:'commonweave.boot-log.v1',time:new Date().toISOString(),version:VERSION,build:'1.0.30-realm-cabinets-r18',kind:`service-worker:${kind}`,detail})})}catch{}
};
...
self.addEventListener('install',event=>event.waitUntil((async()=>{const cache=await caches.open(STATIC_CACHE);const results=await Promise.all(CORE.map(url=>cacheOne(cache,url)));await report('installed',{cached:results.filter(Boolean).length,total:CORE.length,revision:effectiveRevision})})()));
self.addEventListener('activate',event=>event.waitUntil((async()=>{const keys=await caches.keys();const stale=keys.filter(key=>(key.startsWith('commonweave-')&&key!==STATIC_CACHE&&key!==RUNTIME_CACHE)||/^(living-school|cerbanimo|fellowfare|anarchadia)-/.test(key));await Promise.all(stale.map(key=>caches.delete(key)));await self.clients.claim();await report('activated',{deleted:stale,revision:effectiveRevision})})()));
self.addEventListener('message',event=>{if(event.data?.type==='SKIP_WAITING')self.skipWaiting();if(event.data?.type==='GET_VERSION')event.source?.postMessage?.({type:'COMMONWEAVE_VERSION',version:VERSION,revision:effectiveRevision})});
