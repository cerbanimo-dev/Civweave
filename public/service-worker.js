...
async function notifyPackageEvent(detail){try{const clients=await self.clients.matchAll({type:'window',includeUncontrolled:true});for(const client of clients)client.postMessage({type:'COMMONWEAVE_PACKAGE_EVENT',...detail})}catch{}}
async function networkRepair(request){
  const url=new URL(request.url);
  try{
    const headers=new Headers(request.headers);headers.set('x-commonweave-package','runtime-repair');
...
function missingAssetResponse(pathname,fallback=''){
  const body=[
    `Commonweave could not load ${pathname}.`,
