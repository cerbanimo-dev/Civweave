...
const CORE=[
  ...
  '/app/anarchadia-sovereignty-v146.html','/app/anarchadia-sovereignty-v146.css','/app/anarchadia-sovereignty-v146.js','/app/anarchadia-governance-kernel-v145.js','/app/anarchadia-local-sovereignty-v146.js','/app/anarchadia-sovereignty-bridge-v146.js',
  ...
  '/app/shared/commonweave-parity-runtime.js','/app/shared/commonweave-parity-ledger.json',
  ...
  '/app/logos/commonweave.webp','/app/logos/commonweave-app-icon.png','/app/logos/cerbanimo.webp','/app/logos/fellowfare-v2.webp','/app/logos/commonweave-icon-192.png','/app/logos/commonweave-icon-512.png','/app/logos/commonweave-icon-maskable-192.png','/app/logos/commonweave-icon-maskable-512.png',
...
async function modelOnDemand(request){
  ...
  try{
    const headers=new Headers(request.headers);headers.set('x-commonweave-package','model');
  ...
  }catch(error){return new Response(`Local model asset unavailable: ${error.message}`,{status:503,headers:{'content-type':'text/plain; charset=utf-8','cache-control':'no-store','x-commonweave-model-package':'not-installed'}})}
}
async function injectNavigationPolicy(response,pathname=''){const type=response.headers.get('content-type')||'';if(!type.includes('text/html'))return response;const text=await response.text(),scripts=[];if(!text.includes('/app/install-boundary-v146.js'))scripts.push('<script src="/app/install-boundary-v146.js?v=1.0.6"></script>');if(pathname.includes('anarchadia')&&!text.includes('/app/anarchadia-local-sovereignty-v146.js'))scripts.push('<script src="/app/anarchadia-local-sovereignty-v146.js?v=1.0.6"></script><script src="/app/anarchadia-sovereignty-bridge-v146.js?v=1.0.6"></script>');if(!scripts.length)return new Response(text,{status:response.status,statusText:response.statusText,headers:response.headers});const insertion=scripts.join(''),html=/<\/head>/i.test(text)?text.replace(/<\/head>/i,insertion+'</head>'):text.replace(/<\/body>/i,insertion+'</body>');const headers=new Headers(response.headers);headers.delete('content-length');headers.set('x-commonweave-install-boundary','v146');if(pathname.includes('anarchadia'))headers.set('x-commonweave-local-sovereignty','v146');return new Response(html,{status:response.status,statusText:response.statusText,headers})}
