...
const CACHE_REVISION='assistant-orchestration-r20';
const CABINET_REVISION='realm-cabinets-r19';
...
const report=async(kind,detail={})=>{
  try{
    await fetch('/api/boot-log',{
      ...
      body:JSON.stringify({schema:'commonweave.boot-log.v1',time:new Date().toISOString(),version:VERSION,build:'1.0.30-guide-orchestration-v141',kind:`service-worker:${kind}`,detail})
    });
  }catch{}
};
...
async function modelNetworkFirst(request){
  const cache=await caches.open(RUNTIME_CACHE);
  const getRequest=request.method==='HEAD'?new Request(request.url,{method:'GET'}):request;
  try{
    const response=await fetch(request,{cache:'reload'});
    if(response?.ok&&request.method==='GET')await cache.put(getRequest,response.clone());
    return response;
  }catch{
    const cached=await cache.match(getRequest)||await caches.match(getRequest);
...
async function binaryStreamFirst(request){
  try{return await fetch(request,{cache:'reload'})}
    ...
    const getRequest=new Request(request.url,{method:'GET'}),cached=await caches.match(getRequest);
    if(cached&&request.method==='HEAD')return new Response(null,{status:200,headers:cached.headers});
...
self.addEventListener('fetch',event=>{
  ...
  if(request.mode==='navigate'){
    ...
    event.respondWith(networkFirst(request,fallback).catch(()=>caches.match(fallback).then(response=>response||caches.match('/offline.html'))));
    return;
  }
  ...
  if(url.pathname.startsWith(ONNX_BACKEND_PREFIX)||url.pathname.startsWith(MODEL_GRAPH_PREFIX)){
    event.respondWith(binaryStreamFirst(request));
    return;
  }
  if(url.pathname.startsWith(MODEL_PREFIX)){
    ...
    event.respondWith(mutable?modelNetworkFirst(request):cacheFirst(request));
    return;
  }
