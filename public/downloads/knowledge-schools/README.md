...
python batch_unpack_schools.py \
  --base-url https://civweave-host-node.onrender.com/downloads/knowledge-schools/ \
...
The Civweave installer shows every school selected by default, but it downloads nothing without an explicit tap. Each selected ZIP is verified against `catalog.json` with SHA-256 and stored in `civweave-knowledge-schools-v1`, outside the core PWA cache. The compressed seeds can be unpacked by a later Living School runtime without forcing every installation to carry the full library.
