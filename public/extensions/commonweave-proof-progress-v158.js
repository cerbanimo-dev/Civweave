(()=>{
'use strict';
if(globalThis.CommonweaveProofProgressV158)return;
const VERSION='1.0.4-proof-progress-v158';
const KEYS={
  campus:'commonweave.working-campus.v1',
  intentions:'commonweave.intentions.v127',
  inbox:'commonweave.realm-inbox.v1',
...
function cerbanimoCompletion(plan,path){
  ...
  if(!quest)return{complete:false,source:'cerbanimo',reason:'No quest tied to this weave has submitted proof.'};
...
function fellowfareCompletion(plan,path){
  ...
  return{
    ...
    reason:complete.length?'The material or service request has a completed exchange record.':'No fulfilled or settled exchange proof is attached to this weave.'
  };
}
...
function updateInbox(plan){
  ...
  for(const row of rows){
    ...
    const path=list(plan.paths).find(item=>item.realm===row.target);
    if(!path)continue;
...
function syncProgress(){
  ...
  try{
    ...
    for(const {path,result} of snapshot.paths){
      const nextProgress=result.complete?list(path.steps).map((_,index)=>index):[];
      const nextStatus=result.complete?'completed':(plan.state==='review'?'ready':'active');
    ...
    if(changed){
      ...
      dispatchEvent(new CustomEvent('commonweave:proof-progress-synced',{detail:{planId:plan.id,state:plan.state,paths:paths.map(path=>({id:path.id,realm:path.realm,status:path.status,progress:path.progress})),at:plan.updatedAt}}));
    }
...
function boot(){
  ...
  for(const name of ['commonweave:rewards-changed','commonweave:reward-bridge','commonweave:peer-review-recorded','commonweave:domain-synced'])addEventListener(name,syncProgress);
}
if(document.readyState==='loading')addEventListener('DOMContentLoaded',boot,{once:true});else boot();
globalThis.CommonweaveProofProgressV158=Object.freeze({VERSION,KEYS,proofState,syncProgress,lockCheckpointInputs,completionFor});
})();
