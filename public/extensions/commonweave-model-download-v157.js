(()=>{
'use strict';
const VERSION='1.0.7-model-download-v180';
if(globalThis.CommonweaveModelDownloadV157?.version===VERSION)return;
const ADAPTER='/app/models/all-minilm-l6-v2/adapter.js?v=device-package-r40-freeze-boundary';
let running=null;
...
async function download(button){
  ...
  running=(async()=>{
    try{
      ...
      if(before.available){write(button,'MiniLM is already cached for explicit semantic-lab use.');return before}
      ...
      write(button,'MiniLM is cached. It remains dormant until a semantic-lab test explicitly starts it.');
      globalThis.dispatchEvent(new CustomEvent('commonweave:model-package-ready',{detail:{version:VERSION,status:result}}));return result;
...
addEventListener('commonweave:open-semantic-lab',event=>enhance(event.detail?.root||document));
globalThis.CommonweaveModelDownloadV157=Object.freeze({version:VERSION,download,enhance,status:async()=>module().then(adapter=>adapter.status()),settingsHooks:false,automaticStartup:false});
})();
