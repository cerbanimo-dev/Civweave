...
function normalizeRequest(plan,realm,kind){const path=pathFor(plan,realm);if(!path)return null;const profile=profileFromCampus();return{
  schema:`commonweave.${kind}-request.v156`,
...
function canonicalSnapshot(){const plan=planFromCampus();if(!plan)return{schema:'commonweave.domain.v156',version:VERSION,activeIntentionId:null,intentions:[],requests:{learning:[],tasks:[],materials:[]},updatedAt:now()};const learning=normalizeRequest(plan,'living-school','learning'),tasks=normalizeRequest(plan,'cerbanimo','task'),materials=normalizeRequest(plan,'fellowfare','materials');return{
  schema:'commonweave.domain.v156',version:VERSION,activeIntentionId:plan.id,
  intentions:[{id:plan.id,title:clean(plan.title,240),wish:clean(plan.wish,4000),outcome:clean(plan.outcome,4000),state:plan.state||'review',profile:profileFromCampus(),governance:plan.governance||null,updatedAt:plan.updatedAt||now()}],
  requests:{learning:learning?[learning]:[],tasks:tasks?[tasks]:[],materials:materials?[materials]:[]},
  passport:{schema:'commonweave.passport.v156',intentionId:plan.id,consent:plan.governance?.consent||'review required',constraints:profileFromCampus().constraints,paths:list(plan.paths).map(path=>({realm:path.realm,title:path.title,state:path.state||'ready'}))},
  updatedAt:now()
}}
...
function rewardLedger(){const value=read(KEYS.rewards,{schema:'commonweave.reward-ledger.v156',events:[]});value.schema='commonweave.reward-ledger.v156';value.events=list(value.events);return value}
function appendReward(input){const ledger=rewardLedger(),eventRow={schema:'commonweave.reward-event.v156',id:input.id||rewardId(input.system,input.sourceId,input.currency,input.skill),system:input.system,sourceId:input.sourceId,currency:input.currency,amount:Number(input.amount)||0,skill:clean(input.skill,180)||null,validator:input.validator||null,createdAt:input.createdAt||now()};if(eventRow.amount<=0)return null;if(!ledger.events.some(item=>item.id===eventRow.id)){ledger.events.push(eventRow);ledger.updatedAt=now();write(KEYS.rewards,ledger);event('commonweave:rewards-changed',{event:eventRow,balances:balances(ledger.events)})}return eventRow}
...
let patched=false;function patchStorage(){if(patched)return;patched=true;const original=Storage.prototype.setItem;Storage.prototype.setItem=function(key,value){const result=original.call(this,key,value);if(this===localStorage&&Object.values(KEYS).includes(String(key))){queueMicrotask(()=>{syncCampus();syncRewards()})}return result}}
...
boot();
globalThis.CommonweaveDomainBridgeV156=Object.freeze({VERSION,KEYS,syncCampus,syncRewards,canonicalSnapshot,rewardLedger,appendReward,balances,profileFromCampus});
})();
