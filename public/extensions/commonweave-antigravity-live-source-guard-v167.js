(()=>{
'use strict';
if(globalThis.__commonweaveAntigravityLiveSourceGuardV167)return;
globalThis.__commonweaveAntigravityLiveSourceGuardV167=true;
...
function liveKind(body){
  ...
  if(/commonweave live-search verification test|verify the live-search route/.test(text))return'live-test';
...
function enforcePayload(payload,kind){
  ...
  next.commonweave_live_verification={...evidence,kind,verifiedAt:new Date().toISOString()};
...
async function runLiveSearchTest(){
  const runtime=globalThis.CommonweaveModelRuntime,config=runtime?.readSharedConfig?.('agentic');const model=clean(config?.model||config?.modelId,200);
...
function boot(){if(nativeFetch&&globalThis.fetch!==guardedFetch)globalThis.fetch=guardedFetch;document.addEventListener('click',handleLiveTest,true);const root=document.documentElement;new MutationObserver(patchSettings).observe(root,{childList:true,subtree:true});addEventListener('storage',event=>{if(event.key===KEY)patchSettings()});patchSettings();globalThis.CommonweaveAntigravityLiveSourceGuardV167={version:VERSION,evidenceFromPayload,verifiedUrl,enforcePayload,runLiveSearchTest,latest}}
