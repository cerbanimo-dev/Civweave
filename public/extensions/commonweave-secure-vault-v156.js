...
function currentSnapshot(){
  return{
    schema:'commonweave.ai-vault.snapshot.v156',
...
function restore(snapshot){
  if(snapshot?.schema!=='commonweave.ai-vault.snapshot.v156')throw new Error('Unsupported Commonweave AI vault snapshot.');
...
async function remember(passphrase){
  ...
  localStorage.setItem(VAULT_KEY,JSON.stringify({schema:'commonweave.encrypted-ai-vault.v156',kdf:'PBKDF2-SHA256',iterations:250000,cipher:'AES-GCM-256',salt:b64(salt),iv:b64(iv),payload:b64(cipher),savedAt:now()}));
  dispatch('commonweave:vault-remembered',{status:publicStatus()});
  return publicStatus();
}
