(()=>{
'use strict';
const VERSION='1.0.0-generation-lifecycle-v1-cross-frame-runtime-stop';
...
function install(){
  if(!installed){
    ...
    for(const type of ['civweave:local-model-runtime-ready','civweave:gemma4-litert-fast-extension-ready','civweave:litert-structured-tool-adapter-ready','civweave:gemma4-litert-first-request-ready','civweave:guide-loader-reset','pageshow'])addEventListener(type,()=>queueMicrotask(()=>{bindFrames();for(const win of windows())patchRuntime(win)}));
    const observer=new MutationObserver(()=>{bindFrames();for(const win of windows())patchRuntime(win)});observer.observe(document.documentElement,{childList:true,subtree:true});
    for(const delay of [0,100,500,1500,4000,9000])setTimeout(()=>{bindFrames();for(const win of windows())patchRuntime(win)},delay);
  }else{bindFrames();for(const win of windows())patchRuntime(win)}
  return true;
}
globalThis.CivweaveGenerationLifecycleV1=Object.freeze({version:VERSION,install,record,history:()=>history.map(plain),latest,artifactName,inferArtifacts,normalizeArtifacts,mergeArtifacts,artifactKey,stageId,patchRuntime,lastStopAt:()=>lastStopAt});
install();
})();