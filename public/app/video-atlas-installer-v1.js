...
async function stage(){
  ...
  try{
    ...
    for(const extra of ['lookup.json','youtube-availability-current.json','youtube-metadata-current.json.gz']){try{const response=await fetch(BASE+extra,{cache:'no-store'});if(response.ok)await cache.put(new URL(BASE+extra,location.origin).href,response.clone())}catch{}}
