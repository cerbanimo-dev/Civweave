...
if(globalThis.CivweaveMobileRegressionV170?.version===VERSION)return;
let lastLivingError='';
const sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));
async function openSettings(){const controller=globalThis.CivweaveModelSettingsControllerV173;if(!controller?.open)throw new Error('The unified Civweave AI settings controller is unavailable.');return controller.open()}
function patchLoader(){const api=globalThis.CivweaveFamilyAILoaderV105;if(!api||api.__cw175Settings)return false;api.openSettings=openSettings;Object.defineProperty(api,'__cw175Settings',{value:true});return true}
function suppressExtraLaunchers(){document.querySelectorAll('#gc153-launcher,.gc153-launcher,[data-civweave-bottom-launcher],[data-weaveling-launcher]').forEach(node=>{node.hidden=true;node.setAttribute('aria-hidden','true');node.tabIndex=-1})}
...
function resetLiving(){const key='civweave.living-school.cabinet.v151',value=localStorage.getItem(key);if(value)localStorage.setItem(`${key}.backup.${Date.now()}`,value);localStorage.removeItem(key);location.reload()}
...
globalThis.CivweaveMobileRegressionV170={version:VERSION,openSettings,verifyLivingBoot,defaultProvider:'deterministic',transformerActive:false};
})();
