(()=>{
'use strict';
const VERSION='1.0.12-model-settings-controller-v173-gemma4-render-safe';
...
function ensureGemma4Pack(){
  if(globalThis.CivweaveGemma4PackExtensionV1?.version===GEMMA4_PACK_VERSION)return true;
  const path=new URL(GEMMA4_PACK_SRC,location.href).pathname;
  const resident=[...document.scripts].find(script=>{try{return new URL(script.src,location.href).pathname===path}catch{return false}});
  if(resident)return false;
  const script=document.createElement('script');
  script.src=GEMMA4_PACK_SRC;
  script.async=false;
  script.dataset.civweaveGemma4Pack='q4-core-q2-optional-render-safe';
