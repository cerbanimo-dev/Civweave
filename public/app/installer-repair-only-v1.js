...
const REVISION='installer-repair-only-v1-install-only-pwa-boundary';
...
function releaseVersion(){
  ...
  return visible.match(/\d+\.\d+\.\d+/)?.[0]||'1.0.141';
}
function failed(){
...
function installedEntryUrl(){
  ...
  url.searchParams.set('source','installer-repair-only-v1');
  return url.href;
}
function removeBrowserLaunchers(){
  document.querySelector('#open-online-campus-v225')?.remove();
  document.querySelectorAll('[data-civweave-browser-fallback],a[href*="launch=online"]').forEach(node=>node.remove());
}
...
async function requestRepair(){
  ...
  return new Promise((resolve,reject)=>{
    ...
    const timer=setTimeout(()=>{try{channel.port1.close()}catch{}reject(new Error('Shell repair stopped responding.'))},90000);
    channel.port1.onmessage=event=>{clearTimeout(timer);try{channel.port1.close()}catch{}resolve(event.data||null)};
    try{worker.postMessage({type:'REPAIR_DEVICE_PACKAGE'},[channel.port2])}catch(error){clearTimeout(timer);reject(error)}
  });
}
async function repairInstalledShell(){
  ...
  try{
    ...
    if(installedDisplay()){
      ...
      queueMicrotask(()=>globalThis.CivweavePWAInstallV247?.refresh?.());
    }
  }catch(error){
...
function apply(){
  removeBrowserLaunchers();
  if(repairing)return;
  if(!failed())return;
  if(installButton){
    installButton.disabled=false;
    installButton.textContent='Repair shell';
    installButton.dataset.civweaveRepairOnly=REVISION;
  }
...
globalThis.CivweaveInstallerRepairOnlyV1=Object.freeze({
  revision:REVISION,
  installedDisplay,
  installedEntryUrl,
  repairInstalledShell,
