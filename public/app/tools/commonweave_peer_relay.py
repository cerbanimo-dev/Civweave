#!/usr/bin/env python3
"""Small opaque store-and-forward relay for Commonweave encrypted sync packets."""
...
def packet(value):
    if not isinstance(value,dict) or value.get('protocol')!='commonweave.relay-cipher.v1': raise ValueError('unsupported packet')
...
class Handler(BaseHTTPRequestHandler):
    store: Store
    server_version='CommonweavePeerRelay/1.0'
    ...
    def do_GET(self):
        ...
        if query.get('health')==['1']: return self.send_json(200,{'ok':True,'protocol':'commonweave.encrypted-relay.v1','opaqueStorage':True,'relay':'lan-peer'})
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument('--host',default='0.0.0.0'); parser.add_argument('--port',type=int,default=8799); parser.add_argument('--db',default='commonweave-relay.sqlite3'); args=parser.parse_args()
    Handler.store=Store(Path(args.db)); server=ThreadingHTTPServer((args.host,args.port),Handler)
    print(f'Commonweave opaque peer relay listening on http://{args.host}:{args.port}/api/relay')
