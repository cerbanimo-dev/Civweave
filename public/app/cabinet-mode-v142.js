...
function consoleUrl(system,room){const query=new URLSearchParams({embed:'1'});if(system?.id)query.set('system',system.id);if(room?.id)query.set('room',room.id);if(params.get('capability'))query.set('capability',params.get('capability'));if(system?.id==='living-school')return `/app/living-school-cabinet-v150.html?${query}`;return system?.id==='anarchadia'?`/app/anarchadia-console-v139.html?${query}`:`/app/realm-console-v140.html?${query}`}
