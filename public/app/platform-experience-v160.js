...
let patching=false;
let refreshTimer=0;
function rewardEvents(){const value=parse(localStorage.getItem('commonweave.rewards.v156'),{});return Array.isArray(value)?value:Array.isArray(value?.events)?value.events:[]}
...
function syncFrame(frame){
  try{
    ...
    if(doc.documentElement.dataset.commonweaveTheme!==selected){doc.documentElement.dataset.commonweaveTheme=selected;diagnostics.domWrites+=1}
    if(doc.documentElement.dataset.commonweaveResolvedTheme!==mode){doc.documentElement.dataset.commonweaveResolvedTheme=mode;diagnostics.domWrites+=1}
...
function applyTheme(value=theme(),persist=false){
  ...
  if(root.dataset.commonweaveTheme!==selected){root.dataset.commonweaveTheme=selected;diagnostics.domWrites+=1}
  if(root.dataset.commonweaveResolvedTheme!==mode){root.dataset.commonweaveResolvedTheme=mode;diagnostics.domWrites+=1}
  ...
  refreshControls();
  syncFrames();
  try{dispatchEvent(new CustomEvent('commonweave:appearance-changed',{detail:{theme:selected,resolved:mode}}))}catch{}
  return selected;
}
function cycleTheme(){const current=theme(),next=THEMES[(THEMES.indexOf(current)+1)%THEMES.length];return applyTheme(next,true)}
function explicitActionSystem(text,contextSystem='commonweave'){
  ...
  return contextSystem!=='commonweave'&&/\b(request|draft|plan|create|make|start)\b/.test(value)?contextSystem:'';
}
...
function patchAssistant(api){
  ...
  api.respond=async options=>{
    const request={...(options||{})},startedIn=request.systemId||'commonweave';
    let target='';
    if(startedIn==='commonweave'){
      const contextSystem=request.contextSystem||document.documentElement.dataset.commonweaveSystem||'commonweave';
...
function writeActions(rows){
  ...
  try{dispatchEvent(new CustomEvent('commonweave:actions-changed',{detail:{items:rows}}))}catch{}
}
...
function holdFunding(action){
  ...
  const plan={schema:'commonweave.fellowfare.funding-plan.v160',id:index>=0?plans[index].id:`funding-${Date.now().toString(36)}`,actionId:action.id,title:action.title,requiredButtons:required,availableButtons:available,gap,state:'held',visibility:'private',createdAt:index>=0?plans[index].createdAt:now(),updatedAt:now()};
...
function pending(){
  ...
  return[
    ...
    ...intentions.map(item=>({type:'intention',id:item.id,title:item.title||item.plan?.title||'Review weave',at:item.updatedAt||item.createdAt,state:'review',system:'commonweave'}))
...
async function openLatest(){
  ...
  if(item.type==='action')globalThis.CommonweaveActionUI?.open?.(item.id);else globalThis.CommonweaveIntentionUI?.open?.(item.id);
}
...
function boot(){
  ...
  addEventListener('storage',event=>{
    if(![THEME_KEY,ACTION_KEY,INTENTION_KEY,'commonweave.rewards.v156'].includes(event.key))return;
    if(event.key===THEME_KEY)applyTheme();else refreshControls();
  });
  addEventListener('commonweave:actions-changed',()=>refreshControls());
  addEventListener('commonweave:intentions-changed',()=>refreshControls());
  addEventListener('commonweave:rewards-changed',()=>refreshControls());
...
globalThis.CommonweavePlatformExperienceV160={version:VERSION,theme,applyTheme,cycleTheme,pending,openLatest,explicitActionSystem,buttonBalance:balance,refreshControls,diagnostics};
})();
