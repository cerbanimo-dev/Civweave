...
function stageHandoff({target,kind='commonweave.live-handoff.v1',title,payload={}}){const key='commonweave.pending.live-handoffs.v1',list=safeJson(key,[]),item={id:crypto.randomUUID?.()||String(Date.now()),target,kind,title:text(title||'Commonweave handoff'),payload,createdAt:new Date().toISOString(),status:'review-required'};list.unshift(item);localStorage.setItem(key,JSON.stringify(list.slice(0,100)));BUS.dispatchEvent(new CustomEvent('live-data:handoff',{detail:item}));return item}
window.addEventListener('storage',e=>{if(e.key&&/cerbanimo|living|fellowfare|commonweave/.test(e.key))refresh()});
window.CommonweaveLiveData={VERSION,bus:BUS,refresh,snapshot,search,scenePulse,stageHandoff,get records(){return CACHE.records},get signals(){return CACHE.signals},get counts(){return CACHE.counts}};
