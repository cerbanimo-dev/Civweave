(()=>{
'use strict';
const VERSION='1.0.0-knowledge-encyclopedia-bridge-v271';
...
function patchGenerate(runtime,encyclopedia){
  ...
  const wrapped=async function(options={}){
    ...
    return{
      status:'success',
      outputJson:{answer:fallbackAnswer(knowledge),choice:{mode:mode(system),system,room:clean(options?.context?.currentContext?.roomId,240),nextAction:''},assumptions:[],requiresConsent:false,confidence:knowledge.sources?.length?.82:.45,questDraft:null},
      outputText:'',
