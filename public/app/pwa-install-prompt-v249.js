...
const VERSION='pwa-install-prompt-v249-origin-source-truth';
...
function pagesUnderlay(){return location.origin===PAGES_UNDERLAY_ORIGIN}
function contributorOrigin(){return location.hostname.endsWith('.civweave.cc')}
function previewParentOrigin(){
  if(!cloudflarePreview())return null;
  const parent=`https://${location.hostname.split('.').slice(1).join('.')}`;
  return parent===PAGES_UNDERLAY_ORIGIN?CANONICAL_ORIGIN:parent;
}
...
function installOrigin(){return localDevelopment()||location.origin===CANONICAL_ORIGIN||(productionPagesOrigin()&&!pagesUnderlay())||contributorOrigin()}
...
function rerouteUnsafeInstall(){
  if(standalone()||localDevelopment()||installOrigin())return false;
...
async function discoverRelatedInstalls(){
  ...
  const pagesUnderlayInstalled=urls.has(PAGES_UNDERLAY_MANIFEST);
  document.documentElement.dataset.civweaveCanonicalInstall=canonicalInstalled?'installed':'unknown';
  document.documentElement.dataset.civweavePagesUnderlayInstall=pagesUnderlayInstalled?'installed':'absent';
  publish('civweave:related-install-state',{canonicalInstalled,pagesUnderlayInstalled,relatedApps:[...relatedApps]});
  return [...relatedApps];
}
...
const api=Object.freeze({
  ...
  pagesUnderlayOrigin:PAGES_UNDERLAY_ORIGIN,
  previousCanonicalOrigin:PAGES_UNDERLAY_ORIGIN,
  installOrigin,
