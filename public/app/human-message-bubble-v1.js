...
const VERSION='1.0.0-human-message-bubble-v1';
...
@media(max-width:620px){#${BUBBLE_ID}{right:calc(max(10px,env(safe-area-inset-right)) + 56px)!important;bottom:calc(var(--cw-themed-nav-height,58px) + env(safe-area-inset-bottom) + 10px)!important;width:48px!important;height:48px!important;min-width:48px!important;min-height:48px!important}}
...
function openParty(item){const ai=document.getElementById(AI_LAUNCHER_ID);if(ai&&document.getElementById(ROOT_ID)?.hidden!==false)ai.click();const select=()=>{const tab=document.querySelector(`[data-party-tab="${cssEscape(item.threadId)}"]`);if(tab){tab.click();markRead(item);return true}return false};if(!select())setTimeout(()=>{if(!select()){try{dispatchEvent(new CustomEvent('civweave:open-human-thread',{detail:{source:'party',threadId:item.threadId,messageId:item.messageId}}))}catch{}markRead(item)}},80)}
function openPm(item){try{dispatchEvent(new CustomEvent('civweave:private-message-open-request',{detail:{username:item.threadId,messageId:item.messageId}}));dispatchEvent(new CustomEvent('civweave:open-human-thread',{detail:{source:'pm',threadId:item.threadId,messageId:item.messageId}}))}catch{}markRead(item)}
async function openLatest(){const current=lastSnapshot||await snapshot(),item=current.latest;if(!item){try{dispatchEvent(new CustomEvent('civweave:open-human-thread',{detail:{source:'inbox'}}))}catch{}return false}if(item.source==='party')openParty(item);else openPm(item);return true}

function start(){ensureBubble();['civweave:party-thread-changed','civweave:private-message','civweave:private-messaging-ready','civweave:shared-intention-party-ready','pageshow','online'].forEach(name=>addEventListener(name,queueRefresh));addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')queueRefresh()});document.addEventListener('click',event=>{const tab=event.target instanceof Element?event.target.closest('[data-party-tab]'):null;if(tab&&tab.getAttribute('data-party-tab')!=='private')setTimeout(()=>markPartyVisibleRead(tab.getAttribute('data-party-tab')),0)},true);const observer=new MutationObserver(()=>{const button=ensureBubble();alignWithAi(button)});observer.observe(document.documentElement,{childList:true,subtree:true});refreshTimer=setInterval(queueRefresh,5000);addEventListener('pagehide',()=>{clearInterval(refreshTimer);observer.disconnect()},{once:true});void refresh();globalThis.CivweaveHumanMessageBubbleV1=Object.freeze({version:VERSION,refresh,snapshot,openLatest,markRead,attentionKey:ATTENTION_KEY,bubbleId:BUBBLE_ID,aggregates:['party','pm'],readState:'local-only',pmAvatarFallback:'initial-until-encrypted-avatar-snapshot'});try{dispatchEvent(new CustomEvent('civweave:human-message-bubble-ready',{detail:{version:VERSION}}))}catch{}}
