...
const VERSION='1.1.1-human-message-chat-icon';
...
const CHAT_ICON='/app/assets/chat-symbol-v1.png';
...
function chatFaceMarkup(){return`<img src="${CHAT_ICON}" alt="">`}
function ensureBubble(){installStyle();let button=document.getElementById(BUBBLE_ID);if(button)return button;button=document.createElement('button');button.id=BUBBLE_ID;button.type='button';button.setAttribute('aria-label','Human messages');button.innerHTML=`<span class="cw-human-face" aria-hidden="true">${chatFaceMarkup()}</span><span id="${BADGE_ID}" hidden>0</span>`;button.addEventListener('click',()=>void openLatest());document.body?.append(button);return button}
