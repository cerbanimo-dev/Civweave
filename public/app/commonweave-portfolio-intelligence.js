...
const PREF_KEY='commonweave.portfolio.preferences.v1';
const CLAIM_KEY='commonweave.portfolio.resource-claims.v1';
const CONSENT_KEY='commonweave.portfolio.consent-requests.v1';
const BRIEF_KEY='commonweave.portfolio.briefs.v1';
const HISTORY_KEY='commonweave.portfolio.history.v1';
...
const SS=()=>window.CommonweaveStrategySteward;
const AS=()=>window.CommonweaveAutonomousSteward;
const LD=()=>window.CommonweaveLiveData;
...
function saveClaim(item){const all=claims().filter(x=>x.id!==item.id);all.unshift(item);save(CLAIM_KEY,all.slice(0,400));dispatchEvent(new CustomEvent('commonweave:portfolio-claim',{detail:clone(item)}));return clone(item)}
...
function requestConsent({intentionId,stepId='',person,role='',message='',expiresHours=72}){
 ...
 AQ()?.stage?.({target:person?.system||'commonweave',action:'request-collaborator-consent',title:`Ask ${person?.title||person?.name||'a collaborator'} to participate`,payload:item});
 dispatchEvent(new CustomEvent('commonweave:consent-requested',{detail:clone(item)}));return clone(item);
}
...
function prepareRecommendation(rec){
 ...
 return AQ()?.stage?.({target:'commonweave',action:'coordinate-portfolio-dependency',title:rec.title,payload:{...rec,reviewRequired:true}});
}
...
window.CommonweavePortfolioIntelligence={VERSION,preferences,setPreferences,scoreCandidate,rankedGapCandidates,claims,reserveCandidate,releaseClaim,consentRequests,requestConsent,respondConsent,portfolioSnapshot,recommendations,prepareRecommendation,negotiatePriority,briefs,generateBrief,recordHistory,tick};
