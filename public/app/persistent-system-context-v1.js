...
const VERSION='1.0.2-persistent-system-context-v1';
...
function finishGesture(event){if(!gesture||event.pointerId!==gesture.pointerId)return;if(!gesture.moved&&Date.now()-gesture.start>=HOLD_MS){suppressClickSystem=gesture.system;clearTimeout(suppressTimer);suppressTimer=setTimeout(()=>{suppressClickSystem=''},800)}gesture=null}
function interceptNavClick(event){
  const link=event.target?.closest?.(`#${NAV_ID} a[data-system]`);if(!link)return;const system=clean(link.dataset.system);if(!valid(system))return;
  if(suppressClickSystem===system){suppressClickSystem='';event.preventDefault();event.stopImmediatePropagation();return}
...
function bindNav(){
  const nav=document.getElementById(NAV_ID);if(!nav||nav===boundNav)return Boolean(nav);boundNav=nav;
  nav.addEventListener('pointerdown',event=>{const link=event.target?.closest?.('a[data-system]');if(link)markGesture(event,link)},true);nav.addEventListener('pointermove',moveGesture,true);nav.addEventListener('pointerup',finishGesture,true);nav.addEventListener('pointercancel',()=>{gesture=null},true);nav.addEventListener('click',interceptNavClick,true);syncSelection();return true;
}
function bindChatControls(){
  const launcher=document.getElementById(CHAT_LAUNCHER_ID);
  if(launcher&&!boundControls.has(launcher)){boundControls.add(launcher);launcher.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();const system=selected();remember(system);chat()?.open?.({guide:system,focus:true});syncSelection(system)},true)}
...
function patchFamilyAiLoader(){
  ...
  loader.openChat=(system,options={})=>{const requested=clean(options?.contextSystem),host=hostSystem(),sticky=selected(),target=valid(requested)&&requested!==host?requested:valid(sticky)?sticky:valid(requested)?requested:valid(system)?system:host;remember(target);syncSelection(target);return original(system,{...(options||{}),contextSystem:target})};
  loader.persistentSystemContext='v1';patchedLoader=loader;return true;
}
function patchFamilyShell(){const shell=globalThis.CivweaveFamilyShellV104;if(!shell||shell===patchedShell)return Boolean(shell);try{shell.route=system=>switchContext(system,{source:'family-shell-route',open:null,focus:false});shell.persistentSystemContext='v1';patchedShell=shell;return true}catch{return false}}
function maintain(){bindNav();bindChatControls();patchFamilyAiLoader();patchFamilyShell();syncSelection()}
function install(){
  if(!pending()&&requestedContext())remember(requestedContext());maintain();
  ...
  addEventListener('civweave:system-route-changed',()=>queueMicrotask(()=>syncSelection(selected())));addEventListener('pageshow',maintain);addEventListener('focus',maintain);
  document.documentElement.dataset.civweaveSystemContextOwner='persistent-system-context-v1';return true;
}

const api=Object.freeze({version:VERSION,owner:true,systems:SYSTEMS,hostSystem,requestedContext,selected,switchContext,applyPending,syncSelection,bindNav,bindChatControls,patchFamilyAiLoader,patchFamilyShell,install,navigationReload:false,stickyUntilExplicitSwitch:true});
