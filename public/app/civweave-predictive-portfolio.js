...
const SCENARIO_KEY='civweave.portfolio.scenarios.v1';
const COMMITMENT_KEY='civweave.portfolio.commitments.v1';
const CAPACITY_KEY='civweave.portfolio.capacity.v1';
const BUDGET_KEY='civweave.portfolio.budgets.v1';
const AGENDA_KEY='civweave.portfolio.agendas.v1';
...
function setBudgets(patch){const next={...budgets(),...clone(patch||{}),updatedAt:now()};save(BUDGET_KEY,next);dispatchEvent(new CustomEvent('civweave:portfolio-budget',{detail:clone(next)}));return clone(next)}
function setCapacity(kind,id,entry){const all=capacities();if(!all[kind])all[kind]={};all[kind][id]={...(all[kind][id]||{}),...clone(entry||{}),id,kind,updatedAt:now()};all.updatedAt=now();save(CAPACITY_KEY,all);dispatchEvent(new CustomEvent('civweave:portfolio-capacity',{detail:clone(all[kind][id])}));return clone(all[kind][id])}
function createCommitment({intentionId,stepId='',kind='person',subjectId='',subjectTitle='',quantity=1,startAt=null,endAt=null,status='tentative',source='civweave',terms={}}){
 ...
 const all=commitments();all.unshift(item);save(COMMITMENT_KEY,all.slice(0,800));dispatchEvent(new CustomEvent('civweave:commitment',{detail:clone(item)}));return clone(item);
}
function updateCommitment(id,status,patch={}){const all=commitments(),item=all.find(x=>x.id===id);if(!item)return null;item.status=status;Object.assign(item,clone(patch));item.updatedAt=now();item.history=[...(item.history||[]),{status,at:item.updatedAt,note:text(patch.note),source:patch.source||'civweave'}];save(COMMITMENT_KEY,all);dispatchEvent(new CustomEvent('civweave:commitment',{detail:clone(item)}));return clone(item)}
...
function simulate(change={}){
 ...
 const all=scenarios();all.unshift(result);save(SCENARIO_KEY,all.slice(0,100));dispatchEvent(new CustomEvent('civweave:portfolio-scenario',{detail:clone(result)}));return clone(result);
}
function prepareScenario(scenario){if(!scenario)return null;return AQ()?.stage?.({target:'civweave',action:'apply-portfolio-scenario',title:`Apply portfolio scenario: ${scenario.change.type}`,payload:{scenarioId:scenario.id,change:scenario.change,summary:scenario.summary,projected:scenario.projected,reviewRequired:true}})}
function forecast(){
...
window.CivweavePredictivePortfolio={VERSION,scenarios,commitments,capacities,budgets,setBudgets,setCapacity,createCommitment,updateCommitment,activeCommitments,simulate,prepareScenario,forecast,weeklyAgenda,agendas,negotiate};
