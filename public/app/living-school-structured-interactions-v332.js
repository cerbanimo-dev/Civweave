(()=>{
'use strict';
const VERSION='1.0.0-living-school-structured-interactions-v332';
const ID='living-school-structured-interactions-v332';
...
function base(request){return{schema:'civweave-model-result-1.0',requestId:request.requestId||`ls-v332-${Date.now().toString(36)}`,purpose:PURPOSE,requested:{provider:'gemini',model:MODEL,executionProfile:'interactive'},actual:{provider:'gemini',model:MODEL},diagnostics:[`Living School module construction used ${VERSION} via Gemini Interactions.`]}}
async function call(request,ctx,messages,schema){
  ...
  const result=await transport(runtime,{...request,executionProfile:'interactive',config,messages,schema});
...
async function generate(request,ctx){
  const schema=schemaFor(request),boundary='Return exactly one complete JSON module object matching the supplied schema. Do not output XP, Acorns, Buttons, prices, payouts, wages, bonuses, grants, labor values, ledger metadata, video URLs, or media links. Use only supplied SOURCE_ID values; unsupported material must have no fabricated citation.';
  const original=[...(Array.isArray(request.messages)?request.messages:[]),{role:'system',content:boundary}];
  ...
  for(let attempt=0;attempt<=max;attempt++){
    try{
      const response=await call(request,ctx,messages,schema);last=response.text;lastResult=response.result;
      ...
      if(attempt<max)messages=[...original,{role:'assistant',content:last},{role:'user',content:`Correct the preceding JSON and return only the corrected object. Fix:\n- ${errors.slice(0,12).join('\n- ')}`}];
...
function install(){
  ...
  try{dispatchEvent(new CustomEvent('civweave:living-school-structured-interactions-ready',{detail:{version:VERSION,middleware:ID,model:MODEL,at:new Date().toISOString()}}))}catch{}
  return true;
}
...
install();
globalThis.CivweaveLivingSchoolStructuredInteractionsV332=Object.freeze({version:VERSION,install,middlewareId:ID,model:MODEL});
})();