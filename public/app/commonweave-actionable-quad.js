...
const THREAD_KEY='commonweave.quad.thread.v1';
const REVIEW_KEY='commonweave.pending.live-handoffs.v1';
const DECISION_KEY='commonweave.handoff.decisions.v1';
const ROUTE_KEY='commonweave.active.route.v1';
const LIFECYCLE_KEY='commonweave.handoff.lifecycle.v1';
...
function obligations(){const live=window.CommonweaveLiveData?.snapshot?.()||{signals:[]};return (live.signals||[]).filter(x=>x.priority>=3).sort((a,b)=>b.priority-a.priority).slice(0,12)}
function search(query){return window.CommonweaveLivingWorld?.search?.(query)||window.CommonweaveLiveData?.search?.(query)||[]}
function findRecord(id){return window.CommonweaveLiveData?.records?.find(r=>r.id===id)||null}
function openResult(id){const r=findRecord(id);if(!r)return {ok:false,error:'Record not found'};thread.context.activeRecord=id;thread.context.lastScene=r.scene||thread.context.lastScene;persist();if(r.scene)window.CommonweaveWorld?.render?.(r.scene);else if(r.deepLink)location.href=r.deepLink;return {ok:true,record:clone(r)}}
function journeyFromRecord(recordId){const r=findRecord(recordId);if(!r)return null;const map={living:'learn-and-practice',fellowfare:'meet-a-need',cerbanimo:'build-something',anarchadia:'meet-a-need'};const template=map[r.system]||'build-something';const j=window.CommonweaveLivingWorld?.createJourney?.(template,`Continue: ${r.title}`);if(!j)return null;j.sourceRecord={id:r.id,system:r.system,type:r.type,title:r.title,deepLink:r.deepLink};const state=window.CommonweaveLivingWorld.state;state.journeys[j.id].sourceRecord=j.sourceRecord;localStorage.setItem('commonweave.living-world.v1',JSON.stringify(state));thread.context.activeJourney=j.id;persist();return clone(state.journeys[j.id])}
function stage({target,action,title,payload={},sourceRecord=null}){const item=window.CommonweaveLiveData?.stageHandoff?.({target,kind:`commonweave.${action}.v1`,title,payload:{...payload,sourceRecord,threadId:thread.id}});if(item){addMessage('system',`Prepared “${item.title}” for review. Nothing has been changed yet.`,{handoffId:item.id});bus.dispatchEvent(new CustomEvent('quad:staged',{detail:item}))}return item}
...
function decide(id,decision,note=''){const list=pending(),index=list.findIndex(x=>x.id===id);if(index<0)return null;const item=list[index];list.splice(index,1);save(REVIEW_KEY,list);const result={...item,decision,decisionNote:text(note),decidedAt:new Date().toISOString()};const history=decisions();history.unshift(result);save(DECISION_KEY,history.slice(0,250));if(decision==='approved'){
 const inboxKey=`commonweave.approved-handoffs.${item.target}.v1`, inbox=safe(inboxKey,[]);inbox.unshift({...item,status:'approved',approvedAt:result.decidedAt});save(inboxKey,inbox.slice(0,100));
}
...
async function plan(query){const obs=obligations(),results=search(query).slice(0,8),actions=suggestedActions(query,results);addMessage('user',query);let intention=window.CommonweaveAutonomousSteward?.preflight?.(query)||null;if(!intention&&window.CommonweaveModelSteward?.needsModel?.(query)&&window.CommonweaveIntentionOrchestrator?.active?.()){const guided=await window.CommonweaveModelSteward.replan(query);if(guided?.ok)intention={kind:'model-replan',summary:guided.session.summary,modelSession:guided.session};}if(!intention){const strategic=window.CommonweaveStrategySteward?.interpret?.(query)||null;intention=strategic||window.CommonweaveIntentionOrchestrator?.converse?.(query)||null;}const response={summary:intention?.summary||(results.length?`I found ${results.length} useful threads across Commonweave.`:'I did not find a matching existing thread, so I prepared routes for creating one safely.'),obligations:obs.slice(0,4),results,actions,intention};addMessage('assistant',response.summary,{response,intentionId:intention?.intention?.id||intention?.proposal?.intentionId||intention?.modelSession?.intentionId||null});return response}
...
window.CommonweaveActionableQuad={VERSION,bus,snapshot,thread,addMessage,plan,search,openResult,journeyFromRecord,stage,pending,decisions,decide,setRoute,activeRoute,clearRoute,obligations,lifecycle};
})();
