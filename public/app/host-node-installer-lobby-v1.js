...
const REVISION = 'host-node-installer-lobby-v1';
...
function installStyles() {
  ...
  style.textContent = `
    ...
    .cw-host-slot strong{display:block;margin-top:5px;font-size:clamp(1.9rem,6vw,3rem);line-height:1;color:#fff}
    ...
    .cw-host-node-actions button{min-height:44px;padding:9px 15px;border-radius:13px;border:1px solid #ffffff2a;font-weight:900;cursor:pointer}
    ...
    @media(max-width:640px){.cw-host-node-lobby{padding:18px;border-radius:20px}.cw-host-node-head{display:grid}.cw-host-node-live{justify-self:start}.cw-host-node-slots{grid-template-columns:1fr 1fr}.cw-host-node-actions button{flex:1 1 140px}}
  `;
...
function buildLobby() {
  ...
  section.innerHTML = `
    <div class="cw-host-node-head">
      <div><small class="cw-host-node-kicker">HOST NODE</small><h2 class="cw-host-node-title" id="cw-host-node-title">Checking this Host Node…</h2><p class="cw-host-node-meta" id="cw-host-node-meta"></p></div>
    ...
    <div class="cw-host-node-actions"><button class="cw-host-node-join" id="cw-host-node-join" type="button" disabled>Join this Host</button><button class="cw-host-node-refresh" id="cw-host-node-refresh" type="button">Refresh status</button></div>
    <p class="cw-host-node-help" id="cw-host-node-help" role="status">Joining sets this as your device's Host Node. It does not silently start a paid membership.</p>
  `;
...
function renderSelectedState() {
  ...
  button.textContent = selected ? 'This is your Host Node' : 'Join this Host';
}
...
function renderStatus(packet) {
  ...
  if (!packet?.ok) {
    ...
    if (help) help.textContent = 'Refresh to try again. The Join button stays locked until this host answers successfully.';
    return;
  }
  if (live) { live.dataset.state = 'online'; live.textContent = 'Online'; }
  if (title) title.textContent = packet.displayName || 'Civweave Host Node';
  if (meta) meta.textContent = [packet.nodeId, packet.hostOrigin].filter(Boolean).join(' · ');
  ...
  if (help && selectedOrigin() !== normalizedHost()) help.textContent = 'Joining sets this as your device’s Host Node. It does not silently start a paid membership.';
  renderSelectedState();
}
...
async function loadStatus(force = false) {
  ...
  try {
    ...
    const packet = await response.json().catch(() => ({ ok: false, error: `HTTP ${response.status}` }));
    renderStatus(response.ok ? packet : { ...packet, ok: false });
    return packet;
  } catch (error) {
