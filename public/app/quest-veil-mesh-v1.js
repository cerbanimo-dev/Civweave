...
const VERSION='1.0.0-quest-veil-mesh-v1';
...
const BATCH_SIZE=8;
const DEFAULT_BOUNTY=Object.freeze({buttons:1,acorns:1,basis:'accepted-complete-batch'});
...
async function queueStates(jobs,{bounty=DEFAULT_BOUNTY}={}){
  ...
  for(let offset=0;offset<cleanJobs.length;offset+=BATCH_SIZE){
    const chunk=cleanJobs.slice(offset,offset+BATCH_SIZE),batchId=`veil-batch:${crypto.randomUUID?.()||`${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`}`,payload={schema:BATCH_SCHEMA,batchId,requesterNodeId,jobs:chunk.map(job=>({submissionId:clean(job.submissionId,240),sourceHash:clean(job.sourceHash,80),stateHash:clean(job.stateHash,80),veilState:clone(job.veilState)})),bounty:{buttons:Math.max(0,Math.min(5,Number(bounty.buttons||0))),acorns:Math.max(0,Math.min(5,Number(bounty.acorns||0))),basis:'accepted-complete-batch'},privacy:{rawSourceIncluded:false,sourceTitlesIncluded:false,evidenceContentIncluded:false},createdAt:now()};
    await mesh.createObject({id:`quest-veil-batch:${batchId}`,kind:BATCH_KIND,purpose:'Offer a batch of context-stripped Quest Veil writing work to the federated mesh.',consent:'federated',audience:[],payload,hopLimit:6,publish:true});queued+=chunk.length;
  }
  if(queued)dispatchEvent(new CustomEvent('civweave:quest-veil-mesh-queued',{detail:{queued,at:now()}}));return{queued};
}
...
async function acceptResultObject(object){
  ...
  const payload={schema:ACCEPT_SCHEMA,batchId:result.batchId,requesterNodeId:localNodeId,workerNodeId:result.workerNodeId,resultObjectId:object.id,sourceHashes:accepted,bounty:clone(batch.payload.bounty||DEFAULT_BOUNTY),acceptedAt:now()};
  const acceptance=await mesh.createObject({id:`quest-veil-accept:${result.batchId}`,kind:ACCEPT_KIND,purpose:'Acknowledge an accepted Quest Veil batch so the worker can claim its modest local reward.',consent:'federated',audience:[],payload,hopLimit:6,publish:true});dispatchEvent(new CustomEvent('civweave:quest-veil-mesh-result',{detail:{batchId:result.batchId,accepted:accepted.length,workerNodeId:result.workerNodeId,at:now()}}));return{status:'accepted',acceptance};
}
...
function appendLegacyReward({currency,amount,sourceId}){
  ...
  const event={id:`quest-veil-bounty:${currency}:${sourceId}`,system:'civweave',currency,amount,phase:'quest-veil-bounty',sourceId,reason:'Accepted community Quest Veil batch',createdAt:now()};const next=Array.isArray(raw)?[event,...events]:{...(raw&&typeof raw==='object'?raw:{}),events:[event,...events].slice(0,3000),updatedAt:now()};localStorage.setItem(REWARD_KEY,JSON.stringify(next));return true;
}
...
async function grantAcceptedBounty(object){
  ...
  const sourceId=`quest-veil-accept:${acceptance.batchId}`,buttons=Math.max(0,Math.min(5,Number(acceptance.bounty?.buttons||0))),acorns=Math.max(0,Math.min(5,Number(acceptance.bounty?.acorns||0)));let changed=false;
  changed=appendLegacyReward({currency:'button',amount:buttons,sourceId})||changed;changed=appendLegacyReward({currency:'acorn',amount:acorns,sourceId})||changed;
  const accountId=rewardAccountId();if(buttons>0&&accountId){try{const rewards=await ensureRewardWeave();if(rewards?.read&&rewards?.write&&rewards?.core?.recordExternalCoinReward){const state=rewards.read(),already=state?.fellowfare?.receipts?.some?.(receipt=>receipt?.sourceReceiptId===sourceId||receipt?.sourceId===sourceId);if(!already)rewards.write(rewards.core.recordExternalCoinReward(state,{accountId,amount:buttons,kind:'quest-veil-bounty',sourceReceiptId:sourceId,reason:'Accepted community Quest Veil batch'}))}}catch{}}
  if(changed)dispatchEvent(new CustomEvent('civweave:rewards-changed',{detail:{kind:'quest-veil-bounty',buttons,acorns,sourceId,at:now()}}));return{status:changed?'granted':'already-granted',buttons,acorns};
}
async function tick(){
...
globalThis.CivweaveQuestVeilMeshV1=Object.freeze({VERSION,BATCH_SCHEMA,RESULT_SCHEMA,ACCEPT_SCHEMA,BATCH_KIND,RESULT_KIND,ACCEPT_KIND,PROMPT_VERSION,BATCH_SIZE,DEFAULT_BOUNTY,localWriterAvailable,queueStates,processBatchObject,acceptResultObject,grantAcceptedBounty,tick});
dispatchEvent(new CustomEvent('civweave:quest-veil-mesh-ready',{detail:{version:VERSION,batchSize:BATCH_SIZE,bounty:DEFAULT_BOUNTY}}));
})();
