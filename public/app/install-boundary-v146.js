...
const PREVIOUS_ADDITIONS_VERSION='v161-settings-dialog-stability';
const FAST_CORE_COMPATIBILITY_REVISION='v157-fast-core';
const SETTINGS_STABILITY_REVISION='v161-settings-dialog-stability';
const INTENTION_RESEARCH_REVISION='v162-latest-intention-agentic-research';
...
function addScript(src){if(document.querySelector(`script[src^="${src}"]`))return;const script=document.createElement('script');script.src=`${src}?v=latest-intention-agentic-research-v162`;script.defer=true;document.head.append(script)}
function installAdditions(){
  if(!document.querySelector(`link[href^="${ADDITIONS_STYLE}"]`)){
    const link=document.createElement('link');link.rel='stylesheet';link.href=`${ADDITIONS_STYLE}?v=latest-intention-agentic-research-v162`;document.head.append(link);
  }
...
globalThis.CommonweaveInstallBoundaryV146={allowed,installedDisplay,developer,embedded,installerUrl,installAdditions,additionsVersion:'v162-latest-intention-agentic-research',previousAdditionsVersion:PREVIOUS_ADDITIONS_VERSION,fastCoreCompatibilityRevision:FAST_CORE_COMPATIBILITY_REVISION,settingsStabilityRevision:SETTINGS_STABILITY_REVISION,intentionResearchRevision:INTENTION_RESEARCH_REVISION};
})();
