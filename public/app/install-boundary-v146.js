...
const SETTINGS_SAFE_OPEN_SCRIPT='/extensions/commonweave-settings-safe-open-v172.js';
...
const PREVIOUS_ADDITIONS_VERSION='v171-settings-safe-open';
const FAST_CORE_COMPATIBILITY_REVISION='v157-fast-core';
const SETTINGS_STABILITY_REVISION='v161-settings-dialog-stability';
const SETTINGS_SAFE_OPEN_REVISION='v172-settings-window-capture';
...
function addScript(src){if(document.querySelector(`script[src^="${src}"]`))return;const script=document.createElement('script');script.src=`${src}?v=settings-window-capture-v172`;script.async=false;document.head.append(script)}
function installAdditions(){
  if(!document.querySelector(`link[href^="${ADDITIONS_STYLE}"]`)){
    const link=document.createElement('link');link.rel='stylesheet';link.href=`${ADDITIONS_STYLE}?v=settings-window-capture-v172`;document.head.append(link);
  }
