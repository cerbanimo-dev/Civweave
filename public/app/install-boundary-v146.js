...
const ADDITIONS_VERSION='v213-gemini-task-tier-routing';
...
const PREVIOUS_ADDITIONS_VERSION='v207-registration-watchdog';
const EARLIER_ADDITIONS_VERSION='v191-memory-credential';
const FAST_CORE_COMPATIBILITY_REVISION='v213-v106-gemini-task-tier-routing';
...
function installAdditions(){if(!document.querySelector(`link[href^="${ADDITIONS_STYLE}"]`)){const link=document.createElement('link');link.rel='stylesheet';link.href=`${ADDITIONS_STYLE}?v=${ADDITIONS_VERSION}`;document.head.append(link)}addScript(MEMORY_SCRIPT);addScript(MEMORY_BRIDGE_SCRIPT);addScript(DETERMINISTIC_MODE_SCRIPT);addScript(SETTINGS_CONTROLLER_SCRIPT);addScript(SETTINGS_DELEGATION_SCRIPT);addScript(GEMINI_TASK_ROUTER_SCRIPT);addScript(LIVE_SOURCE_GUARD_SCRIPT);addScript(DEVICE_CREDENTIALS_SCRIPT);addScript(ADDITIONS_SCRIPT);addScript(SHARED_TOOLS_CLEANUP_SCRIPT);addScript(PROOF_PROGRESS_SCRIPT);addScript(GEMINI_INTERACTIONS_SCRIPT);addScript(THEMED_SYSTEM_NAV_SCRIPT);addScript(PWA_UPDATE_SCRIPT)}
