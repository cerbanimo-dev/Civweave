...
const GUIDE_RUNTIME_RECOVERY_SCRIPT='/extensions/commonweave-guide-runtime-recovery-v160.js';
...
function addScript(src){if(document.querySelector(`script[src^="${src}"]`))return;const script=document.createElement('script');script.src=`${src}?v=guide-runtime-recovery-v160`;script.defer=true;document.head.append(script)}
function installAdditions(){
  if(!document.querySelector(`link[href^="${ADDITIONS_STYLE}"]`)){
    const link=document.createElement('link');link.rel='stylesheet';link.href=`${ADDITIONS_STYLE}?v=guide-runtime-recovery-v160`;document.head.append(link);
  }
  ...
  addScript(GUIDE_RUNTIME_RECOVERY_SCRIPT);
}
if(!allowed()){
...
globalThis.CommonweaveInstallBoundaryV146={allowed,installedDisplay,developer,embedded,installerUrl,installAdditions,additionsVersion:'v160-guide-runtime-recovery',previousAdditionsVersion:PREVIOUS_ADDITIONS_VERSION};
})();