...
const FAST_CORE_COMPATIBILITY_REVISION='v216-v106-guide-identity-integrity';
...
function installAdditions(){if(!document.querySelector(`link[href^="${ADDITIONS_STYLE}"]`)){const link=document.createElement('link');link.rel='stylesheet';link.href=`${ADDITIONS_STYLE}?v=${ADDITIONS_VERSION}`;document.head.append(link)}addScript(MEMORY_SCRIPT);addScript(MEMORY_BRIDGE_SCRIPT);addScript(DETERMINISTIC_MODE_SCRIPT);addScript(SETTINGS_CONTROLLER_SCRIPT);addScript(SETTINGS_DELEGATION_SCRIPT);addScript(GEMINI_TASK_ROUTER_SCRIPT);addScript(GUIDE_IDENTITY_SCRIPT);addScript(PERSISTENT_GUIDE_CHAT_SCRIPT);addScript(LIVE_SOURCE_GUARD_SCRIPT);addScript(DEVICE_CREDENTIALS_SCRIPT);addScript(ADDITIONS_SCRIPT);addScript(SHARED_TOOLS_CLEANUP_SCRIPT);addScript(PROOF_PROGRESS_SCRIPT);addScript(GEMINI_INTERACTIONS_SCRIPT);addScript(THEMED_SYSTEM_NAV_SCRIPT);addScript(PWA_UPDATE_SCRIPT)}
