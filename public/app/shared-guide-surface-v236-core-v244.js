...
const VERSION='1.0.130-shared-guide-surface-v236-single-shell-context';
...
const CONTEXT_SRC='/app/persistent-system-context-v1.js?v=1.0.2';
...
let currentSystem='civweave';
let layoutObserver=null;
let observedNav=null;
let mounted=false;

function detectSystem(){
  const selected=globalThis.CivweavePersistentSystemContextV1?.selected?.();if(SYSTEMS.includes(selected))return selected;
  const route=globalThis.CivweaveSystemRoutesV227?.identify?.(location.href);if(SYSTEMS.includes(route))return route;
  const declared=clean(document.documentElement?.dataset?.civweaveActiveSystemContext||document.documentElement?.dataset?.civweaveSystemRoute||document.documentElement?.dataset?.civweaveSystem,80).toLowerCase();if(SYSTEMS.includes(declared))return declared;
  const context=new URLSearchParams(location.search).get('context');return SYSTEMS.includes(context)?context:'civweave';
}
...
function canonicalSurface(){const api=globalThis.CivweaveGuideChatSurfaceV350||globalThis.CivweavePersistentGuideChatV215;return api?.canonicalOwner?api:null}
function ownPageGuide(){return Boolean(canonicalSurface())}
async function submitInline(text){
  const value=clean(text,8000),api=canonicalSurface();if(!value||typeof api?.submitText!=='function')return false;
  const active=globalThis.CivweavePersistentSystemContextV1?.selected?.()||api.state?.().activeSystem||currentSystem;if(SYSTEMS.includes(active))api.switchGuide?.(active,{open:false});return(await api.submitText(value,active))!==false;
}
function normalizeFloatingLayout(){const themed=document.getElementById('cw-themed-system-nav');if(themed){const height=Math.max(0,Math.round(themed.getBoundingClientRect().height||0));if(height)document.documentElement.style.setProperty('--cw-guide-nav-offset',`${height}px`)}document.documentElement.dataset.civweaveFloatingContract='v236-single-shell-context-v1'}
function observeNav(){if(!('ResizeObserver'in globalThis))return false;if(!layoutObserver)layoutObserver=new ResizeObserver(normalizeFloatingLayout);const nav=document.getElementById('cw-themed-system-nav');if(!nav)return false;if(observedNav!==nav){if(observedNav)try{layoutObserver.unobserve(observedNav)}catch{};observedNav=nav;layoutObserver.observe(nav)}return true}
function repairSurface(){currentSystem=detectSystem();const api=canonicalSurface();try{api?.ensureLauncher?.()}catch{}ensureSystemContext();try{globalThis.CivweavePersistentSystemContextV1?.applyPending?.('shared-guide-repair')}catch{}observeNav();normalizeFloatingLayout();return Boolean(api)}
function mount(){
  if(mounted)return;mounted=true;currentSystem=detectSystem();installStyle();ensureSystemContext();repairSurface();
  addEventListener('civweave:persistent-guide-chat-ready',repairSurface);addEventListener('civweave:guide-chat-ready',repairSurface);addEventListener('civweave:system-context-changed',event=>{if(SYSTEMS.includes(event?.detail?.system))currentSystem=event.detail.system;observeNav();normalizeFloatingLayout()});addEventListener('pageshow',repairSurface);addEventListener('resize',normalizeFloatingLayout,{passive:true});
  document.documentElement.dataset.civweaveGuideSurface=VERSION;document.documentElement.dataset.civweaveGuideSurfaceMode='bubble-only';document.documentElement.dataset.civweaveGuideLauncherOwner='guide-chat-surface-v350';
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',mount,{once:true});else mount();

globalThis.CivweaveSharedGuideSurfaceV236=Object.freeze({version:VERSION,mode:'bubble-only',sourceTruth:true,launcherOwner:'guide-chat-surface-v350',navigationGeometryOwner:false,navigationOffsetVariable:'--cw-guide-nav-offset',pageGuideOwnership:false,persistentSystemContext:true,singleShellContext:true,detectSystem,guideFor:system=>GUIDE[system]||null,renderTranscript,normalizeFloatingLayout,ownPageGuide,submitInline,syncInlineVisibility,setInlineInteractive,buildInline,repairSurface,removeEmbeddedGuideCards,mount});
})();