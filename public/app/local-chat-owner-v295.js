(()=>{
'use strict';
const VERSION='1.0.114-local-chat-owner-v301',SYSTEMS=['civweave','living-school','cerbanimo','fellowfare','anarchadia'],GUIDE={civweave:['Weaveling','central mirror and orchestrator'],'living-school':['Moss','learning guide'],cerbanimo:['Kamiya','questwright and skilled-work guide'],fellowfare:['Rook','quartermaster and exchange guide'],anarchadia:['Merlin','civic and automation guide']};
...
async function runOne(system,item){
 ...
 catch(error){const stalled=error?.code==='LOCAL_CHAT_STAGE_STALLED',boot=/^LOCAL_RUNTIME_BOOT_/.test(String(error?.code||'')),raw=clean(error?.message||error,500),detail=/^\d{6,}$/.test(raw)?`The browser inference backend returned diagnostic code ${raw} while ${error?.phase||'creating the model session'}.`:raw,hint=stalled?'The worker was reset because that loading stage stopped making progress. ':boot?'The local runtime startup was stopped because it did not become ready. ':'';update(system,id,{text:`The selected local model did not finish this chat turn, so Civweave stopped the run instead of letting the interface hang. ${hint}${detail}\n\nTry again, or open AI settings and choose a lighter downloaded model.`,pending:false,error:true,provider:'local-recovery',model:pick.id,localFastPath:true,localError:{code:error?.code||'LOCAL_MODEL_FAILED',phase:error?.phase||'',message:detail}});return false}
}
...
globalThis.CivweaveLocalChatOwnerV295=Object.freeze({version:VERSION,capturePhase:false,canonicalSubmitOwner:false,fiveGuideWindows:true,truthfulLoadProgress:true,boundedStartupRecovery:true,fifoQueue:true,enqueue,submit,queued,drain});
})();
