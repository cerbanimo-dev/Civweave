...
async function runOne(system,item){
 ...
 catch(error){const cancelled=error?.code==='LOCAL_MODEL_CANCELLED',stalled=error?.code==='LOCAL_CHAT_STAGE_STALLED',boot=/^LOCAL_RUNTIME_BOOT_/.test(String(error?.code||'')),failedId=clean(error?.model||pick.id,160),failedLabel=clean(error?.modelLabel||failedId,160),fallback=failedId&&failedId!==pick.id,raw=clean(error?.message||error,500),detail=/^\d{6,}$/.test(raw)?`The browser inference backend returned diagnostic code ${raw} while ${error?.phase||'creating the model session'}.`:raw,hint=stalled?'The worker was reset because that loading stage stopped making progress. ':boot?'The local runtime startup was stopped because it did not become ready. ':fallback?`${failedLabel}, the compatibility fallback for the selected model, also failed. `:'',released=Boolean(error?.workerReleased||error?.sessionReleased);update(system,id,{text:cancelled?'The local AI run was stopped so AI settings could open safely. Choose or change a model, then send your message again.':`The selected local model did not finish this chat turn, so Civweave stopped the run instead of letting the interface hang. ${hint}${detail}${released?' The failed inference worker was fully released.':''}\n\nTry again, or open AI settings and choose a lighter downloaded model.`,pending:false,error:true,provider:'local-recovery',model:failedId||pick.id,localFastPath:true,localError:{code:error?.code||'LOCAL_MODEL_FAILED',phase:error?.phase||'',selectedModel:pick.id,executionModel:failedId||pick.id,backend:error?.backend||'',workerReleased:released,message:detail}});return false}
}
...
globalThis.CivweaveLocalChatOwnerV295=Object.freeze({version:VERSION,capturePhase:false,canonicalSubmitOwner:false,fiveGuideWindows:true,truthfulLoadProgress:true,boundedStartupRecovery:true,freshWorkerFallback:true,truthfulExecutionModel:true,fifoQueue:true,terminalCancellation:true,settingsTeardown:true,enqueue,submit,queued,drain,cancelQueued});
})();
