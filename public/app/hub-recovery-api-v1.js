(()=>{
'use strict';
const VERSION='1.0.133-hub-recovery-api-v3-cloud-fabric-route';
...
function endpoint(path,h=host(),n=nodeId()){
 ...
 const base=new URL(h),route=clean(path,120).replace(/^\/+/,''),directHost=base.hostname===`${n}.nodes.commonweave.earth`,cloudFabric=/^civweave-node-cloud\./i.test(base.hostname);
 const pathname=directHost?`/api/account/${route}`:cloudFabric?`/n/${encodeURIComponent(n)}/api/account/${route}`:`/nodes/${encodeURIComponent(n)}/api/account/${route}`;
 return new URL(pathname,h)
}
async function post(path,body,h=host(),n=nodeId()){
 ...
 const packet=await response.json().catch(()=>({}));if(!response.ok){const e=new Error(clean(packet?.error||`Hub returned HTTP ${response.status}.`,1200));e.status=response.status;throw e}return packet
}
