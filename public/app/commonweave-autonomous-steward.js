...
const CLARIFY_KEY='commonweave.steward.clarifications.v1';
const POLICY_KEY='commonweave.steward.intention-policies.v1';
const CHECKIN_KEY='commonweave.steward.checkins.v1';
const MERGE_KEY='commonweave.steward.structure-proposals.v1';
...
const MS=()=>window.CommonweaveModelSteward;
...
function saveClarification(c){const all=clarifications().filter(x=>x.id!==c.id);all.unshift(c);save(CLARIFY_KEY,all.slice(0,100));dispatchEvent(new CustomEvent('commonweave:clarification-updated',{detail:clone(c)}));return clone(c)}
...
window.CommonweaveAutonomousSteward={VERSION,clarifications,openClarification,answerClarification,resolveClarification,preflight,policies,policy,setPolicy,shouldContact,checkins,createCheckin,gapCandidates,stageGapCandidate,structuralSuggestions,prepareStructure,reconcileBatchToPlan,dashboard,tick};
