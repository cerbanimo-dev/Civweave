...
function actualLoader(){const api=globalThis.CivweaveFamilyAILoaderV105;return api?.version?.includes('inline-civweave')?api:null}
function loadFamily(){
...
globalThis.CivweaveGuideChatV153={version:'anarchadia-compat-v159',ask:askMerlin,open:async()=>{const api=await loadFamily();return api.openChat('civweave',{contextSystem:'anarchadia'})}};
...
document.addEventListener('click',event=>{
  ...
  if(event.target.closest?.('[data-cwf-chat]')&&!actualLoader()){event.preventDefault();event.stopImmediatePropagation();proxy.openChat('civweave',{contextSystem:'anarchadia'});return}
},true);
compactLegacyState();
