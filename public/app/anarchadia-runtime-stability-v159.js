(()=>{
'use strict';
const VERSION='1.0.4-anarchadia-runtime-stability-v159';
...
const LOADER_SRC='/app/family-ai-loader-v105.js?v=interaction-stability-r41';
let loaderPromise=null;
const clean=(value,max=12000)=>String(value??'').slice(0,max);
const parse=(value,fallback)=>{try{return JSON.parse(value)??fallback}catch{return fallback}};
function compactLegacyState(){
  const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return;
  const state=parse(raw,null);if(!state||state.schema!=='commonweave.anarchadia-console.v1')return;
...
if(!actualLoader())globalThis.CommonweaveFamilyAILoaderV105=proxy;
async function askMerlin(_system,text,rows=[]){
  const api=await loadFamily();await api.ensure();
  const assistant=globalThis.CommonweaveAssistantV141;if(!assistant?.respond)throw new Error('The shared assistant runtime did not become ready.');
...
globalThis.CommonweaveGuideChatV153={version:'anarchadia-compat-v159',ask:askMerlin,open:async()=>{const api=await loadFamily();return api.openChat('commonweave',{contextSystem:'anarchadia'})}};
...
document.addEventListener('click',event=>{
  ...
  if(event.target.closest?.('[data-cwf-chat]')&&!actualLoader()){event.preventDefault();event.stopImmediatePropagation();proxy.openChat('commonweave',{contextSystem:'anarchadia'});return}
},true);
compactLegacyState();
globalThis.AnarchadiaRuntimeStabilityV159={version:VERSION,loadFamily,compactLegacyState};
})();
