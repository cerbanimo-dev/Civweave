...
const FALLBACK_ICONS=Object.freeze({civweave:'/app/assets/ai/chat/weaveling-face-v255.webp','living-school':'/app/assets/ai/chat/moss-face-v255.webp',cerbanimo:'/app/assets/ai/chat/kamiya-face-v255.webp',fellowfare:'/app/assets/ai/chat/rook-face-v255.webp',anarchadia:'/app/assets/ai/chat/merlin-face-v255.webp'});
...
function installDirector(){
  ...
  const head=document.head;if(!head?.isConnected)return Promise.resolve(false);const script=document.createElement('script');script.src=DIRECTOR_SRC;script.async=false;return new Promise(resolve=>{script.onload=()=>resolve(Boolean(director()));script.onerror=()=>resolve(false);head.append(script)})
}
function srcFor(system){return director()?.assetFor?.(system,expressions.get(system)||'neutral')||FALLBACK_ICONS[system]||''}
function installStyle(){
  if(document.getElementById(STYLE_ID))return;const head=document.head;if(!head)return;const style=document.createElement('style');style.id=STYLE_ID;style.textContent=`
...
function explicitSystem(img){const windowButton=img.closest?.('[data-cw242-window]');if(windowButton&&FALLBACK_ICONS[windowButton.dataset.cw242Window])return windowButton.dataset.cw242Window;const shared=img.closest?.(`#${SHARED_ROOT_ID}`);if(shared&&FALLBACK_ICONS[shared.dataset.system])return shared.dataset.system;const root=img.closest?.(`#${ROOT_ID}`);if(root&&img.matches?.('[data-guide-avatar]')&&FALLBACK_ICONS[root.dataset.guide])return root.dataset.guide;const launcher=img.closest?.(`#${LAUNCHER_ID}`);if(launcher){const page=root?.dataset?.pageSystem||document.documentElement.dataset.civweaveSystemRoute;if(FALLBACK_ICONS[page])return page}const alt=String(img.getAttribute?.('alt')||'').trim().toLowerCase().replace(/^open\s+/,'');if(ALT_TO_SYSTEM[alt])return ALT_TO_SYSTEM[alt];const src=String(img.getAttribute?.('src')||'').split('?')[0].split('/').pop()?.toLowerCase()||'';return OLD_SRC_TO_SYSTEM[src]||''}
function apply(scope=document){const images=scope instanceof HTMLImageElement?[scope]:Array.from(scope.querySelectorAll?.('img')||[]);for(const img of images){if(!img.closest?.(`#${ROOT_ID},#${SHARED_ROOT_ID},#${LAUNCHER_ID}`))continue;const system=explicitSystem(img),src=srcFor(system);if(!src)continue;if(img.getAttribute('src')!==src)img.src=src;img.dataset.cwFaceIcon='v313';img.dataset.cwExpression=expressions.get(system)||'neutral'}}
...
function start(){
  ...
  const observer=new MutationObserver(records=>{for(const record of records){for(const node of record.addedNodes){if(node instanceof Element)apply(node)}if(record.type==='attributes'&&record.target instanceof HTMLImageElement)apply(record.target)}});observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['src']});addEventListener('civweave:avatar-expression',expressionEvent);['civweave:guide-workspace-ready','civweave:guide-workspace-state','civweave:realm-guide-thread-changed','civweave:chat-single-owner-ready'].forEach(name=>addEventListener(name,refresh));globalThis.CivweaveSharedChatFaceIconsV255=Object.freeze({version:VERSION,icons:FALLBACK_ICONS,expressions,refresh,directorSrc:DIRECTOR_SRC,destroy:()=>{observer.disconnect();removeEventListener('civweave:avatar-expression',expressionEvent)},switcherDesktopPx:60,switcherMobilePx:54,launcherShape:'circle',launcherPosition:'fixed',launcherDesktopPx:52,launcherMobilePx:48,expressiveAvatars:true})
}
