...
const VERSION='1.0.107-shared-chat-face-icons-v255-mobile-anchor-v342';
...
const ALT_TO_SYSTEM=Object.freeze({
  weaveling:'civweave',moss:'living-school',kamiya:'cerbanimo',rook:'fellowfare',merlin:'anarchadia'
});
const OLD_SRC_TO_SYSTEM=Object.freeze({
  'weaveling.png':'civweave','moss.png':'living-school','kamiya.png':'cerbanimo','rook.png':'fellowfare','merlin.png':'anarchadia'
});
...
function explicitSystem(img){
  ...
  if(launcher){
    const page=root?.dataset?.pageSystem||document.documentElement.dataset.civweaveSystemRoute;
    if(ICONS[page])return page;
  }
...
function apply(scope=document){
  ...
  for(const img of images){
    ...
    const system=explicitSystem(img),src=ICONS[system];
    if(!src||img.getAttribute('src')===src)continue;
    img.src=src;
    img.dataset.cwFaceIcon='v255';
  }
}
...
function start(){
  ...
  const observer=new MutationObserver(records=>{
    for(const record of records){
      for(const node of record.addedNodes){if(node instanceof Element)apply(node)}
      if(record.type==='attributes'&&record.target instanceof HTMLImageElement)apply(record.target);
    }
  });
  ...
  globalThis.CivweaveSharedChatFaceIconsV255=Object.freeze({version:VERSION,icons:ICONS,refresh,destroy:()=>observer.disconnect(),switcherDesktopPx:60,switcherMobilePx:54,launcherShape:'circle',launcherPosition:'fixed',launcherDesktopPx:52,launcherMobilePx:48});
}
