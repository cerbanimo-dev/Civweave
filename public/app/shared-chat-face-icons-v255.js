...
const VERSION='1.0.115-shared-chat-face-icons-v255-open-ui-only-v2-avatar-contain-v1-header-clearance-v1';
...
#${ROOT_ID}>header{grid-template-columns:76px minmax(0,1fr) 40px!important;column-gap:12px!important}
...
@media(max-width:620px){
  ...
  #${ROOT_ID}>header{grid-template-columns:68px minmax(0,1fr) 40px!important;column-gap:10px!important}
...
function explicitSystem(img){
  ...
  if(img.closest?.(`#${LAUNCHER_ID}`)){const page=root?.dataset?.pageSystem||document.documentElement.dataset.civweaveSystemRoute;if(ICONS[page])return page}
  return img.dataset.cwExpressionSystem||'';
}
...
function refreshMiniLmLamp(){
  ...
  const active=Boolean(globalThis.CivweaveContextRouterV344?.status?.().ready);root.dataset.cwMinilmActive=active?'true':'false';
