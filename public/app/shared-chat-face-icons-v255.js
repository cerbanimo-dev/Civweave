...
const VERSION='1.0.108-shared-chat-face-icons-v255-expressive-v343';
...
const EXPRESSION_DIRECTOR='/app/avatar-expression-director-v343.js?v=1.0.0-avatar-v343';
...
function ensureExpressionDirector(){
  ...
  directorPromise=new Promise(resolve=>{
    const existing=Array.from(document.scripts||[]).find(node=>String(node.src||'').includes('/app/avatar-expression-director-v343.js'));
...
function explicitSystem(img){
  const windowButton=img.closest?.('[data-cw242-window]');
  if(windowButton&&ICONS[windowButton.dataset.cw242Window])return windowButton.dataset.cw242Window;
  const shared=img.closest?.(`#${SHARED_ROOT_ID}`);
  if(shared&&ICONS[shared.dataset.system])return shared.dataset.system;
  const root=img.closest?.(`#${ROOT_ID}`);
  if(root&&img.matches?.('[data-guide-avatar]')&&ICONS[root.dataset.guide])return root.dataset.guide;
  const launcher=img.closest?.(`#${LAUNCHER_ID}`);
  if(launcher){const page=root?.dataset?.pageSystem||document.documentElement.dataset.civweaveSystemRoute;if(ICONS[page])return page}
  const alt=String(img.getAttribute?.('alt')||'').trim().toLowerCase().replace(/^open\s+/,'');
  if(ALT_TO_SYSTEM[alt])return ALT_TO_SYSTEM[alt];
  const src=String(img.getAttribute?.('src')||'').split('?')[0].split('/').pop()?.toLowerCase()||'';
  return OLD_SRC_TO_SYSTEM[src]||img.dataset.cwExpressionSystem||'';
}
...
function apply(scope=document){
  ...
  for(const img of images){
    ...
    if(img.dataset.cwExpressionSprite==='v343'&&img.dataset.cwExpressionSystem===system)continue;
    if(img.dataset.cwExpressionSprite==='v343'){delete img.dataset.cwExpressionSprite;delete img.dataset.cwExpression;delete img.dataset.cwExpressionSystem}
    if(img.getAttribute('src')===src)continue;
    img.src=src;img.dataset.cwFaceIcon='v255';
  }
}
...
function applyExpression(detail={}){
  ...
  for(const img of images){if(explicitSystem(img)!==system)continue;img.dataset.cwExpressionSprite='v343';img.dataset.cwExpressionSystem=system;img.dataset.cwExpression=String(detail.expression||'neutral');if(img.getAttribute('src')!==asset)img.src=asset;changed+=1}
  return changed>0;
}

function refresh(){installStyle();apply(document);void ensureExpressionDirector().then(api=>api?.refresh?.())}
function start(){
  refresh();
  const observer=new MutationObserver(records=>{
    for(const record of records){for(const node of record.addedNodes){if(node instanceof Element)apply(node)}if(record.type==='attributes'&&record.target instanceof HTMLImageElement)apply(record.target)}
  });
  ...
  globalThis.CivweaveSharedChatFaceIconsV255=Object.freeze({version:VERSION,icons:ICONS,refresh,applyExpression,ensureExpressionDirector,destroy:()=>observer.disconnect(),switcherDesktopPx:60,switcherMobilePx:54,launcherShape:'circle',launcherPosition:'fixed',launcherDesktopPx:52,launcherMobilePx:48,expressiveSprites:true});
  try{dispatchEvent(new CustomEvent('civweave:shared-chat-face-icons-ready',{detail:{version:VERSION,expressiveSprites:true}}))}catch{}
}
