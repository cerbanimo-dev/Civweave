(()=>{
'use strict';
const VERSION='1.0.4-anarchadia-v158';
const STORAGE_KEY='commonweave.anarchadia.citizen-console.v139';
const CHAT_KEY='commonweave.guide-chat.anarchadia.v153';
...
function defaultState(){
  return {schema:'commonweave.anarchadia-console.v1',passportId:`AC-${crypto?.randomUUID?.().slice(0,8).toUpperCase()||Math.random().toString(36).slice(2,10).toUpperCase()}`,proposals:[],ledger:[{id:uid('evt'),time:now(),kind:'console-ready',detail:'Citizen console initialized locally.'}],settings:{autoRun:true}};
}
let state=load();
...
function load(){const saved=parse(localStorage.getItem(STORAGE_KEY),null);return saved?.schema==='commonweave.anarchadia-console.v1'?saved:defaultState()}
...
function proposalCard(p){const busy=runningPipelines.has(p.id);return `<article class="ac-card"><header><div><small>${esc(p.kind)} · ${esc(p.area)}</small><h3>${esc(p.title)}</h3></div>${badge(p)}</header><p>${esc(p.problem)}</p>${stageTrack(p)}<div class="ac-card-actions"><button type="button" data-open-pipeline="${p.id}">PIPELINE</button>${p.preview?.srcdoc?`<button type="button" data-preview="${p.id}">PREVIEW</button>`:''}<button type="button" data-signal-vote="${p.id}">${p.voteSignal?'SUPPORT SIGNALLED':'VOTE!'}</button><button type="button" data-rerun="${p.id}" ${busy?'disabled':''}>${busy?'RUNNING…':'RUN AGAIN'}</button></div></article>`}
...
function renderObservatory(){
  ...
  const total=state.proposals.length,ready=state.proposals.filter(p=>p.stage==='preview-ready').length,blocked=state.proposals.filter(p=>['blocked','failed'].includes(p.status)).length,votes=state.proposals.filter(p=>p.voteSignal).length,readiness=total?Math.round((ready/total)*100):100;
...
async function semanticReview(p){
  const reflex=globalThis.CommonweaveReflexRuntime||parent?.CommonweaveReflexRuntime;
  if(!reflex?.respond)return 'Deterministic rails completed. The semantic matcher is still warming.';
  try{
    const result=await Promise.race([reflex.respond({purpose:'anarchadia-rail-review',config:{provider:'bundled'},context:{schema:'commonweave.structured-context.v1',userMessage:`${p.kind}: ${p.title}\n${p.problem}\nExpected: ${p.expected}`,routingAnswer:{system:'anarchadia',mode:'Govern',room:'anarchadia.automation'},consent:{consequentialActionDetected:true}},messages:[]}),sleep(500).then(()=>null)]);
...
function deterministicPatch(p){
  ...
  return {generator:'commonweave-safe-scaffolder',summary:`Generated a bounded ${p.kind} preview for ${p.area}.`,files:[{path:'preview/index.html',content:html},{path:'preview/styles.css',content:css},{path:'preview/app.js',content:js}],preview:{html,css,js}};
}
async function providerPatch(p){
  const runtime=globalThis.CommonweaveModelRuntime||parent?.CommonweaveModelRuntime,config=runtime?.readSharedConfig?.('agentic');
...
function openPreview(id){const p=proposal(id);if(!p?.preview?.srcdoc)return;$('#ac-preview-title').textContent=p.title;$('#ac-preview-frame').srcdoc=p.preview.srcdoc;const dialog=$('#ac-preview-dialog');if(dialog&&!dialog.open)dialog.showModal?.()}
...
function submitRequest(event){event.preventDefault();const data=new FormData(event.currentTarget),acceptance=String(data.get('acceptance')||'').split(/\n+/).map(v=>v.trim()).filter(Boolean),p={id:uid('proposal'),kind:String(data.get('kind')||requestKind),title:String(data.get('title')||'').trim(),problem:String(data.get('problem')||'').trim(),expected:String(data.get('expected')||'').trim(),area:String(data.get('area')||'Commonweave'),acceptance,risk:String(data.get('risk')||'').trim(),evidence:String(data.get('evidence')||'').trim(),status:'draft',stage:'intake',pipeline:[],createdAt:now(),updatedAt:now(),voteSignal:false};state.proposals.unshift(p);save('proposal-created',`${p.kind}: ${p.title}`,p.id);setScreen('automation');if(data.get('autoRun'))runPipeline(p.id)}
...
globalThis.AnarchadiaCitizenConsoleV158={version:VERSION,setScreen,runPipeline,render:queueRender,openMerlin:openFullMerlin};
})();
