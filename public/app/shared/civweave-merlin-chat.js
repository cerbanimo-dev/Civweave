...
(function(global){
  ...
  const AI=global.CivweaveAIPersonas||{prompts:fallbackPrompts,realmGuide:{"Living School":"moss",Cerbanimo:"kamiya",Anarchadia:"merlin",FellowFare:"rook","Civweave Campus":"weaveling"},compose:(persona,constraints="")=>`${fallbackPrompts[persona]||fallbackPrompts.weaveling}\n\nTask-specific tool, safety, privacy, consent, schema, and output constraints remain binding and override persona style if they conflict:\n${constraints}`};
  if(!global.CivweaveAIPersonas)global.CivweaveAIPersonas=AI;
  ...
  function upliftRequest(request={}){
    if(request.__civweavePersonaApplied)return request;
    const persona=realmPersona(),next={...request,__civweavePersonaApplied:true};
  ...
  function installPersonaUplift(){const rt=global.CivweaveModelRuntime;if(!rt||rt.__personaUplift)return;const enhanced={...rt,__personaUplift:true,generate:request=>rt.generate(upliftRequest(request)),generateAgentic:request=>(rt.generateAgentic||rt.generate)(upliftRequest(request)),generateInteractive:request=>(rt.generateInteractive||rt.generate)(upliftRequest(request))};global.CivweaveModelRuntime=Object.freeze(enhanced)}
  installPersonaUplift();global.addEventListener("civweave:model-runtime-ready",installPersonaUplift);
  const script=document.currentScript;
  const asset=(name)=>new URL(`../assets/ai/${name}`,script?.src||location.href).href;
  const STORAGE="civweave.merlin-chat.v1";
  const POSITION_KEY="civweave.merlin-launcher-position.v1";
  ...
  function currentRealm(){const path=location.pathname;return path.includes("living-school")?"Living School":path.includes("cerbanimo")?"Cerbanimo":path.includes("anarchadia")?"Anarchadia":path.includes("fellowfare")?"FellowFare":"Civweave Campus"}
  function currentGuide(){return AI.realmGuide?.[currentRealm()]||"weaveling"}
  function summarizeIntentions(){const values=read("civweave.intentions.v2",[]);return (Array.isArray(values)?values:[]).slice(0,12).map(item=>({title:compact(item.title,140),status:item.status,progress:item.health?.progress,health:compact(item.health?.summary,180),steps:Array.isArray(item.steps)?item.steps.length:0,systems:item.systems||[]}))}
  ...
  function platformSnapshot(){
    const live=global.CivweaveLiveData?.records;
...
- Answer questions about the supplied Civweave platform snapshot.
...
Speak naturally. The interface renders your answer as ordinary conversation, not a deterministic wrapper.`;
  ...
  function modelConfig(){const rt=global.CivweaveModelRuntime,base=rt?.readSharedConfig?.("interactive");if(!base)return null;return {...base,stream:true,temperature:Math.max(.45,Number(base.temperature||.75)),maxTokens:Math.max(2048,Number(base.maxTokens||4096)),timeoutMs:Math.max(90000,Number(base.timeoutMs||120000)),service:"civweave-merlin"}}
  ...
  function openModelSettings(){
    ...
    global.dispatchEvent(new CustomEvent("civweave:open-model-settings",{detail:"Weaveling's Compass needs a connected interactive language model. Deterministic mode is intentionally unsupported."}));
    if(global.parent!==global)global.parent.postMessage({type:"civweave:open-model-settings",detail:"Weaveling's Compass needs a connected interactive language model."},location.origin);
    ui.status.textContent="Open the Civweave shared model settings and connect an interactive model.";
  }
  ...
  global.CivweaveMerlinChat=api;global.CivweaveCompass=api;
