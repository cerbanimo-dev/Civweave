(function installCivweaveModelRuntime(global) {
  "use strict";

  const VERSION = "1.0.0-rc6-context-switching";
  const RESULT_SCHEMA = "civweave-model-result-1.0";
  const CAPABILITY_SCHEMA = "civweave-model-capabilities-1.0";
  const SHARED_MODEL_STORAGE_KEY = "civweave-shared-model";
  const MODEL_PROFILES_STORAGE_KEY = "civweave-model-profiles-v1";
  const SESSION_SECRET_STORAGE_KEY = "civweave-model-secrets-v1";
  const CAPABILITY_STORAGE_KEY = "civweave-model-capabilities-v1";
  ...
  function emit(context, phase, detail = {}) {
    ...
    dispatch("civweave:model-event", event);
    return event;
  }
  ...
  function normalizeConfig(input = {}) {
    ...
    return {
      ...
      service: safeString(input.service || "civweave", 80),
  ...
  async function invokeHosted(config, messages, request, context, signal) {
    ...
    const response = await fetch(endpoint, fetchOptions(endpoint, {
      ...
      body: JSON.stringify({
        schema: "civweave-model-request-1.0",
  ...
  async function invokeAntigravity(config, messages, request, context, signal) {
    ...
    let connectedHost = "";
    try { connectedHost = JSON.parse(global.localStorage?.getItem?.("civweave.host-node.v1") || "null")?.baseUrl || ""; } catch {}
    ...
    try {
      const pending = JSON.parse(global.localStorage?.getItem?.("civweave-antigravity-pending-v1") || "{}");
      pending[context.requestId] = { interactionId, purpose: context.purpose, startedAt: nowIso() };
      global.localStorage?.setItem?.("civweave-antigravity-pending-v1", JSON.stringify(pending));
    } catch {}
  ...
  async function generate(request = {}) {
    ...
    if (config.provider === "manual") {
      ...
      try { global.sessionStorage?.setItem?.(`civweave-manual-${context.purpose}`, prompt); } catch {}
  ...
  function readSharedConfig(profile = "interactive") {
    ...
    const session = readJsonStorage(global.sessionStorage, "civweave-model-session", {});
  ...
  function saveSharedConfig(input, options = {}) {
    ...
    dispatch("civweave:model-config-changed", { config: safe, profile, profiles, at: nowIso() });
    return safe;
  }
  ...
  async function detectCapabilities(input = {}, options = {}) {
    ...
    dispatch("civweave:model-capabilities", base);
    return base;
  }
  ...
  global.CivweaveModelRuntime = api;
  dispatch("civweave:model-runtime-ready", { version: VERSION, at: nowIso() });
