...
const icon={
 home:R('assets/generated/commonweave-navigation-icons/commonweave-home.png'),
 compass:R('assets/generated/commonweave-navigation-icons/weaveling-compass.png'),
 realms:R('assets/generated/commonweave-navigation-icons/commonweave-realms.png'),
 route:R('assets/generated/commonweave-navigation-icons/commonweave-route.png'),
 settings:R('assets/generated/commonweave-navigation-icons/commonweave-ai-config.png')
};
...
const stateKey=`commonweave.${system}.visual-state.v2`;
...
function commonweave(query=''){location.href=R(`index.html${query}`)}
function settings(){commonweave(`?open=model-settings&return=${encodeURIComponent(location.href)}`)}
function compass(){if(window.CommonweaveCompass?.open){window.CommonweaveCompass.open();return}if(window.CommonweaveMerlinChat?.open){window.CommonweaveMerlinChat.open();return}document.querySelector('.cw-merlin-launcher')?.click()}
...
function saveSubmission(type,data){const key=`commonweave.${system}.${type}.v1`;let list=[];try{list=JSON.parse(localStorage.getItem(key)||'[]')}catch{}list.unshift({...data,id:crypto.randomUUID(),createdAt:new Date().toISOString()});localStorage.setItem(key,JSON.stringify(list.slice(0,100)));dispatchEvent(new CustomEvent(`commonweave:${system}:${type}`,{detail:list[0]}))}
...
function markup(){return `<section class="cw-recovery-stage" data-system="${system}"><img class="cw-scene-backdrop" alt=""><div class="cw-art-frame"><img class="cw-recovery-art" alt="${cfg.name} visual scene"><div class="cw-scene-layer"></div></div><div class="cw-loading">Loading local ${cfg.name} scene…</div><button class="cw-commonweave-return" aria-label="Return to Commonweave"><img src="${R('ui-icons/back.svg')}" alt=""></button><nav class="cw-district-controls"><button data-info aria-label="Scene information"><span>i</span></button><button data-compass aria-label="Weaveling Compass"><img src="${icon.compass}" alt=""></button><button data-settings aria-label="Commonweave settings"><img src="${icon.settings}" alt=""></button></nav><nav class="cw-district-dock"><button data-home aria-label="${cfg.name} home"><img src="${icon.home}" alt=""></button><button data-route aria-label="Realm route"><img src="${icon.route}" alt=""></button><button data-commonweave aria-label="Commonweave home"><img src="${R('logos/commonweave-icon-96.png')}" alt=""></button><button data-realms aria-label="Realm map"><img src="${icon.realms}" alt=""></button><button data-compass aria-label="Weaveling Compass"><img src="${icon.compass}" alt=""></button></nav></section><aside class="cw-district-info" hidden><button aria-label="Close">×</button><small>COMMONWEAVE v${VERSION}</small><strong>${cfg.name}</strong><p>The image is the interface. Object-shaped hotspots follow the artwork, while Commonweave alone owns installation, caching, models, and updates.</p><span>Top Compass: site-wide AI</span><span>Center dock sigil: return to the Quad</span><span>Home: return to this realm’s main room</span></aside>`}
function mount(){if(document.querySelector('.cw-recovery-stage'))return;document.documentElement.classList.add('cw-lean-visual-shell',`cw-system-${system}`);document.title=`Commonweave Pocket Campus · ${cfg.name}`;document.body.insertAdjacentHTML('beforeend',markup());stage=document.querySelector('.cw-recovery-stage');frame=stage.querySelector('.cw-art-frame');art=stage.querySelector('.cw-recovery-art');backdrop=stage.querySelector('.cw-scene-backdrop');layer=stage.querySelector('.cw-scene-layer');stage.querySelector('.cw-commonweave-return').onclick=()=>commonweave();stage.querySelectorAll('[data-compass]').forEach(b=>b.onclick=compass);stage.querySelector('[data-settings]').onclick=settings;stage.querySelector('[data-info]').onclick=toggleInfo;stage.querySelector('[data-home]').onclick=()=>setScene(cfg.home);stage.querySelector('[data-commonweave]').onclick=()=>commonweave();stage.querySelector('[data-realms]').onclick=()=>commonweave('?open=map');stage.querySelector('[data-route]').onclick=()=>openSurface(system==='anarchadia'?'hub':system==='fellowfare'?'exchange-board':system==='living'?'moss-chat':'kamiya-chat');document.querySelector('.cw-district-info>button').onclick=toggleInfo;addEventListener('resize',fitFrame,{passive:true});setScene(read().scene||cfg.home);unifyWorker()}
