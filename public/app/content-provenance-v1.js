...
function read(record){const metadata=object(record?.metadata),source=object(metadata[METADATA_KEY]);if(!Object.keys(source).length)return null;const origin=normalizeOrigin(source.origin);return{...clone(source),schema:SCHEMA,origin,aiGenerated:origin==='ai-generated',humanValidations:Array.isArray(source.humanValidations)?clone(source.humanValidations):[]}}
