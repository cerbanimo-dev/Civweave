...
const VERSION='1.0.34-guide-workspace-v242';
...
function readWorkspace(){
  ...
  return{
    ...
    activeWindow:SYSTEMS.includes(value.activeWindow)?value.activeWindow:null,
    seen:value.seen&&typeof value.seen==='object'?value.seen:{}
  };
}
function saveWorkspace(){
  const previous=readWorkspace();
  try{localStorage.setItem(STATE_KEY,JSON.stringify({open:workspaceOpen,minimized,activeWindow,seen:previous.seen,updatedAt:now()}))}catch{}
}
function markSeen(system){
  const value=readWorkspace();value.seen={...value.seen,[system]:now()};value.open=workspaceOpen;value.minimized=minimized;value.activeWindow=activeWindow;
  try{localStorage.setItem(STATE_KEY,JSON.stringify(value))}catch{}
}
function hasUnread(system){
  if(system===activeWindow&&workspaceOpen)return false;
  const thread=readThread(system),seen=readWorkspace().seen?.[system]||'';
  return Boolean(Number(thread.unread||0)>0||(thread.updatedAt&&(!seen||thread.updatedAt>seen)));
}

function retireViewportTrap(){
  try{globalThis.CivweavePersistentGuideViewportV216?.destroy?.()}catch{}
  document.documentElement.dataset.civweaveGuideViewport='css-v242';
}
...
function installStyle(){
  ...
  style.id=STYLE_ID;
  style.textContent=`
#${ROOT_ID}{z-index:2147483644!important;right:max(10px,env(safe-area-inset-right))!important;bottom:calc(var(--cw-themed-nav-height,64px) + env(safe-area-inset-bottom) + 10px)!important;width:min(460px,calc(100vw - 20px))!important;height:min(72dvh,680px)!important;max-height:calc(100dvh - var(--cw-themed-nav-height,64px) - env(safe-area-inset-bottom) - env(safe-area-inset-top) - 24px)!important;grid-template-rows:auto auto minmax(96px,1fr) auto!important;overflow:hidden!important;overscroll-behavior:auto!important;touch-action:auto!important;isolation:isolate!important;contain:layout paint style!important;pointer-events:auto!important}
#${ROOT_ID}.is-minimized{height:auto!important;grid-template-rows:auto auto!important}
#${ROOT_ID}.is-minimized [data-log],#${ROOT_ID}.is-minimized [data-persistent-form]{display:none!important}
...
function ensureRoot(){
  ...
  if(!root){
    root=document.createElement('section');root.id=ROOT_ID;root.hidden=true;
    root.innerHTML='<header><img data-guide-avatar alt=""><div><small data-window-label></small><strong data-guide-name></strong><span data-guide-role></span></div><button type="button" data-minimize aria-label="Minimize chat">−</button><button type="button" data-close aria-label="Close chat">×</button></header><nav class="'+SWITCHER_CLASS+'" aria-label="Guide chat windows"></nav><div data-log role="log" aria-live="polite"></div><form data-persistent-form><textarea rows="2" maxlength="12000" required></textarea><button data-send type="submit">Send</button></form>';
    document.body.append(root);
  }
  if(!root.querySelector('.'+SWITCHER_CLASS)){
    const nav=document.createElement('nav');nav.className=SWITCHER_CLASS;nav.setAttribute('aria-label','Guide chat windows');root.querySelector('header')?.after(nav);
  }
  if(!launcher){launcher=document.createElement('button');launcher.id=LAUNCHER_ID;launcher.type='button';launcher.innerHTML='<img alt="">';document.body.append(launcher)}
  return root;
}

function renderSwitcher(){
  ensureRoot();const nav=root.querySelector('.'+SWITCHER_CLASS);if(!nav)return;
  nav.innerHTML=SYSTEMS.map(system=>{const guide=GUIDE[system];return`<button type="button" class="cw242-window" data-cw242-window="${system}" data-here="${system===pageSystem}" aria-pressed="${system===activeWindow}" style="--window-accent:${guide.accent}" aria-label="Open ${esc(guide.name)} chat window"><img src="${guide.avatar}" alt=""><span>${esc(guide.name)}</span><i class="cw242-unread" ${hasUnread(system)?'':'hidden'}></i></button>`}).join('');
}
function renderVisuals(){
  ensureRoot();const guide=GUIDE[activeWindow],pageGuide=GUIDE[pageSystem];if(!guide||!pageGuide)return;
  root.dataset.guide=activeWindow;root.dataset.pageSystem=pageSystem;root.style.setProperty('--guide-accent',guide.accent);root.style.setProperty('--guide-panel',guide.panel);
  const avatar=root.querySelector('[data-guide-avatar]'),label=root.querySelector('[data-window-label]'),name=root.querySelector('[data-guide-name]'),role=root.querySelector('[data-guide-role]'),input=root.querySelector('textarea');
  if(avatar){avatar.src=guide.avatar;avatar.alt=guide.name}if(label)label.textContent=`${guide.label} THREAD${activeWindow===pageSystem?' · HERE':' · WINDOW'}`;if(name)name.textContent=guide.name;if(role)role.textContent=`${guide.role} · ${guide.label}`;if(input)input.placeholder=`Message ${guide.name}`;
  launcher.style.setProperty('--guide-accent',pageGuide.accent);launcher.style.setProperty('--guide-panel',pageGuide.panel);const launchImage=launcher.querySelector('img');if(launchImage){launchImage.src=pageGuide.avatar;launchImage.alt=`Open ${pageGuide.name}`};launcher.setAttribute('aria-label',`Open ${pageGuide.name} chat`);
}
function renderMessages(){
  ensureRoot();const log=root.querySelector('[data-log]'),thread=readThread(activeWindow),guide=GUIDE[activeWindow];if(!log)return;
  const rows=Array.isArray(thread.messages)?thread.messages:[];
  if(!rows.length){log.innerHTML=`<div class="cw237-bubble">${esc(guide.name)} keeps a private ${esc(guide.label)} thread. Switching windows never mixes histories. Cross-realm work appears only as an explicit handover card.</div>`;return}
...
function render({keepScroll=false}={}){
  ensureRoot();const log=root.querySelector('[data-log]'),before=log?.scrollTop||0;root.hidden=!workspaceOpen;root.classList.toggle('is-minimized',minimized);renderVisuals();renderSwitcher();renderMessages();if(log){if(keepScroll)log.scrollTop=before;else log.scrollTop=log.scrollHeight}markSeen(activeWindow)
}

function openWindow(system=pageSystem,options={}){
  if(!SYSTEMS.includes(system))system=pageSystem;activeWindow=system;workspaceOpen=true;minimized=false;saveWorkspace();render();const input=root.querySelector('textarea');if(options.prefill&&input)input.value=clean(options.prefill,12000);if(options.focus===true)queueMicrotask(()=>input?.focus({preventScroll:true}));return root
}
...
function switchWindow(system,{open=true}={}){if(!SYSTEMS.includes(system))return false;activeWindow=system;if(open)workspaceOpen=true;minimized=false;saveWorkspace();render();return true}
...
async function submitActive(text){
  const value=clean(text,12000),system=activeWindow;if(!value||busy||!SYSTEMS.includes(system))return false;
  busy=true;ensureRoot();const button=root.querySelector('[data-send]');if(button)button.disabled=true;
  append(system,{role:'user',text:value});const pendingId=uid('pending');append(system,{id:pendingId,role:'assistant',guide:system,text:`${GUIDE[system].name} is thinking…`,pending:true});const input=root.querySelector('textarea');if(input)input.value='';render();
  try{
    await globalThis.CivweaveFamilyAILoaderV105?.ensure?.();const assistant=globalThis.CivweaveAssistantV141;if(!assistant?.respond)throw new Error('The shared assistant runtime is not ready.');
    ...
    const target=explicitHandoffTarget(result,system);if(target)await realmApi()?.createHandover?.(system,target,result);
  }catch(error){const thread=readThread(system),index=(thread.messages||[]).findIndex(row=>row.id===pendingId),replacement={role:'assistant',guide:system,responderSystem:system,text:`${GUIDE[system].name} could not complete this call: ${clean(error.message,800)}`,provider:'local-recovery'};if(index>=0)thread.messages[index]=replacement;else thread.messages.push(replacement);writeThread(system,thread)}finally{busy=false;if(button)button.disabled=false;render()}
  return true
}

function onClickCapture(event){
  const windowButton=event.target.closest?.('[data-cw242-window]');if(windowButton){event.preventDefault();event.stopImmediatePropagation();switchWindow(windowButton.dataset.cw242Window);return}
...
function onSubmitCapture(event){
  const form=event.target?.closest?.(`#${ROOT_ID} [data-persistent-form]`);if(!form)return;event.preventDefault();event.stopImmediatePropagation();const input=form.querySelector('textarea'),text=clean(input?.value,12000);if(text)submitActive(text)
}
...
function installApi(){
  ...
  const api=Object.freeze({
    version:VERSION,realmIsolated:true,workspace:true,systems:[...SYSTEMS],
    readState:()=>readThread(activeWindow),readThread,
    ...
    destroy(){document.removeEventListener('click',onClickCapture,true);document.removeEventListener('submit',onSubmitCapture,true);removeEventListener('civweave:realm-guide-thread-changed',onThreadChanged);bodyObserver?.disconnect();globalThis.visualViewport?.removeEventListener('resize',onResize);removeEventListener('resize',onResize);document.getElementById(STYLE_ID)?.remove()}
  });
  globalThis.CivweavePersistentGuideChatV215=api;return api
}

function start(){
  pageSystem=detectSystem();const saved=readWorkspace();activeWindow=pageSystem;workspaceOpen=saved.open;minimized=saved.minimized;installStyle();retireViewportTrap();ensureRoot();installApi();document.addEventListener('click',onClickCapture,true);document.addEventListener('submit',onSubmitCapture,true);addEventListener('civweave:realm-guide-thread-changed',onThreadChanged);bodyObserver=new MutationObserver(onBodyChange);bodyObserver.observe(document.body,{childList:true});globalThis.visualViewport?.addEventListener('resize',onResize,{passive:true});addEventListener('resize',onResize,{passive:true});onResize();render();document.documentElement.dataset.civweaveGuideWorkspace='v242';dispatchEvent(new CustomEvent('civweave:guide-workspace-ready',{detail:{version:VERSION,pageSystem,activeWindow,realmIsolated:true,windowCount:5}}))
}

if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();

globalThis.CivweaveGuideWorkspaceV242=Object.freeze({version:VERSION,systems:[...SYSTEMS],detectSystem,readThread,openWindow,switchWindow,closeWorkspace,submitActive,state:()=>({pageSystem,activeWindow,open:workspaceOpen,minimized,busy})});
})();