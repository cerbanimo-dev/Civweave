...
const VERSION='1.0.132-guide-workspace-v242-canonical-v250-hub-neurons';
...
#${ROOT_ID}>header [data-neuron-status]{color:#8df0c6;font-weight:800}
#${ROOT_ID}>header [data-neuron-status][hidden]{display:none}
...
function renderVisuals(){ensureRoot();const guide=GUIDE[activeWindow],pageGuide=GUIDE[pageSystem];if(!guide||!pageGuide)return;root.dataset.guide=activeWindow;root.dataset.pageSystem=pageSystem;root.style.setProperty('--guide-accent',guide.accent);root.style.setProperty('--guide-panel',guide.panel);const avatar=root.querySelector('[data-guide-avatar]'),label=root.querySelector('[data-window-label]'),name=root.querySelector('[data-guide-name]'),role=root.querySelector('[data-guide-role]'),neuron=root.querySelector('[data-neuron-status]'),input=root.querySelector('textarea');if(avatar){avatar.src=guide.avatar;avatar.alt=guide.name}if(label)label.textContent=`${guide.label} THREAD${activeWindow===pageSystem?' · HERE':' · WINDOW'}`;if(name)name.textContent=guide.name;if(role)role.textContent=`${guide.role} · ${guide.label}`;const telemetry=globalThis.CivweaveHostNodeSessionV1?.telemetryFor?.();if(neuron){const charged=Number(telemetry?.chargedNeurons),remaining=Number(telemetry?.remainingNeurons),turns=Number(telemetry?.approximateTurnsLeft),parts=[];if(Number.isFinite(charged)&&charged>0)parts.push(`${Math.floor(charged)} neurons last turn`);else if(Number.isFinite(remaining))parts.push(`${Math.floor(remaining)} neurons left`);if(Number.isFinite(turns))parts.push(`≈${Math.floor(turns)} chats left`);neuron.textContent=parts.join(' · ');neuron.hidden=!parts.length}if(input)input.placeholder=`Message ${guide.name}`;launcher.style.setProperty('--guide-accent',pageGuide.accent);launcher.style.setProperty('--guide-panel',pageGuide.panel);const launchImage=launcher.querySelector('img');if(launchImage){launchImage.src=pageGuide.avatar;launchImage.alt=`Open ${pageGuide.name}`};launcher.setAttribute('aria-label',`Open ${pageGuide.name} chat`)}
function renderMessages(){ensureRoot();const log=root.querySelector('[data-log]'),thread=readThread(activeWindow),guide=GUIDE[activeWindow];if(!log)return;const rows=Array.isArray(thread.messages)?thread.messages:[];if(!rows.length){log.innerHTML=`<div class="cw237-bubble">${esc(guide.name)} keeps a private ${esc(guide.label)} thread. Switching windows never mixes histories. Cross-realm work appears only as an explicit handover card.</div>`;return}log.innerHTML=rows.map(row=>{if(row.role==='user')return`<article class="is-user"><div class="cw237-bubble">${esc(row.text)}</div></article>`;const system=rowSystem(row),meta=GUIDE[system]||guide,handover=row.role==='handover',neurons=Number(row.chargedNeurons),turns=row.approximateTurnsLeft==null?NaN:Number(row.approximateTurnsLeft);return`<article class="${handover?'is-handover':''}" style="--message-accent:${meta.accent}"><img src="${meta.avatar}" alt="${esc(meta.name)}"><div class="cw237-bubble">${handover?`<strong>Handover from ${esc(meta.name)} · ${esc(meta.label)}</strong>\n`:''}${esc(row.text)}<div class="cw237-meta"><b>${esc(meta.name)}</b>${row.provider?`<span>${esc(row.provider)}${row.model?` · ${esc(row.model)}`:''}</span>`:''}${Number.isFinite(neurons)&&neurons>0?`<span>${Math.floor(neurons)} neurons</span>`:''}${Number.isFinite(turns)?`<span>≈${Math.max(0,Math.floor(turns))} chats left</span>`:''}${handover?'<span>handover</span>':''}</div>${gateMarkup(row)}</div></article>`}).join('')}
...
async function fallbackReply(system,text){
  ...
  if(typeof runtime?.generate==='function'){
    try{
      ...
      const answer=clean(result?.outputText,10000);if(answer)return{text:answer,provider:result?.actual?.provider||result?.fallback?.provider||result?.provider||'shared-model',model:result?.actual?.model||result?.model||'',usage:result?.usage||null}
    }catch{}
  }
...
async function submitActive(text){
  ...
  try{
    ...
    const thread=readThread(system),index=(thread.messages||[]).findIndex(row=>row.id===pendingId),next=clean(result?.response?.choice?.nextAction,1200),replacement={role:'assistant',guide:system,responderSystem:system,text:[clean(result?.response?.answer,10000),next?`Next: ${next}`:''].filter(Boolean).join('\n\n'),provider:result?.provider,model:result?.model,chargedNeurons:Number(result?.usage?.chargedNeurons)||0,remainingNeurons:result?.usage?.remainingNeurons!=null&&Number.isFinite(Number(result.usage.remainingNeurons))?Number(result.usage.remainingNeurons):null,approximateTurnsLeft:result?.usage?.approximateTurnsLeft!=null&&Number.isFinite(Number(result.usage.approximateTurnsLeft))?Number(result.usage.approximateTurnsLeft):null,approvalGate:result?.response?.approvalGate||null,planSnapshot:result?.plan?clone(result.plan):null,actionSnapshot:result?.action?clone(result.action):null};if(index>=0)thread.messages[index]=replacement;else thread.messages.push(replacement);writeThread(system,thread);const target=explicitHandoffTarget(result,system);if(target)await realmApi()?.createHandover?.(system,target,result)
  }catch{
    const fallback=await fallbackReply(system,value),thread=readThread(system),index=(thread.messages||[]).findIndex(row=>row.id===pendingId),replacement={role:'assistant',guide:system,responderSystem:system,text:fallback.text,provider:fallback.provider,model:fallback.model||'',chargedNeurons:Number(fallback?.usage?.chargedNeurons)||0,remainingNeurons:fallback?.usage?.remainingNeurons!=null&&Number.isFinite(Number(fallback.usage.remainingNeurons))?Number(fallback.usage.remainingNeurons):null,approximateTurnsLeft:fallback?.usage?.approximateTurnsLeft!=null&&Number.isFinite(Number(fallback.usage.approximateTurnsLeft))?Number(fallback.usage.approximateTurnsLeft):null,recoveredBy:'guide-workspace-v250'};if(index>=0)thread.messages[index]=replacement;else thread.messages.push(replacement);writeThread(system,thread)
...
function start(){pageSystem=detectSystem();const saved=readWorkspace();activeWindow=pageSystem;workspaceOpen=saved.open;minimized=saved.minimized;installStyle();retireViewportTrap();ensureRoot();installApi();document.addEventListener('pointerdown',onPointerDownCapture,true);document.addEventListener('click',onClickCapture,true);document.addEventListener('submit',onSubmitCapture,true);addEventListener('civweave:realm-guide-thread-changed',onThreadChanged);addEventListener('civweave:ai-neuron-usage',()=>queueMicrotask(()=>render({keepScroll:true})));addEventListener('civweave:host-node-health',()=>queueMicrotask(()=>render({keepScroll:true})));bodyObserver=new MutationObserver(onBodyChange);bodyObserver.observe(document.body,{childList:true});globalThis.visualViewport?.addEventListener('resize',onResize,{passive:true});addEventListener('resize',onResize,{passive:true});onResize();render();document.documentElement.dataset.civweaveGuideWorkspace='v250-canonical-owner';dispatchEvent(new CustomEvent('civweave:guide-workspace-ready',{detail:{version:VERSION,pageSystem,activeWindow,realmIsolated:true,windowCount:5,gestureSafe:true,sendSafe:true,canonicalOwner:true,transportFallback:true,neuronTelemetry:true}}))}
