...
function readWorkspace(){
  ...
  return{open:Boolean(value.open),minimized:Boolean(value.minimized),activeWindow:SYSTEMS.includes(value.activeWindow)?value.activeWindow:null,seen:value.seen&&typeof value.seen==='object'?value.seen:{}};
}
...
function renderVisuals(){
  ensureRoot();const guide=GUIDE[activeWindow],pageGuide=GUIDE[pageSystem];if(!guide||!pageGuide)return;root.dataset.guide=activeWindow;root.dataset.pageSystem=pageSystem;root.style.setProperty('--guide-accent',guide.accent);root.style.setProperty('--guide-panel',guide.panel);
  const avatar=root.querySelector('[data-guide-avatar]'),label=root.querySelector('[data-window-label]'),name=root.querySelector('[data-guide-name]'),role=root.querySelector('[data-guide-role]'),input=root.querySelector('textarea');if(avatar){avatar.src=guide.avatar;avatar.alt=guide.name}if(label)label.textContent=`${guide.label} THREAD${activeWindow===pageSystem?' · HERE':' · WINDOW'}`;if(name)name.textContent=guide.name;if(role)role.textContent=`${guide.role} · ${guide.label}`;if(input)input.placeholder=`Message ${guide.name}`;
...
function renderMessages(){
  ensureRoot();const log=root.querySelector('[data-log]'),thread=readThread(activeWindow),guide=GUIDE[activeWindow];if(!log)return;const rows=Array.isArray(thread.messages)?thread.messages:[];
...
async function submitActive(text){
  const value=clean(text,12000),system=activeWindow;if(!value||busy||!SYSTEMS.includes(system))return false;busy=true;ensureRoot();const button=root.querySelector('[data-send]');if(button)button.disabled=true;
...
function onSubmitCapture(event){const form=event.target?.closest?.(`#${ROOT_ID} [data-persistent-form]`);if(!form)return;event.preventDefault();event.stopImmediatePropagation();const input=form.querySelector('textarea'),text=clean(input?.value,12000);if(text)submitActive(text)}
