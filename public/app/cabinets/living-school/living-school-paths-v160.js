(()=>{
'use strict';
const VERSION='1.0.5-living-school-paths-v160-stable-controls';
...
function dispatchState(next,oldValue,text){
  ...
  try{dispatchEvent(new CustomEvent('living-school:state-replaced',{detail:{state:next}}))}catch{}
  return false;
}
function writeState(next){const oldValue=(()=>{try{return localStorage.getItem(STATE_KEY)||''}catch{return''}})(),text=JSON.stringify(next);try{localStorage.setItem(STATE_KEY,text)}catch(error){throw new Error(`Living School could not save this path: ${error.message}`)}return dispatchState(next,oldValue,text)}
...
function toast(message){const node=document.querySelector?.('#toast');if(!node)return;node.textContent=clean(message,1000);node.hidden=false;clearTimeout(node._ls160);node._ls160=setTimeout(()=>node.hidden=true,5000)}
...
function activatePath(id){
  ...
  lib.activePathId=id;lib.snapshots[id]=next;saveLibrary(lib);const stateSynced=writeState(next);
  if(stateSynced)globalThis.LivingSchoolCabinetV151?.setRoom?.('desk');
  globalThis.LivingSchoolWorkbenchV158?.render?.();
  queuePathbar();toast(`Using learning path: ${path.title}`);return true;
}
...
async function generate(form){
  ...
  try{
    ...
    document.querySelector?.('#instrument-dialog')?.close?.();globalThis.LivingSchoolWorkbenchV158?.render?.();queuePathbar();frame(()=>viewCurriculum());
...
function viewCurriculum(){
  ...
  if(globalThis.LivingSchoolCabinetV151?.setRoom)globalThis.LivingSchoolCabinetV151.setRoom('map');else writeState({...current,room:'map'});
  globalThis.LivingSchoolWorkbenchV158?.render?.();queuePathbar();
  frame(()=>frame(()=>document.querySelector?.('.lsw-course-head,.lsw-reader,#ls-generated-workbench')?.scrollIntoView?.({behavior:'auto',block:'start'})));
  return true;
}
...
function handleClick(event){
  const target=event.target?.closest?.('[data-ls160-use],[data-ls160-view],[data-ls160-generate],[data-lsw-action="forge"],[data-lsw-action="map"]');if(!target)return;
  event.preventDefault();event.stopPropagation();
  if(target.matches('[data-ls160-use]'))return runAction('Use path',()=>activatePath(selectedPathId()));
  if(target.matches('[data-ls160-view],[data-lsw-action="map"]'))return runAction('View curriculum',viewCurriculum);
