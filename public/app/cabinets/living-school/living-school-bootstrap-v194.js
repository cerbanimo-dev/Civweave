...
const VERSION='living-school-bootstrap-v202-image-first';
...
function loading(message='Opening the image-native learning engine from the installed package…'){const target=room();if(!target)return;installStyle();target.innerHTML=`<section class="ls-boot-v194 is-loading" role="status"><small>LIVING SCHOOL STARTUP</small><h1>Preparing Pathway Desk</h1><p>${esc(message)}</p></section>`}
function emitReady(reason){document.dispatchEvent(new CustomEvent('commonweave:living-school-ready',{detail:{version:VERSION,attempt,reason}}))}
function failure(error){settled=true;mark('failed',error?.message||'unknown');const target=room();if(!target)return;installStyle();const message=String(error?.message||error||'The learning engine did not finish loading.');target.innerHTML=`<section class="ls-boot-v194" role="alert"><small>LOCAL STARTUP RECOVERY</small><h1>Living School could not finish opening.</h1><p>The illustrated room and five family buttons remain available. Optional learning enhancements were not started.</p><p><code>${esc(message)}</code></p><div class="ls-boot-v194-actions"><button type="button" data-ls-boot-retry>Retry startup</button><button type="button" data-ls-boot-reset>Back up and reset Living School state</button></div></section>`;target.querySelector('[data-ls-boot-retry]')?.addEventListener('click',()=>boot('manual-retry'));target.querySelector('[data-ls-boot-reset]')?.addEventListener('click',()=>{try{const value=localStorage.getItem(STATE_KEY);if(value)localStorage.setItem(`${STATE_KEY}.recovery-backup.${Date.now()}`,value);localStorage.removeItem(STATE_KEY);sessionStorage.setItem('commonweave.living-school.reset.v202','1')}catch{}location.reload()})}
function moduleTimeout(){return new Promise((_,reject)=>setTimeout(()=>reject(new Error(`The installed learning-engine request did not respond within ${MODULE_TIMEOUT_MS/1000} seconds.`)),MODULE_TIMEOUT_MS))}
function frame(){return new Promise(resolve=>(globalThis.requestAnimationFrame||setTimeout)(resolve))}
async function firstVisualPaint(){
  await frame();await frame();
  const image=document.getElementById('ls-world-art');
  if(!image?.decode)return;
  await Promise.race([image.decode().catch(()=>{}),new Promise(resolve=>setTimeout(resolve,800))]);
  await frame();
}
async function loadModule(){
  const promise=import(`${MODULE}?v=${VERSION}&attempt=${attempt}`);
  lateModulePromise=promise;
  promise.then(()=>{if(!settled){settled=true;mark('ready','late-module-ready');emitReady('late-module-ready')}}).catch(()=>{});
  return Promise.race([promise,moduleTimeout()]);
}
async function boot(reason='initial'){
  attempt+=1;settled=false;mark('loading',reason);loading(attempt>1?'Retrying the local learning engine…':'Opening the image-native learning engine from the installed package…');
  const slow=setTimeout(()=>{if(!settled)loading('The core learning engine is still pending. The illustrated shell remains interactive while Commonweave waits.')},3000);
  try{
    await loadModule();
    settled=true;clearTimeout(slow);mark('ready',reason);emitReady(reason);
  }catch(error){clearTimeout(slow);console.error('[Living School] startup failed',error);failure(error)}
}

async function start(){
  await firstVisualPaint();
  if(globalThis.LivingSchoolCabinetV151){settled=true;mark('ready','already-loaded');emitReady('already-loaded');return}
  boot('page-load');
}
