...
function safeRecoveryRequested(){if(params.get('recovery')==='safe')return true;try{return sessionStorage.getItem(SAFE_KEY)==='1'}catch{return false}}
...
function versionFromManifest(manifest){const named=String(manifest?.name||'').match(/\bv(\d+\.\d+\.\d+)\b/i)?.[1];if(named)return named;try{return semver(new URL(manifest?.start_url||'',location.origin).searchParams.get('version'))}catch{return''}}
async function resolveReleaseVersion(){
  ...
  const explicit=semver(scriptUrl?.searchParams.get('v'))||semver(params.get('version'));if(explicit)return explicit;
  const controller=typeof AbortController==='function'?new AbortController():null;const timer=controller?setTimeout(()=>controller.abort(),RELEASE_TIMEOUT_MS):null;
  try{const response=await bounded(fetch(`/app/manifest.webmanifest?boot=${Date.now()}`,{cache:'no-store',signal:controller?.signal}),RELEASE_TIMEOUT_MS,'release manifest');if(response.ok){const resolved=versionFromManifest(await bounded(response.json(),700,'release manifest parse'));if(resolved)return resolved}}catch{}finally{if(timer)clearTimeout(timer)}
  return FALLBACK_VERSION;
}
function waitForControllerChange(timeout=1600){return new Promise(resolve=>{let settled=false;const finish=()=>{if(settled)return;settled=true;clearTimeout(timer);navigator.serviceWorker?.removeEventListener?.('controllerchange',finish);resolve(true)};const timer=setTimeout(finish,timeout);navigator.serviceWorker?.addEventListener?.('controllerchange',finish,{once:true})})}
async function refreshWorker(releaseVersion){
  if(!('serviceWorker'in navigator))return null;
  const ui=recoveryUi();
  try{
    const workerUrl=`/service-worker-v203.js?v=${encodeURIComponent(releaseVersion)}-direct-page-shell-v1&revision=boot-recovery-v429-direct-shell`;
    ...
    if(candidate){const activate=()=>{try{candidate.postMessage({type:'SKIP_WAITING'})}catch{}};if(candidate.state==='installed')activate();else candidate.addEventListener('statechange',()=>{if(candidate.state==='installed')activate()});await waitForControllerChange(1600)}
    return registration;
...
function ensureRoutes(releaseVersion){
  ...
  const routeUrl=`/app/system-routes-v227.js?v=${encodeURIComponent(releaseVersion)}-five-system-route-contract-v228-direct-shell`;
  return bounded(new Promise((resolve,reject)=>{
    ...
    if(existing){if(globalThis.CivweaveSystemRoutesV227){resolve(globalThis.CivweaveSystemRoutesV227);return}existing.addEventListener('load',ready,{once:true});existing.addEventListener('error',()=>reject(new Error('Route contract failed to load.')),{once:true});return}
...
function fallbackDestination(releaseVersion){
  const destination=new URL('/app/working-campus-v156.html',location.origin);destination.searchParams.set('installed','1');destination.searchParams.set('version',releaseVersion);destination.searchParams.set('navigation','five-system-route-contract-v228-direct-shell-fallback');destination.searchParams.set('launch','installed-entry-v430-direct-shell');if(safeRecoveryRequested())destination.searchParams.set('recovery','safe');return destination;
}
function directDestination(routes,system,releaseVersion){
  const target=routes?.routeFor?.(system)?system:'civweave';
  const destination=routes?.urlFor?.(target,{origin:location.origin,version:releaseVersion,source:'installed-entry-v430-direct-shell',developer:localDeveloper(),direct:true})||fallbackDestination(releaseVersion);
...
async function boot(){
  ...
  const ui=recoveryUi();ui?.setStatus?.('Checking this release’s consent requirements…');await ensureLegalConsent();authorize();ui?.setStatus?.('Reading the installed release…');
  const releaseVersion=await bounded(resolveReleaseVersion(),RELEASE_TIMEOUT_MS+350,'release resolution').catch(()=>FALLBACK_VERSION);await refreshWorker(releaseVersion);
  const requested=params.get('system')||params.get('target')||'civweave';const aliases={hub:'civweave',cabinet:'civweave',cabinets:'civweave',cabinetonly:'civweave',lite:'civweave'};const system=aliases[requested]||requested;ui?.setStatus?.('Opening the local campus…');
  let destination;try{destination=directDestination(await ensureRoutes(releaseVersion),system,releaseVersion)}catch{destination=fallbackDestination(releaseVersion)}
  ui?.markRouted?.();location.replace(destination.href);
}
boot().catch(error=>{console.error('[Civweave] Installed bootstrap recovery caught a launch failure.',error);recoveryUi()?.showRecovery?.(`The normal startup path stopped before the campus opened: ${error?.message||error}`)});
globalThis.CivweaveInstalledEntryV146=Object.freeze({version:'1.0.163-boot-recovery-v430-direct-shell',installedDisplay,launchSession,explicitInstalled,installedLaunchAuthorized,resolveReleaseVersion,refreshWorker,safeRecoveryRequested,ensureLegalConsent,installerUrl,directDestination,iframeShell:false,browserRuntimePolicy:'installed-display-or-pwa-launch-session'});
})();
