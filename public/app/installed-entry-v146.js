...
const releaseVersion=scriptUrl?.searchParams.get('v')||params.get('version')||'1.0.9';
const workerUrl=`/service-worker-v203.js?v=${encodeURIComponent(releaseVersion)}-lightweight-shell-v208&revision=release-coherence-v220`;
...
async function boot(){
  await Promise.race([refreshWorker(),new Promise(resolve=>setTimeout(resolve,1500))]);
