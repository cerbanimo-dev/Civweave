...
let guildId='',gateway='',timer=0,unsubscribe=null,processing=false,syncMs=DEFAULT_SYNC_MS,advertised=false;
...
async function tick(){const eligibility=CivweaveEmergencyAiHostV1.status();if(eligibility.eligible){await publishCapability().then(()=>{advertised=true}).catch(()=>{})}else if(advertised){await publishCapability({forceUnavailable:true}).catch(()=>{});advertised=false}await processIncoming().catch(()=>{});await sync().catch(()=>{});return status()}
...
export function status(){const host=CivweaveEmergencyAiHostV1.status();return Object.freeze({schema:EMERGENCY_AI_MESH_SCHEMA,started:Boolean(timer),guildId:guildId||null,gateway:gateway||null,syncIntervalMs:syncMs,maxQueueDepth:MAX_QUEUE_DEPTH,advertised,host})}
