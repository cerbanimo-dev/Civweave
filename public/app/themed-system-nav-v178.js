...
const VERSION='1.0.45-five-system-navigation-v227';
...
function href(item){return ROUTES?.urlFor?.(item.id,{version:'1.0.45',source:currentSystem()||'navigation'}).href||item.fallback}
...
function installStyle(){
  ...
  style.id=STYLE_ID;
  style.textContent=`
:root{--cw-themed-nav-height:clamp(52px,7vw,72px);--cw-themed-nav-button-width:156px}
...
@media(max-width:680px){:root{--cw-themed-nav-height:clamp(50px,14vw,66px);--cw-themed-nav-button-width:132px}}
...
function mount(){
  ...
  nav.addEventListener('click',event=>{
    ...
    ROUTES.navigate(link.dataset.system,{version:'1.0.45',source:current});
  });
