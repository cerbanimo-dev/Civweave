...
function href(item){return ROUTES?.urlFor?.(item.id,{version:'1.0.117',source:currentSystem()||'navigation'}).href||item.fallback}
...
function mount(){
  ...
  nav.addEventListener('click',event=>{
    ...
    ROUTES.navigate(link.dataset.system,{version:'1.0.117',source:current});
  });
