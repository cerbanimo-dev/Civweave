...
const VERSION='1.0.116-five-system-navigation-v227-shell-v304';
...
function href(item){return ROUTES?.urlFor?.(item.id,{version:'1.0.116',source:currentSystem()||'navigation'}).href||item.fallback}
...
#${NAV_ID}::before{content:"";position:absolute;inset:-2px max(0px,calc((100vw - (5 * var(--cw-themed-nav-button-width))) / 2)) -2px;border:1px solid #d6ab4f88;border-radius:14px;pointer-events:none;box-shadow:0 6px 18px #0007}
...
html[data-civweave-system="living-school"] #cw-shared-guide-surface-v236 .cwsg236-note{line-height:1.25!important}
...
@media(max-width:680px){:root{--cw-themed-nav-height:clamp(45px,12.6vw,59.4px);--cw-themed-nav-button-width:132px;--cw-themed-nav-bottom-gap:4px}#${NAV_ID}::before{inset:-2px 6px -2px}html[data-civweave-system="living-school"] #cw-shared-guide-surface-v236 .cwsg236-log{max-height:84px!important}}
...
function mount(){
  ...
  nav.addEventListener('click',event=>{
    ...
    ROUTES.navigate(link.dataset.system,{version:'1.0.116',source:current});
  });
