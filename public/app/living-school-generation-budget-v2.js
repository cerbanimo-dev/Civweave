(()=>{
'use strict';
const VERSION='2.0.0-living-school-generation-budget-v2-terminal-owner';
...
const stats={designCalls:0,duplicateDesignCallsBlocked:0,designTimeouts:0,structureCompiles:0,quizCallsBlocked:0,repairCallsBlocked:0,depthCallsBlocked:0,filteredSourceNoise:0,installCount:0,installedAt:'',lastBlockedAt:''};
...
function boundedRequest(request,tier='small'){
  ...
  return{...prepared,config,maxRepairAttempts:0,taskTier:tier,executionProfile:'interactive',context:{...(prepared?.context||{}),automaticRepairBudget:0,generationBudgetRevision:VERSION,designProviderCallsMax:1,postDesignProviderCallsMax:0,...(tier==='complex'?{designTimeoutMs:DESIGN_TIMEOUT_MS}: {})}};
}
...
function incompleteDesignResult(result,check){
  ...
  return{...result,status:'invalid-response',error:{code:'LIVING_SCHOOL_SINGLE_DESIGN_PACKET_INCOMPLETE',message},diagnostics:[...(result?.diagnostics||[]),message],livingSchoolDesignCompleteness:{revision:VERSION,complete:false,issues:check.issues,retried:false,designProviderCalls:1}};
}
...
async function runOneDesignCall(original,request){
  ...
  return{...result,livingSchoolDesignCompleteness:{revision:VERSION,complete:true,retried:false,moduleCount:count,designProviderCalls:1},diagnostics:[...(result.diagnostics||[]),'Living School completed its single Gemini 3.7 design call. No automatic strong-model retry is permitted.']};
}
...
function install(){
  ...
  try{dispatchEvent(new CustomEvent('civweave:living-school-generation-budget-ready',{detail:{version:VERSION,designProviderCallsMax:1,designTimeoutMs:DESIGN_TIMEOUT_MS,designExecutionProfile:'interactive',postDesignProviderCalls:0,structureCompiler:'grounded-design-compiler-v338',quizCompiler:'assessment-curator-v338',terminalOwner:true,installCount:1,at:stats.installedAt}}))}catch{}
  return true;
}
...
const api=Object.freeze({version:VERSION,install,designPacketCheck,looksLikeIndexNoise,compileStructure,resetRunBudget,get installed(){return installed},get runtime(){return wrappedRuntime},stats:()=>({...stats}),designProviderCallsMax:1,postDesignProviderCallsMax:0});
