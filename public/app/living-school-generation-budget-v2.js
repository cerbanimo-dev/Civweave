...
const FALLBACK_DESIGN_MODEL='gemini-3.5-flash';
const DESIGN_TIMEOUT_MS=360000;
const DESIGN_MAX_TOKENS=16384;
const MIN_LESSON_WORDS=120;
const stats={designCalls:0,duplicateDesignCallsBlocked:0,designTimeouts:0,wrongModelDesignsRejected:0,fallbackDesignsAccepted:0,structureCompiles:0,quizCallsBlocked:0,repairCallsBlocked:0,depthCallsBlocked:0,filteredSourceNoise:0,installCount:0,installedAt:'',lastBlockedAt:''};
...
function publish(){
  try{
    ...
    root.dataset.livingSchoolDesignCallsMax='2';
    ...
    root.dataset.livingSchoolFallbackDesignsAccepted=String(stats.fallbackDesignsAccepted);
...
function boundedRequest(request,tier='small'){
  ...
  return{...prepared,config,maxRepairAttempts:0,taskTier:tier,executionProfile:'interactive',context:{...(prepared?.context||{}),automaticRepairBudget:0,generationBudgetRevision:VERSION,designProviderCallsMax:2,postDesignProviderCallsMax:0,expectedDesignModel:tier==='complex'?EXPECTED_DESIGN_MODEL:undefined,fallbackDesignModel:tier==='complex'?FALLBACK_DESIGN_MODEL:undefined,...(tier==='complex'?{designTimeoutMs:DESIGN_TIMEOUT_MS}: {})}};
}
...
function accepted503Fallback(result={}){
  const fallback=result?.fallback&&typeof result.fallback==='object'?result.fallback:{};
  return actualDesignModel(result)===FALLBACK_DESIGN_MODEL&&fallback.used===true&&lower(fallback.fromModel)===EXPECTED_DESIGN_MODEL&&lower(fallback.toModel)===FALLBACK_DESIGN_MODEL&&lower(fallback.reason)==='http-503-high-demand';
}
function designProviderCallCount(result={}){return accepted503Fallback(result)?2:1}
function incompleteDesignResult(result,check){
  const calls=designProviderCallCount(result),fallback=accepted503Fallback(result);
  const message=`Living School stopped after its bounded design ${fallback?'fallback sequence':'call'} because the packet was incomplete: ${check.issues.join('; ')}.`;
  return{...result,status:'invalid-response',outputText:'',outputJson:null,error:{code:'LIVING_SCHOOL_SINGLE_DESIGN_PACKET_INCOMPLETE',message},diagnostics:[...(result?.diagnostics||[]),message],livingSchoolDesignCompleteness:{revision:VERSION,complete:false,issues:check.issues,retried:fallback,designProviderCalls:calls,model:actualDesignModel(result)}};
}
function designTimeoutResult(request){
  const message=`Living School stopped the Gemini 3.7 design call after ${Math.round(DESIGN_TIMEOUT_MS/1000)} seconds. HTTP 503/high-demand fallback is not triggered by a client-side timeout.`;
...
function wrongDesignModelResult(result){
  ...
  const message=`Living School rejected the design result because curriculum design must start on ${EXPECTED_DESIGN_MODEL}; ${FALLBACK_DESIGN_MODEL} is accepted only as the recorded one-time HTTP 503/high-demand fallback from ${EXPECTED_DESIGN_MODEL}. The runtime returned ${actual}.`;
  return{...result,status:'provider-error',outputText:'',outputJson:null,error:{code:'LIVING_SCHOOL_STRONG_DESIGN_MODEL_MISMATCH',message},diagnostics:[...(result?.diagnostics||[]),message],livingSchoolGenerationBudget:{...(result?.livingSchoolGenerationBudget||{}),version:VERSION,designProviderCalls:designProviderCallCount(result),expectedModel:EXPECTED_DESIGN_MODEL,fallbackModel:FALLBACK_DESIGN_MODEL,actualModel:actual,fallbackRejected:true}};
}
async function runOneDesignCall(original,request){
  ...
  const fallback=accepted503Fallback(result),actual=actualDesignModel(result);
  if((actual!==EXPECTED_DESIGN_MODEL||result?.fallback?.used===true)&&!fallback)return wrongDesignModelResult(result);
  if(fallback){stats.fallbackDesignsAccepted+=1;publish()}
  ...
  const calls=fallback?2:1;
  return{...result,livingSchoolDesignCompleteness:{revision:VERSION,complete:true,retried:fallback,moduleCount:count,designProviderCalls:calls,model:actual,fallbackReason:fallback?'http-503-high-demand':''},diagnostics:[...(result.diagnostics||[]),fallback?'Living School accepted Gemini 3.5 Flash only after the primary Gemini 3.7 Flash design call returned HTTP 503/high demand.':'Living School completed its primary Gemini 3.7 Flash design call without fallback.']};
}
...
function install(){
  ...
  try{dispatchEvent(new CustomEvent('civweave:living-school-generation-budget-ready',{detail:{version:VERSION,designProviderCallsMax:2,designTimeoutMs:DESIGN_TIMEOUT_MS,designExecutionProfile:'interactive',expectedDesignModel:EXPECTED_DESIGN_MODEL,fallbackDesignModel:FALLBACK_DESIGN_MODEL,fallbackReason:'http-503-high-demand',postDesignProviderCalls:0,structureCompiler:'grounded-design-compiler-v338',quizCompiler:'assessment-curator-v338',terminalOwner:true,installCount:1,at:stats.installedAt}}))}catch{}
  return true;
}
...
const api=Object.freeze({version:VERSION,install,designPacketCheck,looksLikeIndexNoise,compileStructure,resetRunBudget,get installed(){return installed},get runtime(){return wrappedRuntime},stats:()=>({...stats}),designProviderCallsMax:2,postDesignProviderCallsMax:0,expectedDesignModel:EXPECTED_DESIGN_MODEL,fallbackDesignModel:FALLBACK_DESIGN_MODEL,fallbackReason:'http-503-high-demand',minLessonWords:MIN_LESSON_WORDS});
