...
const POLICY_PRESETS=Object.freeze({
  ...
  'learning-path':{label:'Learning Path',budgetBytes:448*1024*1024,autoPrefetch:true,maxAutomaticItemBytes:48*1024*1024},
...
export function isRevoked(value={}){const key=typeof value==='string'?clean(value,500):recordKey(value);const hash=typeof value==='object'?clean(value?.contentHash||value?.content_hash,128):'';return Boolean((key&&revokedRecordKeys.has(key))||(hash&&revokedContentHashes.has(hash)))}
...
export async function cacheRecord(record,{automatic=false,pinned=false,force=false}={}){
  ...
  try{
    const response=await fetch(file.url,{cache:'no-store'});if(!response.ok||!response.body)throw new Error(`Media origin returned ${response.status}.`);const originType=clean(response.headers.get('content-type'),160).toLowerCase();if(originType&&!originType.startsWith('video/')&&originType!=='application/octet-stream')throw new Error(`Media origin returned non-video content type ${originType}.`);
...
export async function resolveOpenMedia(query,{schoolSlug='',topicSlug='',automaticCache=true}={}){
  ...
  if(automaticCache&&automaticNetworkAllowed()){const policy=await storagePolicy();if(policy.autoPrefetch&&Number(file.bytes||0)<=Number(policy.maxAutomaticItemBytes||AUTOMATIC_ITEM_MAX_BYTES))cacheRecord(best,{automatic:true}).catch(error=>emit('automatic-cache-skipped',{recordKey:best.recordKey,error:error.message}))}
...
function compactFileBytes(record){const files=(record?.files||[]).filter(file=>file?.url&&playableFile(file)).map(file=>Number(file?.bytes)||0).filter(bytes=>bytes>0);return files.length?Math.min(...files):Number.MAX_SAFE_INTEGER}
...
async function handleBinary(session,data){for(const transfer of incomingTransfers.values()){if(transfer.sessionId!==session.id||!transfer.pendingBinary)continue;const bytes=data instanceof Uint8Array?data:new Uint8Array(data);transfer.pendingBinary=false;if(transfer.received+bytes.byteLength>transfer.expectedBytes){const error=new Error('Peer sent more media bytes than advertised.');incomingTransfers.delete(transfer.transferId);await transfer.writer.abort(error).catch(()=>{});transfer.putPromise.catch(()=>{});try{const cache=await mediaCache();await cache.delete(syntheticRequest(transfer.record.recordKey))}catch{}resolveTransferWaiter(transfer.expectedHash,error);emit('mesh-receive-error',{recordKey:transfer.record.recordKey,error:error.message});return true}transfer.hasher.update(bytes);transfer.received+=bytes.byteLength;await transfer.writer.write(bytes);return true}return false}
async function handleMediaMessage(session,data){if(typeof data!=='string'){await handleBinary(session,data);return}let message;try{message=JSON.parse(data)}catch{return}if(!message?.type?.startsWith?.('cw-media-'))return;if(message.type==='cw-media-manifest'){peerInventory.set(session.peerId||session.id,{sessionId:session.id,at:now(),items:(message.items||[]).slice(0,512).filter(item=>item.recordKey&&item.contentHash&&licenseAllowed(item.license)&&!isRevoked(item))});emit('mesh-manifest',{peerId:session.peerId||session.id,count:message.items?.length||0});return}if(message.type==='cw-media-request'){const prior=peerSendChains.get(session.id)||Promise.resolve();const next=prior.catch(()=>{}).then(()=>sendMedia(session,message)).catch(error=>sendJson(session.channel,{type:'cw-media-reject',contentHash:message.contentHash,recordKey:message.recordKey,error:error.message})).finally(()=>{if(peerSendChains.get(session.id)===next)peerSendChains.delete(session.id)});peerSendChains.set(session.id,next);return}if(message.type==='cw-media-start'){await acceptStart(session,message);return}if(message.type==='cw-media-chunk'){const transfer=incomingTransfers.get(message.transferId);if(transfer&&transfer.sessionId===session.id)transfer.pendingBinary=true;return}if(message.type==='cw-media-end'){await finishTransfer(message.transferId,message);return}if(message.type==='cw-media-reject'){const waiter=transferWaiters.get(message.contentHash);if(waiter?.sessionId===session.id)resolveTransferWaiter(message.contentHash,new Error(message.error||'Peer rejected media request'));return}}
function attachSession(session){const channel=session?.channel;if(!channel||attachedChannels.has(channel))return;attachedChannels.add(channel);channel.binaryType='arraybuffer';channel.addEventListener('open',()=>sendManifest(session).catch(()=>{}));channel.addEventListener('message',event=>{const prior=peerReceiveChains.get(session.id)||Promise.resolve();const next=prior.catch(()=>{}).then(()=>handleMediaMessage(session,event.data)).catch(error=>emit('mesh-protocol-error',{error:error.message})).finally(()=>{if(peerReceiveChains.get(session.id)===next)peerReceiveChains.delete(session.id)});peerReceiveChains.set(session.id,next)});channel.addEventListener('close',()=>{if(session.peerId)peerInventory.delete(session.peerId)});if(channel.readyState==='open')sendManifest(session).catch(()=>{})}
...
export async function warmup(){if(warmPromise)return warmPromise;warmPromise=(async()=>{try{await Promise.all([loadLookup(),loadHarvestPolicy(),loadRevocations()])}catch(error){emit('catalog-unavailable',{error:error.message})}try{await openDb()}catch(error){emit('storage-unavailable',{error:error.message})}scanSessions();if(!meshTimer&&typeof setInterval==='function')meshTimer=setInterval(scanSessions,1000);if(!revocationTimer&&typeof setInterval==='function')revocationTimer=setInterval(()=>loadRevocations({force:true}).catch(()=>{}),6*60*60*1000);emit('ready',{revision:REVISION,status:await status().catch(()=>null)});return api})();return warmPromise}
