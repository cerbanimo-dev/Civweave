(()=>{
'use strict';
const VERSION='1.0.118-server-ai-settings-v306-community-share';
...
function commercePanel(){
  const section=document.createElement('section');section.id=COMMERCE_ID;section.className='cw-clean-panel';section.innerHTML=`<div><h3>Membership & compute</h3><p>Membership and top-up compute strengthen the active Cloudflare community while preserving each member's personal credits.</p></div><div class="cw-community-dividend-note">Every $5/month of active membership contributes +2 potential free seats and a +200 daily-neuron target for every member. Free seats cap at 16, total residents cap at 28, and neuron bonuses are automatically capped by the funds actually available.</div><div class="cw-server-commerce-grid"><div><b>Compute top-up</b><span>Prepay extra compute for the active host. Cloudflare top-ups share at least 5% with everyone, and you can raise that share to 10%.</span><label>Amount<select data-compute-amount><option value="500">${money(500)}</option><option value="1000">${money(1000)}</option><option value="2000">${money(2000)}</option><option value="5000">${money(5000)}</option></select></label><label>Community share<select data-community-share-bps><option value="500">5% shared</option><option value="600">6% shared</option><option value="700">7% shared</option><option value="800">8% shared</option><option value="900">9% shared</option><option value="1000">10% shared</option></select></label><label class="cw-community-share-row"><input type="checkbox" data-node-equal-topup><span>Top up the node equally</span></label><small class="cw-community-share-note" data-community-share-note>5% of this top-up's service value is ring-fenced for the shared compute pool. Your remaining system-backed compute stays personal.</small><button type="button" data-compute-buy>Add compute</button></div><div><b>Monthly membership</b><span>Paid members keep the same community allowance as everyone else, add personal lifetime credits, and raise the whole node's target allowance.</span><label>Tier<select data-membership-tier><option value="member">Member · $5/mo · +200/day for everyone · 100k personal</option><option value="maker">Maker · $10/mo · +400/day for everyone · 250k personal</option><option value="builder">Builder · $20/mo · +800/day for everyone · 600k personal</option><option value="steward">Steward · $40/mo · +1,600/day for everyone · 1.5m personal</option></select></label><small class="cw-community-share-note">Seat boosts scale at +2 potential free seats per $5 of membership until the 16-free-seat instance cap is reached.</small><button type="button" data-membership-buy>Subscribe</button></div></div><output class="cw-server-commerce-status" data-commerce-status role="status"></output>`;return section;
}
...
async function buyCompute(amountCents,{shareBps=500,shareMode='personal'}={}){
...
function syncSharingControls(panel,capacity){
  ...
  note.textContent=`${Math.max(5,Math.min(10,Number(share.value||500)/100))}% of this top-up's service value is ring-fenced for the shared compute pool. Your remaining system-backed compute stays personal.`;
}
function bindCommerce(form){
  ...
  panel.addEventListener('click',async event=>{
    ...
    try{
      if(topup){
        const equal=Boolean(panel.querySelector('[data-node-equal-topup]')?.checked),shareBps=Math.max(500,Math.min(1000,Number(panel.querySelector('[data-community-share-bps]')?.value)||500));
