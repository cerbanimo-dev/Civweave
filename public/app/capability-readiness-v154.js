...
const KEYS={intentions:'commonweave.intentions.v127',readiness:'commonweave.capability-readiness.v154'};
const SYSTEM_LABELS={commonweave:'Commonweave','living-school':'Living School',cerbanimo:'Cerbanimo',fellowfare:'FellowFare',anarchadia:'Anarchadia'};
const POSTURES={learn:{label:'Learn first',prepared:1,realm:'living-school'},practice:{label:'Practice while doing',prepared:.7,realm:'cerbanimo'},recruit:{label:'Recruit help',prepared:1,realm:'fellowfare'},simplify:{label:'Simplify scope',prepared:.8,realm:'anarchadia'},ready:{label:'Ready to use',prepared:0,realm:'commonweave'}};
...
const writeIntentions=value=>{localStorage.setItem(KEYS.intentions,JSON.stringify(value.slice(0,100)));try{dispatchEvent(new CustomEvent('commonweave:intentions-changed',{detail:{items:value}}))}catch{}return value};
const readStore=()=>{const value=parse(localStorage.getItem(KEYS.readiness),{schema:'commonweave.capability-readiness.v154',plans:{}});return{schema:'commonweave.capability-readiness.v154',plans:value?.plans&&typeof value.plans==='object'?value.plans:{}}};
...
function templates(plan){
  ...
  if(signal.game){out.push(
    ...
    capability('feedback','Impact observation and revision','commonweave',2,'Can you decide whether to repeat, revise, or stop using evidence?','A dated review with a next decision.')
  )}
    ...
    capability('outcome','Outcome and scope definition','commonweave',3,'Can you define an observable result and what is outside scope?','A bounded outcome with completion criteria.'),
...
function ensure(plan){
  ...
  const record={schema:'commonweave.capability-map.v154',planId:plan.id,title:plan.title||'Untitled intention',capabilities,metrics:metrics(capabilities),updatedAt:now()};
...
function persist(planId){
  ...
  if(located.item.state==='active')globalThis.CommonweaveCoreLoopV152?.activate?.(located.plan);
...
function lane(planId,system){const record=get(planId),caps=rows(record?.capabilities).filter(cap=>system==='commonweave'||cap.realm===system||POSTURES[cap.posture]?.realm===system);return{system,metrics:record?.metrics||metrics([]),capabilities:caps}}
...
function summaryMarkup(plan,record,system='commonweave'){
