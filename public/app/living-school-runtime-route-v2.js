(()=>{
'use strict';
const VERSION='2.0.0-living-school-runtime-route-v2-middleware-only';
...
function prepare(request={}){
  ...
  return{...request,purpose:DESIGN_PURPOSE,taskTier:'complex',executionProfile:'interactive',config:provider==='gemini'?{...config,provider:'gemini',route:'gemini',model:complexModel}:config,context:{...(request.context||{}),livingSchoolRuntimeRoute:VERSION,livingSchoolSingleStrongDesign:true},messages:[...(Array.isArray(request.messages)?request.messages:[]),{role:'system',content:boundary}]};
}
...
function install(){
  ...
  try{dispatchEvent(new CustomEvent('civweave:living-school-runtime-route-ready',{detail:{version:VERSION,mode:'middleware-only',singleStrongDesign:true,globalRuntimeWrapping:false,at:new Date().toISOString()}}))}catch{}
  return true;
}
...
install();
const api=Object.freeze({version:VERSION,install,prepare,sanitizeGroundedDesign,designRequest,bridge:true,mode:'middleware-only',singleStrongDesign:true});
