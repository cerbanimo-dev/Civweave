const REVISION='civweave-safe-mode-v1';
...
async function aiReview(originalGenerate,item,kind){
  ...
  const request={purpose:'civweave-safe-admission-review-v1',executionProfile:'interactive',schema:safetyReviewSchema(),temperature:0,messages:[{role:'system',content:'You are an independent safety admission reviewer. Classify the supplied material. Allow only general-audience, non-graphic, non-exploitative material that does not enable dangerous or illegal action. Educational context alone does not make actionable harmful instructions safe. Return JSON only.'},{role:'user',content:`KIND: ${kind}\nUNTRUSTED MATERIAL:\n${text}`}],context:{safeModeReview:true,kind,revision:REVISION}};
  try{const result=await originalGenerate(request),output=result?.outputJson,route=clean([result?.provider,result?.requestedProvider,result?.model].filter(Boolean).join(' '),500);if(result?.status!=='success'||!output)return{decision:'review',categories:['ai-review-unavailable'],confidence:0,reason:'AI review did not return a valid decision.',checker:'ai'};if(!route||/deterministic|local-contract|safe-policy/i.test(route))return{decision:'review',categories:['independent-ai-review-unavailable'],confidence:0,reason:'The active route did not prove that an independent AI performed this review.',checker:'ai'};return{decision:['allow','block','review'].includes(output.decision)?output.decision:'review',categories:Array.isArray(output.categories)?output.categories:[],confidence:Number(output.confidence)||0,reason:clean(output.reason,1000),safeSummary:clean(output.safeSummary,1200),checker:'ai',route}}catch(error){return{decision:'review',categories:['ai-review-unavailable'],confidence:0,reason:clean(error?.message||error,800),checker:'ai'}}
}
...
export async function validateAdmissions(originalGenerate,items,{kind='source'}={}){const rows=[];for(const item of(Array.isArray(items)?items:[]))rows.push({item,review:await validateAdmission(originalGenerate,item,{kind})});return rows}
...
function installRuntimeHarness(){
  ...
  const generate=async request=>{
    if(!readSafeMode().enabled||request?.purpose==='civweave-safe-admission-review-v1'||request?.context?.safeModeReview)return original(request);
