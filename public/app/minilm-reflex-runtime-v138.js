...
const MODES={civweave:'Reflect','living-school':'Learn',cerbanimo:'Build',fellowfare:'Acquire',anarchadia:'Govern'};
const LABELS={civweave:'Civweave','living-school':'Living School',cerbanimo:'Cerbanimo',fellowfare:'FellowFare',anarchadia:'Anarchadia'};
const GUIDES={civweave:{name:'Weaveling',role:'central mirror and orchestrator'},'living-school':{name:'Moss',role:'learning guide'},cerbanimo:{name:'Kamiya',role:'questwright and skilled-work guide'},fellowfare:{name:'Rook',role:'quartermaster and exchange guide'},anarchadia:{name:'Merlin',role:'civic and automation guide'}};
...
function routeByRules(text,current='civweave'){
  ...
  if(multi)return {system:'civweave',mode:'Reflect',confidence:.88,evidence:active.flatMap(([,items])=>items).slice(0,8),multiRealm:true};
  ...
  const system=canonicalSystem(current)||'civweave';return {system,mode:MODES[system],confidence:.35,evidence:[],multiRealm:false};
}
function extractContext(request){
  ...
  for(const item of messages.slice().reverse()){
    ...
    const parsed=parse(content);if(isObject(parsed)&&parsed.schema==='civweave.structured-context.v1')return parsed;
  }
  return {};
}
...
function conversationalReply(kind,system,context){
  const guide=GUIDES[system]||GUIDES.civweave,room=safe(context?.routingAnswer?.room||context?.currentContext?.roomId,200);let answer='',nextAction='';
  if(kind==='test'){answer=`Test received. I’m ${guide.name}, Civweave’s ${guide.role}. I have not turned that message into a ${system==='cerbanimo'?'quest':system==='living-school'?'lesson':system==='fellowfare'?'resource request':system==='anarchadia'?'proposal':'plan'}.`;nextAction=guidePrompt(system)}
  else if(kind==='identity'){answer=`I’m ${guide.name}, Civweave’s ${guide.role}. I’m software, not a person or a real boy. I can use the current conversation, local semantic matching, and Civweave’s canonical state without pretending to be alive.`;nextAction=system==='living-school'?'Ask something you genuinely want to understand.':guidePrompt(system)}
  else if(kind==='greeting'){answer=`Hello. I’m ${guide.name}, the ${guide.role} here.`;nextAction=system==='civweave'?'Tell me your wish in your own words.':`Tell me what needs attention in ${LABELS[system]}.`}
...
function matchedReply(entry,text,route,context,{semantic=false,device='lexical',score=0}={}){
  const system=route.system,intent=cleanIntent(text)||safe(text,500),guide=GUIDES[system]||GUIDES.civweave,room=safe(context?.routingAnswer?.room||context?.currentContext?.roomId,200),label=safe(entry?.label,180)||safe(entry?.id,180).replace(/-/g,' '),clarify=safe(entry?.clarify,500)||guidePrompt(system);
  ...
  if(route.multiRealm)answer+=` This spans more than one realm, so Civweave should coordinate the weave while each realm owns its part.`;
...
function clarifyReply(text,route,context,{semantic=false,device='lexical',score=0}={}){
  const system=route.system,guide=GUIDES[system]||GUIDES.civweave,room=safe(context?.routingAnswer?.room||context?.currentContext?.roomId,200),intent=cleanIntent(text)||safe(text,500);
...
function install(){
  const runtime=globalThis.CivweaveModelRuntime;if(!runtime?.generate||runtime.__minilmReflexInstalled)return false;const previous=runtime.generate.bind(runtime);
  ...
  globalThis.CivweaveModelRuntime={...runtime,generate,__minilmReflexInstalled:true};
  globalThis.CivweaveReflexRuntime={model:MODEL_ID,respond:(request,options)=>reflexResponse(request,options),route:routeByRules,status:async()=>{const engine=await adapter();return engine.status()},prewarm:async()=>{try{const engine=await adapter();const value=await engine.prewarm();semanticReady=true;return value}catch(error){return {ready:false,error:error.message}}},benchmark:async cases=>{const engine=await adapter();return engine.benchmark(cases)},semanticReady:()=>semanticReady};
  const prewarm=()=>globalThis.CivweaveReflexRuntime.prewarm();if('requestIdleCallback'in globalThis)requestIdleCallback(prewarm,{timeout:2500});else setTimeout(prewarm,800);return true;
}
