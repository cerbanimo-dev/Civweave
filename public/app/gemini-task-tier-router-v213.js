(()=>{
'use strict';
const VERSION='1.0.65-gemini-task-tier-router-v269-spine';
...
const MIDDLEWARE_ID='gemini-task-tier-v269';
...
function migrateStoredGeminiPolicy(){
  try{
    ...
    localStorage.setItem(PROFILES_KEY,JSON.stringify({...profiles,interactive:small,agentic:complex,agenticEnabled:true,geminiRouting:'capability-spine-v269'}));
    localStorage.setItem(SETTINGS_KEY,JSON.stringify({...saved,...small,consent:Boolean(small.externalConsent??saved.consent),agenticEnabled:true,geminiRouting:'capability-spine-v269'}));
    return true;
  }catch{return false}
}
...
function disclose(decision,model,request={}){
  ...
  notice.textContent=`Capability routing selected Gemini 3.5 Flash-Lite for ${decision.reason}. Routine requests use Gemini 3.1 Flash-Lite.`;notice.hidden=false;clearTimeout(noticeTimer);noticeTimer=setTimeout(()=>{if(notice?.isConnected)notice.hidden=true},7000);return detail;
}
function patchSettings(){
  ...
  if(gemini&&remote){if(!note){note=document.createElement('div');note.id='cw-gemini-routing-note-v213';note.innerHTML='<b>Capability-aware Gemini routing</b><span>Routine requests use Gemini 3.1 Flash-Lite. Planning, research, code generation, and agentic work use Gemini 3.5 Flash-Lite. Capability classification is provider-neutral; Gemini selection happens only after Gemini is the chosen provider.</span>';remote.insertBefore(note,remote.querySelector('.cw-clean-secret-row')||remote.lastElementChild)}note.hidden=false;}else if(note)note.hidden=true;
}
function middleware(){
  return{
    before(request,ctx){
