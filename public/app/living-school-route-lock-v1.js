(()=>{
'use strict';
const VERSION='1.4.0-living-school-route-lock-v1-single-strong-session';
...
function route(request={}){
  ...
  if(purpose===LIVE_RESEARCH)return request;
  const strong=design;
  return{
    ...request,
...
function postRouter(request={}){
  ...
  return{
    ...
    context:{...(request.context||{}),livingSchoolPostRouterBoundary:VERSION,singleStrongDesignProfile:'interactive'}
  };
}
function install(){
  ...
  try{dispatchEvent(new CustomEvent('civweave:living-school-route-lock-ready',{detail:{version:VERSION,priority:PRIORITY,postRouterPriority:POST_PRIORITY,liveResearchException:LIVE_RESEARCH,strongDesignPurpose:DESIGN,strongDesignModel:COMPLEX_MODEL,strongDesignProfile:'interactive',sessionWideFollowupDowngrade:true,at:new Date().toISOString()}}))}catch{}
  return true;
}
...
install();
globalThis.CivweaveLivingSchoolRouteLockV1=Object.freeze({version:VERSION,install,route,postRouter,priority:PRIORITY,postRouterPriority:POST_PRIORITY,liveResearchException:LIVE_RESEARCH,strongDesignPurpose:DESIGN,strongDesignModel:COMPLEX_MODEL,strongDesignProfile:'interactive',generationActive});
})();