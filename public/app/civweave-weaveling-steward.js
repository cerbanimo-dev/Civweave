...
'use strict';
const VERSION='1.3.0';
const KEY='civweave.weaveling.steward.v2';
...
function buildSubplans(s){const paths=[];if(s.knowledge?.length)paths.push({id:'learning',type:'learning',label:'Learning Path',realm:'living',status:'needs-attention',attention:true,steps:s.tasks.filter(x=>x.system==='living').map(x=>x.id),summary:'Close the knowledge and practice gaps that block confident action.'});if(s.tasks?.some(x=>x.system==='cerbanimo'))paths.push({id:'skilled-labor',type:'labor',label:'Skilled Labor Path',realm:'cerbanimo',status:'ready',attention:!paths.length,steps:s.tasks.filter(x=>x.system==='cerbanimo').map(x=>x.id),summary:'Shape, build, test, and complete the work with the right capabilities.'});if(s.materials?.length)paths.push({id:'materials',type:'materials',label:'Material Acquirement Path',realm:'fellowfare',status:'queued',attention:!paths.length,steps:s.tasks.filter(x=>x.system==='fellowfare').map(x=>x.id),summary:'Secure tools, materials, people, permissions, and access.'});return paths.slice(0,3)}
function inferNeeds(s){const gaps=s.skills.filter(x=>x.level<x.required);const knowledge=gaps.map(x=>({id:uid('know'),title:`Learn or verify ${x.name}`,detail:x.description,skillId:x.id,system:'living'}));
 ...
 const tasks=[
 {id:uid('task'),title:'Define the smallest complete outcome',detail:'Write a testable statement of what exists when this intention succeeds.',system:'civweave',status:'planned'},
 ...
 {id:uid('task'),title:'Release, reflect, and choose the next thread',detail:'Publish or deliver the result, record what changed, and decide what continues.',system:'civweave',status:'planned'}
 ];
 const tour=[
 {system:'civweave',label:'Civweave',purpose:'Clarify the intention, success conditions, constraints, capabilities, and route.'},
 ...
 {system:'civweave',label:'Civweave',purpose:'Monitor progress, preserve the chronicle, and transport the next active context.'}
 ];
 const next={...s,knowledge,materials,tasks,tour,stage:'plan',navigationMode:s.navigationMode||'guided'};next.subplans=buildSubplans(next);return next
}
async function enrichPlan(base){const runtime=window.CivweaveModelRuntime;if(!runtime?.generateInteractive)return base;try{const result=await runtime.generateInteractive({purpose:'weaveling-intention-tour-plan',responseFormat:'json',schema:{type:'object',properties:{title:{type:'string'},success:{type:'string'},knowledge:{type:'array',items:{type:'object',properties:{title:{type:'string'},detail:{type:'string'}},required:['title','detail']}},materials:{type:'array',items:{type:'object',properties:{title:{type:'string'},detail:{type:'string'}},required:['title','detail']}},tasks:{type:'array',items:{type:'object',properties:{title:{type:'string'},detail:{type:'string'},system:{type:'string',enum:['civweave','living','fellowfare','cerbanimo','anarchadia']}},required:['title','detail','system']}}},required:['title','success','knowledge','materials','tasks']},system:'You are Weaveling, Civweave’s humble intention steward. Produce a concise, complete, realistic, editable plan. Preserve user agency. Never imply that anything has been created or approved. Separate knowledge gaps, material/resource needs, and executable tasks. Route each task to the best Civweave realm.',prompt:JSON.stringify({intent:base.intent,success:base.success,constraints:base.constraints,skills:base.skills,deterministicDraft:{knowledge:base.knowledge,materials:base.materials,tasks:base.tasks}}),deterministic:()=>({title:base.title,success:base.success||`A demonstrable outcome exists for: ${base.intent}`,knowledge:base.knowledge,materials:base.materials,tasks:base.tasks})});const x=result.outputJson||{};return {...base,title:clean(x.title||base.title),success:clean(x.success||base.success),knowledge:(x.knowledge||base.knowledge).slice(0,16).map(v=>({...v,id:uid('know'),system:'living'})),materials:(x.materials||base.materials).slice(0,16).map(v=>({...v,id:uid('mat'),system:'fellowfare'})),tasks:(x.tasks||base.tasks).slice(0,30).map(v=>({...v,id:uid('task'),status:'planned'}))}}catch{return base}}
async function buildPlan(){let s=read();if(!s)return null;s=inferNeeds(s);s=await enrichPlan(s);s.subplans=buildSubplans(s);write(s);bus.dispatchEvent(new CustomEvent('changed',{detail:clone(s)}));return clone(s)}
function setNavigationMode(mode){const s=read();if(!s)return null;s.navigationMode=mode==='free'?'free':'guided';write(s);bus.dispatchEvent(new CustomEvent('changed',{detail:clone(s)}));return clone(s)}
...
function approve(){const s=read();if(!s||s.stage!=='plan')return null;const intention=window.CivweaveIntentionOrchestrator?.create?.({title:s.title,intent:s.intent,success:s.success,source:'weaveling',systems:[...new Set(s.tasks.map(x=>x.system))],constraints:s.constraints,subplans:s.subplans,navigationMode:s.navigationMode||'guided',steps:s.tasks.map((t,i)=>({id:t.id,title:t.title,system:t.system,status:'planned',priority:i+1,definitionOfDone:t.detail,notes:t.detail}))});s.stage='monitor';s.intentionId=intention?.id||s.activeIntentionId;s.approvedAt=now();write(s);window.CivweaveActionableQuad?.addMessage?.('assistant',`I am watching “${s.title}”. I will keep the next step visible and carry its approved context between realms.`,{intentionId:s.intentionId,weaveling:true});return {session:clone(s),intention}}
function status(){const s=read();const intention=(s?.intentionId&&window.CivweaveIntentionOrchestrator?.list?.().find(x=>x.id===s.intentionId))||window.CivweaveIntentionOrchestrator?.active?.();const explained=intention?window.CivweaveIntentionOrchestrator?.explain?.(intention):null;return {session:clone(s),intention:clone(intention),explained}}
function nextRealm(){const st=status(),next=st.explained?.next;return next?.system||st.session?.tour?.find(x=>x.system!=='civweave')?.system||'civweave'}
function transport(system){const s=read(),st=status();const target=system||nextRealm();const payload={schema:'civweave.weaveling-handoff.v1',source:'civweave',target,intentionId:st.intention?.id||s?.intentionId||null,title:st.intention?.title||s?.title||'Active intention',intent:st.intention?.intent||s?.intent||'',success:st.intention?.success||s?.success||'',skills:s?.skills||[],knowledge:s?.knowledge||[],materials:s?.materials||[],tasks:s?.tasks||[],next:st.explained?.next||null,approved:true,createdAt:now()};localStorage.setItem(`civweave.approved-handoffs.${target}.v1`,JSON.stringify([payload,...(()=>{try{return JSON.parse(localStorage.getItem(`civweave.approved-handoffs.${target}.v1`)||'[]')}catch{return[]}})()].slice(0,100)));window.CivweaveActionableQuad?.setRoute?.('square',target,`Continue “${payload.title}” in ${target}`);return payload}
async function directAnswer(query){const runtime=window.CivweaveModelRuntime;if(!runtime?.generateInteractive)return {answer:'I can help turn this into an intention, or we can continue the active intention. Tell me which outcome you are trying to reach.'};try{const r=await runtime.generateInteractive({purpose:'weaveling-direct-answer',prompt:query,system:'You are Weaveling, a flexible Civweave guide. Answer the user’s direct question clearly and briefly. When their query is actually about beginning or continuing an intention, orient them to the next concrete step rather than inventing a new intention.',deterministic:()=>({answer:'Start by naming the concrete outcome you want to exist. I will then map the skills, knowledge, materials, tasks, and realm-by-realm route.'})});return {answer:r.outputText||r.outputJson?.answer||'I can help you map the next step.'}}catch{return {answer:'Start by naming the concrete outcome you want to exist. I will then map the skills, knowledge, materials, tasks, and realm-by-realm route.'}}}
function interpret(input){const s=read(),q=clean(input);if(vague.test(q)&&s)return {kind:'resume',session:clone(s)};if(vague.test(q)&&window.CivweaveIntentionOrchestrator?.active?.())return {kind:'active',session:newSession(q)};if(question.test(q)&&q.length<180&&!/i want|i wish|my goal|build|make|create|start/.test(q.toLowerCase()))return {kind:'answer'};return {kind:'intake',session:newSession(q)}}
window.CivweaveWeavelingSteward={VERSION,bus,session,reset,set,setNavigationMode,interpret,answerSkill,prepareSkills,allSkillsAnswered,buildPlan,updateField,updateTask,addTask,removeTask,approve,status,nextRealm,transport,directAnswer};
})();
