...
const VERSION='1.0.5';
const REVISION='experience-orchestrator-v299-chat-boot-runtime-fallback';
...
const CHAT_MODULES=[
 ...
 ['/app/local-chat-runtime-v295.js?v=1.0.124-v312-runtime-first-bootstrap',()=>globalThis.CivweaveLocalChatRuntimeV295?.version==='1.0.117-local-chat-runtime-v305-mobile-bootstrap-recovery'&&globalThis.CivweaveLocalChatRuntimeV295?.revision==='v312-runtime-first-bootstrap'],
 ['/app/local-chat-owner-v295.js?v=1.0.115-v302',()=>globalThis.CivweaveLocalChatOwnerV295?.version==='1.0.115-local-chat-owner-v302'&&globalThis.CivweaveLocalChatOwnerV295?.intentPrewarm===true]
];
...
function status(){clearExpired();return Object.freeze({version:VERSION,revision:REVISION,active:activeSlot?{...activeSlot}:null,protectedFlow:protectedFlow(),settingsReady:Boolean(globalThis.CivweaveSettingsParityV295),chatReady:Boolean(globalThis.CivweaveLocalChatOwnerV295?.intentPrewarm),launchReady:Boolean(globalThis.CivweaveLocalChatOwnerV295?.intentPrewarm&&globalThis.CivweaveSettingsParityV295)})}

function appendScript(src,ready,kind){return new Promise((resolve,reject)=>{const s=document.createElement('script'),joiner=src.includes('?')?'&':'?';s.src=`${src}${joiner}v=1.0.106-v299`;s.async=false;s.dataset.civweaveLaunch=kind;const path=new URL(src,location.href).pathname,timer=setTimeout(()=>reject(new Error(`${path} did not become ready.`)),8000);s.onload=()=>{clearTimeout(timer);ready?.()?resolve(true):reject(new Error(`${path} loaded without becoming ready.`))};s.onerror=()=>{clearTimeout(timer);reject(new Error(`Could not load ${path}.`))};const head=document.head;if(!head){clearTimeout(timer);reject(new Error(`${path} could not mount because the document is leaving.`));return}head.append(s)})}
function loadOne(src,ready,kind='v299'){if(ready?.())return Promise.resolve(true);return appendScript(src,ready,kind)}
function ensureSettingsModule(){if(globalThis.CivweaveSettingsParityV295?.version==='1.0.99-settings-parity-v296')return Promise.resolve(true);if(settingsPromise)return settingsPromise;const [src,ready]=SETTINGS_MODULE;settingsPromise=loadOne(src,ready,'v299-settings').then(()=>{emit('SETTINGS_READINESS_READY',{revision:'v299',independentOfChat:true});return true}).catch(error=>{settingsPromise=null;emit('SETTINGS_READINESS_FAILED',{revision:'v299',message:String(error?.message||error)});return false});return settingsPromise}
function ensureChatModules(){if(CHAT_MODULES.every(([,ready])=>ready?.()))return Promise.resolve(true);if(chatPromise)return chatPromise;chatPromise=(async()=>{for(const [src,ready] of CHAT_MODULES)await loadOne(src,ready,'v299-chat');globalThis.CivweaveChatFullscreenV295?.settleViewport?.();emit('CHAT_READINESS_READY',{revision:'v299',fiveChats:true,savedTabs:true,localFastPath:true,androidKeyboardFix:true,deterministicFullscreenBoot:true,localStageWatchdog:true,runtimeOwnedWebGPUFallback:true,runtimeFirstBootstrap:true,localFifoQueue:true,structuralComposerRepair:true,intentPrewarm:true});return true})().catch(error=>{chatPromise=null;emit('CHAT_READINESS_FAILED',{revision:'v299',message:String(error?.message||error)});return false});return chatPromise}
function ensureLaunchModules(){if(launchPromise)return launchPromise;launchPromise=Promise.all([ensureSettingsModule(),ensureChatModules()]).then(([settings,chat])=>{const ok=Boolean(settings&&chat);emit(ok?'LAUNCH_READINESS_READY':'LAUNCH_READINESS_PARTIAL',{revision:'v299',settings,chat});if(!ok)launchPromise=null;return ok});return launchPromise}
...
const api=Object.freeze({version:VERSION,revision:REVISION,claim,release,status,protectedFlow,ensureSettingsModule,ensureChatModules,ensureLaunchModules,openSettingsIndependent,launchReadiness:'v299-deterministic-fullscreen-webgpu-fallback-intent-prewarm'});
globalThis.CivweaveExperienceOrchestratorV232=api;
globalThis.dispatchEvent?.(new CustomEvent('civweave:experience-orchestrator-ready',{detail:{version:VERSION,revision:REVISION,intentPrewarm:true}}));
})();
