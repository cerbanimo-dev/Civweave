...
const VERSION='1.0.5';
const REVISION='experience-orchestrator-v315-mobile-settings-first-open';
...
const SETTINGS_VERSION='1.0.126-settings-parity-v315-mobile-first-open';
...
const MOBILE_SETTINGS_STYLE='cw-mobile-settings-stack-v315';
...
function ensureLaunchModules(){if(launchPromise)return launchPromise;launchPromise=Promise.all([ensureSettingsModule(),ensureChatModules()]).then(([settings,chat])=>{const ok=Boolean(settings&&chat);emit(ok?'LAUNCH_READINESS_READY':'LAUNCH_READINESS_PARTIAL',{revision:'v315',settings,chat});if(!ok)launchPromise=null;return ok});return launchPromise}
...
function finishSettingsOpen(layer){if(!layer)return false;ensureMobileSettingsStack();void ensureSettingsModule().then(ready=>{if(ready)void globalThis.CivweaveSettingsParityV295?.ensureManagement?.()});return true}
...
const api=Object.freeze({version:VERSION,revision:REVISION,legacyRevision:LEGACY_REVISION,legacyScriptEpoch:LEGACY_SCRIPT_EPOCH,claim,release,status,protectedFlow,ensureSettingsModule,ensureChatModules,ensureLaunchModules,openSettingsIndependent,launchReadiness:'v315-mobile-settings-first-open-plus-v299-chat',settingsCaptureOwner:'window-first-v315',settingsVisibleBeforeManagement:true,mobileSettingsStack:true});
globalThis.CivweaveExperienceOrchestratorV232=api;
globalThis.dispatchEvent?.(new CustomEvent('civweave:experience-orchestrator-ready',{detail:{version:VERSION,revision:REVISION,intentPrewarm:true,chatOpenPrewarm:true,smoothFitRuntime:true,mobileSettingsFirstOpen:true}}));
})();