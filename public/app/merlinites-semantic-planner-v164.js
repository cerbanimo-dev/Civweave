(()=>{
'use strict';
const VERSION='1.0.4-merlinites-semantic-v164';
...
function childStepsFor(realm,title,depth){
  ...
  return selected.map((step,index)=>({
    id:uid('merlinites-node'),parentId:'',kind:nodeKind(realmOf(realm),index,selected.length),realm:realmOf(realm),
...
function expandNode(input,{maxDepth=MAX_DEPTH}={}){
  ...
  node.id=clean(node.id,180)||uid('merlinites-node');
...
function pathNode(path,index){
  ...
  const node={id:clean(path?.id,160)||uid('merlinites-path'),parentId:'merlinites-root',kind:path?.type||'path',realm,title:clean(path?.title,260)||`Path ${index+1}`,objective:clean(path?.purpose||path?.completionCriteria,1200),completionEvidence:[clean(path?.completionCriteria,1200)||completionFor(realm,path?.title)],status:clean(path?.status,40)||'draft',confidence:.86,depth:1,children:[]};
  node.children=steps.slice(0,12).map((step,stepIndex)=>{
    const child={id:uid('merlinites-step'),parentId:node.id,kind:nodeKind(realm,stepIndex,steps.length),realm,title:clean(step,420),objective:`Advance ${clean(path?.title,220).toLowerCase()} through a bounded, reviewable action.`,completionEvidence:[completionFor(realm,step)],status:'proposed',confidence:.8,depth:2,children:[]};
...
function enhancePlan(input,{source='deterministic-bridge'}={}){
  ...
  if(existing?.schema==='commonweave.merlinites-semantic-plan.v1'&&existing?.sourcePlanUpdatedAt===plan.updatedAt)return plan;
  const paths=Array.isArray(plan.paths)?plan.paths:[];
  const rootNode={id:'merlinites-root',parentId:null,kind:'outcome',realm:'commonweave',title:clean(plan.title||plan.wish,300)||'Review the intention',objective:clean(plan.outcome||plan.wish,1600),completionEvidence:[clean(plan.outcome,1600)||'The intended outcome has visible evidence and a reviewable route.'],status:clean(plan.state,40)||'review',confidence:.9,depth:0,children:paths.map(pathNode)};
  plan.semanticPlan={schema:'commonweave.merlinites-semantic-plan.v1',version:1,engine:'merlinites',model:MODEL_ID,authority:'advisory-recursive-planning',source,sourcePlanUpdatedAt:plan.updatedAt||'',maxDepth:MAX_DEPTH,generatedAt:now(),root:rootNode,archetype:null,diagnostics:['Original paths and approval gates are preserved.','Only bounded children are generated.','No consequential action is activated by semantic similarity.']};
  plan.merlinites={engine:'merlinites',version:VERSION,semanticPlanSchema:plan.semanticPlan.schema,nonRegressive:true,authority:'advisory'};
  return plan;
}
async function refinePlan(input,{text='',semanticWaitMs=0}={}){
  ...
  const ranked=await semanticRank(query,candidates,{limit:3,cacheKey:'merlinites-archetypes-v1',semanticWaitMs});
  ...
  plan.semanticPlan.suggestedPhases=archetype.phases.map((title,index)=>({id:uid('merlinites-phase'),kind:nodeKind(archetype.realm,index,archetype.phases.length),realm:archetype.realm,title,completionEvidence:[completionFor(archetype.realm,title)],status:'proposed',depth:1}));
...
async function evaluateLearning({prompt='',response='',criteria=[],deterministic=null,lessonExcerpt='',semanticWaitMs=0}={}){
  ...
  const ranked=await semanticRank(`${prompt}\n${answer}\n${lessonExcerpt}`,candidates,{limit:Math.max(1,rows.length),cacheKey:rows.length?`merlinites-learning-${rows.map(criterionText).join('|').slice(0,220)}`:'',semanticWaitMs});
  ...
  const requiredMissing=coverage.filter(item=>item.required&&item.state!=='demonstrated');
...
async function evaluateTask({task={},quest={},semanticWaitMs=0}={}){
  ...
  const ranked=await semanticRank(`${task?.title||''}\n${task?.description||''}\n${proofText}`,candidates,{limit:Math.max(1,criteria.length),cacheKey:criteria.length?`merlinites-task-${criteria.map(item=>item.label).join('|').slice(0,180)}`:'',semanticWaitMs});
  const scores=new Map((ranked.matches||[]).map(item=>[item.id,Number(item.score||0)]));
...
function patchPlanner(api){
  if(!api?.maybeCreate||api.__merlinitesSemanticInstalled)return api;
  ...
  api.maybeCreate=options=>{
    ...
    result.response=result.response?{...result.response,semanticPlanning:{engine:'merlinites',schema:plan.semanticPlan?.schema,authority:'advisory',maxDepth:MAX_DEPTH}}:result.response;
    return result;
  };
  if(originalRestore)api.restore=plan=>originalRestore(enhancePlan(plan));
  Object.defineProperty(api,'__merlinitesSemanticInstalled',{value:true});
  return api;
}
...
function enrichStoredIntentions(){
  ...
  let changed=0;
  const next=rows.map(item=>{if(item?.kind!=='weave-plan'||!item?.plan||item.plan?.semanticPlan?.schema==='commonweave.merlinites-semantic-plan.v1')return item;changed+=1;return{...item,plan:enhancePlan(item.plan),updatedAt:now()}});
...
function dispatchSyntheticStorage(key,value){
  try{root.dispatchEvent?.(new StorageEvent('storage',{key,newValue:value,storageArea:localStorage,url:location.href}))}catch{dispatch('commonweave:merlinites-storage-updated',{key})}
}
async function reviewLatestLiving(form){
  ...
  if(type==='assessment'){
    ...
    attempt.semanticReview=review;attempt.merlinitesFeedback=review.targetedFollowUp;
  }else if(type==='final'){
    ...
    state.final.semanticReview=review;state.final.merlinitesFeedback=review.targetedFollowUp;
  }else return;
  const value=JSON.stringify(state);localStorage.setItem(LIVING_KEY,value);dispatchSyntheticStorage(LIVING_KEY,value);dispatch('commonweave:merlinites-learning-reviewed',{type});
}
async function reviewTaskFromControl(control){
  ...
  const value=JSON.stringify(state);localStorage.setItem(QUEST_KEY,value);dispatch('cerbanimo:quest-engine-changed',{state,source:'merlinites'});dispatch('commonweave:merlinites-task-reviewed',{questId:quest.id,taskId});
}
function bindDOM(){
  if(typeof document==='undefined'||document.documentElement.dataset.merlinitesSemanticBound==='true')return;
  document.documentElement.dataset.merlinitesSemanticBound='true';
...
async function status(){
  ...
  return{version:VERSION,engine:'merlinites',installed,model,maxDepth:MAX_DEPTH,authority:'advisory',bridges:{intention:Boolean(root.CommonweaveIntentionPlanner?.__merlinitesSemanticInstalled),living:typeof document!=='undefined',cerbanimo:typeof document!=='undefined'}};
}
function install(){
  ...
  dispatch('commonweave:merlinites-ready',{version:VERSION,model:MODEL_ID});
  return true;
}

root.CommonweaveMerlinitesV164={version:VERSION,model:MODEL_ID,maxDepth:MAX_DEPTH,authority:'advisory',lexicalRank,rank:semanticRank,expandNode,nextAction,enhancePlan,refinePlan,evaluateLearning,evaluateTask,install,status,archetypes:clone(ARCHETYPES)};
