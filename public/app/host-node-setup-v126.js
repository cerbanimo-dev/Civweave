...
(async()=>{
  try{if(!window.CommonweaveBootLog)await load(`boot-diagnostics-v126.js?v=${VERSION}&hotfix=2`)}catch(error){console.warn('Boot diagnostics could not preload',error)}
  ...
  await installWorker();
  try{await load(`host-node-v126-safe.js?v=${VERSION}&hotfix=2`);log('v126-runtime-ready',{universalAI:Boolean(window.CommonweaveUniversalAI),build:BUILD})}catch(error){console.error('Commonweave v1.0.26 shell failed to load',error);exposeFallback(error)}
})();
