...
function syncAddress(system,room,{replace=true}={}){const query=new URLSearchParams({system:system.id});if(room?.id)query.set('room',room.id);if(params.get('capability'))query.set('capability',params.get('capability'));query.set('from',params.get('from')||'visual');history[replace?'replaceState':'pushState']({system:system.id,room:room?.id||''},'',`/app/cabinet-visual-v141.html?${query}`);for(const [key,value] of query)params.set(key,value)}
function render(systemId=params.get('system')||DEFAULT_SYSTEM,roomId=params.get('room')||''){
  const system=systemFor(systemId),room=roomFor(system,roomId),shell=shellFor(system.id),screen=shell.screen||{},frame=$('#cv141-shell'),art=$('#cv141-art'),iframe=$('#cv141-screen');
