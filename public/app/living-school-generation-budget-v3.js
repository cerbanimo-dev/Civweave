...
const stats={designCalls:0,duplicateDesignCallsBlocked:0,designTimeouts:0,structureCompiles:0,quizCallsBlocked:0,repairCallsBlocked:0,depthCallsBlocked:0,filteredSourceNoise:0,installCount:0,installedAt:'',lastBlockedAt:'',lastProvider:'',lastModel:''};
let wrappedRuntime=null,designConsumed=false;
...
function selectedConfig(request={}){
  const supplied=request?.config&&typeof request.config==='object'?request.config:{};
  let stored={};
  try{stored=globalThis.CivweaveModelRuntime?.readSharedConfig?.('interactive')||{}}catch{}
  return{...stored,...supplied};
}
function routeMeta(result={},request={}){
  const config=selectedConfig(request),provider=clean(result?.actual?.provider||result?.provider||result?.requested?.provider||config.provider||config.route||config.engine,120),model=clean(result?.actual?.model||result?.model||result?.requested?.model||config.model,240);
  return{provider:provider||'selected runtime',model:model||'selected model'};
}
function publish(){
  try{
    const root=document.documentElement;
    root.dataset.livingSchoolGenerationBudget=VERSION;
    root.dataset.livingSchoolGenerationBudgetProviderNeutral='true';
    ...
    root.dataset.livingSchoolBudgetProvider=stats.lastProvider||'';
    root.dataset.livingSchoolBudgetModel=stats.lastModel||'';
  }catch{}
}
...
function blocked(request,kind,message,code='LIVING_SCHOOL_HIDDEN_PROVIDER_CALL_BLOCKED'){
  ...
  return{schema:'civweave-model-result-1.0',requestId:request?.requestId||`ls-budget-${Date.now().toString(36)}`,purpose:clean(request?.purpose,180),status:'error',outputText:'',outputJson:null,usage:{inputTokens:0,outputTokens:0,totalTokens:0,costCents:0,remainingCents:0},stream:{requested:false,used:false},actual:{provider:'civweave',model:VERSION},error:{code,message},diagnostics:[message],livingSchoolGenerationBudget:{version:VERSION,kind,blocked:true,providerNeutral:true,at:stats.lastBlockedAt}};
}
...
function incompleteDesignResult(result,check,request){
  const route=routeMeta(result,request),message=`Living School stopped after the selected provider design call because the packet was incomplete: ${check.issues.join('; ')}.`;
  return{...result,status:'invalid-response',outputText:'',outputJson:null,error:{code:'LIVING_SCHOOL_SINGLE_DESIGN_PACKET_INCOMPLETE',message},diagnostics:[...(result?.diagnostics||[]),message],livingSchoolDesignCompleteness:{revision:VERSION,complete:false,issues:check.issues,designProviderCalls:1,provider:route.provider,model:route.model,providerNeutral:true}};
}
function designTimeoutResult(request){
  const route=routeMeta({},request),message=`Living School stopped the selected provider design call after ${Math.round(DESIGN_TIMEOUT_MS/1000)} seconds.`;
  return{schema:'civweave-model-result-1.0',requestId:request?.requestId||`ls-design-timeout-${Date.now().toString(36)}`,purpose:DESIGN_PURPOSE,status:'error',outputText:'',outputJson:null,usage:{inputTokens:0,outputTokens:0,totalTokens:0,costCents:0,remainingCents:0},stream:{requested:false,used:false},actual:{provider:route.provider,model:route.model},error:{code:'LIVING_SCHOOL_DESIGN_TIMEOUT',message},diagnostics:[message],livingSchoolGenerationBudget:{version:VERSION,designTimeout:true,timeoutMs:DESIGN_TIMEOUT_MS,designProviderCalls:1,providerNeutral:true}};
}
async function runOneDesignCall(original,request){
  if(designConsumed)return blocked(request,'design','Living School blocked a second curriculum design request in the same curriculum action.','LIVING_SCHOOL_DUPLICATE_STRONG_DESIGN_BLOCKED');
  ...
  const selected=routeMeta({},prepared);stats.lastProvider=selected.provider;stats.lastModel=selected.model;publish();
  ...
  const actual=routeMeta(result,prepared);stats.lastProvider=actual.provider;stats.lastModel=actual.model;publish();
  ...
  if(!check.complete)return incompleteDesignResult(result,check,prepared);
  return{...result,livingSchoolDesignCompleteness:{revision:VERSION,complete:true,moduleCount:count,designProviderCalls:1,provider:actual.provider,model:actual.model,providerNeutral:true},diagnostics:[...(result.diagnostics||[]),`Living School completed one bounded design call through ${actual.provider}${actual.model?` · ${actual.model}`:''}; provider/model selection remained owned by the shared runtime.`]};
}
...
function runtimeRouteReady(){return Boolean(globalThis.CivweaveLivingSchoolRuntimeRouteV2?.providerNeutral||globalThis.CivweaveLivingSchoolRuntimeRouteV1?.providerNeutral)}
function install(){
  ...
  if(current.__livingSchoolGenerationBudgetV3===VERSION){wrappedRuntime=current;publish();return true}
  ...
  wrappedRuntime=Object.freeze({...current,generate,livingSchoolGenerationBudgetRevision:VERSION,__livingSchoolGenerationBudgetV3:VERSION,__livingSchoolGenerationBudgetV2:false});
  ...
  stats.installCount+=1;stats.installedAt=new Date().toISOString();publish();
  try{dispatchEvent(new CustomEvent('civweave:living-school-generation-budget-ready',{detail:{version:VERSION,designProviderCallsMax:1,designTimeoutMs:DESIGN_TIMEOUT_MS,designExecutionProfile:'interactive',providerSelectionOwner:'civweave-runtime',providerNeutral:true,postDesignProviderCalls:0,structureCompiler:'grounded-design-compiler-v338',quizCompiler:'assessment-curator-v338',at:stats.installedAt}}))}catch{}
  return true;
}
function schedule(){queueMicrotask(install);setTimeout(install,0);setTimeout(install,120)}
for(const event of ['civweave:living-school-runtime-route-ready','civweave:living-school-video-generation-guard-ready','civweave:assistant-runtime-ready','civweave:model-runtime-ready','civweave:runtime-spine-ready','pageshow'])addEventListener?.(event,schedule);
...
schedule();
const api=Object.freeze({version:VERSION,install,designPacketCheck,looksLikeIndexNoise,compileStructure,resetRunBudget,get installed(){return Boolean(globalThis.CivweaveModelRuntime?.__livingSchoolGenerationBudgetV3===VERSION)},get runtime(){return wrappedRuntime},stats:()=>({...stats}),designProviderCallsMax:1,postDesignProviderCallsMax:0,providerSelectionOwner:'civweave-runtime',providerNeutral:true,minLessonWords:MIN_LESSON_WORDS});
globalThis.CivweaveLivingSchoolGenerationBudgetV3=api;
