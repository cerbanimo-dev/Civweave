...
function externalDataSetting(spec){
  const counts={};
  for(const row of spec?.artifacts||[]){
    const match=String(row?.path||'').match(/(?:^|\/)([^/]+\.onnx)_data(?:_(\d+))?$/i);
    if(match)counts[match[1]]=Math.max(counts[match[1]]||0,Number(match[2]||0)+1);
  }
  return Object.keys(counts).length?counts:false;
}
...
function visibleAnswer(value){
  return String(value??'')
    ...
    .replace(/<\|channel>(?:final|analysis)\s*/g,'')
    .replace(/<\|turn>model\s*|<turn\|>|<end_of_turn>|<eos>|<bos>|<channel\|>/g,'').trim();
}
function makeStreamer(id,requested,timing){
  if(!hfRuntime?.TextStreamer||!tokenizer)return null;
  let emitted='';
  return new hfRuntime.TextStreamer(tokenizer,{skip_prompt:true,skip_special_tokens:false,
    callback_function:text=>{
      timing.decoded+=clean(text,12000);
      const value=visibleAnswer(timing.decoded.replace(/<[^>]*$/,''));
      if(requested&&value.startsWith(emitted)&&value.length>emitted.length){
        post(id,'token',{token:{text:value.slice(emitted.length),index:timing.index++}});emitted=value;
      }
    },token_callback_function:tokens=>{if(!timing.firstTokenAt)timing.firstTokenAt=now();timing.generatedTokens+=Math.max(1,tokens?.length||1)}});
}
...
async function generate(message,id){
  ...
  const timing={firstTokenAt:0,generatedTokens:0,index:0,decoded:''},generationStarted=now(),streamer=makeStreamer(id,Boolean(message.stream)&&!message.thinking,timing),temperature=Math.max(.01,Math.min(2,Number(message.temperature??.7)));
  ...
  if(streamer)options.streamer=streamer;
  const output=await model.generate(options),completed=now(),ids=generatedIds(output?.sequences,promptTokens);let text='';try{text=visibleAnswer(tokenizer.decode(ids,{skip_special_tokens:false}))}catch{}if(!text)text=visibleAnswer(timing.decoded);if(!text)throw new Error('The local model returned no final answer. Its output ended before a usable response.');
