...
function artifactFor(spec,path){return Array.isArray(spec?.artifacts)?spec.artifacts.find(item=>String(item?.path||'')===String(path||'')):null}
function contentTypeFor(path){if(/\.json$/i.test(path))return'application/json;charset=utf-8';if(/\.jinja$|\.txt$/i.test(path))return'text/plain;charset=utf-8';return'application/octet-stream'}
function missingArtifact(spec,path){
  const artifact=artifactFor(spec,path);
  if(artifact?.required===false){
    const body=/\.json$/i.test(path)?'{}':'';
    return new Response(body,{status:200,headers:{'content-type':contentTypeFor(path),'x-civweave-model-cache':'optional-placeholder'}});
  }
  if(artifact)throw new Error(`Downloaded model cache miss: ${path}. Repair or reinstall ${spec?.label||spec?.id||'the selected model'} before using it offline.`);
  throw new Error(`Downloaded model manifest is incomplete: ${path}. Update Civweave or reinstall ${spec?.label||spec?.id||'the selected model'}.`);
}
function cacheAdapter(cache,spec){
  const localPrefix=`/models/${spec.repo}/`,remotePrefix=pinnedRemoteRoot(spec);
  async function resolve(path,remote){const hit=await cache.match(remote);return hit?.ok?hit.clone():missingArtifact(spec,path)}
  return{
    async match(request){
      const key=requestUrl(request);
      if(key.startsWith(localPrefix)){const path=key.slice(localPrefix.length).split(/[?#]/,1)[0];return resolve(path,remotePrefix+path)}
      if(key.startsWith(remotePrefix)){const path=key.slice(remotePrefix.length).split(/[?#]/,1)[0];return resolve(path,remotePrefix+path)}
...
function friendlyError(error,spec){const raw=String(error?.message||error);if(/Downloaded model cache miss|Downloaded model manifest is incomplete/i.test(raw))return raw;if(raw.includes('/models/')&&(raw.includes('env.allowRemoteModels=false')||raw.includes('local_files_only=true')))return `Downloaded model cache miss for ${spec?.label||spec?.id||'the selected model'}. Re-download this model while online before using it offline.`;if(/Unexpected end of JSON input/i.test(raw))return `Downloaded model metadata could not be parsed for ${spec?.label||spec?.id||'the selected model'}. This is a model-load/cache failure before inference. Repair or reinstall the downloaded model package.`;if(/Unexpected token\s*['"]?<|<!doctype|not valid JSON/i.test(raw))return `Downloaded model metadata contained HTML instead of JSON for ${spec?.label||spec?.id||'the selected model'}. Resume the model once so Civweave can repair only the bad metadata file.`;return raw}
