...
async function importFile(packet){const file=packet.file,total=Math.max(0,Number(packet.totalBytes||file?.size||0)),minBytes=Math.max(0,Number(packet.minBytes||0));if(minBytes&&total<minBytes*.97)throw new Error(`The selected model file is incomplete (${total} bytes).`);return packet.storageBackend==='opfs'?writeOpfs(packet,file):writeCache(packet,file)}
