(()=>{
'use strict';
const VERSION='1.1.0-litert-gemma4-fast-runtime-v1-dual-phone';
...
let modulePromise=null,enginePromise=null,enginePromiseModelId='',engine=null,engineModelId='',wrapped=null,generationActive=false,lastMetrics=null;
...
async function unloadEngine(reason='model-switch'){
  ...
  const prior=engine,priorId=engineModelId;
  engine=null;engineModelId='';
  if(prior)try{await prior.delete?.()}catch{}
  if(priorId)emit('civweave:litert-gemma4-engine-released',{model:priorId,reason});
  return true;
}
async function createEngine(modelId){
  ...
  const stream=await modelStream(profile.id),started=now();
  emit('civweave:litert-gemma4-progress',{phase:'loading-model',model:profile.id,backend:'webgpu-gpu-artisan',phoneProfile:'12gb-dual'});
  const value=await mod.Engine.create({model:stream,backend:mod.Backend.GPU_ARTISAN,mainExecutorSettings:{maxNumTokens:profile.contextTokens},benchmarkEnabled:true},'Civweave local assistant');
  engine=value;engineModelId=profile.id;
  emit('civweave:litert-gemma4-progress',{phase:'model-ready',model:profile.id,backend:'webgpu-gpu-artisan',loadMs:Math.round(now()-started),maxNumTokens:profile.contextTokens,webBindingSettings:'supported-only',oneEngineAtATime:true});
  return value;
}
async function ensureEngine(modelId){
  ...
  enginePromise=createEngine(profile.id).catch(error=>{engine=null;engineModelId='';throw error}).finally(()=>{enginePromise=null;enginePromiseModelId=''});
...
async function runFast(args={},forcedModelId=''){
  ...
  try{
    ...
    emit('civweave:litert-gemma4-progress',{phase:'generating',model:profile.id,backend:'webgpu-gpu-artisan',maxOutputTokens,maxNumTokens:profile.contextTokens,oneEngineAtATime:true});
    ...
    for await(const chunk of stream){
      ...
      try{args.onToken?.({text:piece,index:index++,model:profile.id,backend:'litert-webgpu'})}catch{}
    }
    ...
    const metrics={runtime:'litert-lm-web-0.14.0',backend:'webgpu-gpu-artisan',model:profile.id,generationMs,ttftMs,maxNumTokens:profile.contextTokens,maxOutputTokens,approxGeneratedTokens:approxTokens,approxTokensPerSecond:measuredApproxTokensPerSecond,oneEngineAtATime:true,phoneProfile:'12gb-dual',webBindingSpeculativeDecodingConfigured:false,...benchmark};
...
globalThis.CivweaveLiteRTGemma4FastRuntimeV1=Object.freeze({
  ...
  shouldAccelerate,
  unload,
  state:()=>Object.freeze({installed:Boolean(wrapped),engineReady:Boolean(engine),engineLoading:Boolean(enginePromise),engineLoadingModelId:enginePromiseModelId,engineModelId,generationActive,lastMetrics})
});
})();