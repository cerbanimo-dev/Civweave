...
const YIELD_REVISION='v293-yield-between-settings-modules';
...
const runtimeReady=()=>{const value=globalThis.CivweaveLocalModelRuntimeV266;return Boolean(value?.revision===RUNTIME_REVISION&&value?.smallModelFastPath===true&&value?.canonicalCausalLM===true)};
const settingsReady=()=>{const value=globalThis.CivweaveLocalAISettingsV266;return Boolean(value?.version==='1.0.83-local-ai-settings-v282-health'&&value?.truthfulCompletion===true&&value?.cacheIntegrityOnDemand===true&&value?.openPath==='snapshot-first-v287'&&value?.progressPatchOnly===true)};
const files=[
  ...
  ['/app/local-ai/runtime-v266.js?v=1.0.87-v287-v283-coherence-v288',runtimeReady],
...
const ready=(async()=>{for(const [src,test] of files){await load(src,test);await yieldToUI()}dispatchEvent(new CustomEvent('civweave:local-ai-ready',{detail:{version:VERSION,revision:REVISION,componentCompatibility:'capability-contract-v288',byteProgress:true,backgroundFetch:true,capabilityRouting:true,localAgenticReasoning:true,directModelTest:true,runtimeSpine:true,cacheResolvedInference:true,localStreaming:true,integrityRepair:true,runtimeMetadataRequired:true,metadataOnlyRepair:true,metadataRepairRaceSafe:true,truthfulCompletion:true,backendFallback:true,agenticToolSemantics:true,phone1BTier:true,hardwareLadder:true,directDownloads:true,largeExternalDataForeground:true,hardwareTierUI:true,canonicalCausalLM:true,contextAware:true,timingDiagnostics:true,thinkingProfiles:true,artifactRevisionRepair:true,wasmPerformanceDiagnostics:true,embeddedLocalPrimary:true,smallModelFastPath:true,adaptiveOutput:true,continuationValidation:true,completionMetadata:true,uiYieldBetweenModules:true}}));return true})().catch(error=>{console.warn('[civweave local ai]',error);dispatchEvent(new CustomEvent('civweave:local-ai-unavailable',{detail:{version:VERSION,revision:REVISION,componentCompatibility:'capability-contract-v288',message:String(error?.message||error)}}));return false});
globalThis.CivweaveLocalAIBootstrapV266=Object.freeze({version:VERSION,revision:REVISION,yieldRevision:YIELD_REVISION,ready,componentCompatibility:'capability-contract-v288',smallModelFastPath:true,adaptiveOutput:true,continuationValidation:true,embeddedLocalPrimary:true,wasmPerformanceDiagnostics:true,uiYieldBetweenModules:true});
})();
