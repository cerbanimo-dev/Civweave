(()=>{
'use strict';
const VERSION='1.0.1-gemma4-inference-repair-v1';
  ...
  refreshFlight=(async()=>{
    ...
    for(const name of cacheNames){
      try{
        ...
        for(const request of keys){let url;try{url=new URL(request.url)}catch{continue}if(url.pathname!==WORKER_PATH)continue;await cache.delete(request);evicted+=1;touched=true}
    ...
    refreshDone=true;emit('civweave:gemma4-worker-repaired',{workerRevision:EXPECTED_WORKER_REVISION,cachesUpdated:targets.length,entriesEvicted:evicted});return true;
...
patch();
globalThis.CivweaveGemma4InferenceRepairV1=Object.freeze({version:VERSION,expectedWorkerRevision:EXPECTED_WORKER_REVISION,runtimeWorkerUrl:RUNTIME_WORKER_URL,workerPath:WORKER_PATH,patch,refreshWorkerAsset,activeGemma4,state:()=>Object.freeze({installed:Boolean(wrapped),refreshDone,refreshing:Boolean(refreshFlight)})});
})();