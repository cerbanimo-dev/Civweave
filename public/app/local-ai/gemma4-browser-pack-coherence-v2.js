...
const VERSION='1.0.2-gemma4-browser-pack-coherence-v2-event-driven';
...
let bridgeFlight=null,decorateQueued=false;
...
function setDisabled(node,value){if(!node)return false;const next=Boolean(value);if(node.disabled===next)return false;node.disabled=next;return true}
function setDataset(node,key,value){if(!node)return false;const next=String(value??'');if(node.dataset?.[key]===next)return false;node.dataset[key]=next;return true}
...
async function startModelDownload(id,button){
  ...
  setDisabled(button,true);showError('');
  try{
    const installed=await manager()?.status?.(id).catch?.(()=>null);if(installed?.available){scheduleDecorate();return true}
    const {current,receipt}=await prepareCurrentPack(progress=>{if(progress?.message)setText(button,progress.message)});
    ...
    if(started.has(row.key)){
      setText(button,'Select downloaded file…');
      const result=await current.pickAndImport(PREMIER,{onProgress:progress=>{if(progress?.message)setText(button,progress.message)}});
      if(result?.cancelled){setDisabled(button,false);setText(button,`Import ${fmt(row.expectedBytes||row.sizeBytes)}`);return false}
      await syncFastStatus();
    }else{
      dispatchBrowserDownload(current,row);setText(button,'Browser download started');
    }
    scheduleDecorate();return true;
  }catch(error){
    const message=String(error?.message||error);showError(message);emit('civweave:gemma4-browser-download-error',{modelId:id,message});setDisabled(button,false);setText(button,'Try browser download');return false;
  }
}
async function startPair(button){
  ...
  try{
    ...
    if(next){dispatchBrowserDownload(current,next);setText(button,'Browser download started');scheduleDecorate();return true}
    const missing=current.unimportedRecords(receipt);
    if(missing.length){setText(button,'Select downloaded files…');const result=await current.pickAndImport(PREMIER,{onProgress:progress=>{if(progress?.message)setText(button,progress.message)}});if(result?.cancelled){setDisabled(button,false);setText(button,'Import downloaded files');return false}await syncFastStatus();scheduleDecorate();return true}
    await syncFastStatus();setText(button,'Current models ready');scheduleDecorate();return true;
  }catch(error){
    const message=String(error?.message||error);showError(message);emit('civweave:gemma4-browser-download-error',{pair:true,message});setDisabled(button,false);setText(button,'Try browser download');return false;
  }
}
function decoratePremier(){
  const card=premierCard();if(!card)return false;
  const direct=[...card.children].filter(node=>node.tagName==='P');
  if(direct[0])setText(direct[0],'Current mid-range phone pack: Gemma 4 E2B LiteRT for fast work, Gemma 4 E4B LiteRT for deeper work, multilingual speech, and a CPU-safe fallback. Existing Q4F16 Gemma models remain compatibility fallbacks.');
  if(direct[1])setHtml(direct[1],'<b>Target:</b> 12 GB RAM · modern Android-class WebGPU');
  if(direct[2])setHtml(direct[2],'<b>Storage:</b> ~6.9 GB current pack · old Q2F16 files can be deleted after migration');
  if(direct[3])setText(direct[3],'Gemma 4 E2B LiteRT 2.0 GB · Gemma 4 E4B LiteRT 3.0 GB · Qwen 3 0.6B CPU fallback · Silero · Parakeet INT8 · Omnilingual 300M INT8 · Supertonic 3');
  setDataset(card,'gemma4CurrentMidrange','litert-e2-e4-q4-compat');return true;
}
function decorateProfile(){
  const card=profileCard();if(!card)return false;
  const intro=card.querySelector(':scope > div > p');if(intro)setText(intro,'These are the current Gemma 4 phone models used by the Premier Phone Pack. Large LiteRT files use the browser download manager; after each download finishes, import it back into Civweave. Q4F16 remains the compatibility fallback.');
  for(const button of card.querySelectorAll('[data-litert-fast-download]')){
    ...
    if(imported.has(row.key))setText(button,'Imported');
    else if(started.has(row.key))setText(button,`Import ${fmt(row.expectedBytes||row.sizeBytes)}`);
    else setText(button,`Download ${fmt(row.expectedBytes||row.sizeBytes)}`);
    setDisabled(button,false);
  }
  const pair=card.querySelector('[data-litert-fast-pair]');if(pair){const current=bridge(),receipt=current?.pending?.(PREMIER);const missing=receipt&&current?.unimportedRecords?current.unimportedRecords(receipt):[];const next=current?.nextRecord?.(PREMIER);setText(pair,next?'Download next optimized model':missing.length?'Import downloaded files':'Start optimized downloads');setDisabled(pair,false)}
  ...
  setText(note,'Multi-gigabyte LiteRT models no longer use the legacy in-app Cache Storage downloader.');
  return true;
}
