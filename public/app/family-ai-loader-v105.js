...
const COMMONWEAVE_CHAT_KEY='commonweave.weaveling-chat.v127';
const RETIRED_OVERLAY=['/app/guide-chat-v153.js','CommonweaveGuideChatV153'];
...
const OPTIONAL=[
  ...
  ['/app/local-object-mesh-v146.js?v=1.0.4',()=>globalThis.CommonweaveLocalMeshV146],
...
const GUIDE={
  commonweave:{name:'Weaveling',role:'Central mirror and project-memory assistant',avatar:'/app/assets/ai/weaveling.png',prompt:'Ask about this workspace or say “remember that…”'},
...
const guideFor=system=>GUIDE[system]||GUIDE.commonweave;
const chatKey=system=>system==='commonweave'?COMMONWEAVE_CHAT_KEY:`commonweave.guide-chat.${system}.v128`;
...
function reset(reason='manual reset'){generation++;promise=null;optionalPromise=null;dispatchEvent(new CustomEvent('commonweave:guide-loader-reset',{detail:{reason,at:new Date().toISOString()}}))}
...
async function ensure(){
  if(globalThis.CommonweaveAssistantV141&&globalThis.CommonweaveDeterministicModeV175&&globalThis.CommonweaveWeavelingMemoryBridgeV191){
    globalThis.CommonweaveWeavelingMemoryBridgeV191.install?.();
    loadOptional();
    return true;
  }
  ...
  promise=(async()=>{
    ...
    if(ticket!==generation)throw new Error('Commonweave loading was reset.');
    await loadScript(...FAST_RUNTIME);
    await loadScript(...ASSISTANT);
    await Promise.all(PATCHES.map(([src,ready])=>loadScript(src,ready)));
    globalThis.CommonweaveDeterministicModeV175?.installAssistantPatch?.();
    globalThis.CommonweaveWeavelingMemoryBridgeV191?.install?.();
    loadOptional();
    return true;
...
function prepare(band,system=detect()){
  CSS.forEach(addCss);
  const meta=guideFor(system),alreadyBound=band.dataset.commonweaveInlineChat==='r46'&&band.dataset.contextSystem===system&&band.querySelector('[data-cwf-form]')?.dataset.bound;
  if(alreadyBound)return band;
  band.dataset.commonweaveInlineChat='r46';band.dataset.contextSystem=system;guide(band,system);
...
function workspaceSnapshot(system,query=''){
  const living=obj('commonweave.living-school.cabinet.v151'),fellow=obj('fellowfare.mvp.state.v3'),anarchadia=obj('commonweave.anarchadia.citizen-console.v139');
  const memory=system==='commonweave'?null:globalThis.CommonweaveWeavelingMemoryV191?.snapshot?.(query,{limit:6})||null;
  const snapshot={schema:'commonweave.inline-workspace-context.v4-fast',contextSystem:system,page:{path:location.pathname,room:new URLSearchParams(location.search).get('room')},memory,intentions:compact(arr('commonweave.intentions.v127'),8)};
  ...
  if(system==='cerbanimo')snapshot.cerbanimo={quests:compact(arr('commonweave.cerbanimo.quest-queue.v1'),10),actions:compact(arr('commonweave.realm-actions.v141').filter(item=>item?.system==='cerbanimo'),10)};
  if(system==='fellowfare')snapshot.fellowfare={resources:compact(arr('commonweave.fellowfare.resource-queue.v152'),10),threads:compact(fellow.threads,8),listings:compact(fellow.listings||fellow.offers||fellow.posts,8)};
  if(system==='anarchadia')snapshot.anarchadia={proposals:compact(anarchadia.proposals,8),passport:obj('commonweave.anarchadia.passport.v152')};
...
async function submit(event,band){
  ...
  try{
    ...
    const result=await globalThis.CommonweaveAssistantV141.respond({text,systemId:system,history:conversation});
...
function handleGate(event,band){const button=event.target.closest('[data-cwf-gate]');if(!button)return;event.preventDefault();const operation=button.dataset.cwfGate,id=button.dataset.id;if(operation==='open-plan')globalThis.CommonweaveIntentionUI?.open?.(id);if(operation==='activate-plan')globalThis.CommonweaveIntentionUI?.activate?.(id);if(operation==='open-action')globalThis.CommonweaveActionUI?.open?.(id);if(operation==='approve-action')globalThis.CommonweaveActionUI?.approve?.(id);setTimeout(()=>render(band,band.dataset.contextSystem||detect()),40)}
...
async function openSettings(){const system=detect();try{const controller=globalThis.CommonweaveModelSettingsControllerV173;if(!controller?.open)throw new Error('The direct model settings controller is unavailable.');return await controller.open()}catch(error){const band=ensureBand(system),rows=history(system);rows.push({role:'assistant',text:`Universal AI settings could not load: ${error.message}`});save(system,rows);render(band,system);return null}}
function warm(){return ensure()}
function patch(){patchHeader();const system=detect();document.querySelectorAll('.ch142-control-band').forEach(band=>{if(band.dataset.commonweaveInlineChat!=='r46'||band.dataset.contextSystem!==system)prepare(band,system)})}
...
addEventListener('pageshow',event=>{if(event.persisted&&promise&&!globalThis.CommonweaveAssistantV141)reset('restored page contained an incomplete Commonweave load');patch();ensure().catch(()=>{})});
globalThis.CommonweaveFamilyAILoaderV105={version:VERSION,ensure,warm,openChat,openSettings,reset,ensureBand,workspaceSnapshot,retiredOverlay:RETIRED_OVERLAY,settingsOwner:'CommonweaveModelSettingsControllerV173',defaultProvider:'deterministic',transformerActive:false,memoryRevision:'v192-fast-relevant-memory',latencyRevision:'v192-prewarmed-no-mesh'};
})();
