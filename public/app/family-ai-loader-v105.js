...
const VERSION='1.0.122-headless-canonical-chat-r50-settings-gateway-v317';
...
let promise=null,optionalPromise=null,generation=0;
...
async function ensure(){if(globalThis.CivweaveAssistantV141&&globalThis.CivweaveDeterministicModeV175&&globalThis.CivweaveWeavelingMemoryBridgeV191&&globalThis.CivweaveKnowledgeEncyclopediaBridgeV271){globalThis.CivweaveWeavelingMemoryBridgeV191.install?.();await globalThis.CivweaveKnowledgeEncyclopediaBridgeV271.install?.();loadOptional();emitAssistantReady();return true}if(promise)return promise;const ticket=++generation;promise=(async()=>{CSS.forEach(addCss);await Promise.all(PREREQUISITES.map(([src,ready])=>loadScript(src,ready)));if(ticket!==generation)throw new Error('Civweave loading was reset.');await loadScript(...FAST_RUNTIME);await loadScript(...ASSISTANT);await Promise.all(PATCHES.map(([src,ready])=>loadScript(src,ready)));await globalThis.CivweaveKnowledgeEncyclopediaBridgeV271?.install?.();globalThis.CivweaveDeterministicModeV175?.installAssistantPatch?.();globalThis.CivweaveWeavelingMemoryBridgeV191?.install?.();loadOptional();emitAssistantReady();return true})().catch(error=>{if(ticket===generation)reset(error.message);throw error});return promise}
...
function waitForCanonicalChat(timeout=1600){if(canonicalChatApi())return Promise.resolve(true);return new Promise(resolve=>{let done=false;const finish=value=>{if(done)return;done=true;clearTimeout(timer);removeEventListener('civweave:guide-workspace-ready',ready);resolve(value)};const ready=()=>finish(true),timer=setTimeout(()=>finish(Boolean(canonicalChatApi())),timeout);addEventListener('civweave:guide-workspace-ready',ready,{once:true})})}
