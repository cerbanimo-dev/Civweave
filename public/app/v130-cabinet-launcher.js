...
function currentRoom(systemId){const system=systemRecord(systemId);if(!system)return'';const query=new URLSearchParams(location.search).get('room'),saved=localStorage.getItem(`commonweave.realm-room.${systemId}`);return system.rooms.find(room=>room.id===(query||saved))||system.rooms[0]||null}
...
function context(){const systemId=realmId(),system=systemRecord(systemId),room=currentRoom(systemId),count=Math.max(1,Array.isArray(room?.capabilityIds)?room.capabilityIds.length:0);return{systemId,system,room,count}}
...
function openWorkstation(){const ctx=context();location.assign(CommonweaveParity.visualUrl({systemId:ctx.systemId,roomId:ctx.room?.id||'',from:'loom'}))}
document.addEventListener('click',event=>{const control=event.target.closest?.('[data-action]');if(control?.dataset.action==='lite'){event.preventDefault();event.stopImmediatePropagation();openWorkstation()}},true);
