...
function answerFor(text){
  const lower=text.toLowerCase();let mode='Reflect',realm='Commonweave';
  ...
  localStorage.setItem('commonweave.weaveling-mode',mode);const label=document.querySelector('#cw127-context-label');if(label)label.textContent=mode;
...
function chat(){
  ...
  node.querySelector('form').onsubmit=event=>{event.preventDefault();const input=node.querySelector('textarea'),text=input.value.trim();if(!text)return;const parsed=parse(localStorage.getItem(CHAT_KEY),[]);const items=Array.isArray(parsed)?parsed:[];items.push({role:'user',text,time:new Date().toISOString()});items.push({role:'assistant',text:answerFor(text),time:new Date().toISOString()});localStorage.setItem(CHAT_KEY,JSON.stringify(items.slice(-80)));input.value='';render();report('weaveling-message',{characters:text.length,mode:localStorage.getItem('commonweave.weaveling-mode')||'Reflect'})};
...
const mode=localStorage.getItem('commonweave.weaveling-mode')||MODES[new Date().getHours()%MODES.length];const modeLabel=document.querySelector('#cw127-context-label');if(modeLabel)modeLabel.textContent=mode;
localStorage.setItem('commonweave.host-build',BUILD);document.documentElement.dataset.commonweaveBuild=BUILD;document.documentElement.dataset.commonweavePatch=PATCH;window.CommonweaveUniversalAISettings={open:settings};
