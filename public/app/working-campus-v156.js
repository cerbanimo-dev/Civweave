...
const routeScript='/app/system-routes-v227.js?v=1.0.16-five-system-route-contract-v227';
...
function ensureRouteContract(){
  if(globalThis.CommonweaveSystemRoutesV227){globalThis.CommonweaveSystemRoutesV227.authorize();return Promise.resolve(true)}
  return new Promise((resolve,reject)=>{
    ...
    const ready=()=>{if(globalThis.CommonweaveSystemRoutesV227){globalThis.CommonweaveSystemRoutesV227.authorize();resolve(true)}else reject(new Error('The five-system route contract loaded without becoming ready.'))};
...
async function fetchPart(pathname){
  ...
  const response=await fetch(url,{cache:'no-store',signal:controller.signal,redirect:'follow',headers:{'x-commonweave-package':'working-campus-v227'}});
...
async function boot(){
  ...
  dispatchEvent(new CustomEvent('commonweave:working-campus-runtime-ready',{detail:{revision:REVISION,parts:parts.length,at:new Date().toISOString(),policy:'canonical-core-only-five-system-routing'}}));
}
boot().catch(error=>{
