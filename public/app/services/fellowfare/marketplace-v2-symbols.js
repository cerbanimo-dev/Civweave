...
function receipts(){const raw=parse(localStorage.getItem(RECEIPT_KEY),{});return Array.isArray(raw)?raw:Array.isArray(raw?.receipts)?raw.receipts:[]}
...
function listingShareText(listing){
 ...
 if(Number(pricing.buttons)>0)terms.push(`${Number(pricing.buttons)} ${format('Buttons')}`);
 if(Number(pricing.acorns)>0)terms.push(`${Number(pricing.acorns)} ${format('Acorns')}`);
...
async function onCapture(event){
 ...
 if(wallet){event.preventDefault();event.stopImmediatePropagation();const p=projection();const text=`FellowFare balances\n${SYMBOLS.button} Buttons: ${Number(p.buttons||0)}\n${SYMBOLS.acorn} Acorns: ${Number(p.acorns||0)}\nSkill XP: ${Number(p.skillXp||0)}\nCommerce receipts: ${receipts().length}`;notify(await copyText(text)?'Balance diagnostics copied.':'Could not copy diagnostics.');return}
...
const api=Object.freeze({version:'1.0.0',symbols:SYMBOLS,format,decorate});
