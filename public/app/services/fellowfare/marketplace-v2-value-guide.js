...
function baselineText(kind,input={}){
 ...
 if(b.buttons>0)parts.push(g?.formatButtons?.(b.buttons)||`${b.buttons} 🔘 Buttons`);
 if(b.acorns>0)parts.push(g?.formatAcorns?.(b.acorns)||`${b.acorns} 🌰 Acorns`);
 if(kind==='learning'&&!parts.length)parts.push('5–50 🌰 Acorns');
 if(!parts.length)return b.hours>0?'No baseline currency applies to this type.':'Add or import human-equivalent labor hours to calculate a baseline.';
...
function renderGuide(){
 ...
 const html=`<section id="ffv2BasicValueGuide" class="ffv2-value-guide" aria-labelledby="ffv2ValueGuideTitle"><div class="ffv2-section-head"><div><p class="ffv2-eyebrow">SHARED BASIC VALUE GUIDE</p><h2 id="ffv2ValueGuideTitle">Baseline first. Market second.</h2></div></div><p class="ffv2-value-intro">This chart is shared by Cerbanimo, Living School, and FellowFare. It establishes a transparent pre-market baseline; live comparables can then move the asking price without rewriting the underlying labor or learning value.</p><div class="ffv2-value-table" role="table" aria-label="Civweave basic value guide">${rows.map(row=>`<article role="row"><strong role="cell">${esc(row.label)}</strong><b role="cell">${esc(row.value)}</b><small role="cell">${esc(row.note)}</small></article>`).join('')}</div></section>`;
 priceDesk.insertAdjacentHTML('beforebegin',html);
 const heading=priceDesk.querySelector('h2');if(heading)heading.textContent='Compare the baseline with the live market.';
 const copy=priceDesk.querySelector('#ffv2PriceAdvice')||priceDesk.querySelector('p:not(.ffv2-eyebrow)');if(copy&&/only calculate|invent a market/i.test(copy.textContent))copy.textContent='Live comparables are the market layer. They never replace the shared labor/learning baseline and Rook still will not invent market evidence.';
}
...
function updateComposer(form){
 ...
 const box=form.querySelector('#ffv2ValueComposerBaseline');if(box)box.innerHTML=`<strong>Shared baseline:</strong> ${esc(baselineText(kind,{hours,recommendedAcorns:candidate(clean(form.elements.draftId?.value,220))?.curriculumAcorns}))}<small>${hours?'Automation speed does not reduce these human-equivalent hours.':'Cerbanimo/Living School model-generated tasks carry this estimate automatically; you can also enter or revise it here.'}</small>`;
}
...
function decorateDrafts(){
 for(const card of document.querySelectorAll('.ffv2-draft-card[data-use-draft]')){
   ...
   const small=document.createElement('small');small.className='ffv2-draft-baseline';small.textContent=`Baseline: ${baselineText(kind,{hours,recommendedAcorns:row?.curriculumAcorns||row?.valuation?.curriculumAcorns})}`;card.append(small);
 }
}
function decorateListings(){
 ...
 for(const card of document.querySelectorAll('.ffv2-listing[data-listing-id]')){
   ...
   if(!(b.buttons>0||b.acorns>0))continue;const node=document.createElement('div');node.className='ffv2-saved-baseline';node.textContent=`Basic value: ${baselineText(row.kind,{hours:valuation.laborWorthHours,recommendedAcorns:valuation.curriculumAcorns})}`;card.querySelector('.ffv2-price')?.insertAdjacentElement('afterend',node);
 }
}
...
const api=Object.freeze({version:'1.0.0',enhance,baselineText});globalThis.CivweaveFellowFareValueGuideV1=api;
