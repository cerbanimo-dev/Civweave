...
exports.REWARD_PROTOCOL = "civweave.reward-weave.v1.1";
exports.LEGACY_REWARD_PROTOCOL = "civweave.reward-weave.v1";
exports.VALIDATION_PROTOCOL = "civweave.validation-packet.v1.1";
exports.LEGACY_VALIDATION_PROTOCOL = "civweave.validation-packet.v1";
exports.THRESHOLD_PROTOCOL = "civweave.validation-threshold.v1";
...
exports.LEGACY_REWARD_STORAGE_KEYS = {
    ...
    validation: "civweave.validation-ledger.v1",
    chronicle: "civweave.chronicle-ledger.v1",
};
...
function emptyRewardState() {
    ...
    return { living: emptyLiving(updatedAt), fellowfare: emptyFellowfare(updatedAt), co: { schema: "cerbanimo.co-ledger.v1", credits: [], updatedAt },
        validation: { schema: "civweave.validation-ledger.v1.1", submissions: [], packets: [], receipts: [], thresholdReceipts: [], completedSubmissionIds: [], processedExchangeIds: [], updatedAt },
        chronicle: { schema: "civweave.chronicle-ledger.v1.1", entries: [], updatedAt } };
}
...
function migrateRewardWeaveState(value) {
    ...
    const next = {
        ...
        validation: { ...empty.validation, ...(value.validation || {}), schema: "civweave.validation-ledger.v1.1", thresholdReceipts: value.validation?.thresholdReceipts || [], processedExchangeIds: value.validation?.processedExchangeIds || [] },
        chronicle: { ...empty.chronicle, ...(value.chronicle || {}), schema: "civweave.chronicle-ledger.v1.1" },
    };
...
const core=exports;
const EXCHANGE_PROTOCOL='civweave.validation-exchange.v1.1';
...
function write(state){const next=core.migrateRewardWeaveState(state);for(const room of Object.keys(keys))localStorage.setItem(keys[room],JSON.stringify(next[room]));dispatchEvent(new CustomEvent('civweave:reward-state-changed',{detail:next}));return next}
function vaultIdentity(){try{const vault=JSON.parse(localStorage.getItem('civweave-identity-vault')||'null');return{identityId:vault?.identity?.identityId,deviceId:vault?.deviceId}}catch{return{}}}
...
async function exchange(kind,payload,vault){if(!vault?.identity||!vault?.deviceId)throw Error('Portable identity is required.');const device=vault.identity.devices?.find(x=>x.deviceId===vault.deviceId&&!x.revokedAt);if(!device)throw Error('This device is not authorized by the portable identity.');const unsigned={schema:EXCHANGE_PROTOCOL,exchangeId:core.rewardId('validation-exchange'),kind,signerIdentityId:vault.identity.identityId,signerDeviceId:vault.deviceId,signerIdentity:vault.identity,signerPublicKey:device.publicKey,createdAt:new Date().toISOString(),payload};return{...unsigned,signature:await CivweaveIdentitySync.signValue(vault,unsigned)}}
function equal(a,b){return JSON.stringify(a)===JSON.stringify(b)}
async function verifyExchange(value){if(value?.schema!==EXCHANGE_PROTOCOL||!value.signature||!value.signerIdentity||!value.signerPublicKey)return false;if(value.signerIdentity.identityId!==value.signerIdentityId)return false;const device=value.signerIdentity.devices?.find(x=>x.deviceId===value.signerDeviceId&&!x.revokedAt);if(!device||!equal(device.publicKey,value.signerPublicKey))return false;const unsigned={...value};delete unsigned.signature;if(!await CivweaveIdentitySync.verifyValue(device.publicKey,unsigned,value.signature))return false;if(value.kind==='packet')return !value.payload?.contributorIdentityId||value.payload.contributorIdentityId===value.signerIdentityId;const receipt=value.payload;if(receipt?.validatorId!==value.signerIdentityId||receipt?.signature?.identityId!==value.signerIdentityId||receipt?.signature?.deviceId!==value.signerDeviceId||!equal(receipt?.signature?.publicKey,value.signerPublicKey))return false;const unsignedReceipt={...receipt};delete unsignedReceipt.signature;return CivweaveIdentitySync.verifyValue(value.signerPublicKey,unsignedReceipt,receipt.signature.value)}
