const CACHE="cerbanimo-rc22.3.14-wave-two-sync";
...
self.addEventListener("activate",event=>event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("cerbanimo-")&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
