...
(function(global){
  ...
  class CerbanimoWorldEngine extends EventTarget{
    ...
    renderObject(o,i){const b=o.bounds||{};return `<button type="button" class="visual-hotspot cw-world-object ${esc(o.className||'')}" data-world-object="${i}" aria-label="${esc(o.label||'Interact')}" style="left:${clamp(b.x,0,100)}%;top:${clamp(b.y,0,100)}%;width:${clamp(b.w,1,100)}%;height:${clamp(b.h,1,100)}%"><span>${esc(o.label||'Interact')}</span>${o.badge?`<b class="cw-world-object-badge">${esc(o.badge)}</b>`:''}</button>`}
