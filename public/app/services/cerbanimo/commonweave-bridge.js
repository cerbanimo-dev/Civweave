(() => {
  ...
  function applyContext(context) {
    ...
    if (
      ...
      typeof api.configureCommonweaveModelSession === "function"
    ) {
      api.configureCommonweaveModelSession(
        String(
    ...
    if (!badge) {
      ...
      badge.id = "commonweave-suite-badge";
  ...
  function renderQuestBoard(items = []) {
    let board = document.getElementById("commonweave-quest-board");
    if (!items.length) {
      board?.remove();
      return;
    }
    if (!board) {
      ...
      board.id = "commonweave-quest-board";
    ...
    board.innerHTML = `<header style="display:flex;align-items:center;justify-content:space-between;gap:12px"><div><small style="color:#b8ff6a;font:700 9px ui-monospace,monospace;letter-spacing:.12em">ANARCHADIA · APPROVED QUEST BOARD</small><strong style="display:block;margin-top:3px">${items.length} community plan${items.length === 1 ? "" : "s"} ready for member review</strong></div><button type="button" data-close-commonweave-board style="border:1px solid #ffffff42;border-radius:999px;background:transparent;color:#fff;padding:6px 10px">Hide</button></header><div style="display:grid;gap:8px;margin-top:10px">${items
      ...
      .join("")}</div>`;
    board.querySelector("[data-close-commonweave-board]")?.addEventListener(
      "click",
      () => board.remove(),
    );
  }
  ...
  function selectedPlanPayload() {
    ...
    return {
      schema: "commonweave.cerbanimo-plan.v1",
  ...
  const PROJECT_CONTRACT = "commonweave.cerbanimo-project.v1";
  ...
  function projectRequest(data, expectedType) {
    ...
    if (data.sourceApplication !== "commonweave") return null;
    ...
    if (
      expectedType === "commonweave:project-handoff-request" &&
  ...
  function ensureProjectLinks(state) {
    state.commonweave = state.commonweave || {};
    if (!Array.isArray(state.commonweave.projectLinks)) state.commonweave.projectLinks = [];
    return state.commonweave.projectLinks;
  }
  ...
  function projectStatusDescriptor(state, link) {
    ...
    if (proposal?.status === "cancelled" || proposal?.status === "rejected") {
      return {status:"rejected",event:"project-rejected",type:"commonweave:project-rejected",detail:"The linked Cerbanimo proposal was rejected or closed."};
    }
    const quest = questForProjectLink(state, link);
    if (!quest) {
      return {status:"submitted",event:proposal?"project-linked":"handoff-accepted",type:proposal?"commonweave:project-linked":"commonweave:project-status-returned",detail:proposal?"Kamiya prepared a proposal linked to this Living School project. It still requires learner ratification.":"Cerbanimo has the saved project packet. A quest has not yet been ratified."};
    }
    const tasks = state.tasks.filter((task) => task.questId === quest.id);
    if (!tasks.length) return {status:"submitted",event:"project-created",type:"commonweave:project-created",detail:"The linked Cerbanimo quest exists, but no project work units have been accepted yet.",quest};
    ...
    if (revision) {
      ...
      return {status:"revision-requested",event:"revision-requested",type:"commonweave:project-revision-requested",detail:"Cerbanimo returned at least one work unit for revision.",reviewFeedback:projectText([revision.review?.reason,instructions].filter(Boolean).join(" "),2400),reviewId:revision.review?.id,evidenceRef:revision.submission?.id,quest};
    }
    ...
    if (everyAccepted) {
      return {status:"accepted",event:"project-accepted",type:"commonweave:project-accepted",detail:"Cerbanimo verified that every linked work unit has evidence and a passing selected-model review.",reviewId:evidence[0]?.review?.id,evidenceRef:evidence[0]?.submission?.id,quest};
    }
    ...
    if (reviewed) return {status:"under-review",event:"review-pending",type:"commonweave:project-review-pending",detail:"Cerbanimo has review activity for the linked project. Acceptance still requires passing evidence review for every work unit.",reviewId:reviewed.review?.id,evidenceRef:reviewed.submission?.id,quest};
    if (submitted) return {status:"under-review",event:"evidence-submitted",type:"commonweave:project-evidence-submitted",detail:"Project evidence has reached Cerbanimo and is waiting for review.",evidenceRef:submitted.submission?.id,quest};
    return {status:"submitted",event:"project-linked",type:"commonweave:project-linked",detail:"The Cerbanimo project is linked and active. No accepted evidence receipt has returned yet.",quest};
  }
  ...
  function projectReceipt(type, request, link, detail, status, options = {}) {
    ...
    const receiptType = type || descriptor.type || "commonweave:project-status-returned";
    ...
    return {
      ...
      projectUrl:`/services/cerbanimo/index.html?commonweave=1&projectRef=${encodeURIComponent(request.projectRef)}`,
  ...
  function postProjectError(request, detail, status = "integration-unavailable") {
    post(projectReceipt(status==="rejected"?"commonweave:project-rejected":"commonweave:project-unavailable",request,null,detail,status,{event:status==="rejected"?"project-rejected":"project-unavailable"}));
  }

  async function acceptProjectHandoff(data) {
    const request = projectRequest(data,"commonweave:project-handoff-request");
    if (!request) return;
    ...
    state.commonweave.pendingProject={id:link.id,projectRef:link.projectRef,title:link.title,creativeIntention:link.creativeIntention,receivedAt:new Date().toISOString()};
    touchLinkStatus(link,{status:"submitted",event:"handoff-accepted"});
    api.setState(state);
    post(projectReceipt("commonweave:project-handoff-accepted",request,link,"Cerbanimo acknowledged and saved the final-project packet. Kamiya is preparing a reviewable project proposal; no quest has been created without learner ratification.","submitted",{event:"handoff-accepted"}));
    ...
    try {
      ...
      if(updatedLink&&proposal){updatedLink.proposalId=proposal.id;updatedLink.threadId=proposal.threadId||state.ui.selectedThreadId;updatedLink.status="submitted";updatedLink.lastRequestIdentity={...request};updatedLink.updatedAt=new Date().toISOString();state.commonweave.pendingProject.proposalId=proposal.id;api.setState(state);post(projectReceipt("commonweave:project-linked",request,updatedLink,"Kamiya prepared and linked the project proposal. It is awaiting learner inspection and ratification in Cerbanimo.","submitted",{event:"project-linked"}));}
    } catch(error) {
      ...
      post(projectReceipt("commonweave:project-status-returned",request,updatedLink||link,`Cerbanimo saved the project packet, but Kamiya's selected model needs attention before it can draft the proposal: ${projectText(error?.message||error,700)}`,"submitted",{event:"status-returned"}));
    }
  }

  async function returnProjectStatus(data) {
    const request=projectRequest(data,"commonweave:project-status-request");
    if(!request)return;
    const api=await waitForHost();
  ...
  const ACTION_SIGNAL_CONTRACT = "commonweave.action-signal.v1";
  const NAVIGATION_CONTRACT = "commonweave.navigation.v1";
  const PARTY_DISCOVERY_CONTRACT = "commonweave.party-discovery.v1";
  const PARTY_STATUS_CONTRACT = "commonweave.party-status.v1";
  ...
  function broadcastAttentionChanges() {
    ...
    const state = api.getState();
    state.commonweave = state.commonweave || {};
    const previous = state.commonweave.attentionSignals && typeof state.commonweave.attentionSignals === "object" ? state.commonweave.attentionSignals : {};
    const current = {};
    for (const signal of attentionSignals(state)) {
      ...
      post({ type: "commonweave:action-signal", contractVersion: ACTION_SIGNAL_CONTRACT, sourceApplication: "cerbanimo", ...signal, timestamp: new Date().toISOString() });
    }
    for (const [signalId, prior] of Object.entries(previous)) {
      if (current[signalId] || !prior || prior.state === "resolved") continue;
      post({ type: "commonweave:action-signal", contractVersion: ACTION_SIGNAL_CONTRACT, sourceApplication: "cerbanimo",
    ...
    state.commonweave.attentionSignals = current;
    api.setState(state);
  }
  ...
  function partyStatusMessage() {
    ...
    const party = (state.parties || []).find((item) => item.questId === selectedQuestId)
      ...
      || (state.parties || [])[0];
    if (!party) return { type: "commonweave:party-status", contractVersion: PARTY_STATUS_CONTRACT, sourceApplication: "cerbanimo", status: "empty", updatedAt: "1970-01-01T00:00:00.000Z", revision: 0 };
    const quest = (state.quests || []).find((item) => item.id === party.questId || item.sourcePartyId === party.id);
    if (!quest) return { type: "commonweave:party-status", contractVersion: PARTY_STATUS_CONTRACT, sourceApplication: "cerbanimo", status: "empty", updatedAt: "1970-01-01T00:00:00.000Z", revision: 0 };
    ...
    return {
      type: "commonweave:party-status", contractVersion: PARTY_STATUS_CONTRACT, sourceApplication: "cerbanimo", status: "active",
  ...
  async function receivePartyDiscovery(data) {
    ...
    if (data.contractVersion !== PARTY_DISCOVERY_CONTRACT || data.sourceApplication !== "commonweave" || data.authoritativeMembership !== false) return;
    ...
    if (!discoveryId || !partyId || !discoveryId.startsWith("host-party:")) {
      post({ type: "commonweave:navigation-receipt", contractVersion: NAVIGATION_CONTRACT, sourceApplication: "cerbanimo", actionId, status: "unavailable", detail: "The host-node party discovery record was malformed." }); return;
    }
    const api = await waitForHost();
    if (!api) { post({ type: "commonweave:navigation-receipt", contractVersion: NAVIGATION_CONTRACT, sourceApplication: "cerbanimo", actionId, status: "unavailable", detail: "Cerbanimo did not finish initializing." }); return; }
    const state = api.getState();
    state.commonweave = state.commonweave || {};
    state.commonweave.partyDiscoveries = Array.isArray(state.commonweave.partyDiscoveries) ? state.commonweave.partyDiscoveries : [];
    if (!state.commonweave.partyDiscoveries.some((item) => item.discoveryId === discoveryId)) state.commonweave.partyDiscoveries.unshift({ discoveryId, partyId, title: projectText(data.title, 220), purpose: projectText(data.purpose, 1200), expiresAt: projectText(data.expiresAt, 80), receivedAt: new Date().toISOString(), authoritativeMembership: false });
    api.setState(state);
    const rawRoute = projectText(data.invitationUrl, 16000);
    if (!rawRoute) {
      post({ type: "commonweave:navigation-receipt", contractVersion: NAVIGATION_CONTRACT, sourceApplication: "cerbanimo", actionId, status: "unavailable", detail: "The beacon was discovered, but its owner did not attach a signed Cerbanimo invitation. No membership changed." }); return;
    }
    try {
      ...
      post({ type: "commonweave:navigation-receipt", contractVersion: NAVIGATION_CONTRACT, sourceApplication: "cerbanimo", actionId, status: "opened", detail: "Cerbanimo validated and opened the signed invitation for review. Membership has not been granted yet." });
      broadcastAttentionChanges();
    } catch (error) {
      post({ type: "commonweave:navigation-receipt", contractVersion: NAVIGATION_CONTRACT, sourceApplication: "cerbanimo", actionId, status: "unavailable", detail: `The party beacon was found, but Cerbanimo could not validate its signed invitation: ${projectText(error?.message || error, 420)}` });
    }
  }

  async function navigateObject(data) {
    if (data.contractVersion !== NAVIGATION_CONTRACT || data.sourceApplication !== "commonweave") return;
    const api = await waitForHost();
    ...
    if (!api || !id || !["project","quest","task","invitation","review","audit","appeal","receipt"].includes(object)) {
      post({ type: "commonweave:navigation-receipt", contractVersion: NAVIGATION_CONTRACT, sourceApplication: "cerbanimo", actionId, status: "unavailable", detail: "Cerbanimo could not resolve that object." }); return;
    }
    const state = api.getState();
    try {
      if (object === "task") {
      ...
      post({ type: "commonweave:navigation-receipt", contractVersion: NAVIGATION_CONTRACT, sourceApplication: "cerbanimo", actionId, status: "opened", detail: `Opened the requested ${object} in Cerbanimo.` });
    } catch (error) {
      post({ type: "commonweave:navigation-receipt", contractVersion: NAVIGATION_CONTRACT, sourceApplication: "cerbanimo", actionId, status: "unavailable", detail: projectText(error?.message || error, 500) });
    }
  }
  ...
  window.addEventListener("message", async (event) => {
    if (
      ...
      return;
    if (event.data.type === "commonweave:reward-settled") {
      ...
      post({type:"commonweave:reward-settlement-ack",contractVersion:event.data.contractVersion,sourceApplication:"cerbanimo",submissionId:event.data.submissionId,taskId:task.id,status:"applied",detail:"Cerbanimo recorded the Fellowfare labor mint and vested Co credits without copying their balances."});
      return;
    }
    if (event.data.type === "commonweave:party-discovery") {
      await receivePartyDiscovery(event.data);
      return;
    }
    if (event.data.type === "commonweave:navigate-object") {
      await navigateObject(event.data);
      return;
    }
    if (event.data.type === "commonweave:project-handoff-request") {
      await acceptProjectHandoff(event.data);
      return;
    }
    if (event.data.type === "commonweave:project-status-request") {
      await returnProjectStatus(event.data);
      return;
    }
    if (event.data.type === "commonweave:context") applyContext(event.data);
    if (event.data.type === "commonweave:governance-sync") {
    ...
    if (event.data.type === "commonweave:ai-intention") {
      const api = host();
      if (!api) return;
      ...
      const journey =
        event.data.journey?.schema === "commonweave.intention-journey.v1"
          ? event.data.journey
          : null;
      const state = api.getState();
      if (state.commonweave?.lastRoutedRequestId === requestId) {
        post({
      ...
      state.commonweave = {
        ...
        activeJourney: journey || state.commonweave?.activeJourney || null,
    ...
    if (event.data.type === "commonweave:request-plan") {
    ...
    if (event.data.type === "commonweave:governance-receipt") {
      const api = host();
      if (!api) return;
      const state = api.getState();
      state.commonweave = {
    ...
    if (event.data.type === "commonweave:import-capstone") {
      post({
        type: "commonweave:handoff",
        source: "living",
        target: "cerbanimo",
  ...
  async function broadcastRewardChanges() {
    ...
    for (const quest of state.quests || []) {
      ...
      post({
        type:"commonweave:quest-rewards-proposed",
        contractVersion:"commonweave.reward-weave.v1.1",
...
Reflection: ${submission?.reflection||""}`.trim();
      ...
      post({
        type:"commonweave:reward-submission",
        contractVersion:"commonweave.reward-weave.v1.1",
        sourceApplication:"cerbanimo",
        submission:{
          ...
          contributorId:(()=>{try{return JSON.parse(localStorage.getItem("commonweave-identity-vault")||"null")?.identity?.identityId||state.identity?.fingerprint||state.profile?.alias||"local-contributor"}catch{return state.identity?.fingerprint||state.profile?.alias||"local-contributor"}})(),
          contributorName:state.profile?.name||state.profile?.alias||"Local contributor",
          contributors:[{contributorId:(()=>{try{return JSON.parse(localStorage.getItem("commonweave-identity-vault")||"null")?.identity?.identityId||state.identity?.fingerprint||state.profile?.alias||"local-contributor"}catch{return state.identity?.fingerprint||state.profile?.alias||"local-contributor"}})(),contributorName:state.profile?.name||state.profile?.alias||"Local contributor",shareBps:10000}],
    ...
    for (const settlement of state.settlements || []) {
      ...
      post({
        type:"commonweave:reward-threshold-reached",
        contractVersion:"commonweave.reward-weave.v1.1",
        sourceApplication:"cerbanimo",
    ...
    for (const capsule of state.capsules || []) {
      ...
      post({
        type:"commonweave:validator-reward",
        contractVersion:"commonweave.reward-weave.v1.1",
        sourceApplication:"cerbanimo",
