...
function ensureGuildUsage(){
  if(String(globalThis.CivweaveGuildChatUsageV1?.version||'').startsWith(GUILD_USAGE_VERSION))return true;
  if(typeof document==='undefined')return false;
  const path=new URL(GUILD_USAGE_SRC,globalThis.location?.href||'https://civweave.invalid').pathname;
  if([...document.scripts].some(script=>{try{return new URL(script.src,globalThis.location?.href||'https://civweave.invalid').pathname===path&&script.dataset.civweaveGuildUsageRepair==='v2'}catch{return false}}))return false;
  const script=document.createElement('script');script.src=`${GUILD_USAGE_SRC}&repair=${Date.now()}`;script.async=false;script.dataset.civweaveGuildUsageRepair='v2';(document.head||document.documentElement).append(script);return true;
}
