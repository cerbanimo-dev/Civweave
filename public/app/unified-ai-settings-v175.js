(()=>{
'use strict';
const VERSION='175.0-deterministic-single-authority';
...
const DETERMINISTIC_MODEL='commonweave-deterministic-v175';
...
function providerName(value){
  ...
  if(LEGACY_LOCAL.has(raw)||raw==='deterministic')return'deterministic';
...
function migrateDeterministicDefault(){
  const legacy=parse(localStorage.getItem(SETTINGS_KEY),{}),legacyRoute=providerName(legacy.provider||legacy.route);
  const profiles=parse(localStorage.getItem(PROFILES_KEY),{}),interactive=profiles.interactive;
  const profileRoute=interactive?providerName(interactive.provider||interactive.route):null;
  if(!interactive||profileRoute==='deterministic'&&LEGACY_LOCAL.has(String(interactive.provider||interactive.route||'').toLowerCase())){
    profiles.interactive={route:'deterministic',provider:'deterministic',model:DETERMINISTIC_MODEL,endpoint:'',externalConsent:false};
    profiles.agenticEnabled=false;
    profiles.agentic=null;
    localStorage.setItem(PROFILES_KEY,JSON.stringify(profiles));
  }
  if(!legacy.route||legacyRoute==='deterministic'&&LEGACY_LOCAL.has(String(legacy.provider||legacy.route||'').toLowerCase())){
    localStorage.setItem(SETTINGS_KEY,JSON.stringify({route:'deterministic',provider:'deterministic',model:DETERMINISTIC_MODEL,endpoint:'',consent:false,agenticEnabled:false}));
  }
}
function readState(){
  migrateDeterministicDefault();
  const legacy=parse(localStorage.getItem(SETTINGS_KEY),{route:'deterministic',provider:'deterministic',model:DETERMINISTIC_MODEL,endpoint:'',consent:false});
...
function markup({inline=false}={}){
  return`<form class="cw-ai-settings-form${inline?' cw-ai-inline-form':''}" data-unified-ai-settings-v175 data-version="${VERSION}">
    ${inline?'':`<header class="cw-ai-header"><div class="cw-ai-header-copy"><small>COMMONWEAVE AI</small><h2>Commonweave AI settings</h2><p>This is the single configuration surface used by Weaveling and every realm guide.</p></div><button class="cw-ai-close" type="button" data-close aria-label="Close settings">×</button></header>`}
    ${inline?'<div class="cw-ai-inline-intro"><small>COMMONWEAVE AI</small><h2>Commonweave AI settings</h2></div>':''}
    ...
    <section class="cw-ai-route-panel" data-route-panel="deterministic">
      ...
      <p>Commonweave uses inspectable rules, canonical local state, and explicit approval gates. No model is loaded and no prompt leaves this device.</p>
      <div class="cw-ai-fallback-contract"><b>Transformer laboratory is parked</b><span>The archived Transformer and MiniLM files remain in the repository for later testing, but the installed application does not load, download, warm, probe, or validate them.</span></div>
    </section>
    ...
    <section class="cw-ai-route-panel" data-route-panel="gemini" hidden>
      ...
      <label class="cw-ai-agent-toggle"><input name="agenticEnabled" type="checkbox"><span><b>Use Antigravity for agentic and background work</b><small>Conversation stays on Gemini. Tool-using work can use Antigravity when your key has access.</small></span></label>
      <div class="cw-ai-agent-options" data-agent-options hidden><label>Agentic model<input name="agenticModel" value="antigravity" readonly></label><p>Antigravity is tested separately. A Gemini response is not reported as an Antigravity success.</p></div>
    ...
    <footer class="cw-ai-form-footer"><p>Provider preferences are stored locally. Credentials stay in session storage and are excluded from exports and offline seeds.</p><div class="cw-ai-actions"><button type="submit" class="cw-ai-save">Save Commonweave AI settings</button></div><output data-save-status role="status"></output></footer>
  </form>`;
}
function sync(form){
...
function fill(form){
  ...
  keyInput.value='';
  keyInput.placeholder=state.hasKey?'A session key is loaded. Enter another to replace it.':'Paste a Google Gemini API key';
...
function setupFrom(form){
  ...
  if(route==='deterministic')return{route,key:'',consent:false,agentic:false,interactive:{route:'deterministic',provider:'deterministic',model:DETERMINISTIC_MODEL,endpoint:'',externalConsent:false},agenticConfig:null};
  if(route==='gemini'){
...
function persist(setup){
  ...
  if(modelRuntime){
    ...
    else modelRuntime.saveModelProfiles({agenticEnabled:false,agentic:null});
  }else{
    localStorage.setItem(PROFILES_KEY,JSON.stringify({interactive:setup.interactive,agentic:setup.agenticConfig,agenticEnabled:setup.agentic}));
  }
...
async function pasteKey(form,button){
  const input=byName(form,'apiKey');button.disabled=true;
  try{const key=extractKey(await navigator.clipboard.readText());if(!key)throw new Error('The clipboard did not contain a recognizable key.');input.value=key;form.querySelector('[data-secret-note]').textContent='Clipboard key loaded for this session. Save to apply it.'}
  catch(error){form.querySelector('[data-secret-note]').textContent=`Paste failed: ${error.message}`}
  finally{button.disabled=false;input.focus()}
}
async function importKey(form,file){
  if(!file)return;const key=extractKey(await file.text());if(!key)throw new Error('The file did not contain GEMINI_API_KEY, GOOGLE_API_KEY, apiKey, or a raw key.');byName(form,'apiKey').value=key;form.querySelector('[data-secret-note]').textContent=`Loaded ${file.name}. Save to apply it.`;
}
function forgetKey(form){
  sessionStorage.removeItem(SESSION_KEY);
  try{sessionStorage.removeItem('commonweave-model-secrets-v1')}catch{}
  byName(form,'apiKey').value='';
  form.querySelector('[data-secret-note]').textContent='The session key was forgotten.';
}
async function testGemini(form,profile,button){
  const setup=setupFrom(form),status=form.querySelector(`[data-test-status="${profile}"]`);button.disabled=true;status.textContent=`Testing ${profile==='agentic'?'Antigravity':'Gemini'}…`;status.className='cw-ai-test-status';
  try{
    if(!setup.key)throw new Error('Enter a Gemini API key first.');
    if(!setup.consent)throw new Error('Allow Gemini requests before testing.');
    if(!runtime()?.generate)throw new Error('The shared model runtime has not loaded.');
    const config=profile==='agentic'?(setup.agenticConfig||{...setup.interactive,model:'antigravity'}):setup.interactive;
    const result=await runtime().generate({purpose:`commonweave-settings-${profile}-test`,executionProfile:profile==='agentic'?'agentic':'interactive',config:{...config,apiKey:setup.key},messages:[{role:'user',content:'Reply with the single word READY.'}]});
    ...
    const actualModel=String(result.actual?.model||'');if(profile==='agentic'&&!actualModel.toLowerCase().includes('antigravity'))throw new Error(`The request completed with ${actualModel||'another model'}, not Antigravity.`);
    status.textContent=`${profile==='agentic'?'Antigravity':'Gemini'} answered successfully with ${actualModel||config.model}.`;status.className='cw-ai-test-status is-ok';
...
function bind(form){
  if(!form||form.dataset.unifiedAiBound==='true')return form;
  form.dataset.unifiedAiBound='true';
  byName(form,'route').addEventListener('change',()=>sync(form));
  byName(form,'agenticEnabled').addEventListener('change',()=>sync(form));
  ...
  form.addEventListener('submit',event=>{event.preventDefault();const setup=setupFrom(form);persist(setup);form.querySelector('[data-save-status]').textContent=setup.route==='deterministic'?'Saved deterministic local mode. No model will load.':setup.agentic?'Saved Gemini and Antigravity. Deterministic mode remains available as a manual route.':'Saved the selected provider.'});
  return form;
}
function build(){
  let dialog=document.querySelector('#cw-ai-settings-v175');if(dialog)return{dialog,created:false};
  dialog=document.createElement('dialog');dialog.id='cw-ai-settings-v175';dialog.className='cw-ai-settings-dialog';dialog.innerHTML=markup();document.body.append(dialog);
...
function open(){const{dialog,created}=build();if(!created)fill(dialog.querySelector('form'));if(!dialog.open)dialog.showModal();return dialog}
function renderInline(target){const host=typeof target==='string'?document.querySelector(target):target;if(!host)return null;host.innerHTML=markup({inline:true});const form=bind(host.querySelector('form'));fill(form);return form}
migrateDeterministicDefault();
const api=Object.freeze({version:VERSION,eventOwnership:'controller-only',open,renderInline,readState,migrateDeterministicDefault,providerName,extractKey});
globalThis.CommonweaveUnifiedAISettingsV175=api;
globalThis.CommonweaveModelSettingsV133=api;
globalThis.CommonweaveModelSettingsV157=api;
})();
