...
self.addEventListener('activate',event=>event.waitUntil((async()=>{
  ...
  const stale=keys.filter(key=>key.startsWith('commonweave-')||/^(living-school|cerbanimo|fellowfare|anarchadia)-/.test(key));
  ...
  await Promise.allSettled(windows.filter(client=>new URL(client.url).pathname.startsWith('/app/')).map(client=>client.navigate(new URL(TARGET,self.location.origin).href)));
