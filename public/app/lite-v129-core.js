...
function setRoute(next,{replace=false}={}){workspace=null;state={...state,...next};const url=CivweaveParity.liteUrl(state);history[replace?'replaceState':'pushState'](state,'',url);render()}
...
function applyShell(system){
  ...
  $('#screen-guide').textContent=`${system.guide} · ${shell.motif||'shared Civweave workstation'}`;
  $('#screen-system').textContent=system.name;
}
function renderCabinetControls(system){
  const target=$('#cabinet-controls');const order=system.interfaceShell?.controlOrder||['anarchadia','fellowfare','civweave','living-school','cerbanimo'];
...
function journeyProgress(){
  ...
  if(parse(localStorage.getItem(SETTINGS_KEY),null))done.add('civweave.model-setup');
  if(w.wish)done.add('civweave.state-wish');
  if(w.clarification?.outcome||w.clarification?.context||w.clarification?.constraints)done.add('civweave.clarify-wish');
  if(w.skill?.posture)done.add('civweave.skill-posture');
  if(w.weave)done.add('civweave.generate-weave');
  if(w.weave?.reviewedAt)done.add('civweave.review-weave');
  if(w.activatedAt)done.add('civweave.activate-weave');
  if(w.passport)done.add('civweave.store-intention');
  return done;
}
...
function renderRoom(system,room,caps){
  const journey=system.id==='civweave'?renderJourney():'';
