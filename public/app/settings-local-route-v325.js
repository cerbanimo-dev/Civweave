...
const REVISION='settings-v325-parent-source-recovery-v2-card-owned-progress';
...
const STATUS_PLACEMENT_SRC='/app/settings-local-progress-placement-v1.js?v=1.0.0-settings-local-progress-card-owned';
...
function ensureProgressPlacement(){
  ...
  if(existing?.localize){existing.schedule?.([0,80,250]);return Promise.resolve(existing)}
  ...
  placementPromise=new Promise((resolve,reject)=>{
    ...
    const ready=()=>{const api=globalThis.CivweaveSettingsLocalProgressPlacementV1;if(api?.localize){api.schedule?.([0,80,250]);resolve(api);return}reject(new Error('Local AI progress placement loaded without becoming ready.'))};
    if(script){
      if(globalThis.CivweaveSettingsLocalProgressPlacementV1?.localize){ready();return}
      script.addEventListener('load',ready,{once:true});
      script.addEventListener('error',()=>reject(new Error('Local AI progress placement could not load.')),{once:true});
      return;
    }
    script=document.createElement('script');script.src=STATUS_PLACEMENT_SRC;script.async=false;script.dataset.civweaveLocalProgressPlacement='v1';
...
function ensureDirect(layer=document.getElementById(LAYER_ID),force=false){
  ...
  if(existing?.render){if(layer?.isConnected&&!layer.hidden)existing.render(layer);void ensureProgressPlacement().catch(()=>{});return Promise.resolve(existing)}
...
BRIDGE=Object.freeze({
  ...
  cardOwnedProgress:true,globalProgressBanner:false,progressPlacementPath:STATUS_PLACEMENT_SRC,
