...
function boot(){installStyle();apply();globalThis.visualViewport?.addEventListener('resize',schedule,{passive:true});addEventListener('resize',schedule,{passive:true});addEventListener('orientationchange',schedule,{passive:true});try{dispatchEvent(new CustomEvent('civweave:persistent-guide-viewport-ready',{detail:{version:VERSION,visualViewport:Boolean(globalThis.visualViewport),scrollTrap:false,at:new Date().toISOString()}}))}catch{}}
...
globalThis.CivweavePersistentGuideViewportV216=Object.freeze({version:VERSION,rootId:ROOT_ID,launcherId:LAUNCHER_ID,refresh:schedule,destroy,state:()=>({visualViewportHeight:globalThis.visualViewport?.height||innerHeight,scrollTrap:false,mutationObserver:false,autoScroll:false})});
})();
