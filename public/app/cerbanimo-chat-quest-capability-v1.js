...
async function createQuest(request={}){
  ...
  try{
    ...
    try{dispatchEvent(new CustomEvent('civweave:cerbanimo-chat-quest-created',{detail:{questId:saved.id,title:saved.title,taskCount:count,provider,model,sourceActionId}}))}catch{}
    return questPacket(`Quest created: “${clean(saved.title||plan.title,240)}”\n\n${count} work unit${count===1?'':'s'} are active in Cerbanimo, each with a proof gate and completion criteria.${plan.assumptions.length?`\n\nAssumptions recorded: ${plan.assumptions.join('; ')}`:''}`,first?`Start: ${first}`:'Open the Workboard to begin.',{provider,model,assumptions:plan.assumptions,action:{kind:'cerbanimo-quest-created',system:'cerbanimo',state:'active',questId:saved.id||'',title:saved.title||plan.title,taskCount:count,source:'kamiya-chat-ai-quest',canonicalArtifact:'Quest'}});
...
function install(){
  ...
  try{dispatchEvent(new CustomEvent('civweave:cerbanimo-chat-quest-capability-ready',{detail:{version:VERSION,system:'cerbanimo',structuredQuestAuthoring:true,deterministicQuestCreation:false}}))}catch{}
  return true;
}
...
globalThis.CivweaveCerbanimoChatQuestCapabilityV1=Object.freeze({version:VERSION,install,handler,createQuest,questIntent:text=>QUEST_INTENT.test(clean(text,12000)),completionText,modelObject,normalizeQuestPlan,schema:QUEST_SCHEMA,state:()=>({installed})});
})();