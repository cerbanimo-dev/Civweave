...
async function sendMesh(envelope){const runtime=await mesh();if(!runtime?.createObject)return{ok:false,reason:'mesh-unavailable'};await configureMeshGroups();const peers=(runtime.status?.().sessions||[]).filter(row=>row?.peerId&&row?.peerVerified&&Array.isArray(row.peerClaimedGroups)&&row.peerClaimedGroups.includes(envelope.channelId)).map(row=>row.peerId);if(!peers.length)return{ok:false,reason:'no-chat-peer'};const object=await runtime.createObject({kind:MESH_KIND,purpose:'Lud Mode Guild/Party chat',audience:[...new Set(peers)],consent:'direct',payload:envelope,hopLimit:1,priority:85});runtime.flushAll?.().catch(()=>{});return{ok:true,objectId:object.id,peers:peers.length}}
