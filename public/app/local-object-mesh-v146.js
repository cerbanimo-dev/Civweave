...
function makeSession(connection,role,groups=[]){const session={id:uid('session'),connection,channel:null,peerId:null,peerGroups:[],peerClaimedGroups:[],peerCapabilities:null,peerProtocol:null,peerVerified:false,role,groups:[...new Set(groups.map(String))].slice(0,64),openedAt:null,lastSyncAt:null};sessions.set(session.id,session);connection.addEventListener('connectionstatechange',()=>emit('peer-state',{sessionId:session.id,peerId:session.peerId,state:connection.connectionState}));return session}
