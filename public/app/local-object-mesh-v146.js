...
async function validateObject(object){if(object?.schema!==OBJECT_SCHEMA||!object.id||!object.revisionHash||!object.signature)return{ok:false,error:'invalid object envelope'};if(await sha256(canonical(object.payload))!==object.payloadHash)return{ok:false,error:'payload hash mismatch'};if(await sha256(canonical(signableObject(object)))!==object.revisionHash)return{ok:false,error:'revision hash mismatch'};if(!await verify(object.origin?.credential,signableObject(object),object.signature))return{ok:false,error:'signature rejected'};if(object.expiresAt&&Date.parse(object.expiresAt)<=Date.now())return{ok:false,error:'object expired'};return{ok:true}}
async function ingest(object,context={}){
  ...
  if(local?.revisionHash===object.revisionHash)return{status:'duplicate',object:local};
  if(local&&Number(local.revision)>Number(object.revision))return{status:'older',object:local};
...
function makeSession(connection,role,groups=[]){const session={id:uid('session'),connection,channel:null,peerId:null,peerGroups:[],peerCapabilities:null,peerProtocol:null,role,groups:[...new Set(groups.map(String))].slice(0,64),openedAt:null,lastSyncAt:null};sessions.set(session.id,session);connection.addEventListener('connectionstatechange',()=>emit('peer-state',{sessionId:session.id,peerId:session.peerId,state:connection.connectionState}));return session}
...
async function sendHello(session){await sendJson(session,{type:'hello',protocol:PROTOCOL,nodeId:await deviceId(),groups:session.groups,schemas:[OBJECT_SCHEMA],capabilities:{foregroundOnly:true,sleepingPhones:false,nativeCompanion:false,wifiDirect:false,resumableChunks:true,priorities:true,storeForwardPublic:true,directRelay:false,chunkBytes:CHUNK_BYTES,transportEncryption:'webrtc-dtls'}})}
...
async function candidateRows(session){const pending=await listOutbox('pending'),byKey=new Map();for(const delivery of pending){const object=await getObject(delivery.objectId).catch(()=>null);if(object)byKey.set(`${object.id}|${object.revisionHash}`,object)}for(const object of await listObjects())if(mayRelay(object))byKey.set(`${object.id}|${object.revisionHash}`,object);const rows=[];for(const object of byKey.values()){if(!intendedFor(object,session.peerId,session.peerGroups))continue;if(await acked(session.peerId,object))continue;if(object.expiresAt&&Date.parse(object.expiresAt)<=Date.now())continue;const transit=await transitFor(object),hopLimit=Math.max(0,Number(object.hopLimit)||0);if(transit.visited.includes(session.peerId)||transit.hopsUsed>=hopLimit||transit.hopsUsed>0&&!mayRelay(object))continue;const bytes=encoder.encode(JSON.stringify(object));if(bytes.byteLength>MAX_OBJECT_BYTES)continue;rows.push({object,transit,bytes,priority:await storedPriority(object),serializedHash:await sha256(bytes)})}rows.sort((a,b)=>b.priority-a.priority||Date.parse(a.object.createdAt||0)-Date.parse(b.object.createdAt||0));return rows}
async function flushSession(session){if(!session?.channel||session.channel.readyState!=='open'||!session.peerId)return{sent:0};if(session.peerProtocol&&session.peerProtocol!==PROTOCOL){const pending=await listOutbox('pending'),items=[];for(const delivery of pending){const object=await getObject(delivery.objectId).catch(()=>null);if(!object||!intendedFor(object,session.peerId,session.peerGroups)||await acked(session.peerId,object))continue;const transit=await transitFor(object);if(transit.visited.includes(session.peerId)||transit.hopsUsed>=Math.max(0,Number(object.hopLimit)||0))continue;items.push(object)}if(items.length)await sendJson(session,{type:'objects',items});session.lastSyncAt=now();emit('peer-flush',{peerId:session.peerId,count:items.length,protocol:'legacy'});return{sent:items.length}}const rows=await candidateRows(session),entries=rows.map(({object,transit,bytes,priority,serializedHash})=>({objectId:object.id,revisionHash:object.revisionHash,revision:Number(object.revision)||1,kind:object.kind,consent:object.consent,priority,bytes:bytes.byteLength,chunks:Math.max(1,Math.ceil(bytes.byteLength/CHUNK_BYTES)),serializedHash,hopsUsed:transit.hopsUsed,visited:transit.visited,expiresAt:object.expiresAt||null}));await sendJson(session,{type:'manifest',schema:`${PROTOCOL}.manifest`,entries});session.lastSyncAt=now();emit('peer-flush',{peerId:session.peerId,count:entries.length,protocol:PROTOCOL});return{sent:entries.length}}
...
async function exportBundle(){const owner=await credential();return{schema:'civweave.community-bundle.v1',createdAt:now(),node:{id:owner.id,publicKey:owner.publicKey},objects:await listObjects(),receipts:await tx(['receipts'],'readonly',stores=>req(stores.receipts.getAll()))}}
async function importBundle(bundle){if(bundle?.schema!=='civweave.community-bundle.v1'||!Array.isArray(bundle.objects))throw new Error('Unsupported community bundle');const results=[];for(const object of bundle.objects)try{results.push(await ingest(object,{fromPeer:bundle.node?.id,localNodeId:await deviceId()}))}catch(error){results.push({status:'rejected',id:object?.id,error:error.message})}return results}
...
function status(){return{protocol:PROTOCOL,foregroundOnly:true,sleepingPhones:false,nativeCompanion:false,wifiDirect:false,transport:'webrtc-datachannel',transportEncryption:'webrtc-dtls',chunkBytes:CHUNK_BYTES,sessions:[...sessions.values()].map(session=>({id:session.id,peerId:session.peerId,role:session.role,state:session.channel?.readyState||null,peerProtocol:session.peerProtocol,lastSyncAt:session.lastSyncAt}))}}
...
globalThis.CivweaveLocalMeshV146={DB_NAME,OBJECT_SCHEMA,DELIVERY_SCHEMA,RECEIPT_SCHEMA,PROTOCOL,deviceId,credential,createObject,queue,getObject,listObjects,listOutbox,ingest,validateObject,createOffer,acceptOffer,acceptAnswer,flushSession,flushAll,syncGateway,exportBundle,importBundle,requestBackgroundSync,setPriority,configure,status,subscribe,sessions};
})();
