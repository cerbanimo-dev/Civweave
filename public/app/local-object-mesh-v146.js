...
function chooseCanonicalRevision(local,incoming){
  ...
  return{relation:'conflict',winner:incomingHash<localHash?'incoming':'local'};
}
...
async function finalizeIncoming(session,row){const parts=[];let total=0;for(let i=0;i<row.totalChunks;i++){const part=unb64(row.chunks[i]);parts.push(part);total+=part.byteLength}if(total!==row.bytes)throw new Error('transfer length mismatch');const bytes=new Uint8Array(total);let offset=0;for(const part of parts){bytes.set(part,offset);offset+=part.byteLength}if(await sha256(bytes)!==row.serializedHash)throw new Error('transfer hash mismatch');const object=JSON.parse(decoder.decode(bytes));if(object.id!==row.objectId||object.revisionHash!==row.revisionHash)throw new Error('transfer identity mismatch');const localId=await deviceId();if(!intendedFor(object,localId,configuredGroups))throw new Error('object is outside this device audience');const result=await ingest(object,{fromPeer:session.peerId,localNodeId:localId}),canonical=result.object||object,acceptedIncoming=canonical.revisionHash===object.revisionHash,transit={hopsUsed:row.hopsUsed+1,visited:[...new Set([...row.visited,session.peerId,localId].filter(Boolean))],receivedFrom:session.peerId};if(result.status!=='conflict'||acceptedIncoming)await saveTransit(canonical,transit);await tx(['incoming'],'readwrite',stores=>stores.incoming.delete(row.transferId));await sendJson(session,{type:'receipt',objectId:object.id,revisionHash:object.revisionHash,status:result.status||'accepted'});if(result.status==='conflict'&&!acceptedIncoming){queueMicrotask(()=>flushAll().catch(()=>{}));return}if(mayRelay(canonical)&&transit.hopsUsed<Math.max(0,Number(canonical.hopLimit)||0))queueMicrotask(()=>flushAll().catch(()=>{}))}
...
async function handleLegacyObjects(session,message){const localId=await deviceId();for(const object of message.items||[])try{if(!intendedFor(object,localId,configuredGroups))throw new Error('object is outside this device audience');const result=await ingest(object,{fromPeer:session.peerId,localNodeId:localId}),canonical=result.object||object,acceptedIncoming=canonical.revisionHash===object.revisionHash;if(result.status!=='conflict'||acceptedIncoming)await saveTransit(canonical,{hopsUsed:1,visited:[...new Set([...(object.visited||[]),session.peerId,localId].filter(Boolean))],receivedFrom:session.peerId});if(result.status==='conflict'&&!acceptedIncoming)queueMicrotask(()=>flushAll().catch(()=>{}));if(result.receipt)await sendJson(session,{type:'receipt',receipt:result.receipt})}catch(error){await sendJson(session,{type:'rejection',objectId:object?.id,error:error.message})}}
