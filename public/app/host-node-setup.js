(()=>{
  ...
  function subscribeReleases(config=current()){
    const root=String(config?.baseUrl||DEFAULT_HOST).replace(/\/$/,'');if(!root||typeof EventSource==='undefined')return;
  ...
  const host=suppliedHost||current()?.baseUrl||DEFAULT_HOST;
