...
const STORE_KEY='civweave.boot-log.v1';
const SESSION_ID=sessionStorage.getItem('civweave.boot-session')||crypto.randomUUID();
sessionStorage.setItem('civweave.boot-session',SESSION_ID);
...
function log(kind,detail={}){
  const entry={schema:'civweave.boot-log.v1',id:crypto.randomUUID(),time:new Date().toISOString(),elapsedMs:Date.now()-startedAt,sessionId:SESSION_ID,version:VERSION,build:BUILD,kind:String(kind).slice(0,100),url:location.href,visibility:document.visibilityState,detail:clean(detail)};
  events.push(entry);persist();console.info('[CW-BOOT]',entry);transmit(entry);render();return entry;
}
function snapshot(){
  const nav=performance.getEntriesByType?.('navigation')?.[0];
  return {href:location.href,referrer:document.referrer,displayMode:matchMedia('(display-mode: standalone)').matches?'standalone':'browser',navigationType:nav?.type||'unknown',hostBuild:localStorage.getItem('civweave.host-build'),recoveryCompleted:localStorage.getItem('civweave.recovery.completed'),recoveryEntry:sessionStorage.getItem('civweave.recovery.entry'),userAgent:navigator.userAgent.slice(0,300),online:navigator.onLine};
}
...
window.CivweaveBootLog={log,events:()=>events.slice(),snapshot,inspectWorkers,download,copy,clear,version:VERSION,build:BUILD};
