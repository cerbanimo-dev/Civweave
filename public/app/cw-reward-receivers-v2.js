(()=>{
'use strict';
if(globalThis.CommonweaveRewardReceiversV2)return;
const api=globalThis.CommonweaveCanonicalRewardsV2;if(!api)throw Error('Canonical reward ledger must load before receivers.');
const V='2.0.0',EVENTS='commonweave.rewards.v156',SKILLS=['living-school.reward-ledger.v1.1','living-school.reward-ledger.v1'],BUTTONS=['fellowfare.reward-ledger.v1.1','fellowfare.reward-ledger.v1'],seen=new Set();let migrating=false;
const clean=(v,n=500)=>String(v??'').trim().slice(0,n),num=v=>Number.isFinite(Number(v))?Number(v):0,parse=(v,f)=>{try{return JSON.parse(v)??f}catch{return f}},copy=v=>JSON.parse(JSON.stringify(v));
const CONTRACT=`COMMONWEAVE REWARD CONTRACT commonweave.reward-contract.v2
Learning and doing are provenance categories, not separate XP currencies. Both add Skill XP to Living School's canonical skill ledger.
...
Use sourceKind "learning" for Living School and "doing" for Cerbanimo. Each skill amount is absolute. Never emit a generic rewardXp pool, never use amounts as weights, and never redistribute declared amounts. Acorns and Buttons are ledger transactions, not mutable counters. A receiver hashes, signs, validates, and deduplicates each entry. Return JSON only when a JSON schema is supplied.`;
...
function contractGenerate(runtime){
  ...
  const generate=async request=>{
    ...
    if(next&&relevant(next)){
      ...
      next={...next,schema:schema(next.schema),context:{...(next.context||{}),rewardContract:{version:'commonweave.reward-contract.v2',authority:'living-school',sourceKind,exactAmounts:true,levelFormula:'floor(sqrt(skillXp / 40)) + 1',assets:['skill-xp','acorn','button']}},messages:[...(next.messages||[]),{role:'system',content:CONTRACT}]};
    }
...
async function migrateEvents(){const value=parse(localStorage.getItem(EVENTS),null);for(const [i,row] of rows(value).entries()){const id=clean(row?.id||row?.receiptId||`${row?.createdAt||''}:${row?.currency||''}:${row?.skill||''}:${row?.amount||0}:${i}`);if(!id||id.startsWith('canonical:')||seen.has(`event:${id}`))continue;const currency=clean(row.currency||row.assetType,30).toLowerCase(),assetType=currency==='xp'?'skill-xp':currency==='acorn'?'acorn':currency==='button'?'button':'',amount=num(row.amount);if(assetType&&amount)await api.appendEntry({assetType,skillId:row.skill||row.skillId,skillName:row.skillName||row.skill,amount,sourceSystem:row.system||row.sourceSystem||'commonweave',sourceKind:row.sourceKind||row.phase||'migration',sourceId:row.sourceId||row.completionId||id,sourceKey:`legacy-event:${id}`,createdAt:row.createdAt,metadata:{legacyKey:EVENTS,legacyId:id}});seen.add(`event:${id}`)}}
async function migrateSkills(){for(const key of SKILLS){const state=parse(localStorage.getItem(key),null),receipts=Array.isArray(state?.xpReceipts)?state.xpReceipts:[];for(const [i,row] of receipts.entries()){const id=clean(row.id||row.receiptId||`${key}:${row.createdAt||''}:${row.skillSlug||row.skillId||row.skill||''}:${row.amount||row.xp||0}:${i}`);if(seen.has(`skill:${id}`))continue;const amount=num(row.amount??row.xp),skillId=row.skillSlug||row.skillId||row.skill;if(amount&&skillId)await api.appendEntry({assetType:'skill-xp',skillId,skillName:row.skillName||row.skill||skillId,amount,sourceSystem:row.sourceSystem||row.system||'living-school',sourceKind:row.sourceKind||row.phase||'migration',sourceId:row.sourceId||row.completionId||id,sourceKey:`legacy-skill:${id}`,createdAt:row.createdAt,metadata:{legacyKey:key,legacyId:id}});seen.add(`skill:${id}`)}}}
async function migrateButtons(){for(const key of BUTTONS){const state=parse(localStorage.getItem(key),null),receipts=Array.isArray(state?.receipts)?state.receipts:[];for(const [i,row] of receipts.entries()){const id=clean(row.id||row.receiptId||`${key}:${row.createdAt||''}:${row.amount||0}:${i}`);if(seen.has(`button:${id}`))continue;const amount=num(row.amount);if(amount)await api.appendEntry({assetType:'button',amount,sourceSystem:row.sourceSystem||row.system||'fellowfare',sourceKind:row.sourceKind||row.phase||'migration',sourceId:row.sourceId||id,sourceKey:`legacy-button:${id}`,createdAt:row.createdAt,metadata:{legacyKey:key,legacyId:id,operation:row.operation||row.type}});seen.add(`button:${id}`)}}}
...
function mirror(){const current=parse(localStorage.getItem(EVENTS),{}),legacy=rows(current).filter(x=>!String(x?.id||'').startsWith('canonical:')),events=api.readLedger().entries.map(e=>({id:`canonical:${e.id}`,currency:e.assetType==='skill-xp'?'xp':e.assetType,skill:e.skillId,amount:e.amount,system:e.sourceSystem,phase:e.sourceKind,sourceId:e.sourceId,createdAt:e.createdAt,canonicalHash:e.hash}));rawSet.call(localStorage,EVENTS,JSON.stringify({...(!Array.isArray(current)&&current&&typeof current==='object'?current:{}),schema:'commonweave.compatible-reward-events.v2',events:[...legacy,...events].slice(-25000),canonicalProjection:true,updatedAt:new Date().toISOString()}))}
function eventBundle(d){return d?.rewards||d?.rewardBundle||d?.module?.rewards||d?.task?.rewards||d?.completion?.rewards}
function bindEvents(){for(const name of ['living-school:module-completed','living-school:lesson-completed','living-school:assessment-completed','cerbanimo:task-completed','cerbanimo:quest-completed','commonweave:reward-bundle'])addEventListener(name,event=>{const d=event.detail||{},bundle=eventBundle(d);if(!bundle)return;const system=name.startsWith('living-school')?'living-school':name.startsWith('cerbanimo')?'cerbanimo':d.sourceSystem||'commonweave';api.issueRewardBundle(bundle,{sourceSystem:system,sourceKind:system==='cerbanimo'?'doing':system==='living-school'?'learning':d.sourceKind,sourceId:d.completionId||d.moduleId||d.taskId||d.id,evidenceHash:d.evidenceHash,validatorIds:d.validatorIds,accountId:d.accountId}).catch(console.warn)})}
function boot(){patchStorage();watch('CommonweaveModelRuntime',patchRuntime);bindEvents();addEventListener('storage',e=>{if([EVENTS,...SKILLS,...BUTTONS].includes(e.key))migrate().catch(console.warn)});addEventListener('commonweave:canonical-rewards-changed',mirror);migrate().then(mirror).catch(console.warn);setInterval(()=>{const current=globalThis.CommonweaveModelRuntime,patched=patchRuntime(current);if(patched&&patched!==current)globalThis.CommonweaveModelRuntime=patched},1500)}
globalThis.CommonweaveRewardReceiversV2=Object.freeze({version:V,promptContract:CONTRACT,rewardProperty,normalizeGeneratedTree:normalizeTree,patchRuntime,migrate});
boot();
})();
