...
async function recordThermal(){
  const start=Number($('battery-start').value),end=Number($('battery-end').value),thermal=$('thermal').value,minutes=Number($('observation-minutes').value),notes=$('physical-notes').value.trim(),battery=await batterySnapshot();
