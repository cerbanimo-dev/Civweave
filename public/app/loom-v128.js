...
function chat(){
  const historyValue=parse(localStorage.getItem(CHAT_KEY),[]);
...
function intentions(){
  ...
  const render=()=>{const savedItems=parse(localStorage.getItem(INTENTIONS_KEY),[]);const items=Array.isArray(savedItems)?savedItems:[];node.querySelector('[data-list]').innerHTML=items.length?items.map((item,index)=>`<label style="display:grid;grid-template-columns:auto 1fr;align-items:center;gap:9px;padding:9px;border:1px solid #7ee5ff33;border-radius:11px"><input type="checkbox" data-index="${index}" ${item.done?'checked':''}><span>${escapeHtml(item.text)}</span></label>`).join(''):'<p>No active intentions yet. The loom is quiet, not broken.</p>';node.querySelectorAll('[data-index]').forEach(box=>box.onchange=()=>{const saved=parse(localStorage.getItem(INTENTIONS_KEY),[]);const current=Array.isArray(saved)?saved:[];if(current[Number(box.dataset.index)])current[Number(box.dataset.index)].done=box.checked;localStorage.setItem(INTENTIONS_KEY,JSON.stringify(current));render()})};
  node.querySelector('form').onsubmit=event=>{event.preventDefault();const input=event.currentTarget.intention,text=input.value.trim();if(!text)return;const savedItems=parse(localStorage.getItem(INTENTIONS_KEY),[]);const items=Array.isArray(savedItems)?savedItems:[];items.push({text,done:false,createdAt:new Date().toISOString()});localStorage.setItem(INTENTIONS_KEY,JSON.stringify(items));input.value='';render();document.querySelector('#cw127-context-label').textContent='Plan';report('intention-added',{characters:text.length})};
  node.querySelector('[data-clear]').onclick=()=>{const savedItems=parse(localStorage.getItem(INTENTIONS_KEY),[]);const items=(Array.isArray(savedItems)?savedItems:[]).filter(item=>!item.done);localStorage.setItem(INTENTIONS_KEY,JSON.stringify(items));render()};render();show(node);report('dialog-opened',{dialog:'intentions'});
}
function realms(){
